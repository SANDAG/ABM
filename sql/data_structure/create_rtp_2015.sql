...
SELECT
	...
	,SUM(CASE	WHEN [mode_id] BETWEEN 1 AND 2 THEN [hwy_flow_mode].[flow]
				...
				ELSE 0 END) AS [total_person_flow]
FROM
	[abm].[hwy_flow_mode]
INNER JOIN
	[abm].[hwy_flow]
...
with taz_results AS (
	...
	INNER JOIN (	
		...
		INNER JOIN
			[ref].[mgra13_xref]
		ON
			[lu_hh].[geography_zone_id] = [mgra13_xref].[mgra13_geography_zone_id]
		WHERE 
			[lu_person].[scenario_id] = @scenario_id
			AND [lu_hh].[scenario_id] = @scenario_id
		...
		ON 
			...
			AND [hwy_skims].[orig_geography_zone_id] = orig.[taz13_geography_zone_id]
	INNER JOIN (	
		SELECT	
			[scenario_id]
			,[taz13_geography_zone_id]
		...
		INNER JOIN
			[ref].[mgra13_xref]
		ON
			[lu_mgra_input].[geography_zone_id] = [mgra13_xref].[mgra13_geography_zone_id]
		WHERE 
			[scenario_id] = @scenario_id
		...
		ON 
			...
			AND [hwy_skims].[dest_geography_zone_id] = dest.[taz13_geography_zone_id]
	WHERE 
...
ON 
	[hwy_skims].[dest_geography_zone_id] = dest.[taz13_geography_zone_id]
WHERE 
	[scenario_id] = @scenario_id 
...
INNER JOIN
	#tt
ON 
	#pop_seg_by_taz.[taz13_geography_zone_id] = #tt.orig_taz
GROUP BY
	#pop_seg_by_taz.[scenario_id]
)
-- and output results
