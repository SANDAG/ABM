...
class CreateScenarioGUI(Tkinter.Frame):
        def __init__(self, root):
            ...
            Tkinter.Label(body, text=u"Copy network files to a study folder?", font=("Helvetica", 10, 'bold')).grid(row=1,columnspan=3)
            ...
            self.studybutton = Tkinter.Button(body, text=u"...",width=4,command=self.get_studypath)
        ...
        def get_networkpath(self):
            ...
            if path:
                networkpath = os.path.normpath(path)
                self.networkpath.delete(0, Tkconstants.END)
                self.networkpath.insert(0, networkpath)
            return

        #set study path
        def get_studypath(self):
            self.setPathOptions()
            studypath = tkFileDialog.askdirectory(**self.dir_opt)
            if studypath:
                studypath = os.path.normpath(studypath)
                self.studypath.delete(0, Tkconstants.END)
                self.studypath.insert(0, studypath)
            return

        #set network path to be copied to study
        def get_studynetworkpath(self):
            self.setPathOptions()
            path = tkFileDialog.askdirectory(**self.dir_opt)
            if path:
        ...
        def checkPath(self):
            ...
            if os.path.exists(self.scenariopath.get()):
                ...
                popupMsg.popupmsg(self,"Scenario is empty!",1,"scenario")
            else:
                self.executeBatch("scenario")
            return

        #check study path and network path validity
        def checkStudyPath(self):
            self.popup=Tkinter.Tk()
            if os.path.exists(self.studypath.get()):
                if not self.studynetworkpath.get():
                    popupMsg.popupmsg(self,"Network folder is empty!",1,"study")
                else:
                    popupMsg.popupmsg(self,"Selected study folder already exists! Proceeding will overwrite existing files!",2,"study")
            else:
                if not self.studypath.get():
                    popupMsg.popupmsg(self,"Study folder is empty!",1,"study")
                elif not self.studynetworkpath.get():
                    popupMsg.popupmsg(self,"Network folder is empty!",1,"study")
                else:
                    self.executeBatch("study")
            return
