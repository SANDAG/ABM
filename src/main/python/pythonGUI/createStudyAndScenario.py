...
class SelectStudyYears(tkinter.Frame):
    def __init__(self, root, parent):
        ...
        current_row = 0
        tkinter.Label(body, text="Year Selection", font=("Helvetica", 10, 'bold')).grid(row=current_row)
        current_row += 1
        ...
        tkinter.Button(body, text="Select", font=("Helvetica", 8, 'bold'),width=10,command=lambda: self.record_selection()).grid(row=current_row)
    ...
    def record_selection(self):
        ...
        self.parent.studyyears.delete(0, tkinter.constants.END)
...
class CreateScenarioGUI(tkinter.Frame):
        def __init__(self, root, emme_version = "4.3.7", year = "2016", geo = "1", lu = "DS42"):
            tkinter.Frame.__init__(self, root, border=5)
            body = tkinter.Frame(self)
            body.pack(fill=tkinter.constants.X, expand=1)
            ...
            self.lu_options = {"DS41": {"name": "Baseline",
                               ...
                               "DS42": {"name": "Sustainable Community Strategy",
                                        "years": ["2016", "2019", "2022", "2023", "2025", "2026", "2029", "2030", "2032", "2035", "2040", "2050"]}}
            ...
            self.studyYearList = ["2016", "2019", "2020", "2022", "2023", "2025_Vision", "2025nb", "2026_Vision", "2029_Vision",
            ...
            self.defaultNetworkDir="T:\\projects\\sr14\\version14_3_0\\network_build"
            ...
            tkinter.Label(body, text="Release Version 14.3.0\n"+divider, font=("Helvetica", 11, 'bold'), width=50, fg='royal blue').grid(row=current_row,columnspan=5)
            current_row += 1
            tkinter.Label(body, text="Create an ABM Work Space", font=("Helvetica", 10, 'bold')).grid(row=current_row,columnspan=n_columns)
            current_row += 1
            ...
            tkinter.Label(body, text="Study Folder", font=("Helvetica", 8, 'bold')).grid(row=current_row)
            self.studypath = tkinter.Entry(body, width=40)
            self.studypath.grid(row=current_row, column=1, sticky=sticky)
            self.studypath.delete(0, tkinter.constants.END)
            self.studypath.insert(0, self.defaultScenarioDir)
            self.studybutton = tkinter.Button(body, text="...",width=4,command=lambda:self.get_path("study"))
            self.studybutton.grid(row=current_row, column=n_columns-1)
            current_row += 1

            tkinter.Label(body, text="Network Folder",font=("Helvetica", 8, 'bold')).grid(row=current_row)
            self.studynetworkpath = tkinter.Entry(body, width=40)
            self.studynetworkpath.grid(row=current_row, column=1, sticky=sticky)
            self.studynetworkpath.delete(0, tkinter.constants.END)
            self.studynetworkpath.insert(0, self.defaultNetworkDir)
            self.studynetworkbutton = tkinter.Button(body, text="...",width=4,command=lambda: self.get_path("studynetwork"))
            self.studynetworkbutton.grid(row=current_row, column=n_columns-1)
            self.selected_study_years = ''
            current_row += 1

            tkinter.Label(body, text="Year Selection",font=("Helvetica", 8, 'bold')).grid(row=current_row)
            self.studyyears = tkinter.Entry(body, width=40)
            self.studyyears.grid(row=current_row, column=1, sticky=sticky)
            self.studyyears.delete(0, tkinter.constants.END)
            self.studyyearsbutton = tkinter.Button(body, text="...",width=4,command=lambda: self.select_study_years())
            self.studyyearsbutton.grid(row=current_row, column=n_columns-1)
            current_row += 1
            
            self.copyButton = tkinter.Button(body, text="Create", font=("Helvetica", 8, 'bold'),width=10, command=lambda: self.checkPath("study"))
            ...
            current_row += 1
            tkinter.Label(body, text="Create an ABM scenario", font=("Helvetica", 10, 'bold')).grid(row=current_row,columnspan=n_columns)
            current_row += 1

            self.version="version_14_4_0"


            tkinter.Label(body, text="Emme Version", font=("Helvetica", 8, 'bold')).grid(row=current_row)
            var = StringVar(root)
            optionList = ["4.3.7", "4.5.0"]
            ...
            tkinter.Label(body, text="Year", font=("Helvetica", 8, 'bold')).grid(row=current_row)
            var = StringVar(root)
            #if self.select_lu:
            var.set(self.year)
            #else:
            ...
            tkinter.Label(body, text="Land Use", font=("Helvetica", 8, 'bold')).grid(row=current_row)
            #self.lu="DS41"
            #if self.select_lu:
            var = StringVar(root)
            ...
            tkinter.Label(body, text="Geography ID", font=("Helvetica", 8, 'bold')).grid(row=current_row)
            #self.geo="1"
            self.geo = tkinter.Entry(body, width=40)
            self.geo.grid(row=current_row, column=1, sticky=sticky)
            self.geo.delete(0, tkinter.constants.END)
            self.geo.insert(0, 1)
            current_row += 1
            #option.pack(expand = True)
			
            tkinter.Label(body, text="Scenario Folder", font=("Helvetica", 8, 'bold')).grid(row=13)
            self.scenariopath = tkinter.Entry(body, width=40)
            self.scenariopath.grid(row=current_row, column=1, sticky=sticky)
            button = tkinter.Button(body, text="...",width=4,command=lambda: self.get_path("scenario"))
            button.grid(row=current_row, column=2)
            current_row += 1

            tkinter.Label(body, text="Network Folder",font=("Helvetica", 8, 'bold')).grid(row=14)
            self.networkpath = tkinter.Entry(body, width=40)
            self.networkpath.grid(row=current_row, column=1, sticky=sticky)
            button = tkinter.Button(body, text="...",width=4,command=lambda: self.get_path("network"))
            button.grid(row=current_row, column=2)
            current_row += 1

            buttons = tkinter.Frame(self)
            buttons.pack()
            botton = tkinter.Button(buttons, text="Create", font=("Helvetica", 8, 'bold'),width=10, command=lambda: self.checkPath("scenario"))
            botton.pack(side=tkinter.constants.LEFT)
            #botton.grid(row=13, column = 0)
            tkinter.Frame(buttons, width=10).pack(side=tkinter.constants.LEFT)
            button = tkinter.Button(buttons, text="Quit", font=("Helvetica", 8, 'bold'), width=10, command=self.quit)
            ...
            self.networkpath.delete(0, tkinter.constants.END)
        ...
        def setyear(self,value):
            ...
            self.destroy()
            CreateScenarioGUI(self.root, self.emme_version, self.year, self.geo, self.lu).pack(fill=tkinter.constants.X, expand=1)
            return
        ...
        def select_study_years(self):
            ...
            SelectStudyYears(selection_root, self).pack(fill=tkinter.constants.X, expand=1)
        ...
        def executeBatch(self, type):
            self.popup.destroy()
            if type=="scenario":
                commandstr = "create_scenario.cmd %s %s %s %s" % (
                ...
                f.write('\n'.join(exclude))
                f.close()
                commandstr="copy_networkfiles_to_study.cmd "+self.studypath.get()+" "+self.studynetworkpath.get()
                print(commandstr)
                os.chdir(self.releaseDir+"\\"+self.version+'\\')
                os.system(commandstr)    
...
root.minsize(370, 0)
logo = tkinter.PhotoImage(file=r"T:\ABM\release_test\ABM\SANDAG_logo.gif")
