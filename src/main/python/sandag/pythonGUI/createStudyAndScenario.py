...
#sxu update 12/2020 for release 13.3.3
import Tkinter
import Tkconstants
import tkFileDialog
import os
...
class CreateScenarioGUI(Tkinter.Frame):
        def __init__(self, root):
            ...
            #create new option list for Network Build years_Susan
            Tkinter.Label(body, text=u"Year", font=("Helvetica", 8, 'bold')).grid(row=5)
            valores = StringVar(root)
            #self.studynetworkyear = "2012 2014 2020rc 2025rc 2030rc 2035rc 2040rc 2045rc 2050rc"
            valores.set("2012 2014 2020rc 2025rc 2030rc 2035rc 2040rc 2045rc 2050rc")
            self.lstbox = Listbox(body, listvariable=valores, selectmode=MULTIPLE, width=50, height=10)
            self.lstbox.config(width=50)
            self.lstbox.grid(row=5, column=1)
            #end_Susan

            self.copyButton = Tkinter.Button(body, text=u"Create", font=("Helvetica", 8, 'bold'),width=10, command=lambda: self.checkPath("study"))
            self.copyButton.grid(row=6,column=0,columnspan=4)

            Tkinter.Label(body, text=divider, font=("Helvetica", 11, 'bold'), width=50, fg='royal blue').grid(row=7,columnspan=5)
            Tkinter.Label(body, text=u"Create an ABM scenario", font=("Helvetica", 10, 'bold')).grid(row=8,columnspan=3)
        ...
        def executeBatch(self, type):
            ...
            if type=="scenario":
				...
				reslist = [self.lstbox.get(i) for i in self.lstbox.curselection()]
				for value in reslist:
					self.defaultpath=self.defaultNetworkDir+'\\'+value
					self.studynetworkpath.delete(0, Tkconstants.END)
					self.studynetworkpath.insert(0, self.defaultpath)
					self.studynetworkyear=value
					commandstr=u"copy_networkfiles_to_study.cmd "+self.studypath.get()+" "+self.studynetworkpath.get()+" "+self.studynetworkyear+" "+self.defaultNetworkDir
					print (commandstr)
					os.chdir(self.releaseDir+"\\"+self.version+'\\')
					os.system(commandstr)
            self.popup=Tkinter.Tk()
