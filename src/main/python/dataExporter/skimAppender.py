...
class SkimAppender(object):
    ...
    def mgra_xref(self) -> pd.DataFrame:
        ...
        scenario input MGRA file (input/mgra15_based_input<<year>>.csv).
        """
    ...
    def drive_transit_skims(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # create MGRA-TAP origin-destinations based on outboundbound direction
        records["MGRA"] = np.where(records["outbound"],
                                   records["originMGRA"],
    ...
    def omx_auto_skim_appender(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for omx_fn in df_map.omxFileName.unique():
            ...
            fn = os.path.join(self.scenario_path, "output", "skims", omx_fn + ".omx")
            ...
            for matrix in records_omx.matrixName.unique():
                ...
                skims = list(zip(
                    ...
                    [matrix] * len(o),
                    o, d,
                    omx_file[matrix[:-4] + "_TIME_" + matrix[-3:]][o_idx, d_idx],
                    omx_file[matrix[:-4] + "_DIST_" + matrix[-3:]][o_idx, d_idx],
                    omx_file[matrix[:-4] + "_TOLLCOST_" + matrix[-3:]][o_idx, d_idx] / 100))
    ...
    def omx_transit_skims(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        #switch the period to the end of the skim name
        for i in range(len(mapping)):
            mapping[i][2] = mapping[i][2][3:] + "__" + mapping[i][2][:2]
        ...
        for period in ['EA', 'AM', 'MD', 'PM', 'EV']:
            f = omx.open_file(r"C:\abm_runs\abm3_dev\emme_setup\output\skims\transit_skims_{}.omx".format(period))
            omx_file[period] = f
    
        omx_map = omx_file['EA'].mapping("zone_number")
