...
public final class XBorderSkimsAppender {
	...
	protected transient Logger logger = Logger
			.getLogger(XBorderSkimsAppender.class);
	...
	private String[] inputFormats = { "NUMBER", "NUMBER", "NUMBER", "STRING",
	...
	private void runSkimsAppender(ResourceBundle rb) {
		...
		AutoTazSkimsCalculator tazDistanceCalculator = new AutoTazSkimsCalculator(
				rbMap);
		...
		for (int[] hisOdt : hisOdts) {
			...
			try {
				...
				writeSkimsToFile(seq, outStreamHis, false, odt, anm, wtw,
						loggers);
			} catch (Exception e) {
	...
	private void writeSkimsToFile(int sequence, PrintWriter outStream,
			...
			WalkTransitWalkSkimsCalculator wtw, Logger[] loggers) {
	...
	private TableDataSet getInputTableDataSet(HashMap<String, String> rbMap) {
		...
		try {
			...
			inTds = reader.readFileWithFormats(new File(hisFileName),
					inputFormats);
	...
	private int[][] getInputOrigDestTimes(TableDataSet hisTds) {
		...
		for (int r = 1; r <= hisTds.getRowCount(); r++) {
			if (departStrings[r - 1].equalsIgnoreCase("am"))
				departs[r - 1] = 2;
			else if (departStrings[r - 1].equalsIgnoreCase("pm"))
				departs[r - 1] = 5;
			else if (departStrings[r - 1].equalsIgnoreCase("op"))
				departs[r - 1] = 1;
			else
				departs[r - 1] = -1;
