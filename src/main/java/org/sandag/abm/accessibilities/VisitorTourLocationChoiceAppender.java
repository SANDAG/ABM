...
public final class VisitorTourLocationChoiceAppender extends McLogsumsAppender {
	...
	private void runLogsumAppender(ResourceBundle rb) {
		...
		String outputAllKey = Util.getStringValueFromPropertyMap(rbMap,
				OUTPUT_SAMPLE_DEST_LOGSUMS);
		...
		if (outputAllKey.equalsIgnoreCase("all"))
			outputName1 = baseName + "_" + "all" + extension;
		...
		try {
			...
			outStream2 = new PrintWriter(new BufferedWriter(new FileWriter(
					new File(outputName2))));
		} catch (IOException e) {
		...
		logger.info("total part 1 runtime = " + (totalTime1 / 1000)
				+ " seconds.");
		logger.info("total part 2 runtime = " + (totalTime2 / 1000)
				+ " seconds.");

	}

	private void writeDcFile(HashMap<String, String> rbMap,
			PrintWriter outStream1, PrintWriter outStream2) {

		// print the chosen destMgra and the depart/arrive logsum field names to
		// both
		// files
		if (outStream1 != null)
			outStream1.print("seq,sampno,chosenMgra");
		...
		SandagAppendMcLogsumDMU mcDmuObject = new SandagAppendMcLogsumDMU(
				modelStructure);
		...
		for (int i = 0; i < estDataOdts.length; i++) {
			...
			calculateModeChoiceLogsums(rbMap, category == -1 ? null
					: mcModel[category], mcDmuObject, odtSet, mgraSet,
					departAvailable, arriveAvailable, false);
			...
			outStream2.printf(",%.8f",
					departArriveLogsums[0][chosenLogsumTodIndex]);
			...
			for (int m = 1; m < departArriveLogsums.length; m++) {
				...
				outStream2.printf(",%.8f",
						departArriveLogsums[m][chosenLogsumTodIndex]);
			}
			if (outStream1 != null)
				outStream1.print("\n");
			outStream2.print("\n");
			...
			if (seq % 1000 == 0)
				logger.info("wrote DC Estimation file record: " + seq);
	...
	private int[][] getDcEstimationDataOrigDestTimes(TableDataSet hisTds) {
		...
		for (int r = 1; r <= hisTds.getRowCount(); r++) {
			...
			odts[r - 1][PARTYSIZE] = jointPerson1Participates[r - 1]
					+ jointPerson2Participates[r - 1]
					+ jointPerson3Participates[r - 1]
					+ jointPerson4Participates[r - 1]
					+ jointPerson5Participates[r - 1];

			odts[r - 1][TOUR_PURPOSE] = odts[r - 1][JOINT] == 1
					&& purposeIndex > 4 ? jtPurpose[r - 1] : purposeIndex;
	...
	public static void main(String[] args) {
		...
		VisitorTourLocationChoiceAppender appender = new VisitorTourLocationChoiceAppender(
				rbMap);
