...
public class StoredTransitSkimData {
	...
	public static synchronized StoredTransitSkimData getInstance(
			int numServiceTypes, int numPeriods, int maxTap) {
		if (objInstance == null) {
			objInstance = new StoredTransitSkimData();
			objInstance.setupStoredDataArrays(numServiceTypes, numPeriods,
					maxTap);
			return objInstance;
		} else {
			return objInstance;
	...
	private void setupStoredDataArrays(int numServiceTypes, int numPeriods,
			int maxTap) {
