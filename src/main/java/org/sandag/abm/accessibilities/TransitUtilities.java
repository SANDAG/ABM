 ...
 * @version May, 2009
 */
public class TransitUtilities implements Serializable {

	protected transient Logger logger = Logger
			.getLogger(TransitUtilities.class);
	...
	public TransitUtilities(HashMap<String, String> rbMap) {
		...
		int peakModelPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.transit.peak.page");
		...
		walkUEC[0] = new TransitWalkAccessUEC(rbMap, uecFileName,
				offpeakModelPage, dataPage);
		walkUEC[1] = new TransitWalkAccessUEC(rbMap, uecFileName,
				peakModelPage, dataPage);

		// store the walk-access UEC with the first period UEC
		walkUEC[0].createWalkAccessUEC(rbMap, uecFileName,
				walkTransitModelPage, dataPage);

		// drive uec
		driveUEC = new TransitDriveAccessUEC[TAPPERIODS.length];
		driveUEC[0] = new TransitDriveAccessUEC(rbMap, uecFileName,
				offpeakModelPage, dataPage, AccessMode.PARK_N_RIDE);
		driveUEC[1] = new TransitDriveAccessUEC(rbMap, uecFileName,
				peakModelPage, dataPage, AccessMode.PARK_N_RIDE);

		driveUEC[0].createDriveAccessUEC(rbMap, uecFileName,
				driveTransitModelPage, dataPage);

		trace = Util.getBooleanValueFromPropertyMap(rbMap, "Trace");
		int[] traceOtaz = Util.getIntegerArrayFromPropertyMap(rbMap,
				"Trace.otaz");
		int[] traceDtaz = Util.getIntegerArrayFromPropertyMap(rbMap,
				"Trace.dtaz");
	...
	public void calculateUtilityComponents() {
		...
		for (int period = 0; period < TAPPERIODS.length; ++period) {
			...
			for (int i = 1; i < taps.length; ++i) {
				...
				if (iTap <= 10 || (iTap % 500) == 0)
					logger.info("......Origin TAP " + iTap);
				...
				for (int j = 1; j < taps.length; ++j) {
					...
					trace = false;
					if (tracer.isTraceOn() && traceOTap[iTap]
							&& traceDTap[jTap])
						trace = true;

					double[] results = walkUEC[period]
							.calculateUtilitiesForTapPair(iTap, jTap, trace);

					for (int mode = 0; mode < results.length; ++mode) {
						if (results[mode] > -500)
							tapExpUtilities[period][iTap][jTap][mode] = Math
									.exp(results[mode]);
		...
		for (Integer mgra : mgraManager.getMgras()) { // MGRA
			...
			if (mgra <= 10 || (mgra % 500) == 0)
				logger.info("...Origin MGRA " + mgra);
			...
			for (int tapPosition = 0; tapPosition < tapsInMgra; ++tapPosition) {
				double result = walkUEC[0].calculateWalkAccessUtilityForMgra(
						mgra, tapPosition, trace);
		...
		for (int taz : tazManager.tazs) { // TAZ

			int[] drvTaps = tazManager.getParkRideOrKissRideTapsForZone(taz,
			...
			for (int tapPosition = 0; tapPosition < drvTaps.length; ++tapPosition) {
				...
				double result = driveUEC[0].calculateDriveAccessUtilityForTaz(
						taz, tapPosition, AccessMode.PARK_N_RIDE, trace);
	 ...
	 */
	public double[] calculateWalkTransitExpUtilities(int oMgra, int dMgra,
			int period) {
		...
		// skip mgras with no TAPs within walking distance
		if (mgraManager.getMgraWlkTapsDistArray()[oMgra][0] == null
				|| mgraManager.getMgraWlkTapsDistArray()[dMgra][0] == null)
			return expUtilities;
		...
		if (tracer.isTraceOn() && tracer.isTraceZonePair(iTaz, jTaz)) {
			logger.info("");
			logger.info("Walk-Transit Mgra-Mgra calculations for " + oMgra
					+ " to " + dMgra + " period " + period);
			logger.info("");
		...
		for (int iTap : mgraManager.getMgraWlkTapsDistArray()[oMgra][0]) {
			...
			for (int jTap : mgraManager.getMgraWlkTapsDistArray()[dMgra][0]) {
				...
				// interchange.
				walkUEC[period].comparePaths(expUtilities, iTap, jTap,
						writeCalculations);
		...
		if (writeCalculations) {
			logger.info("");
			logger.info("Best Utilities for mgra pair " + oMgra + " to "
					+ dMgra + " period " + period);
	 ...
	 * @return An array of exponentiated utilities, by mode.
	 */
	public double[] calculateDriveTransitExpUtilities(int oTaz, int dMgra,
			int period) {
		...
		if (writeCalculations) {
			logger.info("");
			logger.info("Drive-Transit Taz-Mgra calculations for " + oTaz
					+ " to " + dMgra + " period " + period);
			logger.info("");
		...
		int iPos = -1;
		for (int iTap : tazManager.getParkRideOrKissRideTapsForZone(oTaz,
				accMode)) {
			iPos++; // used to know where we are in time/dist arrays for taps
			...
			for (int jTap : mgraManager.getMgraWlkTapsDistArray()[dMgra][0]) {
				...
				// the uec object stores the best utilities for this
				// interchange.
				driveUEC[period].comparePaths(expUtilities, iTap, jTap,
						writeCalculations);
		...
		if (trace) {
			logger.info("");
			logger.info("Best Utilities for mgra pair " + oTaz + " to " + dMgra
					+ " period " + period);
			logger.info("ModeNumber, Mode, ExpUtility, bestITap, bestJTap");
			Modes.TransitMode[] mode = Modes.TransitMode.values();
			for (int i = 0; i < bestUtilities.length; ++i) {
	...
	public static void main(String[] args) {
		...
		if (os64bit) {
			...
			try {
				...
				System.out
						.println(String
								.format("UnknownHostException. serverAddress = %s, serverPort = %d -- exiting.",
			...
			try {
				...
				System.out
						.println(String
								.format("RemoteException. serverAddress = %s, serverPort = %d -- exiting.",
										serverAddress, serverPort));
				e.printStackTrace();
				matrixServer.stop32BitMatrixIoServer();
				throw new RuntimeException();
