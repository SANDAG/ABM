...
public final class TripSkimsAppender {
	...
	protected transient Logger logger = Logger
			.getLogger(TripSkimsAppender.class);
	...
	private void runSkimsAppender(ResourceBundle rb) {
		...
		WalkTransitWalkSkimsCalculator wtw = logsumHelper
				.getWtwSkimCalculator();
		WalkTransitDriveSkimsCalculator wtd = logsumHelper
				.getWtdSkimCalculator();
		DriveTransitWalkSkimsCalculator dtw = logsumHelper
				.getDtwSkimCalculator();
		...
		try {
			...
			logger.fatal(String.format(
					"Exception occurred opening output skims file: %s.",
					outputFileNameHis));
		...
		for (int[] hisOdt : hisOdts) {
			...
			try {
				...
				logger.error("Exception caught processing record: " + seq
						+ " of " + hisOdts.length + ".");
				break;
			}
	...
	private void writeSkimsToFile(int sequence, PrintWriter outStream,
		...
		outStream.print(String.format("%d,%d,%d,%d,%d", sequence, odt[3],
				odt[0], odt[1], odt[2]));
		...
		for (int i = 0; i < bestTapPairs.length; i++) {
			if (bestTapPairs[i] == null)
				...
				returnedSkims[i] = wtw.getWalkTransitWalkSkims(i,
						BestTransitPathCalculator.findWalkTransitAccessTime(
								odt[0], bestTapPairs[i][0]),
						BestTransitPathCalculator.findWalkTransitEgressTime(
								odt[1], bestTapPairs[i][1]),
						bestTapPairs[i][0], bestTapPairs[i][1], odt[2],
						loggingEnabled);
			}
		}
		if (loggingEnabled)
		...
		for (int i = 0; i < bestTapPairs.length; i++) {
			if (bestTapPairs[i] == null)
				...
				returnedSkims[i] = wtd.getWalkTransitDriveSkims(i,
						BestTransitPathCalculator.findWalkTransitAccessTime(
								odt[0], bestTapPairs[i][0]),
						BestTransitPathCalculator.findDriveTransitEgressTime(
								odt[1], bestTapPairs[i][1]),
						bestTapPairs[i][0], bestTapPairs[i][1], odt[2],
						loggingEnabled);
			}
		...
		for (int i = 0; i < bestTapPairs.length; i++) {
			if (bestTapPairs[i] == null)
				...
				returnedSkims[i] = dtw.getDriveTransitWalkSkims(i,
						BestTransitPathCalculator.findDriveTransitAccessTime(
								odt[0], bestTapPairs[i][0]),
						BestTransitPathCalculator.findWalkTransitEgressTime(
								odt[1], bestTapPairs[i][1]),
						bestTapPairs[i][0], bestTapPairs[i][1], odt[2],
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("");
			System.out.println("");
			serverAddress = (String) propertyMap
					.get("RunModel.MatrixServerAddress");

			String serverPortString = (String) propertyMap
			...
			if (serverAddress != null && serverPort > 0) {
				try {
					System.out
							.println("attempting connection to matrix server "
					...
					ms.testRemote(Thread.currentThread().getName());
					mdm.setMatrixDataServerObject(ms);
					System.out.println("connected to matrix server "
							+ serverAddress + ":" + serverPort);
				...
				System.out
						.println("starting matrix data server in a 32 bit process.");
			...
			MgraDataManager mgraManager = MgraDataManager
					.getInstance(propertyMap);
			...
			if (ms == null) {
				...
				ioVm32Bit.stopJVM32();
				System.out
						.println("matrix data server 32 bit process stopped.");
			} else {
