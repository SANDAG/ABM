...
public final class NonMandatoryTodEstimationMcLogsumsAppender extends
		McLogsumsAppender {
	...
	private NonMandatoryTodEstimationMcLogsumsAppender(
	...
	private void writeTodFile(HashMap<String, String> rbMap,
		...
		for (int i = 0; i < estDataOdts.length; i++) {
			...
			if (seq % 1000 == 0)
				logger.info("wrote TOD Estimation file record: " + seq);
	...
	private int[][] getTodEstimationDataOrigDestTimes(TableDataSet hisTds) {
		...
		int[] jointParticipants = hisTds
				.getColumnAsInt(JOINT_PARTICIPANTS_FIELD);
		...
		for (int r = 1; r <= hisTds.getRowCount(); r++) {
			...
			odts[r - 1][ADULTS] = adultsFt[r - 1] + adultsPt[r - 1]
					+ adultsUn[r - 1] + adultsRt[r - 1] + adultsNw[r - 1];
			...
			odts[r - 1][TOUR_PURPOSE] = odts[r - 1][JOINT] == 1
					&& purpose[r - 1] > 4 ? jtPurpose[r - 1] : purpose[r - 1];
