 ...
 * @version March, 2010
 */
public class AutoTazSkimsCalculator implements Serializable {
	...
	public AutoTazSkimsCalculator(HashMap<String, String> rbMap) {
		...
		String uecFileName = uecPath
				+ Util.getStringValueFromPropertyMap(rbMap,
						"taz.distance.uec.file");
		int dataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"taz.distance.data.page");
		int autoSkimEaOdPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"taz.od.distance.ea.page");
		int autoSkimAmOdPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"taz.od.distance.am.page");
		int autoSkimMdOdPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"taz.od.distance.md.page");
		int autoSkimPmOdPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"taz.od.distance.pm.page");
		int autoSkimEvOdPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"taz.od.distance.ev.page");

		File uecFile = new File(uecFileName);
		autoDistOD_UECs = new UtilityExpressionCalculator[NUM_PERIODS];
		autoDistOD_UECs[EA] = new UtilityExpressionCalculator(uecFile,
				autoSkimEaOdPage, dataPage, rbMap, dmu);
		autoDistOD_UECs[AM] = new UtilityExpressionCalculator(uecFile,
				autoSkimAmOdPage, dataPage, rbMap, dmu);
		autoDistOD_UECs[MD] = new UtilityExpressionCalculator(uecFile,
				autoSkimMdOdPage, dataPage, rbMap, dmu);
		autoDistOD_UECs[PM] = new UtilityExpressionCalculator(uecFile,
				autoSkimPmOdPage, dataPage, rbMap, dmu);
		autoDistOD_UECs[EV] = new UtilityExpressionCalculator(uecFile,
				autoSkimEvOdPage, dataPage, rbMap, dmu);
