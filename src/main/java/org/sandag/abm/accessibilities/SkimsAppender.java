...
public final class SkimsAppender {
	...
	private static final int[] SURVEY_PERIOD_TO_SKIM_PERIOD = { 0, 3, 1, 3, 3,
			2, 3 };
	...
	private void runSkimsAppender(ResourceBundle rb) {
		...
		if (outputFileNameObs != "" || outputFileNameHis != "") {
			...
			heading += getNonMotorizedSkimsHeaderRecord("nm",
					anm.getNmSkimNames());
			heading += getTransitSkimsHeaderRecord("wtw",
					wtw.getLocalSkimNames(), wtw.getPremiumSkimNames());
			heading += getTransitSkimsHeaderRecord("wtd",
					wtd.getLocalSkimNames(), wtd.getPremiumSkimNames());
			heading += getTransitSkimsHeaderRecord("dtw",
					dtw.getLocalSkimNames(), dtw.getPremiumSkimNames());
			...
			if (outputFileNameObs != "") {
				try {
					...
					for (int i = 0; i < SKIM_PERIOD_LABELS.length; i++) {
						...
						String newName = outputFileNameObs.substring(0,
								dotIndex)
								+ "_"
								+ SKIM_PERIOD_LABELS[i]
								+ outputFileNameObs.substring(dotIndex);
						writer = new FileWriter(new File(newName));
						outStreamObsTod[i] = new PrintWriter(
								new BufferedWriter(writer));
					}
				} catch (IOException e) {
					logger.fatal(String
							.format("Exception occurred opening output skims file: %s.",
									outputFileNameObs));
			...
			if (outputFileNameHis != "") {
				try {
					...
					for (int i = 0; i < SKIM_PERIOD_LABELS.length; i++) {
						...
						String newName = outputFileNameHis.substring(0,
								dotIndex)
								+ "_"
								+ SKIM_PERIOD_LABELS[i]
								+ outputFileNameHis.substring(dotIndex);
						writer = new FileWriter(new File(newName));
						outStreamHisTod[i] = new PrintWriter(
								new BufferedWriter(writer));
					}
				} catch (IOException e) {
					logger.fatal(String
		...
		if (outputFileNameObs != "") {
			...
			for (int[] obsOdt : obsOdts) {
				...
				if (odt[0] == 0 || odt[1] == 0) {
					outStreamObs.println(String.format("%d,%d,%d,%d,%d", seq,
							odt[4], odt[0], odt[1], odt[2]));
					seq++;
					continue;
				}
				...
				if (odt[0] == 25646 && odt[1] == 4319)
					debugFlag = true;

				writeSkimsToFile(seq, outStreamObs, debugFlag, odt, anm, wtw,
						wtd, dtw, loggers);
				...
				for (int i = 0; i < SKIM_PERIOD_LABELS.length; i++) {
					odt[2] = i + 1;
					writeSkimsToFile(seq, outStreamObsTod[i], false, odt, anm,
							wtw, wtd, dtw, loggers);
				}
				...
				if (seq % 1000 == 0)
					logger.info("wrote OBS record: " + seq);
		...
		if (outputFileNameHis != "") {
			...
			for (int[] hisOdt : hisOdts) {
				...
				odt[2] = SURVEY_PERIOD_TO_SKIM_PERIOD[hisOdt[2]]; // depart skim
																	// period
				// index
				writeSkimsToFile(seq, outStreamHis, false, odt, anm, wtw, wtd,
						dtw, loggers);
				...
				for (int i = 0; i < SKIM_PERIOD_LABELS.length; i++) {
					odt[2] = i + 1;
					writeSkimsToFile(seq, outStreamHisTod[i], false, odt, anm,
							wtw, wtd, dtw, loggers);
				}
				...
				if (seq % 1000 == 0)
					logger.info("wrote HIS record: " + seq);
	...
	private void writeSkimsToFile(int sequence, PrintWriter outStream,
			boolean loggingEnabled, int[] odt,
			AutoAndNonMotorizedSkimsCalculator anm,
			WalkTransitWalkSkimsCalculator wtw,
			WalkTransitDriveSkimsCalculator wtd,
			DriveTransitWalkSkimsCalculator dtw, Logger[] loggers) {
		...
		if (outStream != null)
			outStream.print(String.format("%d,%d,%d,%d,%d", sequence, odt[4],
					odt[0], odt[1], odt[2]));

		double[] skims = anm.getAutoSkims(odt[0], odt[1], odt[2],
				loggingEnabled, autoLogger);
		if (loggingEnabled)
			anm.logReturnedSkims(odt[0], odt[1], odt[2], skims, "auto",
					autoLogger);
		...
		skims = anm.getNonMotorizedSkims(odt[0], odt[1], odt[2],
				loggingEnabled, autoLogger);
		if (loggingEnabled)
			anm.logReturnedSkims(odt[0], odt[1], odt[2], skims,
					"non-motorized", autoLogger);
		...
		bestTapPairs = wtw.getBestTapPairs(odt[0], odt[1], odt[2],
				loggingEnabled, wtwLogger);
		...
		for (int i = 0; i < bestTapPairs.length; i++) {
			if (bestTapPairs[i] == null)
				...
				returnedSkims[i] = wtw.getWalkTransitWalkSkims(i,
						...
						bestPathUEC.getBestEgressTime(i), bestTapPairs[i][0],
						bestTapPairs[i][1], odt[2], loggingEnabled);
			}
		}
		if (loggingEnabled)
		...
		bestTapPairs = wtd.getBestTapPairs(odt[0], odt[1], odt[2],
				loggingEnabled, wtdLogger);
		returnedSkims = new double[bestTapPairs.length][];
		for (int i = 0; i < bestTapPairs.length; i++) {
			if (bestTapPairs[i] == null)
		...
		bestTapPairs = dtw.getBestTapPairs(odt[0], odt[1], odt[2],
				loggingEnabled, dtwLogger);
		returnedSkims = new double[bestTapPairs.length][];
		for (int i = 0; i < bestTapPairs.length; i++) {
			if (bestTapPairs[i] == null)
				returnedSkims[i] = dtw.getNullTransitSkims(i);
			else {
	 ...
	 *            period index
	 */
	private String getTransitSkimsHeaderRecord(String transitServiveLabel,
			String[] localNames, String[] premiumNames) {
		...
		for (int i = 0; i < mode.length; i++) {
			if (mode[i].isPremiumMode(mode[i])) {
				for (int j = 0; j < premiumNames.length; j++)
					heading += String.format(",%s_%s_%s", transitServiveLabel,
							mode[i], premiumNames[j]);
			} else {
	...
	private TableDataSet getOnBoardSurveyTableDataSet(
			HashMap<String, String> rbMap) {

		String obsFileName = Util.getStringValueFromPropertyMap(rbMap,
				OBS_DATA_RECORDS_FILE_KEY);
		if (obsFileName == null) {
			logger.error("Error getting the filename from the properties file for the Sandag on-board survey data records file.");
			logger.error("Properties file target: " + OBS_DATA_RECORDS_FILE_KEY
					+ " not found.");
			logger.error("Please specify a filename value for the "
					+ OBS_DATA_RECORDS_FILE_KEY + " property.");
			throw new RuntimeException();
		}
	...
	private TableDataSet getHomeInterviewSurveyTableDataSet(
			HashMap<String, String> rbMap) {

		String hisFileName = Util.getStringValueFromPropertyMap(rbMap,
		...
		if (hisFileName == null) {
			...
			logger.error("Properties file target: " + HIS_DATA_RECORDS_FILE_KEY
					+ " not found.");
			logger.error("Please specify a filename value for the "
					+ HIS_DATA_RECORDS_FILE_KEY + " property.");
			throw new RuntimeException();
		}
