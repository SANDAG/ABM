 ...
 * @version May, 2009
 */
public class NonTransitUtilities implements Serializable {

	protected transient Logger logger = Logger
			.getLogger(NonTransitUtilities.class);
	...
	public NonTransitUtilities(HashMap<String, String> rbMap,
			double[][][] mySovExpUtilities, double[][][] myHovExpUtilities,
			double[][][] myNMotorExpUtilities) {
		...
		int dataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.data.page");
		int offpeakSOVPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.sov.offpeak.page");
		int offpeakHOVPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.hov.offpeak.page");
		int peakSOVPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.sov.peak.page");
		int peakHOVPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.hov.peak.page");
		int nonMotorPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.nonmotorized.page");

		sovUEC = new AutoUEC[SOVPERIODS.length];
		hovUEC = new AutoUEC[HOVPERIODS.length];
		nMotorUEC = new NonMotorUEC[NMTPERIODS.length];

		sovUEC[OFFPEAK_PERIOD_INDEX] = new AutoUEC(rbMap, uecFileName,
				offpeakSOVPage, dataPage);
		sovUEC[PEAK_PERIOD_INDEX] = new AutoUEC(rbMap, uecFileName,
				peakSOVPage, dataPage);
		hovUEC[OFFPEAK_PERIOD_INDEX] = new AutoUEC(rbMap, uecFileName,
				offpeakHOVPage, dataPage);
		hovUEC[PEAK_PERIOD_INDEX] = new AutoUEC(rbMap, uecFileName,
				peakHOVPage, dataPage);
		nMotorUEC[OFFPEAK_PERIOD_INDEX] = new NonMotorUEC(rbMap, uecFileName,
				nonMotorPage, dataPage);
	 ...
	 */
	public void setNonMotorUtilsMap(
			HashMap<Integer, Double>[][] aMgraNMotorExpUtilities) {
	...
	public void buildUtilitiesForOrigMgraAndPeriod(int iMgra, int period) {
		...
		for (int jTaz = 1; jTaz <= maxTaz; ++jTaz) {
			...
			double sovUtility = sovUEC[period].calculateUtilitiesForTazPair(
					iTaz, jTaz, avgTazHourlyParkingCost[jTaz]);
			// exponentiate the SOV utility
			if (sovUtility > -500)
				sovExpUtilities[period][iTaz][jTaz] = Math.exp(sovUtility);

			double hovUtility = hovUEC[period].calculateUtilitiesForTazPair(
					iTaz, jTaz, avgTazHourlyParkingCost[jTaz]);
			// exponentiate the SOV utility
			if (hovUtility > -500)
				hovExpUtilities[period][iTaz][jTaz] = Math.exp(hovUtility);
		...
		if (nMotorExpUtilities[OFFPEAK_PERIOD_INDEX][iTaz] == null) {
			...
			for (int jTaz = 1; jTaz <= maxTaz; ++jTaz) {
				...
				double nmtUtility = nMotorUEC[OFFPEAK_PERIOD_INDEX]
						.calculateUtilitiesForTazPair(iTaz, jTaz);
				// exponentiate the SOV utility
				if (nmtUtility > -500)
					nMotorExpUtilities[OFFPEAK_PERIOD_INDEX][iTaz][jTaz] = Math
							.exp(nmtUtility);
	...
	public double getNMotorExpUtility(int iMgra, int jMgra, int period) {
		...
		if (mgraNMotorExpUtilities[period][iMgra] == null) {
			...
			if (dMgras == null) {
				mgraNMotorExpUtilities[period][iMgra] = new HashMap<Integer, Double>(
						0);
			} else {
				mgraNMotorExpUtilities[period][iMgra] = new HashMap<Integer, Double>(
						dMgras.length);
				...
				for (int dMgra : dMgras) {
					...
					double nmtUtility = nMotorUEC[period]
							.calculateUtilitiesForMgraPair(iMgra, dMgra);

					// exponentiate the utility
					if (nmtUtility > -500)
						mgraNMotorExpUtilities[period][iMgra].put(dMgra,
								Math.exp(nmtUtility));
				}
	...
	public static void main(String[] args) {
		...
		os64bit = Boolean.parseBoolean(Util.getStringValueFromPropertyMap(
				rbMap, "operatingsystem.64bit"));
		if (os64bit) {

			String serverAddress = Util.getStringValueFromPropertyMap(rbMap,
					"server.address");

			int serverPort = Util.getIntegerValueFromPropertyMap(rbMap,
					"server.port");
		...
		NonTransitUtilities au = new NonTransitUtilities(rbMap,
