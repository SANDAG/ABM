...
public final class StopLocationSampleCalculator extends McLogsumsAppender {
	...
	private void runSampleProbabilitiesCalculator(ResourceBundle rb) {
		...
		SandagTripModeChoiceDMU mcDmuObject = new SandagTripModeChoiceDMU(
				modelStructure);

		ChoiceModelApplication[] mcModel = new ChoiceModelApplication[5 + 1];
		mcModel[WORK_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				WORK_SHEET, 0, rbMap, (VariableTable) mcDmuObject);
		mcModel[UNIVERSITY_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				UNIVERSITY_SHEET, 0, rbMap, (VariableTable) mcDmuObject);
		mcModel[SCHOOL_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				SCHOOL_SHEET, 0, rbMap, (VariableTable) mcDmuObject);
		mcModel[MAINTENANCE_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				MAINTENANCE_SHEET, 0, rbMap, (VariableTable) mcDmuObject);
		mcModel[DISCRETIONARY_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				DISCRETIONARY_SHEET, 0, rbMap, (VariableTable) mcDmuObject);
		mcModel[SUBTOUR_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				SUBTOUR_SHEET, 0, rbMap, (VariableTable) mcDmuObject);
		...
		double[] logsums = null;
		try {
			logsums = calculateTripModeChoiceLogsumForEstimationRecord(rbMap,
					mcModel[category], mcDmuObject, odtSet, mgra);
		} catch (Exception e) {
		...
		try {
			...
			slcSoaSizeTerms = getSlcSoaSizeTermsForStopPurpose(stopPurpose,
					preSchoolInHh, gradeSchoolInHh, highSchoolInHh, sizeTerms);
		} catch (Exception e) {
		...
		try {
			...
			int skimPeriodIndex = modelStructure
					.getSkimPeriodIndex(departPeriod) + 1; // depart skim period
			double odDist = logsumHelper.getAnmSkimCalculator().getAutoSkims(
					origMgra, destMgra, skimPeriodIndex, false, logger)[AUTO_DIST_SKIM_INDEX];
			double osDist = logsumHelper.getAnmSkimCalculator().getAutoSkims(
					origMgra, mgra, skimPeriodIndex, false, logger)[AUTO_DIST_SKIM_INDEX];
			double sdDist = logsumHelper.getAnmSkimCalculator().getAutoSkims(
					mgra, destMgra, skimPeriodIndex, false, logger)[AUTO_DIST_SKIM_INDEX];
			...
			double util = -999;
			if (availability == 1)
				util = Math.log(slcSoaSizeTerms[mgra])
						+ absoulteDistanceDeviationCoefficient * distance;

			logUtilityCalculation(origMgra, mgra, destMgra, departPeriod,
					skimPeriodIndex, osDist, sdDist, odDist, distance,
					slcSoaSizeTerms[mgra], util, logsums);
		} catch (Exception e) {
	...
	private void logUtilityCalculation(int origMgra, int sampleMgra,
			int destMgra, int departPeriodIndex, int skimPeriodIndex,
			double osDist, double sdDist, double odDist, double distance,
			double size, double util, double[] logsums) {
	...
	private double[] getSlcSoaSizeTermsForStopPurpose(int stopPurpose,
			boolean preSchoolInHh, boolean gradeSchoolInHh,
			boolean highSchoolInHh, double[][] sizeTerms) {
	...
	private double[][] calculateSlcSoaSizeTerms(HashMap<String, String> rbMap,
			int sampleMgra) {
		...
		int slcSoaSizeUecData = Integer.parseInt(rbMap
				.get(PROPERTIES_UEC_STOP_SOA_SIZE_DATA));
		int slcSoaSizeUecModel = Integer.parseInt(rbMap
				.get(PROPERTIES_UEC_STOP_SOA_SIZE_MODEL));

		IndexValues iv = new IndexValues();
		UtilityExpressionCalculator slcSoaSizeUec = new UtilityExpressionCalculator(
				new File(slcSoaSizeUecFile), slcSoaSizeUecModel,
				slcSoaSizeUecData, rbMap, null);
		...
		for (int mgra : mgras) {
			...
			if (mgra == sampleMgra)
				slcSoaSizeUec.logAnswersArray(slcSoaLogger,
	...
	private int[][] getDcEstimationDataOrigDestTimes(TableDataSet hisTds) {
		...
		int[] jTourParticipants = hisTds
				.getColumnAsInt(J_TOUR_PARTICIPANTS_FIELD);
	...
	public static void main(String[] args) {
		...
		HashMap<String, String> rbMap = ResourceUtil
				.changeResourceBundleIntoHashMap(rb);
		StopLocationSampleCalculator appender = new StopLocationSampleCalculator(
				rbMap);
