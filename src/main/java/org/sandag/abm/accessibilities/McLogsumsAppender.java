...
public class McLogsumsAppender implements Serializable {
	...
	protected transient Logger logger = Logger
			.getLogger(McLogsumsAppender.class);
	protected transient Logger slcSoaLogger = Logger.getLogger("slcSoa");
	protected transient Logger nonManLogsumsLogger = Logger
			.getLogger("nonManLogsums");
	...
	protected TableDataSet getEstimationDataTableDataSet(
		...
		if (estFileName == null) {
			...
			logger.error("Properties file target: "
					+ ESTIMATION_DATA_RECORDS_FILE_KEY + " not found.");
	...
	protected void calculateModeChoiceLogsums(HashMap<String, String> rbMap,
			...
			SandagAppendMcLogsumDMU mcDmuObject, int[] odt, int[] mgraSample,
			int[] departAvailable, int[] arriveAvailable, boolean chosenTodOnly) {
		...
		if (odt[INCOME] == 1)
			inc = 7500;
		else if (odt[INCOME] == 2)
			inc = 22500;
		else if (odt[INCOME] == 3)
			inc = 37500;
		else if (odt[INCOME] == 4)
			inc = 52500;
		else if (odt[INCOME] == 5)
			inc = 67500;
		else if (odt[INCOME] == 6)
			inc = 87500;
		else if (odt[INCOME] == 7)
			inc = 112500;
		else if (odt[INCOME] == 8)
			inc = 137500;
		else if (odt[INCOME] == 9)
			inc = 175000;
		else if (odt[INCOME] == 10)
			inc = 250000;
		else if (odt[INCOME] == 99)
			inc = 37500;
		...
		mcDmuObject.setWorkTourModeIsSOV(odt[WORK_TOUR_MODE] == 1
				|| odt[WORK_TOUR_MODE] == 2 ? 1 : 0);
		mcDmuObject.setWorkTourModeIsBike(odt[WORK_TOUR_MODE] == 10 ? 1 : 0);
		mcDmuObject.setWorkTourModeIsHOV(odt[WORK_TOUR_MODE] >= 3
				|| odt[WORK_TOUR_MODE] <= 8 ? 1 : 0);
		...
		for (int destMgra : mgraSetForLogsums) {
			...
			if (mcModel != null && destMgra > 0) {
				...
				// UEC
				// spreadsheets
				mcDmuObject.setDmuIndexValues(odt[0], origMgra, origMgra,
						destMgra, false);
				...
				modeChoiceLogsums[m] = new double[modelStructure
						.getSkimPeriodCombinationIndices().length];
			...
			for (int[] combo : departArriveCombinations) {
				...
				if (!chosenTodOnly || isChosenDepartArriveCombo) {
					...
					if (unavailableCombination(departPeriod, arrivePeriod,
							departAvailable, arriveAvailable)) {
						departArriveLogsums[m][i++] = -999;
						continue;
					}

					int logsumIndex = modelStructure
							.getSkimPeriodCombinationIndex(departPeriod,
									arrivePeriod);
					...
					double logsum = logsumHelper.calculateTourMcLogsum(
							origMgra, destMgra, departPeriod, arrivePeriod,
							mcModel, mcDmuObject);
					...
					if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2)
						...
						nonManLogsumsLogger
								.info("Logsum["
										+ i
										+ "] calculation for estimation file record number "
										+ odt[0]);
						nonManLogsumsLogger.info("");
						nonManLogsumsLogger
								.info("--------------------------------------------------------------------------------------------------------");
						nonManLogsumsLogger.info("tour purpose = "
								+ odt[TOUR_PURPOSE]);
						nonManLogsumsLogger.info("mc purpose sheet = "
								+ MC_PURPOSE_SHEET_INDICES[odt[TOUR_PURPOSE]]);
						nonManLogsumsLogger
								.info("purpose category = "
										+ PURPOSE_CATEGORIES[odt[TOUR_PURPOSE]]
										+ ": "
										+ PURPOSE_CATEGORY_LABELS[PURPOSE_CATEGORIES[odt[TOUR_PURPOSE]]]);
						nonManLogsumsLogger.info("origin mgra = "
								+ odt[ORIG_MGRA]);
						nonManLogsumsLogger.info("destination mgra = "
								+ odt[DEST_MGRA]);
						nonManLogsumsLogger.info("origin taz = "
								+ mgraManager.getTaz(origMgra));
						nonManLogsumsLogger.info("destination taz = "
								+ mgraManager.getTaz(destMgra));
						nonManLogsumsLogger.info("depart interval = "
								+ departArriveCombinationLabels[i][0]
								+ ", @timeOutbound = "
								+ mcDmuObject.getTimeOutbound()
								+ ", chosen depart = " + departPeriod);
						nonManLogsumsLogger.info("arrive interval = "
								+ departArriveCombinationLabels[i][1]
								+ ", @timeInbound = "
								+ mcDmuObject.getTimeInbound()
								+ ", chosen arrive = " + arrivePeriod);
						nonManLogsumsLogger.info("income category = "
								+ odt[INCOME] + ", @income = "
						...
						nonManLogsumsLogger.info("gender = " + odt[FEMALE]
								+ ", @female = " + mcDmuObject.getFemale());
						nonManLogsumsLogger.info("jointTourCategory = "
								+ odt[JOINT] + ", @tourcategoryJoint = "
								+ mcDmuObject.getTourCategoryJoint());
						nonManLogsumsLogger.info("partySize = "
								+ odt[PARTYSIZE]);
						nonManLogsumsLogger
								.info("--------------------------------------------------------------------------------------------------------");
						nonManLogsumsLogger.info("");

						mcModel.logUECResults(nonManLogsumsLogger,
								"Est Record: " + odt[0]);
						nonManLogsumsLogger
								.info("Logsum Calculation for index: "
										+ logsumIndex + " , Logsum value: "
										+ modeChoiceLogsums[m][logsumIndex]);
						nonManLogsumsLogger.info("");
						nonManLogsumsLogger.info("");
	...
	protected void calculateTripModeChoiceLogsums(
			HashMap<String, String> rbMap, ChoiceModelApplication mcModel,
			SandagTripModeChoiceDMU mcDmuObject, int[] odt, int[] mgraSample) {
		...
		mcDmuObject.setTourModeIsDA(modelStructure
				.getTourModeIsSov(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsS2(modelStructure
				.getTourModeIsS2(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsS3(modelStructure
				.getTourModeIsS3(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsWalk(modelStructure
				.getTourModeIsWalk(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsBike(modelStructure
				.getTourModeIsBike(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsWTran(modelStructure
				.getTourModeIsWalkLocal(odt[TOUR_MODE])
				|| modelStructure.getTourModeIsWalkPremium(odt[TOUR_MODE]) ? 1
				: 0);
		mcDmuObject.setTourModeIsPnr(modelStructure
				.getTourModeIsPnr(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsKnr(modelStructure
				.getTourModeIsKnr(odt[TOUR_MODE]) ? 1 : 0);
		mcDmuObject.setTourModeIsSchBus(modelStructure
				.getTourModeIsSchoolBus(odt[TOUR_MODE]) ? 1 : 0);
		...
		for (int sampleMgra : mgraSetForLogsums) {
			...
			if (mcModel != null && sampleMgra > 0) {
				...
				mcDmuObject.setDestEmpDen(mgraManager
						.getEmpDenValue(sampleMgra));
				mcDmuObject.setDestTotInt(mgraManager
						.getTotIntValue(sampleMgra));
			...
			if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2)) {
				...
				nonManLogsumsLogger
						.info("IK Logsum calculation for estimation file record number "
								+ odt[0]);
				nonManLogsumsLogger.info("");
				nonManLogsumsLogger
						.info("--------------------------------------------------------------------------------------------------------");
				...
				nonManLogsumsLogger.info("sample destination mgra = "
						+ sampleMgra);
				nonManLogsumsLogger
						.info("final destination mgra = " + destMgra);
				nonManLogsumsLogger.info("origin taz = "
						+ mgraManager.getTaz(origMgra));
				nonManLogsumsLogger.info("sample destination taz = "
						+ mgraManager.getTaz(sampleMgra));
				nonManLogsumsLogger.info("final destination taz = "
						+ mgraManager.getTaz(destMgra));
			...
			if ((odt[DIRECTION] == INBOUND_DIRCETION_CODE)) {
				logsumHelper.setWtdTripMcDmuAttributes(mcDmuObject, origMgra,
						sampleMgra, departPeriod, mcDmuObject
								.getDmuIndexValues().getDebug());
			} else
				logsumHelper.setDtwTripMcDmuAttributes(mcDmuObject, origMgra,
						sampleMgra, departPeriod, mcDmuObject
								.getDmuIndexValues().getDebug());

			logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject, origMgra,
					sampleMgra, departPeriod, mcDmuObject.getDmuIndexValues()
							.getDebug());

			double logsum = logsumHelper.calculateTripMcLogsum(origMgra,
					sampleMgra, departPeriod, mcModel, mcDmuObject,
					nonManLogsumsLogger);
			tripModeChoiceLogsums[m][0] = logsum;

			if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2)) {
				nonManLogsumsLogger.info("IK Logsum value: "
						+ tripModeChoiceLogsums[m][0]);
				nonManLogsumsLogger.info("");
				nonManLogsumsLogger.info("");
			}
			...
			if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2)) {
				...
				nonManLogsumsLogger
						.info("KJ Logsum calculation for estimation file record number "
								+ odt[0]);
				nonManLogsumsLogger.info("");
				nonManLogsumsLogger
						.info("--------------------------------------------------------------------------------------------------------");
			...
			logsum = logsumHelper.calculateTripMcLogsum(origMgra, sampleMgra,
					departPeriod, mcModel, mcDmuObject, nonManLogsumsLogger);
			tripModeChoiceLogsums[m][1] = logsum;

			if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2)) {
				nonManLogsumsLogger.info("KJ Logsum value: "
	...
	protected double[] calculateTripModeChoiceLogsumForEstimationRecord(
		...
		if (origMgra == 0 || destMgra == 0 || sampleMgra == 0
				|| odt[TOUR_MODE] == 0 || mcModel == null)
		...
		// spreadsheets
		mcDmuObject.setDmuIndexValues(odt[0], origMgra, origMgra, sampleMgra,
				false);
		...
		mcDmuObject.setATazTerminalTime(tazs
				.getDestinationTazTerminalTime(mgraManager.getTaz(sampleMgra)));
		...
		if (mcDmuObject.getDmuIndexValues().getDebug()) {
			...
			slcSoaLogger.info("mc purpose sheet = "
					+ MC_PURPOSE_SHEET_INDICES[odt[TOUR_PURPOSE]]);
			slcSoaLogger
					.info("purpose category = "
							+ PURPOSE_CATEGORIES[odt[TOUR_PURPOSE]]
			...
			slcSoaLogger.info("sample destination taz = "
					+ mgraManager.getTaz(sampleMgra));
			slcSoaLogger.info("final destination taz = "
					+ mgraManager.getTaz(destMgra));
			slcSoaLogger.info("depart interval = " + departPeriod);
			slcSoaLogger.info("income category = " + odt[INCOME]
					+ ", @income = " + mcDmuObject.getIncome());
			slcSoaLogger.info("adults = " + odt[ADULTS]);
			slcSoaLogger.info("autos = " + odt[AUTOS]);
			slcSoaLogger.info("hhsize = " + odt[HHSIZE]);
			slcSoaLogger.info("gender = " + odt[FEMALE] + ", @female = "
		...
		if (mcDmuObject.getDmuIndexValues().getDebug()) {
			...
			slcSoaLogger.info("IK Mode Choice Logsum value: "
					+ tripModeChoiceLogsums[0]);
			slcSoaLogger.info("");
			slcSoaLogger.info("");
		...
		if ((odt[DIRECTION] == INBOUND_DIRCETION_CODE)) {
			logsumHelper.setWtdTripMcDmuAttributes(mcDmuObject, sampleMgra,
					destMgra, departPeriod, mcDmuObject.getDmuIndexValues()
							.getDebug());
		} else
			logsumHelper.setDtwTripMcDmuAttributes(mcDmuObject, sampleMgra,
					destMgra, departPeriod, mcDmuObject.getDmuIndexValues()
							.getDebug());

		logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject, sampleMgra,
				destMgra, departPeriod, mcDmuObject.getDmuIndexValues()
						.getDebug());

		logsum = logsumHelper.calculateTripMcLogsum(sampleMgra, destMgra,
				departPeriod, mcModel, mcDmuObject, nonManLogsumsLogger);
		tripModeChoiceLogsums[1] = logsum;

		if (mcDmuObject.getDmuIndexValues().getDebug()) {

			slcSoaLogger.info("KJ Mode Choice Logsum value: "
					+ tripModeChoiceLogsums[1]);
			slcSoaLogger.info("");
			slcSoaLogger.info("");
	 ...
	 */
	protected boolean unavailableCombination(int departPeriod,
			int arrivePeriod, int[] departAvailable, int[] arriveAvailable) {
		...
		if (departAvailable[departModelPeriod] == 1
				&& arriveAvailable[arriveModelPeriod] == 1)
			returnValue = false;
	...
	protected void startMatrixServer(ResourceBundle rb) {
		...
		try {
			...
			logger.error(
					"exception caught running ctramp model components -- exiting.",
					e);
			throw new RuntimeException();
	...
	public void setTazDistanceSkimArrays(
		...
		AutoAndNonMotorizedSkimsCalculator anm = logsumHelper
				.getAnmSkimCalculator();
