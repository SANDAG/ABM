...
public class DcUtilitiesTaskJppf implements Callable<List<Object>> {

	private static final int MIN_EXP_FUNCTION_ARGUMENT = -500;

	private static final String[] LOGSUM_SEGMENTS = { "SOV       ",
			"HOV       ", "Transit   ", "NMotorized", "SOVLS_0   ",
			"SOVLS_1   ", "SOVLS_2   ", "HOVLS_0_OP", "HOVLS_1_OP",
			"HOVLS_2_OP", "HOVLS_0_PK", "HOVLS_1_PK", "HOVLS_2_PK", "TOTAL" };
	...
	public DcUtilitiesTaskJppf(int taskIndex, int startRange, int endRange,
			...
			double[][][] myNMotorExpUtilities, boolean[] hasSizeTerm,
			double[][] expConstants, double[][] sizeTerms,
			double[][] workSizeTerms, double[][] schoolSizeTerms,
			int[][] myExternalLuzsForCordonLuz,
			int[] myCordonLuzForExternalLuz,
		...
		String dcUecFileName = Util.getStringValueFromPropertyMap(rbMap,
				"acc.dcUtility.uec.file");
		int dcDataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.dcUtility.data.page");
		int dcUtilityPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.dcUtility.page");

		File dcUecFile = new File(dcUecFileName);
		dcUEC = new UtilityExpressionCalculator(dcUecFile, dcUtilityPage,
				dcDataPage, rbMap, aDmu);
		...
		if (calculateLuAccessibilities) {
			...
			dcUecFileName = Util.getStringValueFromPropertyMap(rbMap,
					"lu.acc.dcUtility.uec.file");
			dcDataPage = Util.getIntegerValueFromPropertyMap(rbMap,
					"lu.acc.dcUtility.data.page");
			dcUtilityPage = Util.getIntegerValueFromPropertyMap(rbMap,
					"lu.acc.dcUtility.page");

			File luUecFile = new File(dcUecFileName);
			luUEC = new UtilityExpressionCalculator(luUecFile, dcUtilityPage,
					dcDataPage, rbMap, luDmu);
	...
	public List<Object> call() {
		...
		NonTransitUtilities ntUtilities = new NonTransitUtilities(rbMap,
		...
		for (int i = startRange; i <= endRange; i++) { // Origin MGRA
			...
			for (Integer jMgra : mgraManager.getMgras()) { // Destination MGRA

				if (!hasSizeTerm[jMgra])
					continue;
				...
				try {
					...
					logger.error("exception for op sov/hov utilitiy taskIndex="
							+ taskIndex + ", i=" + i + ", startRange="
							+ startRange + ", endRange=" + endRange, e);
					System.exit(-1);
				}
				...
				bestPathCalculator.findBestWalkTransitWalkTaps(
						TransitWalkAccessUEC.MD, iMgra, jMgra, false, logger);

				// sum the exponentiated utilities over modes
				double opWTExpUtility = 0;
				double[] transitUtilities = bestPathCalculator
						.getBestUtilities();
				...
				try {
					...
					logger.error("exception for pk sov/hov utility taskIndex="
							+ taskIndex + ", i=" + i + ", startRange="
							+ startRange + ", endRange=" + endRange, e);
					System.exit(-1);
				}
				...
				bestPathCalculator.findBestWalkTransitWalkTaps(
						TransitWalkAccessUEC.AM, iMgra, jMgra, false, logger);
				...
				try {
					nmExpUtility = ntUtilities.getNMotorExpUtility(iMgra,
							jMgra, NonTransitUtilities.OFFPEAK_PERIOD_INDEX);
				} catch (Exception e) {
					logger.error(
							"exception for non-motorized utilitiy taskIndex="
									+ taskIndex + ", i=" + i + ", startRange="
									+ startRange + ", endRange=" + endRange, e);
					System.exit(-1);
				}
				...
				// 2: Walk-Transit
				if (opWTExpUtility > 0)
					logsums[2] = Math.log(opWTExpUtility);

				// 3: Non-Motorized
				if (nmExpUtility > 0)
					logsums[3] = Math.log(nmExpUtility);

				// 4: SOVLS_0
				logsums[4] = Math.log(opSovExpUtility * expConstants[0][0]
						+ opWTExpUtility * expConstants[0][2] + nmExpUtility
						* expConstants[0][3]);
				// 5: SOVLS_1
				logsums[5] = Math.log(opSovExpUtility * expConstants[1][0]
						+ opWTExpUtility * expConstants[1][2] + nmExpUtility
						* expConstants[1][3]);

				// 6: SOVLS_2
				logsums[6] = Math.log(opSovExpUtility * expConstants[2][0]
						+ opWTExpUtility * expConstants[2][2] + nmExpUtility
						* expConstants[2][3]);

				// 7: HOVLS_0_OP
				logsums[7] = Math.log(opHovExpUtility * expConstants[0][1]
						+ opWTExpUtility * expConstants[0][2] + nmExpUtility
						* expConstants[0][3]);

				// 8: HOVLS_1_OP
				logsums[8] = Math.log(opHovExpUtility * expConstants[1][1]
						+ opWTExpUtility * expConstants[1][2] + nmExpUtility
						* expConstants[1][3]);

				// 9: HOVLS_2_OP
				logsums[9] = Math.log(opHovExpUtility * expConstants[2][1]
						+ opWTExpUtility * expConstants[2][2] + nmExpUtility
						* expConstants[2][3]);

				// 10: HOVLS_0_PK
				logsums[10] = Math.log(pkHovExpUtility * expConstants[0][1]
						+ pkWTExpUtility * expConstants[0][2] + nmExpUtility
						* expConstants[0][3]);

				// 11: HOVLS_1_PK
				logsums[11] = Math.log(pkHovExpUtility * expConstants[1][1]
						+ pkWTExpUtility * expConstants[1][2] + nmExpUtility
						* expConstants[1][3]);

				// 12: HOVLS_2_PK
				logsums[12] = Math.log(pkHovExpUtility * expConstants[2][1]
						+ pkWTExpUtility * expConstants[2][2] + nmExpUtility
						* expConstants[2][3]);

				// 13: ALL
				logsums[13] = Math.log(pkSovExpUtility * expConstants[3][0]
						+ pkHovExpUtility * expConstants[3][1] + pkWTExpUtility
						* expConstants[3][2] + nmExpUtility
						* expConstants[3][3]);
				...
				if (calculateLuAccessibilities) {
					...
					luUtilities[0] = pkSovExpUtility * expConstants[0][0]
					...
					// sufficiency
					luUtilities[1] = pkSovExpUtility * expConstants[1][0]
					...
					// sufficiency
					luUtilities[2] = pkSovExpUtility * expConstants[2][0]
					...
					luUtilities[3] = opSovExpUtility * expConstants[0][0]
					...
					// sufficiency
					luUtilities[4] = opSovExpUtility * expConstants[1][0]
					...
					// sufficiency
					luUtilities[5] = opSovExpUtility * expConstants[2][0]
					...
					luUtilities[6] = pkSovExpUtility * expConstants[3][0]
					...
					if (trace) {
						...
						for (int j = 0; j < luUtilities.length; ++j) {
							printString += ","
									+ String.format("%9.2f", luUtilities[j]);
						}
						logger.info(printString);
			...
			for (int alt = 0; alt < alts; ++alt) {
				if (accessibilities[iMgra][alt] > 0)
					accessibilities[iMgra][alt] = (float) Math
							.log(accessibilities[iMgra][alt]);
			}
	...
	private void accumulateLandUseModeChoiceLogsums(float[] luUtilitiesValues,
		...
		accumulateSimple(iLuz, jLuz, luUtilities,
				accumulatedLandUseLogsumsCount, accumulatedLandUseLogsums);
		accumulateLogit(iLuz, jLuz, luUtilities,
				accumulatedLandUseLogsumsCount, accumulatedLandUseLogsums);
	...
	private void accumulateSimple(int iLuz, int jLuz, float[] luUtilities,
		...
		if (externalLuzsForCordonLuz[iLuz] != null) {
			...
			for (int exLuz : externalLuzsForCordonLuz[iLuz]) {
				...
				double additionalUtility = Math
						.exp(cordonLuzMinutesForExternalLuz[exLuz]
