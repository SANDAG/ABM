...
public class BestTransitPathCalculator implements Serializable {
	...
	public BestTransitPathCalculator(HashMap<String, String> rbMap) {
		...
		String uecFileName = uecPath
				+ Util.getStringValueFromPropertyMap(rbMap,
						"utility.bestTransitPath.uec.file");
		...
		walkAccessUEC = createUEC(uecFile, walkAccessPage, dataPage, rbMap,
				walkDmu);
		driveAccessUEC = createUEC(uecFile, driveAccessPage, dataPage, rbMap,
				driveDmu);
		walkEgressUEC = createUEC(uecFile, walkEgressPage, dataPage, rbMap,
				walkDmu);
		driveEgressUEC = createUEC(uecFile, driveEgressPage, dataPage, rbMap,
				driveDmu);

		for (int i = 0; i < NUM_PERIODS; i++)
			tapToTapUEC[i] = createUEC(uecFile, tapToTapPages[i], dataPage,
					rbMap, walkDmu);
		...
		StoredUtilityData storedDataObject = StoredUtilityData.getInstance(
				maxMgra, maxTap, maxTaz, NUM_ACC_EGR, NUM_PERIODS);
		...
		storedDepartPeriodTapTapUtils = storedDataObject
				.getStoredDepartPeriodTapTapUtils();
	...
	public static double findDriveTransitAccessTime(int pMgra, int pTap) {
		...
		int[] pTapSet = tazManager.getParkRideOrKissRideTapsForZone(pTaz,
		...
		if (pTapSet != null) {
			...
			for (int tap : pTapSet) {
				...
				if (tap == pTap) {
					accTime = tazManager.getTapTime(pTaz, pPos,
							AccessMode.PARK_N_RIDE);
					break;
				}
			}
	...
	public static double findDriveTransitEgressTime(int aMgra, int aTap) {
		...
		// get taps with walk access from mgra
		TazDataManager tazManager = TazDataManager.getInstance();
		int[] aTapSet = tazManager.getParkRideOrKissRideTapsForZone(aTaz,
				AccessMode.PARK_N_RIDE);
		...
		if (aTapSet != null) {
			...
			for (int tap : aTapSet) {
				...
				if (tap == aTap) {
					egrTime = tazManager.getTapTime(aTaz, aPos,
							AccessMode.PARK_N_RIDE);
					break;
				}
			}
	 ...
	 * 
	 */
	public void findBestWalkTransitWalkTaps(int period, int pMgra, int aMgra,
			boolean debug, Logger myLogger) {
		...
		for (int pTap : pMgraSet) {
			...
			for (int aTap : aMgraSet) {
				...
				// been
				// computed.
				setUtilitiesForTapPair(WTW, period, pTap, aTap,
						writeCalculations, myLogger);

				// Set the aTap to aMgra walk egress utility values, if they
				// haven't
				// already been computed.
				setWalkEgressUtility(aTap, aMgra, aPos, writeCalculations,
						myLogger);

				if (aTap == 0 || pTap == 0 || aMgra == 0 || pMgra == 0) {
					logger.error("aTap=" + aTap + "pTap=" + pTap + "aMgra="
							+ aMgra + "pMgra=" + pMgra + "period=" + period);
				...
				try {
					...
					logger.error("aTap=" + aTap + "pTap=" + pTap + "aMgra="
							+ aMgra + "pMgra=" + pMgra + "period=" + period, e);
					throw new RuntimeException();
				}

				comparePaths(combinedUtilities, pTap, aTap, writeCalculations,
						myLogger);
	 ...
	 * 
	 */
	public void findBestDriveTransitWalkTaps(int period, int pMgra, int aMgra,
			boolean debug, Logger myLogger) {
		...
		int pPos = -1;
		int[] pTapArray = tazManager.getParkRideOrKissRideTapsForZone(pTaz,
				accMode);
		for (int pTap : pTapArray) {
			...
			// already been computed.
			setDriveAccessUtility(pTaz, pPos, pTap, accMode, writeCalculations,
					myLogger);
			...
			if ((accMode == AccessMode.PARK_N_RIDE && tapManager
				...
				for (int aTap : mgraManager.getMgraWlkTapsDistArray()[aMgra][0]) {
					...
					// already
					// been computed.
					setUtilitiesForTapPair(DTW, period, pTap, aTap,
							writeCalculations, myLogger);

					if (aTap == 0 || pTap == 0 || aMgra == 0 || pMgra == 0) {
						logger.error("aTap=" + aTap + ",pTap=" + pTap
								+ ",aMgra=" + aMgra + ",pMgra=" + pMgra
								+ ",period=" + period);
						throw new RuntimeException();
					...
					comparePaths(combinedUtilities, pTap, aTap,
							writeCalculations, myLogger);
	 ...
	 * 
	 */
	public void findBestWalkTransitDriveTaps(int period, int pMgra, int aMgra,
			boolean debug, Logger myLogger) {
		...
		for (int pTap : mgraManager.getMgraWlkTapsDistArray()[pMgra][0]) {
			...
			for (int aTap : tazManager.getParkRideOrKissRideTapsForZone(aTaz,
				...
				if ((accMode == AccessMode.PARK_N_RIDE && tapManager
					...
					// haven't already been computed.
					setDriveEgressUtility(aTap, aTaz, aPos, accMode,
							writeCalculations, myLogger);

					// set the pTap to aTap utility values, if they haven't
					// already
					// been computed.
					setUtilitiesForTapPair(WTD, period, pTap, aTap,
							writeCalculations, myLogger);
	...
	private void setWalkAccessUtility(int pMgra, int pPos, int pTap,
		...
		if (storedWalkAccessUtils[pMgra][pTap] == null) {
			...
			storedWalkAccessUtils[pMgra][pTap] = walkAccessUEC.solve(index,
					walkDmu, null);

			// logging
			if (myTrace) {
				walkAccessUEC.logAnswersArray(myLogger, "Walk Orig Mgra="
						+ pMgra + ", to pTap=" + pTap + " Utility Piece");
			}
		}
	}

	private void setDriveAccessUtility(int pTaz, int pPos, int pTap,
			Modes.AccessMode accMode, boolean myTrace, Logger myLogger) {
		pDriveTime = tazManager.getTapTime(pTaz, pPos, accMode);
		if (storedDriveAccessUtils[pTaz][pTap] == null) {
			driveDmu.setDriveDistToTap(tazManager.getTapDist(pTaz, pPos,
					accMode));
			driveDmu.setDriveTimeToTap(pDriveTime);
			driveDmu.setEscalatorTime(tapManager.getEscalatorTime(pTap));
			storedDriveAccessUtils[pTaz][pTap] = driveAccessUEC.solve(index,
					driveDmu, null);

			// logging
			if (myTrace) {
				driveAccessUEC.logAnswersArray(myLogger, "Drive Orig Taz="
						+ pTaz + ", to pTap=" + pTap + " Utility Piece");
			}
		}
	}

	private void setWalkEgressUtility(int aTap, int aMgra, int aPos,
			boolean myTrace, Logger myLogger) {
		...
		if (storedWalkEgressUtils[aTap][aMgra] == null) {
			...
			storedWalkEgressUtils[aTap][aMgra] = walkEgressUEC.solve(index,
					walkDmu, null);

			// logging
			if (myTrace) {
				walkEgressUEC.logAnswersArray(myLogger, "Walk aTap=" + aTap
						+ ", from dest mgra=" + aMgra + " Utility Piece");
			}
		}
	}

	private void setDriveEgressUtility(int aTap, int aTaz, int aPos,
			Modes.AccessMode accMode, boolean myTrace, Logger myLogger) {
		aDriveTime = tazManager.getTapTime(aTaz, aPos, accMode);
		if (storedDriveEgressUtils[aTap][aTaz] == null) {
			driveDmu.setDriveDistToTap(tazManager.getTapDist(aTaz, aPos,
					accMode));
			driveDmu.setDriveTimeToTap(aDriveTime);
			driveDmu.setEscalatorTime(tapManager.getEscalatorTime(aTap));
			storedDriveEgressUtils[aTap][aTaz] = driveEgressUEC.solve(index,
					driveDmu, null);

			// logging
			if (myTrace) {
				driveEgressUEC.logAnswersArray(myLogger,
						"Drive Tap to Dest Taz Utility Piece");
				driveEgressUEC.logAnswersArray(myLogger, "Drive aTap=" + aTap
						+ ", from dest taz=" + aTaz + " Utility Piece");
			}
		}
	}
	...
	public void setUtilitiesForTapPair(int accEgr, int period, int pTap,
		...
		if (storedDepartPeriodTapTapUtils[accEgr][period][pTap] == null) {
			...
			if (storedDepartPeriodTapTapUtils[accEgr][period][pTap] == null) {
				logger.error("error allocating array of length "
						+ (maxTap + 1)
		...
		if (storedDepartPeriodTapTapUtils[accEgr][period][pTap][aTap] == null) {
			...
			if (results == null) {
				...
				logger.error("accEgr=" + accEgr + ",period=" + period
						+ ",pTap=" + pTap + ",aTap=" + aTap, e);
				throw e;
			}
			...
			// logging
			if (myTrace) {
				tapToTapUEC[period].logAnswersArray(myLogger, "pTap=" + pTap
						+ " to aTap=" + aTap + " Utility Piece");
	 ...
	 */
	public void comparePaths(double[] calculatedUtilities, int pTap, int aTap,
			boolean myTrace, Logger myLogger) {

		for (int i = 0; i < bestUtilities.length; i++) {

			if (myTrace) {
				myLogger.info("Mode " + i + " calculatedUtility "
						+ calculatedUtilities[i] + ", best utility "
						+ bestUtilities[i]);
			}

			if (calculatedUtilities[i] > bestUtilities[i]
					&& calculatedUtilities[i] > -999) {
				...
				if (myTrace) {
					myLogger.info("Best utility so far for mode " + tm[i]
	 ...
	 */
	public UtilityExpressionCalculator createUEC(File uecSpreadsheet,
			int modelSheet, int dataSheet, HashMap<String, String> rbMap,
			VariableTable dmu) {
		return new UtilityExpressionCalculator(uecSpreadsheet, modelSheet,
				dataSheet, rbMap, dmu);
	}
