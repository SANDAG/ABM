...
public class MandatoryAccessibilities {
	...
	protected transient Logger logger = Logger
			.getLogger(MandatoryAccessibilities.class);
	...
	public MandatoryAccessibilities(HashMap<String, String> rbMap) {
		...
		String accUECFileName = Util.getStringValueFromPropertyMap(rbMap,
				"acc.uec.file");
		File accUECFile = new File(accUECFileName);
		int accDataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.data.page");
		constantsUEC = new UtilityExpressionCalculator(accUECFile,
				constantsPage, accDataPage, rbMap, dmu);
	...
	public void buildAccessibilityComponents(HashMap<String, String> rbMap) {
		...
		ntUtilities = new NonTransitUtilities(rbMap, sovExpUtilities,
	...
	public void readData(HashMap<String, String> rbMap) {
		...
		File surveyDataFile = new File(Util.getStringValueFromPropertyMap(
				rbMap, "survey.input.file"));
		OLD_CSVFileReader csvReader = new OLD_CSVFileReader();

		try {
			surveyData = csvReader.readFile(surveyDataFile, true);
		} catch (IOException e) {
			logger.fatal("Error: Trying to read survey data file "
					+ surveyDataFile);
	...
	public void calculateWorkLogsums(ResourceBundle rb) {
		...
		logger.info("Reading and calculating work location choice logsums from "
				+ inFileName);
		...
		try {
			...
			logger.fatal("Error trying to read survey file " + inFileName
					+ " line " + linesRead);
			throw new RuntimeException(e);
		}
	...
	public double calculateWorkLogsum(int hhMgra, int workMgra,
		...
		double logsum = Math.log(Math.exp(sovUtility)
	...
	public void calculateMandatoryAccessibilities(HashMap<String, String> rbMap) {
		...
		if (ntUtilities == null)
			buildAccessibilityComponents(rbMap);
		...
		for (int row = 1; row <= records; ++row) {
			...
			double[] recordAccessibilities = calculateAccessibilitiesForMgraPair(
					hhMgra, workMgra);

			for (int i = 0; i < recordAccessibilities.length; ++i)
				accessibilities[i][row - 1] = (float) recordAccessibilities[i];

			recordAccessibilities = calculateAccessibilitiesForMgraPair(hhMgra,
					schoolMgra);
			for (int i = 0; i < recordAccessibilities.length; ++i)
	...
	public double[] calculateAccessibilitiesForMgraPair(int oMgra, int dMgra) {
		...
		if (oMgra > 0 && dMgra > 0) {
			...
			// auto logsum
			double pkSovExpUtility = ntUtilities
					.getSovExpUtility(oTaz, dTaz, 1);
			double pkHovExpUtility = ntUtilities
					.getHovExpUtility(oTaz, dTaz, 1);
			...
			// best
			// mode
			double[] walkTransitExpUtilities = transitUtilities
					.calculateWalkTransitExpUtilities(oMgra, dMgra, 1);
			...
			Modes.TransitMode bestMode = transitUtilities
					.getBestWalkTransitMode(1);
			...
			if (bestMode != null) {
				...
				accessibilities[5] = Math.log(walkTransitExpUtilities[bestMode
						.ordinal()]);
			...
			// best
			// mode
			double[] driveTransitExpUtilities = transitUtilities
					.calculateDriveTransitExpUtilities(oTaz, dMgra, 1);
	...
	public static void main(String[] args) {
		...
		ma.os64bit = Boolean
				.parseBoolean(rb.getString("operatingsystem.64bit"));
		if (ma.os64bit)
			ma.startMatrixServer(rb);
