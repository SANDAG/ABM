 ...
 * @version May, 2009
 */
public final class BuildAccessibilities implements Serializable {

	protected transient Logger logger = Logger
			.getLogger(BuildAccessibilities.class);
	...
	private static final String[] WORK_OCCUP_SEGMENT_NAME_LIST = {
			"Management Business Science and Arts", "Services",
			"Sales and Office",
	...
	public static final String[] SCHOOL_DC_SIZE_SEGMENT_NAME_LIST = {
			"preschool", "k-8", "unified k-8", "9-12", "unified 9-12",
			"univ typical", "univ non-typical" };
	...
	private static final int[] SCHOOL_LOC_SEGMENT_TO_UEC_SHEET_INDEX = { 3, 4,
			5, 6, 6 };
	private static final int[] SCHOOL_LOC_SOA_SEGMENT_TO_UEC_SHEET_INDEX = { 5,
			4, 3, 2, 2 };
	private static final int[] SCHOOL_SEGMENT_TO_STF_UEC_SHEET_INDEX = { 3, 3,
			3, 2, 2 };
	...
	public static final int[] EXTERNAL_LUZS = { 230, 231, 232, 233, 234, 235,
			236 };

	public static final int[] EXTERNAL_LUZ_CORDON_LUZS = { 156, 156, 213, 157,
			122, 156, 156 };

	public static final int[] MINUTES_TO_ADD_TO_CORDON_FOR_EXTERNAL_LUZ = {
			105, 480, 1800, 1740, 155, 8160, 8760 };

	public static final int[] CORDON_LUZS = { 122, 156, 157, 213 };
	...
	public void setupBuildAccessibilities(HashMap<String, String> rbMap,
		...
		for (int i = 0; i < WORK_OCCUP_SEGMENT_VALUE_LIST.length; ++i) {
			workerOccupValueSegmentIndexMap.put(
					WORK_OCCUP_SEGMENT_VALUE_LIST[i], i);
		}
		...
		int dataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.data.page");
		int constantsPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.constants.page");
		int sizeTermPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.sizeTerm.page");
		...
		constantsUEC = new UtilityExpressionCalculator(uecFile, constantsPage,
				dataPage, rbMap, aDmu);
		sizeTermUEC = new UtilityExpressionCalculator(uecFile, sizeTermPage,
				dataPage, rbMap, aDmu);
		workerSizeTermUEC = new UtilityExpressionCalculator(uecFile,
				workerSizeTermPage, dataPage, rbMap, aDmu);
		schoolSizeTermUEC = new UtilityExpressionCalculator(uecFile,
				schoolSizeTermPage, dataPage, rbMap, aDmu);
		...
		nonMandatorySizeSegmentNameIndexMap.put(
				ModelStructure.ESCORT_PRIMARY_PURPOSE_NAME, ESCORT_INDEX);
		nonMandatorySizeSegmentNameIndexMap.put(
				ModelStructure.SHOPPING_PRIMARY_PURPOSE_NAME, SHOP_INDEX);
		nonMandatorySizeSegmentNameIndexMap.put(
				ModelStructure.OTH_MAINT_PRIMARY_PURPOSE_NAME, OTH_MAINT_INDEX);
		nonMandatorySizeSegmentNameIndexMap.put(
				ModelStructure.EAT_OUT_PRIMARY_PURPOSE_NAME, EATOUT_INDEX);
		nonMandatorySizeSegmentNameIndexMap.put(
				ModelStructure.VISITING_PRIMARY_PURPOSE_NAME, VISIT_INDEX);
		nonMandatorySizeSegmentNameIndexMap.put(
				ModelStructure.OTH_DISCR_PRIMARY_PURPOSE_NAME, OTH_DISCR_INDEX);
	...
	public void calculateSizeTerms() {
		...
		for (int mgra : mgras) {
			...
			for (int purp = 0; purp < alternatives; ++purp) {
				...
				if (sizeTerms[mgra][purp] > 0)
					hasSizeTerm[mgra] = true;
			}
	...
	public void calculateDCUtilitiesDistributed(HashMap<String, String> rbMap) {
		...
		while (endIndex < numPackets - 1) {
			...
			if (endIndex + packetSize > numPackets)
				endIndex = numPackets - 1;
		...
		if (landUseLogsums != null) {
			...
			if (useSimpleMethod)
				accessibilitiesTableObject
						.writeLandUseAccessibilityTableToFile(
								luAccFileName.substring(0,
										luAccFileName.indexOf('.'))
										+ "_simple" + ".csv",
								luzAccessibilities[SIMPLE]);
			if (useLogitMethod)
				accessibilitiesTableObject
						.writeLandUseAccessibilityTableToFile(
								luAccFileName.substring(0,
			...
			String luMcLogsumsFileName = projectDirectory
					+ Util.getStringValueFromPropertyMap(rbMap,
							"lu.acc.mc.logsums.output.file");
			if (useSimpleMethod)
				accessibilitiesTableObject.writeLandUseLogsumTablesToFile(
						luMcLogsumsFileName.substring(0,
								luMcLogsumsFileName.indexOf('.'))
	...
	private float[][][] aggregateMgraAccessibilitiesToLuz(
			float[][] landUseAccessibilities) {
		...
		for (int i = 1; i < landUseAccessibilities.length; i++) {
			...
			if (landUseAccessibilities[i] == null)
				continue;
			...
			for (int j = 0; j < numColumns - 1; j++) {
				...
				returnValues[LOGIT][luz][j] += Math
						.exp(landUseAccessibilities[i][j]);
			}
			...
			if (externalLuzsForCordonLuz[luz] != null) {
				...
				for (int exLuz : externalLuzsForCordonLuz[luz]) {
					...
					for (int j = 0; j < numColumns - 1; j++) {
						...
						returnValues[LOGIT][exLuz][j] += Math
								.exp(landUseAccessibilities[i][j]);
					}
		...
		for (int i = 0; i < returnValues[SIMPLE].length; i++) {
			...
			for (int j = 0; j < numColumns - 1; j++) {
				...
				returnValues[LOGIT][luz][j] = (float) Math
						.log(returnValues[LOGIT][luz][j]);
			}
	 ...
	 */
	private float[][] submitTasks(ArrayList<int[]> startEndIndexList,
		...
		for (int[] startEndIndices : startEndIndexList) {
			...
			logger.info(String.format("creating TASK: %d range: %d to %d.",
					taskIndex, startIndex, endIndex));

			DcUtilitiesTaskJppf task = new DcUtilitiesTaskJppf(taskIndex,
					startIndex, endIndex, sovExpUtilities, hovExpUtilities,
					nMotorExpUtilities, hasSizeTerm, expConstants, sizeTerms,
					workerSizeTerms, luSchoolSizeTerms,
					externalLuzsForCordonLuz, cordonLuzForExternalLuz,
					cordonLuzMinutesForExternalLuz, rbMap,
					calculateLuAccessibilities);
		...
		for (Future<List<Object>> fs : results) {
			...
			try {
				...
				if (landUseLogsums != null) {
					...
					accumulatedLandUseLogsums = (float[][][][][]) resultBundle
							.get(5);
					accumulatedLandUseLogsumsCount = (int[][]) resultBundle
							.get(6);
					accumulateLandUseModeChoiceLogsums(
	...
	private void accumulateLandUseModeChoiceLogsums(
			float[][][][][] accumulatedLandUseLogsums,
			int[][] accumulatedLandUseLogsumsCount) {
	...
	public void createSchoolSegmentNameIndices() {
		...
		for (int gsDist : gsDistrictSet) {
			if (gsDist > UNIFIED_DISTRICT_OFFSET)
				segmentName = SCHOOL_DC_SIZE_SEGMENT_NAME_LIST[UNIFIED_GRADE_SCHOOL_SEGMENT_GROUP_INDEX]
						+ "_" + (gsDist - UNIFIED_DISTRICT_OFFSET);
			else
				segmentName = SCHOOL_DC_SIZE_SEGMENT_NAME_LIST[GRADE_SCHOOL_SEGMENT_GROUP_INDEX]
						+ "_" + gsDist;
			segmentNames.add(segmentName);
			schoolSegmentIndexNameMap.put(sizeSegmentIndex, segmentName);
			schoolSegmentNameIndexMap.put(segmentName, sizeSegmentIndex);
		...
		for (int hsDist : hsDistrictSet) {
			if (hsDist > UNIFIED_DISTRICT_OFFSET)
				segmentName = SCHOOL_DC_SIZE_SEGMENT_NAME_LIST[UNIFIED_HIGH_SCHOOL_SEGMENT_GROUP_INDEX]
						+ "_" + (hsDist - UNIFIED_DISTRICT_OFFSET);
			else
				segmentName = SCHOOL_DC_SIZE_SEGMENT_NAME_LIST[HIGH_SCHOOL_SEGMENT_GROUP_INDEX]
						+ "_" + hsDist;
			segmentNames.add(segmentName);
			schoolSegmentIndexNameMap.put(sizeSegmentIndex, segmentName);
			schoolSegmentNameIndexMap.put(segmentName, sizeSegmentIndex);
		...
		nonUniversitySegments = new int[segmentNames.size()
				- universitySegments.length];
	...
	public int getEscortSizeArraySegmentIndex() {
		return nonMandatorySizeSegmentNameIndexMap
				.get(ModelStructure.ESCORT_PRIMARY_PURPOSE_NAME);
	}
