...
*/
    private static final String[]       WORK_OCCUP_SEGMENT_NAME_LIST                     = {
        "Management Business Science and Arts", "Services", "Sales and Office", "Natural Resources Construction and Maintenance", "Production Transportation and Material Moving", "Military"  };
    private static final int[]          WORK_OCCUP_SEGMENT_VALUE_LIST                    = {
        50, 51, 53, 54, 55, 56                                                               };
	...
	public static final int[] EXTERNAL_LUZS = { 
		230, 231, 232, 233, 234, 235, 236
	};
	
	public static final int[] EXTERNAL_LUZ_CORDON_LUZS = { 
		156, 156, 213, 157, 122, 156, 156
	};
	...
	public static final int[] CORDON_LUZS = {
		122, 156, 157, 213
	};
	
	public static final int[][] EXTERNAL_LUZS_FOR_CORDON_LUZ = { 
		{ 234 },
		{ 230, 231, 235, 236 }, 
		{ 233 },
		{ 232 }
    ...
    public void setupBuildAccessibilities(HashMap<String, String> rbMap, boolean calculateLuAccessibilities)
    {
...
//            }
    ...
    public void calculateDCUtilitiesDistributed(HashMap<String, String> rbMap)
    {
        ...
        if ( landUseLogsums != null ) {
        	...
        	useSimpleMethod = Util.getBooleanValueFromPropertyMap( rbMap, "lu.acc.simple.averaging.method" );
        	
        	boolean useLogitMethod = false;
        	useLogitMethod = Util.getBooleanValueFromPropertyMap( rbMap, "lu.acc.logit.averaging.method" );
        	
        	// output land use data
	        String luAccFileName = projectDirectory + Util.getStringValueFromPropertyMap(rbMap, "lu.acc.output.file");
	        float[][][] luzAccessibilities = aggregateMgraAccessibilitiesToLuz( landUseAccessibilities );
	        if ( useSimpleMethod )
	        	accessibilitiesTableObject.writeLandUseAccessibilityTableToFile( luAccFileName.substring( 0, luAccFileName.indexOf('.') ) + "_simple" + ".csv", luzAccessibilities[SIMPLE] );
	        if ( useLogitMethod )
	        	accessibilitiesTableObject.writeLandUseAccessibilityTableToFile( luAccFileName.substring( 0, luAccFileName.indexOf('.') ) + "_logit" + ".csv", luzAccessibilities[LOGIT] );
	
	        // output land use mode choice logsums
	        String luMcLogsumsFileName = projectDirectory + Util.getStringValueFromPropertyMap(rbMap, "lu.acc.mc.logsums.output.file");
	        if ( useSimpleMethod )
	        	accessibilitiesTableObject.writeLandUseLogsumTablesToFile( luMcLogsumsFileName.substring( 0, luMcLogsumsFileName.indexOf('.') ) + "_simple" + ".csv", landUseLogsums[SIMPLE] );
	        if ( useLogitMethod )
	        	accessibilitiesTableObject.writeLandUseLogsumTablesToFile( luMcLogsumsFileName.substring( 0, luMcLogsumsFileName.indexOf('.') ) + "_logit" + ".csv", landUseLogsums[LOGIT] );
            ...
            if ( externalLuzsForCordonLuz[luz] != null ) {
            	...
            	for( int exLuz : externalLuzsForCordonLuz[luz] ) {
            		...
            		for ( int j=0; j < numColumns-1; j++ ) {
            			...
            			returnValues[LOGIT][exLuz][j] = (float) Math.log( returnValues[LOGIT][exLuz][j] );
            		}
    ...
    private float[][] submitTasks(ArrayList<int[]> startEndIndexList, HashMap<String, String> rbMap )
    {
        ...
        for (int[] startEndIndices : startEndIndexList)
        {
            ...
            DcUtilitiesTaskJppf task = new DcUtilitiesTaskJppf( 
            	taskIndex, startIndex, endIndex, sovExpUtilities, hovExpUtilities, nMotorExpUtilities,
                hasSizeTerm, expConstants, sizeTerms, workerSizeTerms, luSchoolSizeTerms, 
                externalLuzsForCordonLuz, cordonLuzForExternalLuz, cordonLuzMinutesForExternalLuz,
                rbMap, calculateLuAccessibilities );
        ...
        for (Future<List<Object>> fs : results)
        {
            ...
            try
            {
                ...
                if (landUseLogsums != null) {
                    ...
                    accumulateLandUseModeChoiceLogsums( accumulatedLandUseLogsums, accumulatedLandUseLogsumsCount );
        	...
        	for ( int j=0; j < NUM_SUFFICIENCY_SEGMENTS; j++ ) {        		
            	for ( int k=1; k <= MAX_LUZ; k++ ) {        		
                	for ( int m=1; m <= MAX_LUZ; m++ ) {     
                		landUseLogsums[SIMPLE][i][j][k][m] = landUseLogsums[SIMPLE][i][j][k][m] / landUseLogsumsCount[k][m];
                		landUseLogsums[LOGIT][i][j][k][m] = Math.log( landUseLogsums[LOGIT][i][j][k][m] );
                	}
            	}
        	}
    	}
    ...
    private void accumulateLandUseModeChoiceLogsums( float[][][][][] accumulatedLandUseLogsums, int[][] accumulatedLandUseLogsumsCount ) {
