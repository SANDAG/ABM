...
public final class StopLocationEstimationMcLogsumsAppender extends
		McLogsumsAppender {
	...
	private void writeDcFile(HashMap<String, String> rbMap,
		...
		for (int i = 0; i < estDataOdts.length; i++) {
			...
			outStream.print(seq + "," + odtSet[PORTION] + "," + odtSet[SAMPNO]
					+ "," + odtSet[PERNO] + "," + odtSet[TOUR_ID] + ","
					+ odtSet[TRIPNO] + "," + odtSet[STOPID] + ","
					+ odtSet[STOPNO]);

			int category = PURPOSE_CATEGORIES[odtSet[TOUR_PURPOSE]];

			try {
				calculateTripModeChoiceLogsums(rbMap, mcModel[category],
						mcDmuObject, odtSet, mgraSet);
			} catch (Exception e) {
				logger.error("exception caught processing survey record for i =  "
						+ i);
				throw new RuntimeException();
			}

			outStream.print("," + odtSet[CHOSEN_MGRA]);
			outStream.printf(",%.8f,%.8f", tripModeChoiceLogsums[0][0],
					tripModeChoiceLogsums[0][1]);
