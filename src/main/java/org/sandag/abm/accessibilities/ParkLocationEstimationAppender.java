...
public final class ParkLocationEstimationAppender {
	...
	private transient Logger logger = Logger
			.getLogger(ParkLocationEstimationAppender.class);
	...
	private void runAppender(ResourceBundle rb) {
		...
		AutoAndNonMotorizedSkimsCalculator anm = new AutoAndNonMotorizedSkimsCalculator(
				rbMap);
		...
		for (int i = 0; i < estDataOdts.length; i++) {
			...
			if (odtSet[ORIG] > 0 && odtSet[DEST] > 0) {
				...
				int skimPeriodIndex = modelStructure
						.getSkimPeriodIndex(odtSet[DEPART]) + 1; // depart skim
																	// period
				double[] autoSkims = anm.getAutoSkims(odtSet[ORIG],
						odtSet[DEST], skimPeriodIndex,
				...
				int[] walkMgras = mgraManager
						.getMgrasWithinWalkDistanceFrom(odtSet[ORIG]);
				...
				if (walkMgras != null) {
					...
					for (int wMgra : walkMgras) {
						if (wMgra == odtSet[DEST]) {
							wDist = mgraManager.getMgraToMgraWalkDistFrom(
									odtSet[ORIG], wMgra) / 5280.0;
			...
			outStream.println(seq + "," + odtSet[ID] + "," + odtSet[ORIG] + ","
					+ odtSet[DEST] + "," + odtSet[DEPART] + ","
					+ TYPE_LABELS[odtSet[TYPE]] + "," + aDist + "," + aTime
					+ "," + wDist + "," + wTime);
