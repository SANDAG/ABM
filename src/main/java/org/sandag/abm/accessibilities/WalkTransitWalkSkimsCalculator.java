...
/**
 * This class is used to return walk-transit-walk skim values for MGRA pairs associated with estimation data file records.
 * 
 * @author Jim Hicks
 * @version March, 2010
 */
        ...
        implements Serializable
{
            ...
            BestTransitPathCalculator myBestPathUEC)
    {
        ...
        String uecFileName = uecPath
                + Util.getStringValueFromPropertyMap(rbMap, "skim.walk.transit.walk.uec.file");
        File uecFile = new File(uecFileName);

        walkLocalWalkSkimUECs = new UtilityExpressionCalculator[NUM_PERIODS];
        walkLocalWalkSkimUECs[EA] = new UtilityExpressionCalculator(uecFile, wtwLocSkimEaPage,
                dataPage, rbMap, dmu);
        walkLocalWalkSkimUECs[AM] = new UtilityExpressionCalculator(uecFile, wtwLocSkimAmPage,
                dataPage, rbMap, dmu);
        walkLocalWalkSkimUECs[MD] = new UtilityExpressionCalculator(uecFile, wtwLocSkimMdPage,
                dataPage, rbMap, dmu);
        walkLocalWalkSkimUECs[PM] = new UtilityExpressionCalculator(uecFile, wtwLocSkimPmPage,
                dataPage, rbMap, dmu);
        walkLocalWalkSkimUECs[EV] = new UtilityExpressionCalculator(uecFile, wtwLocSkimEvPage,
                dataPage, rbMap, dmu);

        walkPremiumWalkSkimUECs = new UtilityExpressionCalculator[NUM_PERIODS];
        walkPremiumWalkSkimUECs[EA] = new UtilityExpressionCalculator(uecFile, wtwPremSkimEaPage,
                dataPage, rbMap, dmu);
        walkPremiumWalkSkimUECs[AM] = new UtilityExpressionCalculator(uecFile, wtwPremSkimAmPage,
                dataPage, rbMap, dmu);
        walkPremiumWalkSkimUECs[MD] = new UtilityExpressionCalculator(uecFile, wtwPremSkimMdPage,
                dataPage, rbMap, dmu);
        walkPremiumWalkSkimUECs[PM] = new UtilityExpressionCalculator(uecFile, wtwPremSkimPmPage,
                dataPage, rbMap, dmu);
        walkPremiumWalkSkimUECs[EV] = new UtilityExpressionCalculator(uecFile, wtwPremSkimEvPage,
                dataPage, rbMap, dmu);
    ...
    /**
     * Return the array of walk-transit best tap pairs for the given origin MGRA, destination MGRA, and departure time period.
     * 
     * @param origMgra
     *            Origin MGRA
     * @param workMgra
    ...
    /**
     * Return the array of walk-transit skims for the ride mode, origin TAP, destination TAP, and departure time period.
     * 
     * @param rideModeIndex
     ...
     * @param departPeriod
     *            skim period index for the departure period - 0 = AM period, 1 = PM period, 2 = OffPeak period
     * @return Array of skim values for the MGRA pair and departure period for the ride mode type - local or premium
     */
    public double[] getWalkTransitWalkSkims(int rideModeIndex, double pWalkTime, double aWalkTime,
            int origTap, int destTap, int departPeriod, boolean debug)
    {
        ...
        if (Modes.getIsPremiumTransit(rideModeIndex))
        {
            ...
            {
                logger.error("departPeriod=" + departPeriod + ", origTap=" + origTap + ", destTap="
                        + destTap + ", pWalkTime=" + pWalkTime);
                logger.error(
    ...
    public static void main(String[] args)
    {
        ...
        for (int[] odt : testOdts)
        {
            ...
            for (int i = 0; i < bestTapPairs.length; i++)
            {
                ...
                {
                    returnedSkims[i] = wtw.getWalkTransitWalkSkims(i, logsumHelper
                            .getBestTransitPathCalculator().getBestAccessTime(i), logsumHelper
                            .getBestTransitPathCalculator().getBestAccessTime(i),
