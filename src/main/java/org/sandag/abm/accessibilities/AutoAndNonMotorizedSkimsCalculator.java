 ...
 * @version March, 2010
 */
public class AutoAndNonMotorizedSkimsCalculator implements Serializable {

	private transient Logger logger = Logger
			.getLogger(AutoAndNonMotorizedSkimsCalculator.class);
	...
	private static final String[] AUTO_SKIM_NAMES = { "DA_NT_Time",
			"DA_NT_FFTime", "DA_NT_Dist", "DA_TL_Time", "DA_TL_FFTime",
			"DA_TL_Dist", "DA_TL_Value", "DA_TL_FacLen", "HOV_NT_Time",
			"HOV_NT_FFTime", "HOV_NT_Dist", "HOV_NT_FacLen" };
	...
	private static final String[] NM_SKIM_DESCRIPTIONS = { "walk time",
			"bike time" };
	...
	public AutoAndNonMotorizedSkimsCalculator(HashMap<String, String> rbMap) {
		...
		int dataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"skims.auto.data.page");
		int autoSkimEaPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"skims.auto.ea.page");
		int autoSkimAmPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"skims.auto.am.page");
		int autoSkimMdPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"skims.auto.md.page");
		int autoSkimPmPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"skims.auto.pm.page");
		int autoSkimEvPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"skims.auto.ev.page");

		File uecFile = new File(uecFileName);
		autoSkimUECs = new UtilityExpressionCalculator[NUM_PERIODS];
		autoSkimUECs[EA] = new UtilityExpressionCalculator(uecFile,
				autoSkimEaPage, dataPage, rbMap, dmu);
		autoSkimUECs[AM] = new UtilityExpressionCalculator(uecFile,
				autoSkimAmPage, dataPage, rbMap, dmu);
		autoSkimUECs[MD] = new UtilityExpressionCalculator(uecFile,
				autoSkimMdPage, dataPage, rbMap, dmu);
		autoSkimUECs[PM] = new UtilityExpressionCalculator(uecFile,
				autoSkimPmPage, dataPage, rbMap, dmu);
		autoSkimUECs[EV] = new UtilityExpressionCalculator(uecFile,
				autoSkimEvPage, dataPage, rbMap, dmu);
		iv = new IndexValues();
	 ...
	 */
	public double[] getAutoSkims(int origMgra, int destMgra, int departPeriod,
		...
		if (debug) {
			logger.info("");
			logger.info("");
			header = "get auto skims debug info for origMgra=" + origMgra
					+ ", destMgra=" + destMgra + ", period index="
		...
		if (origMgra > 0 && destMgra > 0) {
			...
			if (debug)
				autoSkimUEC.logAnswersArray(logger, String.format(
						"autoSkimUEC:  oMgra=%d, dMgra=%d, period=%d",
						origMgra, destMgra, departPeriod));
	 ...
	 */
	public double[] getNonMotorizedSkims(int origMgra, int destMgra,
			int departPeriod, boolean debug, Logger logger) {
		...
		if (debug) {
			logger.info("");
			logger.info("");
			header = "get non-motorized skims debug info for origMgra="
					+ origMgra + ", destMgra=" + destMgra;
			for (int i = 0; i < header.length(); i++)
				separator += "^";
		}
		...
		if (walkMgras != null) {
			...
			for (int wMgra : walkMgras) {
				...
				if (wMgra == destMgra) {
					nmSkims[WALK_INDEX] = mgraManager.getMgraToMgraWalkTime(
							origMgra, destMgra);
					nmSkims[BIKE_INDEX] = mgraManager.getMgraToMgraBikeTime(
							origMgra, destMgra);
					...
					if (debug) {
						...
						logger.info(String.format("%5s %40s %15s", "i",
								"skimName", "value"));
						logger.info(String.format("%5s %40s %15s", "-----",
								"----------", "----------"));
	...
	public void getDistancesFromMgra(int origMgra, double[] distances,
			boolean tourModeIsAuto) {
		...
		if (!tourModeIsAuto) {
			...
			int[] walkMgras = mgraManager
					.getMgrasWithinWalkDistanceFrom(origMgra);
			...
			if (walkMgras != null) {
				...
				for (int wMgra : walkMgras)
					distances[wMgra] = mgraManager.getMgraToMgraWalkDistFrom(
							origMgra, wMgra) / 5280.0;
	...
	public void getDistancesToMgra(int destMgra, double[] distances,
			boolean tourModeIsAuto) {
		...
		if (!tourModeIsAuto) {
			...
			// get the array of mgras within walking distance of the destination
			int[] walkMgras = mgraManager
					.getMgrasWithinWalkDistanceTo(destMgra);
			...
			if (walkMgras != null) {
				...
				for (int wMgra : walkMgras)
					distances[wMgra] = mgraManager.getMgraToMgraWalkDistTo(
							destMgra, wMgra) / 5280.0;
	 ...
	 */
	public void logReturnedSkims(int orig, int dest, int depart,
			double[] skims, String skimLabel, Logger logger) {
		...
		logger.info("");
		logger.info("");
		header = skimLabel + " skim value tables for origMgra=" + orig
				+ ", destMgra=" + dest + ", departperiod=" + depart;
		for (int i = 0; i < header.length(); i++)
			separator += "^";
