...
public class StoredUtilityData {
	...
	public static synchronized StoredUtilityData getInstance(int maxMgra,
			int maxTap, int maxTaz, int numAccEgrSegments, int numPeriods) {
		if (objInstance == null) {
			objInstance = new StoredUtilityData();
			objInstance.setupStoredDataArrays(maxMgra, maxTap, maxTaz,
					numAccEgrSegments, numPeriods);
			return objInstance;
		} else {
			return objInstance;
		}
	...
	private void setupStoredDataArrays(int maxMgra, int maxTap, int maxTaz,
