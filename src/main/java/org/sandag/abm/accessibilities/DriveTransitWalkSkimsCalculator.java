 ...
 * @version March, 2010
 */
public class DriveTransitWalkSkimsCalculator implements Serializable {
	...
	public DriveTransitWalkSkimsCalculator() {
		...
		storedDepartPeriodTapTapSkims = storedDataObject
				.getStoredDtwDepartPeriodTapTapSkims();
	}
	...
	public void setup(HashMap<String, String> rbMap, Logger logger,
		...
		String uecFileName = uecPath
				+ Util.getStringValueFromPropertyMap(rbMap,
						"skim.drive.transit.walk.uec.file");
		...
		driveLocalWalkSkimUECs[EA] = new UtilityExpressionCalculator(uecFile,
				dtwLocSkimEaPage, dataPage, rbMap, dmu);
		driveLocalWalkSkimUECs[AM] = new UtilityExpressionCalculator(uecFile,
				dtwLocSkimAmPage, dataPage, rbMap, dmu);
		driveLocalWalkSkimUECs[MD] = new UtilityExpressionCalculator(uecFile,
				dtwLocSkimMdPage, dataPage, rbMap, dmu);
		driveLocalWalkSkimUECs[PM] = new UtilityExpressionCalculator(uecFile,
				dtwLocSkimPmPage, dataPage, rbMap, dmu);
		driveLocalWalkSkimUECs[EV] = new UtilityExpressionCalculator(uecFile,
				dtwLocSkimEvPage, dataPage, rbMap, dmu);

		drivePremiumWalkSkimUECs = new UtilityExpressionCalculator[NUM_PERIODS];
		drivePremiumWalkSkimUECs[EA] = new UtilityExpressionCalculator(uecFile,
				dtwPremSkimEaPage, dataPage, rbMap, dmu);
		drivePremiumWalkSkimUECs[AM] = new UtilityExpressionCalculator(uecFile,
				dtwPremSkimAmPage, dataPage, rbMap, dmu);
		drivePremiumWalkSkimUECs[MD] = new UtilityExpressionCalculator(uecFile,
				dtwPremSkimMdPage, dataPage, rbMap, dmu);
		drivePremiumWalkSkimUECs[PM] = new UtilityExpressionCalculator(uecFile,
				dtwPremSkimPmPage, dataPage, rbMap, dmu);
		drivePremiumWalkSkimUECs[EV] = new UtilityExpressionCalculator(uecFile,
				dtwPremSkimEvPage, dataPage, rbMap, dmu);
	...
	public int[][] getBestTapPairs(int origMgra, int destMgra,
		...
		if (debug) {
			logger.info("");
			logger.info("");
			header = "best drive-transit-walk tap pairs debug info for origMgra="
					+ origMgra
					+ ", destMgra="
					+ destMgra
					+ ", period index="
		...
		bestPathUEC.findBestDriveTransitWalkTaps(departPeriod, origMgra,
				destMgra, debug, primaryLogger);
		...
		if (debug) {
			...
			for (int i = 0; i < bestUtilities.length; i++) {
				if (bestTaps[i] == null) {
					...
					logValues[2] = Double.toString(bestPathUEC
							.getBestAccessTime(i));
					logValues[3] = Double.toString(bestPathUEC
							.getBestEgressTime(i));
					availableModeCount++;
				}

				logger.info(i + "," + mode[i] + "," + bestUtilities[i] + ","
						+ logValues[0] + "," + logValues[1] + ","
						+ logValues[2] + "," + logValues[3]);
			}
	 ...
	 */
	public double[] getDriveTransitWalkSkims(int rideModeIndex,
			double pDriveTime, double aWalkTime, int origTap, int destTap,
		...
		if (Modes.getIsPremiumTransit(rideModeIndex)) {
			...
			if (storedDepartPeriodTapTapSkims[PREM][departPeriod][origTap][destTap] == null) {
				double[] results = drivePremiumWalkSkimUECs[departPeriod]
						.solve(iv, dmu, null);
				if (debug)
					drivePremiumWalkSkimUECs[departPeriod].logAnswersArray(
							primaryLogger, "Drive-Premium-Walk Skims");
			...
			try {
				...
				primaryLogger.error("departPeriod=" + departPeriod
						+ ", origTap=" + origTap + ", destTap=" + destTap
						+ ", pDriveTime=" + pDriveTime);
				primaryLogger
			...
			if (storedDepartPeriodTapTapSkims[LOC][departPeriod][origTap][destTap] == null) {
				double[] results = driveLocalWalkSkimUECs[departPeriod].solve(
						iv, dmu, null);
				if (debug)
					driveLocalWalkSkimUECs[departPeriod].logAnswersArray(
							primaryLogger, "Drive-Local-Walk Skims");
	...
	private void startMatrixServer(ResourceBundle rb) {
		...
		primaryLogger.info("connecting to matrix server " + serverAddress + ":"
				+ serverPort);
		...
		try {
			...
			primaryLogger
					.error(String
							.format("exception caught running ctramp model components -- exiting."),
							e);
	...
	public void logReturnedSkims(int[] odt, int[][] bestTapPairs,
		...
		primaryLogger.info("");
		primaryLogger.info("");
		header = "Returned drive-transit-walk skim value tables for origMgra="
		...
		for (int i = 1; i < bestTapPairs.length; i++) {
			...
			tapHeading += String.format("   %4s-%4s   ", logValues[0],
					logValues[1]);
		}
	...
	public static void main(String[] args) {
		...
		for (int[] odt : testOdts) {
			int[][] bestTapPairs = dtw.getBestTapPairs(odt[0], odt[1], odt[2],
					true, logger);
