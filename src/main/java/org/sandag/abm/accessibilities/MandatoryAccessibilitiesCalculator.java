 ...
 * @version May, 2009
 */
public class MandatoryAccessibilitiesCalculator implements Serializable {

	protected transient Logger logger = Logger
			.getLogger(MandatoryAccessibilitiesCalculator.class);
	...
	public MandatoryAccessibilitiesCalculator(HashMap<String, String> rbMap,
		...
		// Create the UECs
		String uecFileName = Util.getStringValueFromPropertyMap(rbMap,
				"acc.mandatory.uec.file");
		int dataPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.mandatory.data.page");
		int autoSkimPage = Util.getIntegerValueFromPropertyMap(rbMap,
				"acc.mandatory.auto.page");
		...
		autoSkimUEC = new UtilityExpressionCalculator(uecFile, autoSkimPage,
				dataPage, rbMap, dmu);
		bestWalkTransitUEC = new UtilityExpressionCalculator(uecFile,
				bestWalkTransitPage, dataPage, rbMap, dmu);
		bestDriveTransitUEC = new UtilityExpressionCalculator(uecFile,
				bestDriveTransitPage, dataPage, rbMap, dmu);
		autoLogsumUEC = new UtilityExpressionCalculator(uecFile,
				autoLogsumPage, dataPage, rbMap, dmu);
		transitLogsumUEC = new UtilityExpressionCalculator(uecFile,
				transitLogsumPage, dataPage, rbMap, dmu);
	...
	public double[] calculateWorkerMandatoryAccessibilities(int hhMgra,
			int workMgra) {
		return calculateAccessibilitiesForMgraPair(hhMgra, workMgra, false,
				null);
	}

	public double[] calculateStudentMandatoryAccessibilities(int hhMgra,
			int schoolMgra) {
		return calculateAccessibilitiesForMgraPair(hhMgra, schoolMgra, false,
				null);
	}
	 ...
	 */
	public double calculateWorkLogsum(int hhMgra, int workMgra,
			int autoSufficiency, boolean debug, Logger aLogger) {
		...
		if (debug) {
			aLogger.info("");
			aLogger.info("");
			header = "calculateWorkLogsum() debug info for homeMgra=" + hhMgra
					+ ", workMgra=" + workMgra;
			for (int i = 0; i < header.length(); i++)
				separator += "^";
		}

		double[] accessibilities = calculateAccessibilitiesForMgraPair(hhMgra,
				workMgra, debug, aLogger);
		...
		double utilSum = Math.exp(sovUtility)
				* expConstants[autoSufficiency][0] + Math.exp(hovUtility)
				* expConstants[autoSufficiency][1] + Math.exp(transitLogsum)
		...
		if (debug) {
			...
			aLogger.info(String
					.format("%5s %15s %15s", "i", "accName", "value"));
			aLogger.info(String.format("%5s %15s %15s", "-----", "----------",
					"----------"));
			for (int i = 0; i < accessibilities.length; i++) {
				aLogger.info(String.format("%5d %15s %15.5e", i, accNames[i],
						accessibilities[i]));
			}
			...
			aLogger.info(String.format(
					"%-15s = %15.5e, %-18s = %15.5e, %-18s = %15.5e",
					"sovUtility", sovUtility, "exp(sovUtility)",
					Math.exp(sovUtility),
					String.format("expConst suff=%d 0", autoSufficiency),
					expConstants[autoSufficiency][0]));
			aLogger.info(String.format(
					"%-15s = %15.5e, %-18s = %15.5e, %-18s = %15.5e",
					"hovUtility", hovUtility, "exp(hovUtility)",
					Math.exp(hovUtility),
					String.format("expConst suff=%d 1", autoSufficiency),
					expConstants[autoSufficiency][1]));
			aLogger.info(String.format(
					"%-15s = %15.5e, %-18s = %15.5e, %-18s = %15.5e",
					"transitLogsum", transitLogsum, "exp(transitLogsum)",
					Math.exp(transitLogsum),
					String.format("expConst suff=%d 2", autoSufficiency),
					expConstants[autoSufficiency][2]));
			aLogger.info(String.format("%-15s = %15.5e, %-18s = %15.5e",
					"nmExpUtility", nmExpUtility,
					String.format("expConst suff=%d 3", autoSufficiency),
					expConstants[autoSufficiency][3]));
	 ...
	 */
	public double calculateSchoolLogsum(int hhMgra, int schoolMgra,
			int autoSufficiency, int studentType, boolean debug, Logger aLogger) {
		...
		if (debug) {
			aLogger.info("");
			aLogger.info("");
			header = "calculateSchoolLogsum() debug info for homeMgra="
					+ hhMgra + ", schoolMgra=" + schoolMgra;
			for (int i = 0; i < header.length(); i++)
				separator += "^";
		}

		double[] accessibilities = calculateAccessibilitiesForMgraPair(hhMgra,
				schoolMgra, debug, aLogger);

		double sovUtility = accessibilities[4];
		double hovUtility = accessibilities[12];
		double transitLogsum = accessibilities[8]; // includes both walk and
													// drive access
		double nmExpUtility = ntUtilities.getNMotorExpUtility(hhMgra,
				schoolMgra, NonTransitUtilities.OFFPEAK_PERIOD_INDEX);
		...
		if (studentType >= 2) {
			logsum = logsum + Math.exp(sovUtility)
					* expConstants[autoSufficiency][0];
		}
		...
		if (debug) {
			...
			aLogger.info(String.format("%-15s = %15.5e, %-18s = %15.5e",
					...
					String.format("expConst suff=%d 3", autoSufficiency),
					expConstants[autoSufficiency][3]));
			aLogger.info(String.format("%s = %15.5e",
					"utility sum (before adding sovUtility)", logsum1));
			if (studentType >= 2) {
				...
				aLogger.info(String.format(
						...
						expConstants[autoSufficiency][0]));
				aLogger.info(String.format("%s = %15.5e",
						"utility sum (after adding sovUtility)", logsum2));
			} else {
				aLogger.info(String
						.format("studentType = %d, no additional contribution to utility sum",
								studentType));
			}
			aLogger.info(String.format("%s = %15.5e, %s = %15.5e",
					"final utility sum", logsum2, "final logsum", logsum));
			aLogger.info(separator);
		}
	...
	public double[] calculateAccessibilitiesForMgraPair(int oMgra, int dMgra,
		...
		if (oMgra > 0 && dMgra > 0) {
			...
			if (debug)
				autoSkimUEC.logAnswersArray(aLogger, String.format(
						"autoSkimUEC:  oMgra=%d, dMgra=%d", oMgra, dMgra));
			...
			bestPathCalculator.findBestWalkTransitWalkTaps(
					TransitWalkAccessUEC.AM, oMgra, dMgra, debug, aLogger);

			// sum the exponentiated utilities over modes
			double sumWlkExpUtilities = 0;
			double[] walkTransitWalkUtilities = bestPathCalculator
					.getBestUtilities();
			...
			Modes.TransitMode bestMode = bestPathCalculator
					.getBestTransitMode();

			if (bestMode != null) {
				int[] bestTaps = bestPathCalculator.getBestTaps(bestMode);
				dmu.setBestMode(bestMode.ordinal());
				int oTapPosition = mgraManager.getTapPosition(oMgra,
						bestTaps[0]);
				int dTapPosition = mgraManager.getTapPosition(dMgra,
						bestTaps[1]);

				if (oTapPosition == -1 || dTapPosition == -1) {
					logger.fatal("Error:  Best walk mode " + bestMode
							+ " found for origin mgra " + oMgra
							+ " to destination mgra " + dMgra
							+ " but oTap pos " + oTapPosition
							+ " and dTap pos " + dTapPosition);
				...
				accessibilities[5] = Math.log(walkTransitWalkUtilities[bestMode
						.ordinal()]);

				dmu.setMgraTapWalkTime((float) bestPathCalculator
						.getBestAccessTime(bestMode.ordinal()));
				dmu.setTapMgraWalkTime((float) bestPathCalculator
						.getBestEgressTime(bestMode.ordinal()));
				iv.setOriginZone(bestTaps[0]);
				iv.setDestZone(bestTaps[1]);
				double[] wlkTransitTimes = bestWalkTransitUEC.solve(iv, dmu,
						null);

				if (debug) {
					bestWalkTransitUEC.logAnswersArray(aLogger, String.format(
							"bestWalkTransitUEC:  oMgra=%d, dMgra=%d", oMgra,
							dMgra));
				}
			...
			// determine the best transit path, which also stores the best
			// utilities array and the best mode
			bestPathCalculator.findBestDriveTransitWalkTaps(
					TransitWalkAccessUEC.AM, oMgra, dMgra, debug, aLogger);

			// sum the exponentiated utilities over modes
			double sumDrvExpUtilities = 0;
			double[] driveTransitWalkUtilities = bestPathCalculator
					.getBestUtilities();
			for (int i = 0; i < driveTransitWalkUtilities.length; i++) {
				if (driveTransitWalkUtilities[i] > MIN_EXP_FUNCTION_ARGUMENT)
					sumDrvExpUtilities += Math
							.exp(driveTransitWalkUtilities[i]);
			}
			...
			if (bestMode != null) {
				...
				dmu.setDriveTimeToTap((float) bestPathCalculator
						.getBestAccessTime(bestMode.ordinal()));
				...
				if (debug) {
					bestDriveTransitUEC.logAnswersArray(aLogger, String.format(
							"bestDriveTransitUEC:  oMgra=%d, dMgra=%d", oMgra,
							dMgra));
				}
			...
			double[] transitLogsumResults = transitLogsumUEC.solve(iv, dmu,
					null);
			if (debug) {
				transitLogsumUEC.logAnswersArray(aLogger, String.format(
						"transitLogsumUEC:  oMgra=%d, dMgra=%d", oMgra, dMgra));
			}
