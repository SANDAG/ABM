...
/**
 * This class is the TOD choice model for IE tours.  It is currently based on a static probability distribution
 * stored in an input file, and indexed into by purpose.  Since there are no IE purposes, the purpose is 0.
 * 
 * @author Freedman
 *
 */
public class InternalExternalTourTimeOfDayChoiceModel {
	...
	public InternalExternalTourTimeOfDayChoiceModel(HashMap<String,String> rbMap ){
        ...
        String stationDiurnalFile = Util.getStringValueFromPropertyMap(rbMap,"internalExternal.tour.tod.file");
	...
	public void calculateTourTOD(InternalExternalTour tour){
		...
		if(tour.getDebugChoiceModels()){
			logger.info("Choosing tour time of day for tour ID "+tour.getID()+" using random number "+random);
