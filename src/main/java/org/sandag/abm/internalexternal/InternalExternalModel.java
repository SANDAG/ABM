...
public class InternalExternalModel {
	...
	public void runModel(){
		...
		InternalExternalTourTimeOfDayChoiceModel todChoiceModel = new InternalExternalTourTimeOfDayChoiceModel(rbMap);
        InternalExternalTourDestChoiceModel destChoiceModel = new InternalExternalTourDestChoiceModel(rbMap, modelStructure, dmuFactory );
	    destChoiceModel.calculateTazProbabilities(dmuFactory);
        
        InternalExternalTripModeChoiceModel tripModeChoiceModel = new InternalExternalTripModeChoiceModel(rbMap, modelStructure, dmuFactory,logsumsCalculator );
		...
		for(int i = 0; i<tours.length;++i){
        	...
        	//generate trips and choose mode for them - note this assumes two trips per tour
	...
	public static void main(String[] args) {
        ...
        //sampleRate is not relevant for internal-external model, since sampling
        ...
        try
        {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
            {
                ...
                if (matrixServerAddress.equalsIgnoreCase("localhost")) {
                    matrixServer = internalExternalModel.startMatrixServerProcess(matrixServerAddress, serverPort, mt);
                    internalExternalModel.ms = matrixServer;
                }
                else {
                	internalExternalModel.ms = new MatrixDataServerRmi( matrixServerAddress, serverPort, MatrixDataServer.MATRIX_DATA_SERVER_NAME );
                	internalExternalModel.ms.testRemote(Thread.currentThread().getName());
        ...
        if (matrixServerAddress.equalsIgnoreCase("localhost")) {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
                internalExternalModel.ms.stop32BitMatrixIoServer();
        }
