...
public class InternalExternalTourManager {
	 ...
	 */
	public InternalExternalTourManager(HashMap<String, String> rbMap,
			int iteration) {
		...
		String personFile = Util.getStringValueFromPropertyMap(rbMap,
				"Results.PersonDataFile");
		// Remove extension from filename
		String extension = getFileExtension(personFile);
		personFile = removeFileExtension(personFile) + iterationString
				+ extension;
		...
		householdFile = removeFileExtension(householdFile) + iterationString
				+ extension;
	...
	public void generateTours() {
		...
		for (int i = 1; i <= rows; ++i) {
			...
			if (((int) personData.getValueAt(i, "ie_choice")) == 2) {
				...
				String gender = (String) personData.getStringValueAt(i,
						"gender");
	...
	private void writeTrip(InternalExternalTrip trip, PrintWriter writer) {
		...
		String record = new String(trip.getOriginMgra() + ","
	...
	public static void main(String args[]) {
		...
		InternalExternalTourManager apm = new InternalExternalTourManager(pMap,
				1);
		apm.generateTours();
