...
public class CompositeShortestPathResultSet<N extends Node>
        implements ShortestPathResultSet<N>
{
        ...
        for (NodePair<N> nodePair : spResults)
            if (spResultsLookup.put(nodePair, spResults) != null)
                throw new IllegalArgumentException(
                        "Repeated shortest path results for node pair: ("
                                + nodePair.getFromNode().getId() + ","
                                + nodePair.getToNode().getId() + ")");
    }
