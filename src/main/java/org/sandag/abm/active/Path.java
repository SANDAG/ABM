        ...
        implements Iterable<N>
{
    ...
    public N getNode(int index)
    {
        if (index < 0 || index >= length)
            throw new IllegalArgumentException("Invalid index " + index + " for path of length "
                    + length);
        return getNodeNoChecks(index + 1);
    }

    private N getNodeNoChecks(int index)
    { // index here is 1-based, not zero based!
    ...
    public Iterator<N> iterator()
    {
        return new Iterator<N>()
        {
            ...
            public N next()
            {
                if (point < length) return getNodeNoChecks(++point);
