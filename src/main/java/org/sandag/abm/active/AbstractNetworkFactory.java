...
public abstract class AbstractNetworkFactory<N extends Node, E extends Edge<N>, T extends Traversal<E>>
        extends NetworkFactory<N, E, T>
{
    ...
    protected Collection<T> getTraversals()
    {
        ...
        for (E toEdge : getEdges())
        {
            for (E fromEdge : predecessors.get(toEdge.getFromNode()))
                if (!isReversal(fromEdge, toEdge)) traversals.add(getTraversal(fromEdge, toEdge));
        }
