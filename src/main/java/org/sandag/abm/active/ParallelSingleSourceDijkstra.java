...
public class ParallelSingleSourceDijkstra<N extends Node>
        implements ShortestPathStrategy<N>
{
            ...
            double maxCost)
    {
        switch (method)
        {
            case FORK_JOIN:
            {
                ShortestPathRecursiveTask task = new ShortestPathRecursiveTask(sp, originNodes,
                        destinationNodes, maxCost);
            ...
            case QUEUE:
            {
                ...
                for (int i = 0; i < threadCount; i++)
                    executor.execute(new QueueMethodTask(sp, originNodeQueue, destinationNodes,
                            maxCost, counter, sprcThreadLocal, latch));
                try
                {
                    latch.await();
                ...
                for (ModifiableShortestPathResultSet<N> sprc : sprcQueue)
                    if (finalContainer == null) finalContainer = sprc;
                    else finalContainer.addAll(sprc);
            ...
            implements Runnable
    {
        ...
        private QueueMethodTask(ShortestPathStrategy<N> sp, Queue<N> originNodes,
                Set<N> destinationNodes, double maxCost, AtomicInteger counter,
                ThreadLocal<ModifiableShortestPathResultSet<N>> spr, CountDownLatch latch)
        {
            this.sp = sp;
        ...
        public void run()
        {
            ...
            while (originNodes.size() > 0)
            {
                ...
                ShortestPathResultSet<N> result = sp.getShortestPaths(origins, destinationNodes,
                        maxCost);
                ...
                if (c % segmentSize < origins.size())
                    System.out.println("   done with " + ((c / segmentSize) * segmentSize)
                            + " origins");
                origins.clear();
            }
            latch.countDown();
    ...
    private class ShortestPathRecursiveTask
            extends DnCRecursiveTask<ModifiableShortestPathResultSet<N>>
    {
        ...
        protected ShortestPathRecursiveTask(ShortestPathStrategy<N> sp, Set<N> origins,
                Set<N> destinations, double maxCost)
        {
        ...
        protected ShortestPathRecursiveTask(long start, long length,
                DnCRecursiveTask<ModifiableShortestPathResultSet<N>> next,
                ShortestPathStrategy<N> sp, Node[] origins, Set<N> destinations, double maxCost,
                AtomicInteger counter)
        {
        ...
        // origins only hold N, we just can't declare as such because of
        // generics
        ...
        @Override
        protected DnCRecursiveTask<ModifiableShortestPathResultSet<N>> getNextTask(long start,
                long length, DnCRecursiveTask<ModifiableShortestPathResultSet<N>> next)
        {
            return new ShortestPathRecursiveTask(start, length, next, sp, origins, destinations,
                    maxCost, counter);
        }

        @Override
        protected ModifiableShortestPathResultSet<N> joinResults(
                ModifiableShortestPathResultSet<N> spr1, ModifiableShortestPathResultSet<N> spr2)
        {
