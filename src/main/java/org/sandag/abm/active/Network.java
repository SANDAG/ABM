...
public interface Network<N extends Node, E extends Edge<N>, T extends Traversal<E>>
{
    ...
    // throw new RuntimeException("Network already contains Node with id " +
    // node.getId());
    // }
    // nodeIndex.put(node.getId(), nodes.size());
    // nodes.add(node);
    // if (! successorIndex.containsKey(node.getId()) ) {
    // successorIndex.put(node.getId(), new ArrayList<Integer>()); }
    // if (! predecessorIndex.containsKey(node.getId()) ) {
    // predecessorIndex.put(node.getId(), new ArrayList<Integer>()); }
    // }
    //
    ...
    // throw new RuntimeException("Network already contains Edge with fromId " +
    // edge.getFromNode() + " and toId " + edge.getToNode());
    // }
    //
    // edgeIndex.put(edgeIndexKey, edges.size());
    // edges.add(edge);
    //
    // if ( ! successorIndex.containsKey(fromId) ) { successorIndex.put(fromId,
    // new ArrayList<Integer>()); }
    // if ( ! predecessorIndex.containsKey(toId) ) { predecessorIndex.put(toId,
    // new ArrayList<Integer>()); }
    //
    // if ( ! successorIndex.get(fromId).contains(toId) ) {
    // successorIndex.get(fromId).add(toId); }
    // if ( ! predecessorIndex.get(toId).contains(fromId) ) {
    // predecessorIndex.get(toId).add(fromId); }
    // }
    //
    ...
    // TraversalKey traversalIndexKey = new TraversalKey(startId, thruId,
    // endId);
    //
    ...
    // return traversalIndex.containsKey(new
    // TraversalKey(ids[0],ids[1],ids[2]));
    // }
    //
