...
public class SandagBikePathChoiceLogsumMatrixApplication extends AbstractPathChoiceLogsumMatrixApplication<SandagBikeNode,SandagBikeEdge,SandagBikeTraversal>
{
    		                                           ...
    		                                           final Map<String,String> propertyMap)
    {
        ...
        debugOrigin = propertyMap.containsKey(PROPERTIES_DEBUG_ORIGIN) ? Integer.parseInt(this.propertyMap.get(PROPERTIES_DEBUG_ORIGIN)) : -1;
        debugDestination = propertyMap.containsKey(PROPERTIES_DEBUG_DESTINATION) ? Integer.parseInt(this.propertyMap.get(PROPERTIES_DEBUG_DESTINATION)) : -1;
    ...
    protected double[] calculateMarketSegmentLogsums(PathAlternativeList<SandagBikeNode, SandagBikeEdge> alternativeList)
    {
        ...
        boolean debug = runDebug && (alternativeList.getODPair().getFromNode().getId() == debugOrigin) &&(alternativeList.getODPair().getToNode().getId() == debugDestination);
    ...
    protected List<IntrazonalCalculation<SandagBikeNode>> getMarketSegmentIntrazonalCalculations() {
    	...
    	IntrazonalCalculation<SandagBikeNode> logsumIntrazonalCalculation =
    		IntrazonalCalculations.maxFactorIntrazonalCalculation(IntrazonalCalculations.positiveNegativeFactorizer(0.5,0,2,0),1);
//			IntrazonalCalculations.maxFactorIntrazonalCalculation(IntrazonalCalculations.simpleFactorizer(1,Math.log(2)),1); 
    	...
    	//do distance
    	intrazonalCalculations.add(IntrazonalCalculations.<SandagBikeNode>minFactorIntrazonalCalculation(IntrazonalCalculations.simpleFactorizer(0.5,0),1));
    	return intrazonalCalculations;
    }
    
    //these methods might be moved to the network classes...
    public static void writeDerivedNetworkEdges(Network<SandagBikeNode,SandagBikeEdge,SandagBikeTraversal> network, Path outputFile) {
    	try (PrintWriter writer = new PrintWriter(outputFile.toFile())) {
            ...
            sb.append("fromNode").append(",")
              ...
              .append("bikeClass").append(",")
              .append("lanes").append(",")
              .append("functionalClass").append(",")
              .append("centroidConnector").append(",")
              .append("autosPermitted").append(",")
              .append("cycleTrack").append(",")
              .append("bikeBlvd").append(",")
              .append("distance").append(",")
              .append("gain").append(",")
              .append("bikeCost").append(",")
              .append("walkCost");
            ...
            while (it.hasNext()) {
                ...
                sb.append(edge.getFromNode().getId()).append(",")
                  .append(edge.getToNode().getId()).append(",")
                  .append(edge.bikeClass).append(",")
                  .append(edge.lanes).append(",")
                  .append(edge.functionalClass).append(",")
                  .append(edge.centroidConnector).append(",")
                  .append(edge.autosPermitted).append(",")
                  .append(edge.cycleTrack).append(",")
                  .append(edge.bikeBlvd).append(",")
                  .append(edge.distance).append(",")
                  .append(edge.gain).append(",")
                  .append(edge.bikeCost).append(",")
                  .append(edge.walkCost);
                writer.println(sb.toString());
            }
        } catch (IOException e) {
        	logger.fatal(e);
    ...
    public static void writeDerivedNetworkNodes(Network<SandagBikeNode,SandagBikeEdge,SandagBikeTraversal> network, Path outputFile) {
    	try (PrintWriter writer = new PrintWriter(outputFile.toFile())) {
            ...
            while (it.hasNext()) {
                ...
                sb.append(node.getId()).append(",")
                  ...
                  .append(node.mgra).append(",")
                  .append(node.taz).append(",")
                  .append(node.tap).append(",")
                  .append(node.signalized).append(",")
    ...
    public static void writeDerivedNetworkTraversals(Network<SandagBikeNode,SandagBikeEdge,SandagBikeTraversal> network, Path outputFile) {
    	try (PrintWriter writer = new PrintWriter(outputFile.toFile())) {
            ...
            sb.append("start").append(",")
              .append("thru").append(",")
              .append("end").append(",")
              .append("turnType").append(",")
              .append("bikecost").append(",")
              .append("thruCentroid").append(",")
              .append("signalExclRight").append(",")
              .append("unlfrma").append(",")
              .append("unlfrmi").append(",")
              .append("unxma").append(",")
              .append("unxmi");
            writer.println(sb.toString());
            ...
            while (it.hasNext()) {
                ...
                sb.append(traversal.getFromEdge().getFromNode().getId()).append(",")
                  ...
                  .append(traversal.turnType.getKey()).append(",")
                  .append(traversal.cost).append(",")
                  .append(traversal.thruCentroid).append(",")
...
//                                   propertyMap.get(BikeLogsum.BIKE_LOGSUM_MGRA_FILE_PROPERTY));
        ...
        if (Boolean.parseBoolean(propertyMap.get(PROPERTIES_WRITE_DERIVED_BIKE_NETWORK))) {
        	Path outputDirectory = Paths.get(propertyMap.get(SandagBikePathAlternativeListGenerationConfiguration.PROPERTIES_OUTPUT));
	        writeDerivedNetworkEdges(network,outputDirectory.resolve(propertyMap.get(PROPERTIES_DERIVED_NETWORK_EDGES_FILE)));
	        writeDerivedNetworkNodes(network,outputDirectory.resolve(propertyMap.get(PROPERTIES_DERIVED_NETWORK_NODES_FILE)));
	        writeDerivedNetworkTraversals(network,outputDirectory.resolve(propertyMap.get(PROPERTIES_DERIVED_NETWORK_TRAVERSALS_FILE)));
        }
