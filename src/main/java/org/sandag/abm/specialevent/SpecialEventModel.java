...
public class SpecialEventModel {
	...
	private SpecialEventModel(HashMap<String, String> rbMap) {
		...
		// read the event data
		String eventFile = Util.getStringValueFromPropertyMap(rbMap,
				"specialEvent.event.file");
	...
	public void runModel() {
		...
		SpecialEventDmuFactoryIf dmuFactory = new SpecialEventDmuFactory(
				modelStructure);

		SpecialEventTourManager tourManager = new SpecialEventTourManager(
				rbMap, eventData);
		...
		SpecialEventOriginChoiceModel originChoiceModel = new SpecialEventOriginChoiceModel(
				rbMap, dmuFactory, eventData);
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (!matrixServerAddress.equalsIgnoreCase("none")) {
				...
				if (matrixServerAddress.equalsIgnoreCase("localhost")) {
					matrixServer = specialEventModel.startMatrixServerProcess(
							matrixServerAddress, serverPort, mt);
					specialEventModel.ms = matrixServer;
				} else {
					specialEventModel.ms = new MatrixDataServerRmi(
		...
		if (matrixServerAddress.equalsIgnoreCase("localhost")) {
			...
			if (!matrixServerAddress.equalsIgnoreCase("none"))
				specialEventModel.ms
						.stop32BitMatrixIoServer("SpecialEventModel");
		}
