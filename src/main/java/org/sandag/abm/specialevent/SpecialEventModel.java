...
public class SpecialEventModel {
    ...
    public void runModel() {
        ...
        SpecialEventOriginChoiceModel originChoiceModel = new SpecialEventOriginChoiceModel(rbMap,  dmuFactory, eventData );
		originChoiceModel.calculateSizeTerms(dmuFactory);
        originChoiceModel.calculateTazProbabilities(dmuFactory);
        
        SpecialEventTripModeChoiceModel tripModeChoiceModel = new SpecialEventTripModeChoiceModel(rbMap, modelStructure, dmuFactory,logsumsCalculator,eventData );
    ...
    public static void main(String[] args) {
        ...
        try
        {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
            {
                ...
                if (matrixServerAddress.equalsIgnoreCase("localhost")) {
                    matrixServer = specialEventModel.startMatrixServerProcess(matrixServerAddress, serverPort, mt);
                    specialEventModel.ms = matrixServer;
                }
                else {
                	specialEventModel.ms = new MatrixDataServerRmi( matrixServerAddress, serverPort, MatrixDataServer.MATRIX_DATA_SERVER_NAME );
                	specialEventModel.ms.testRemote(Thread.currentThread().getName());
                	specialEventModel.ms.start32BitMatrixIoServer( mt );
        ...
        if (matrixServerAddress.equalsIgnoreCase("localhost")) {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
                specialEventModel.ms.stop32BitMatrixIoServer();
        }
