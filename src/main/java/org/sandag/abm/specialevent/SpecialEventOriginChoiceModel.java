...
public class SpecialEventOriginChoiceModel {
	...
	private HashMap<String, Integer> purposeMap; // string is purpose, int is
	...
	public SpecialEventOriginChoiceModel(HashMap<String, String> rbMap,
		...
		SpecialEventOriginChoiceDMU dmu = dmuFactory
				.getSpecialEventOriginChoiceDMU();
		sizeTermUEC = new UtilityExpressionCalculator(
				new File(destUecFileName), sizePage, dataPage, rbMap, dmu);
	...
	public void calculateSizeTerms(SpecialEventDmuFactoryIf dmuFactory) {
		...
		SpecialEventOriginChoiceDMU aDmu = dmuFactory
				.getSpecialEventOriginChoiceDMU();
		...
		for (int purpose = 0; purpose < purposes; ++purpose) {
			...
			for (int taz = 0; taz < tazs.length; ++taz) {
				...
				for (int i = 0; i < probabilities.length; ++i) {
					...
					tazProbabilities[purpose].setValueAt(originTaz,
							destinationTaz, (float) cumProb);
				}
			}
		}
	 ...
	 */
	public int chooseMGRA(String eventType, int destinationMgra, double random,
			boolean debug) {
		...
		for (int i = 0; i < tazCumProb.getColumnCount(); ++i) {
			...
			if (tazCumProb.getValueAt(originTaz, destinationTaz) > random) { // the
																				// probabilities
																				// are
																				// stored
																				// column-wise
		...
		if (debug)
			logger.info("Chose origin MGRA " + mgraNumber);
		// return the chosen MGRA number
		return mgraNumber;
	}
	...
	public void chooseOrigin(SpecialEventTour tour) {
		...
		int mgra = chooseMGRA(eventType, destinationMgra, random,
