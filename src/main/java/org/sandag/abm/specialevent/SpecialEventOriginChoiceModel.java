...
public class SpecialEventOriginChoiceModel {
     ...
     */
	public SpecialEventOriginChoiceModel(HashMap<String,String> rbMap, SpecialEventDmuFactoryIf dmuFactory, TableDataSet eventData){
        ...
        String destUecFileName = Util.getStringValueFromPropertyMap(rbMap, "specialEvent.dc.uec.file");
        destUecFileName = uecFileDirectory + destUecFileName;
        
        int dataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "specialEvent.dc.data.page" ) );
        int sizePage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "specialEvent.dc.size.page" ) );
       	int modelPage =Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "specialEvent.dc.model.page" ) );
        ...
        destModel = new ChoiceModelApplication( destUecFileName, modelPage, dataPage, rbMap, (VariableTable)dcDmu );
        ...
        sizeTermUEC = new UtilityExpressionCalculator(new File(destUecFileName), sizePage, dataPage, rbMap,dmu );
     	...
     	for(int purpose = 0; purpose<purposes;++purpose){
    		...
    		for(int taz =0;taz<tazs.length;++taz){
            	...
            	for(int i =0; i<probabilities.length;++i){
            		...
            		tazProbabilities[purpose].setValueAt(originTaz, destinationTaz, (float)cumProb);
            	}	
    		}        
    	}
	...
	/**
	 * Choose an MGRA
	 * @param eventType  Event type corresponding to size term
	 * @param destinationMgra MGRA of destination
	...
	public int chooseMGRA(String eventType, int destinationMgra, double random, boolean debug){
		...
		for(int i=0; i < tazCumProb.getColumnCount(); ++ i){
			...
			if(tazCumProb.getValueAt(originTaz,destinationTaz) > random){ //the probabilities are stored column-wise
				if(i != 0){
	 ...
	 * 
	 * @param tour  A Special Event tour
	 */
