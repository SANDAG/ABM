...
public class SandagTripTables {
	...
	// Some parameters
	private int[] modeIndex; // an index array, dimensioned by number of total
								// modes, returns 0=auto modes, 1=non-motor,
								// 2=transit, 3= other
	private int[] matrixIndex; // an index array, dimensioned by number of
								// modes, returns the element of the matrix
	...
	private String purposeName[] = { "Work", "University", "School", "Escort",
			"Shop", "Maintenance", "EatingOut", "Visiting", "Discretionary",
			"WorkBased" };
	...
	private float[][] CBDVehicles; // an array of parked vehicles in MGRAS by
									// period
	private float[][] PNRVehicles; // an array of parked vehicles at TAPs by
									// period
	...
	private String[] indivColumns = { "stop_period", "orig_mgra", "dest_mgra",
			"trip_mode", "inbound", "trip_board_tap", "trip_alight_tap",
			"parking_mgra", "tour_purpose" };

	private String[] jointColumns = { "stop_period", "orig_mgra", "dest_mgra",
			"trip_mode", "inbound", "trip_board_tap", "trip_alight_tap",
			"parking_mgra", "tour_purpose", "num_participants" };

	private HashMap<String, Float> averageOcc3Plus; // a HashMap of average
													// occupancies for 3+
													// vehicles by tour purpose
	 ...
	 *            _iteration appended
	 */
	public SandagTripTables(HashMap<String, String> rbMap, float sampleRate,
			int iteration) {
	...
	public void readOccupancies() {
		...
		for (int i = 0; i < purposeName.length; ++i) {
			...
			float occupancy = new Float(Util.getStringValueFromPropertyMap(
					rbMap, searchString));
		...
		for (int i = 0; i < numberOfModes; ++i) {
			...
			if (i == 0) {
				...
				for (int j = 0; j < autoModes; ++j) {
					...
					matrix[i][j] = new Matrix(modeName + "_" + periodName, "",
							maxTaz, maxTaz);
	...
	public void createTripTables(MatrixType mt) {
		...
		indivTripFile = removeFileExtension(indivTripFile) + iterationString
				+ extension;
		...
		jointTripFile = removeFileExtension(jointTripFile) + iterationString
				+ jntExtension;
		...
		String CBDFile = Util.getStringValueFromPropertyMap(rbMap,
				"Results.CBDFile");
		writeCBDFile(directory + CBDFile);

		// write the vehicles by PNR lot TAP
		String PNRFile = Util.getStringValueFromPropertyMap(rbMap,
				"Results.PNRFile");
	...
	public void processTrips(int timePeriod, TableDataSet tripData) {
		...
		for (int i = 1; i <= tripData.getRowCount(); ++i) {
			...
			if (modelStructure.getTourModeIsS2(tripMode) && !jointTour) {
				...
				String tourPurpose = tripData.getStringValueAt(i,
						"tour_purpose");
			...
			if (mode == 0) {
				...
				float value = matrix[mode][mat].getValueAt(originTAZ,
						destinationTAZ);
				matrix[mode][mat].setValueAt(originTAZ, destinationTAZ,
						(value + vehicleTrips));
			} else if (mode == 1) {
				float value = matrix[mode][mat].getValueAt(originTAZ,
						destinationTAZ);
				...
				matrix[mode][mat].setValueAt(boardTap, alightTap,
						(value + personTrips));
				...
				if (modelStructure.getTourModeIsDriveTransit(tripMode)) {
					...
					if (inbound == 0) { // from origin to lot (boarding tap)
						...
						matrix[0][0].setValueAt(originTAZ, PNRTAZ,
								(value + vehicleTrips));
						...
						matrix[0][0].setValueAt(PNRTAZ, destinationTAZ,
								(value + vehicleTrips));
					}
	...
	public void writeTrips(int period, MatrixType mt) {
		...
		fileName[0] = dir
				+ Util.getStringValueFromPropertyMap(rbMap,
						"Results.AutoTripMatrix") + end;
		fileName[1] = dir
				+ Util.getStringValueFromPropertyMap(rbMap,
						"Results.NMotTripMatrix") + end;
		fileName[2] = dir
				+ Util.getStringValueFromPropertyMap(rbMap,
						"Results.TranTripMatrix") + end;
		fileName[3] = dir
				+ Util.getStringValueFromPropertyMap(rbMap,
						"Results.OthrTripMatrix") + end;
		...
		for (int i = 0; i < 4; ++i) {
			try {
				if (ms != null)
					...
					writeMatrixFile(fileName[i], matrix[i]);
			} catch (Exception e) {
				logger.error("exception caught writing " + mt.toString()
						+ " matrix file = " + fileName[i]
						+ ", for mode index = " + i, e);
			}
		}
	...
	public void writeMatrixFile(String fileName, Matrix[] m) {
		...
		for (int i = 0; i < m.length; i++) {
			names[i] = m[i].getName();
			logger.info(m[i].getName() + " has " + m[i].getRowCount()
					+ " rows, " + m[i].getColumnCount()
					+ " cols, and a total of " + m[i].getSum());
		}
	...
	private void setupMatrixServer(HashMap<String, String> pMap) {
		...
		try {
			...
			if (!matrixServerAddress.equalsIgnoreCase("none")) {
				...
				if (matrixServerAddress.equalsIgnoreCase("localhost")) {
					...
					try {
						...
						logger.error(
								"RuntimeException caught starting matrix server: "
										+ matrixServerAddress + ":"
										+ serverPort
			...
			logger.error(
					String.format("exception caught setting up matrix server -- exiting."),
					e);
			throw new RuntimeException();
	...
	public static void main(String[] args) {
		...
		logger.info(String
				.format("SANDAG Trip Table Generation Program using CT-RAMP version %s",
		...
		SandagTripTables tripTables = new SandagTripTables(pMap, sampleRate,
				iteration);
