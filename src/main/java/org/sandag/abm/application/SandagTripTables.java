...
public class SandagTripTables {
    ...
    private MatrixDataServerRmi ms;
 	private static MatrixIO32BitJvm ioVm32Bit = null;
  	...
  	private String[] indivColumns = {"stop_period",
 									"orig_mgra",
 									"dest_mgra",
 									"trip_mode",
 									"inbound",
 	...
 	private String[] jointColumns = {"stop_period",
				"orig_mgra",
				"dest_mgra",
				"trip_mode",
				"inbound",
     ...
     * Constructor.
     * 
     * @param rbMap  HashMap formed from a property map, which includes environment variables and arguments passed in as -d to VM
     * @param sampleRate  Sample rate 0->1.0
     * @param iteration  Iteration number, program will look for trip file names with _iteration appended
     */
	...
	public void createTripTables( MatrixType mt ){
		...
		String indivTripFile = Util.getStringValueFromPropertyMap(rbMap,"Results.IndivTripDataFile");
		...
		String jointTripFile = Util.getStringValueFromPropertyMap(rbMap,"Results.JointTripDataFile");
	...
	public void writeTrips(int period, MatrixType mt){
		...
		String dir  = Util.getStringValueFromPropertyMap(rbMap,
        "scenario.path");
		...
		fileName[0] =dir+Util.getStringValueFromPropertyMap(rbMap,"Results.AutoTripMatrix")+end;
		fileName[1] =dir+Util.getStringValueFromPropertyMap(rbMap,"Results.NMotTripMatrix")+end;
		fileName[2] =dir+Util.getStringValueFromPropertyMap(rbMap,"Results.TranTripMatrix")+end; 
		fileName[3] =dir+Util.getStringValueFromPropertyMap(rbMap,"Results.OthrTripMatrix")+end; 
		...
		for(int i=0;i<4;++i) {
		    try {
		        ...
		        logger.error( "exception caught writing " + mt.toString() + " matrix file = " + fileName[i] + ", for mode index = " + i, e );
		    }
		}
    ...
    */
     ...
     * @param sampleRate  The iteration number, should be appended to trip files as _iteration
     */
