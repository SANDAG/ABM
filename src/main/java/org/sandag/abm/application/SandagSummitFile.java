...
public class SandagSummitFile
{
    ...
    // Some parameters
    private int                    modes;
    private int                    upperEA;                                                 // Upper limit on time of day for the Early morning time
                                                                                             // period
    ...
    private final static String    purposeName[] = {"Work", "University", "School", "Escort",
    ...
    /**
     * This is the main workhorse method in this class. It creates a SummitRecordTable, and then iterates over records in the tour file. If the tour
     * purpose for the record is mapped to the fileNumber argument, a ConcreteSummitRecord is created and the attributes are set, and the record is
     * added to the SummitRecordTable. After all tour records have been read, the table is finalized.
     * 
     * After this method is run, the file header information can be set and the SummitRecordTable can be written to a SUMMIT file.
     * 
     * @param fileNumber
     */
     ...
     * 
     * Market 0 = Departure & arrival in peak Market 1 = Departure & arrival in mixed periods (peak and off-peak) Market 2 = Departure & arrival in
     * off-peak
     * 
     * @param departPeriod
    ...
    public int calculatePeriodMarket(int departPeriod, int arrivePeriod)
    {
        ...
        /*
         * Arrival & departure = peak, period = 0 Mixed peak & off-peak, period = 1 Arrival & departure = off-peal, period = 2
         */
    ...
    /**
     * Calculate and return market segment based on auto sufficiency and time period combination.
     * 
     * Market AutoSuff Period 1 0 0 Peak 2 0 1 Mixed 2 0 2 Off-Peak 3 1 0 Peak 4 1 1 Mixed 4 1 2 Off-Peak 5 2 0 Peak 6 2 1 Mixed 6 2 2 Off-Peak
     * 
     * 
    ...
    /**
     * Determine what table to use for tour purpose, based on fileNumber array set from properties file.
     * 
     * @param tourPurpose
    ...
    /**
     * Look up the purpose string based on the file number, for use in SUMMIT file header.
     * 
     * @param fileNumber
     ...
     * 
     * 0 = 0 autos owned 1 = autos > 0 & autos < adults (persons 18+) 2 = autos > adults
     * 
     * @param hhID
     *            Household ID
    ...
    /**
     * Calculate the total non-transit exponentiated utility. The method uses the walkTransitModes array and the driveTransitModes array to determine
     * which modes are non-transit, and the sum of their exponentiated utilities is calculated and returned.
     * 
     * @param util
     *            An array of utilities, by mode. -999 indicates mode not available.
    ...
    /**
     * Calculate the share of walk-transit available: 1 if any walk-transit mode is available, as indicated by a non-zero probability, else 0. The
     * method iterates through the probability array and returns a 1 if the probability is non-zero for any walk-transit mode, as indicated by the
    ...
    /**
     * Calculate the share of drive-transit only available: 1 if any drive-transit mode is available, as indicated by a non-zero probability, and all
     * walk-transit modes are not available. The method iterates through the probability array and returns a 1 if the probability is non-zero for any
     * drive-transit mode, as indicated by the driveTransitModes array.
     * 
     * @param prob
     *            An array of probabilities, dimensioned by modes.
    ...
    /**
     * Calculate the total transit probability for records with walk-transit available. The method returns 0 if walk-transit is not available. If
     * walk-transit is available, the method iterates through the probability array, adding all transit mode probabilities. The sum is returned.
     * 
     * @param prob
    ...
    /**
     * Calculate the total transit probability for records where only drive-transit available. The method returns 0 if only drive-transit is not
     * available. If only drive-transit is available, the method iterates through the probability array, adding all drive-transit mode probabilities.
     * The sum is returned.
     * 
     * @param prob
     *            An array of probabilities, one per mode.
    ...
    /**
     * Get the in-vehicle time coefficient for the file, based on the values read in the properties file.
     * 
     * @param fileNumber
