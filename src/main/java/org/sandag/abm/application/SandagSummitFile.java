...
public class SandagSummitFile {
	...
	// Some parameters
	private int modes;
	private int upperEA; // Upper limit on time of day for the Early morning
							// time period
	private int upperAM; // Upper limit on time of day for the AM peak time
							// period
	private int upperMD; // Upper limit on time of day for the Midday time
							// period
	private int upperPM; // Upper limit on time of day for the PM time peak
							// period
	...
	private final static String purposeName[] = { "Work", "University",
			"School", "Escort", "Shop", "Maintenance", "EatingOut", "Visiting",
			"Discretionary", "WorkBased" };
	...
	public SandagSummitFile(String resourceFile) {
		...
		mdm = MgraDataManager.getInstance(ResourceUtil
				.changeResourceBundleIntoHashMap(rb));
		tdm = TazDataManager.getInstance(ResourceUtil
				.changeResourceBundleIntoHashMap(rb));
	...
	public void createSummitFile(int fileNumber) {
		...
		for (int i = 1; i <= tourData.getRowCount(); ++i) {
			...
			short marketSegment = (short) calculateMarketSegment(
					autoSufficiency, periodMarket);
			float expUtility = calculateNonTransitExpUtility(util); // not used
			float wtAvailShare = calculateWalkTransitAvailableShare(prob);
			float dtAvailShare = calculateDriveTransitOnlyShare(prob,
					wtAvailShare);
			float wtProb = calculateTransitShareOfWalkTransit(prob,
					wtAvailShare);
			float dtProb = calculateTransitShareOfDriveTransitOnly(prob,
					dtAvailShare);
			float aggExpUtility = calculateAggregateExpUtility(util);

			float participants = 1.0f;
			if (jointTour) {
				String participantString = tourData.getStringValueAt(i,
						participantsCol);
	...
	public int calculateSummitTable(String tourPurpose) {
		...
		if (tourPurpose.contentEquals("Work"))
			...
			logger.error("Error: Tour purpose " + tourPurpose
					+ " not recognized");
		}
	...
	public float calculateDriveTransitOnlyShare(float prob[],
			float walkTransitAvailableShare) {
	 ...
	 *         0.
	 */
	public float calculateTransitShareOfWalkTransit(float prob[],
			float walkTransitAvailableShare) {
