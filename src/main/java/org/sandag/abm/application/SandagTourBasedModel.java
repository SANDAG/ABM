...
public final class SandagTourBasedModel {
	 ...
	 */
	private SandagTourBasedModel(ResourceBundle aRb,
			HashMap<String, String> aPropertyMap, int aGlobalIterationNumber,
	...
	private void runTourBasedModel(HashMap<String, String> propertyMap) {
		...
		SandagCtrampApplication ctrampApplication = new SandagCtrampApplication(
				rb, propertyMap, calculateLandUseAccessibilities);
		...
		try {
			...
			try {
				...
				hhServerPort = Integer.parseInt(rb
						.getString("RunModel.HouseholdServerPort"));
				localHandlers = false;
		...
		try {
			...
			if (localHandlers) {
				...
				householdDataManager.setHouseholdSampleRate(
				...
				householdDataManager = new HouseholdDataManagerRmi(
				...
				try {
					...
					// inmtod,
					// stf, stl
					String restartModel = rb
							.getString("RunModel.RestartWithHhServer");
				...
				if (restartHhServer) {
					...
					householdDataManager.setHouseholdSampleRate(
							iterationSampleRate, sampleSeed);
					householdDataManager.setupHouseholdDataManager(
							modelStructure, inputHouseholdFileName,
							inputPersonFileName);
			...
			// run the models
			ctrampApplication.runModels(householdDataManager, dmuFactory,
					globalIterationNumber, iterationSampleRate);
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			for (int i = 1; i < args.length; i++) {
				...
				if (args[i].equalsIgnoreCase("-luAcc")) {
					calculateLandUseAccessibilities = Boolean
							.parseBoolean(args[i + 1]);
					logger.info(String.format("-luAcc %s.",
			...
			if (globalIterationNumber < 0) {
				...
				logger.info(String.format(
						"no -iteration flag, default value %d used.",
						globalIterationNumber));
			}
			...
			if (iterationSampleRate < 0) {
				...
				logger.info(String.format(
						"no -sampleRate flag, default value %.4f used.",
						iterationSampleRate));
			}
			...
			if (sampleSeed < 0) {
				...
				logger.info(String.format(
						"no -sampleSeed flag, default value %d used.",
						sampleSeed));
			}
		...
		SandagTourBasedModel mainObject = new SandagTourBasedModel(rb, pMap,
				globalIterationNumber, iterationSampleRate, sampleSeed,
				calculateLandUseAccessibilities);
