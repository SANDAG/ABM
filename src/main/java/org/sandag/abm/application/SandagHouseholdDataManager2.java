 ...
 */
public class SandagHouseholdDataManager2 extends HouseholdDataManager {
	...
	public void mapTablesToHouseholdObjects() {
		...
		// the original order
		int[] firstSortedIndices = getRandomOrderHhIndexArray(hhTable
				.getRowCount());

		// get a second index array for households sorted in random order - to
		// select a sample from the randomly ordered hhs
		int[] randomSortedIndices = getRandomOrderHhIndexArray(hhTable
				.getRowCount());
		...
		logger.info(mgrasInSample + " unique MGRA values in the "
				+ (sampleRate * 100) + "% sample.");
		...
		for (int i = 0; i < numHouseholdsInSample; i++) {
			...
			try {
				...
				for (p = personHhStart[hhid]; p <= personHhEnd[hhid]; p++) {
					...
					// in
					persId = (int) personTable.getValueAt(p, personTable
					...
					// object
					int age = (int) personTable.getValueAt(p, personTable
					 ...
					 */
					int military = (int) personTable.getValueAt(p, personTable
							.getColumnPosition(PERSON_MILITARY_FIELD_NAME));
					int pecasOccup = 0;

					String occsoc = personTable.getStringValueAt(p, personTable
							.getColumnPosition(PERSON_SOC_FIELD_NAME));

					int indcen = (int) personTable.getValueAt(p, personTable
					...
					if (military == 1) // in active military
						...
						if (!occCodes.containsKey(occsoc)) {
							logger.fatal("Error:  Occupation code " + occsoc
									+ " for hhid " + hhid + " person " + p
									+ " not found in occupation file");
							throw new RuntimeException();
						}
						pecasOccup = occCodes.get(occsoc); // everyone else
						if (pecasOccup == 0)
							logger.warn("pecasOccup==0 for occsoc==" + occsoc);
					}
					...
					int studentCat = (int) personTable
							.getValueAt(
									p,
									personTable
					...
					int personType = (int) personTable
							.getValueAt(
									p,
									personTable
					...
					// non-student
							...
							.ordinal()
							&& empCat == Person.EmployStatus.FULL_TIME
									.ordinal()) {
					...
					if (studentCat == Person.StudentStatus.NON_STUDENT
							.ordinal()) {
	...
	public static void main(String args[]) throws Exception {
		...
		System.out.println(String.format(
				"SandagHouseholdDataManager2 server class started on: %s:%d",
	...
	private HashMap<String, Integer> readOccupCorrespondenceData() {
		...
		int[] militaryRange = PropertyMap.getIntegerArrayFromPropertyMap(
				propertyMap, PROPERTIES_MILITARY_INDUSTRY_RANGE);
