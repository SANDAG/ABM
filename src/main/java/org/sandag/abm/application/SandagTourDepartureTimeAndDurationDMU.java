 ...
 */
public class SandagTourDepartureTimeAndDurationDMU extends
	...
	public int getFemaleWithPreschooler() {
		return ((getPersonIsFemale() == 1) && (getNumPreschoolChildrenInHh() > 1)) ? 1
				: 0;
	}
	...
	public int getSchoolChildWithMandatoryTour() {
		return (getStudentNonDrivingAge() == 1 && getPersonMandatoryTotal() > 0) ? 1
				: 0;
	}

	public int getUniversityWithMandatoryPattern() {
		return (getUniversityStudent() == 1 && person.getCdapActivity()
				.equalsIgnoreCase(Definitions.MANDATORY_PATTERN)) ? 1 : 0;
	}

	public int getWorkerWithMandatoryPattern() {
		return ((getFullTimeWorker() == 1 || getPartTimeWorker() == 1) && person
				.getCdapActivity().equalsIgnoreCase(
	...
	public int getJointTour() {
		return (tour.getTourCategory()
				.equalsIgnoreCase(ModelStructure.JOINT_NON_MANDATORY_CATEGORY)) ? 1
				: 0;
	}

	public int getIndividualTour() {
		return (tour.getTourCategory()
				.equalsIgnoreCase(ModelStructure.INDIVIDUAL_NON_MANDATORY_CATEGORY)) ? 1
				: 0;
	}
