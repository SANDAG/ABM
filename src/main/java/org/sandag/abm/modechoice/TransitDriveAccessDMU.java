 ...
 */
public class TransitDriveAccessDMU implements Serializable, VariableTable {

	protected transient Logger logger = Logger
			.getLogger(TransitDriveAccessDMU.class);
	...
	public void logValues(Logger localLogger) {
		...
		localLogger.info("");
		localLogger.info(String.format("Drive Time To Tap:     %9.4f",
				driveTimeToTap));
		localLogger.info(String.format("Drive Dist To Tap:     %9.4f",
				driveDistToTap));
		localLogger.info(String.format("Drive Time From Tap:     %9.4f",
				driveTimeFromTap));
		localLogger.info(String.format("Drive Dist From Tap:     %9.4f",
				driveDistFromTap));
		localLogger.info(String.format("TAP to MGRA walk time:    %9.4f",
				tapToMgraWalkTime));
		localLogger.info(String.format("MGRA to TAP walk time:    %9.4f",
				mgraToTapWalkTime));
		localLogger.info(String.format("Car to station walk time: %9.4f",
				carToStationWalkTime));
		localLogger.info(String.format("Escalator time:           %9.4f",
				escalatorTime));
