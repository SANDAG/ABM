...
/**
 * ParkRidePathUEC calculates the best PNR utilities for a given production/origin TDZ and attraction/destination MGRA.
 * 
 * @author Christi Willison
 * @version 1.0, Feb 12, 2009
 */
public class TransitDriveAccessUEC
        extends TransitPathUEC
{
    ...
    /**
     * Create a drive-transit access UECFile. This method is only required if piece-wise utilities are built, where TAP-TAP pair utilities are
     * computed separately from MGRA-TAP walk utilities, to cut down on processing time.
     * 
     * @param rb
     *            ResourceBundle
     * @param UECFileName
    ...
    {
        UECFile = new File(UECFileName);
        driveAccessUEC = new UtilityExpressionCalculator(UECFile, modelSheet, dataSheet, rbMap, dmu);
    ...
    public void findBestDriveTransitWalkTaps(int pMgra, int aMgra, boolean debug)
    {
        ...
        int pPos = -1;
        for (int pTap : tazManager.getParkRideOrKissRideTapsForZone(pTaz, accMode))
        {
                    ...
                    || (accMode == AccessMode.KISS_N_RIDE))
            {
                ...
                for (int aTap : mgraManager.getMgraWlkTapsDistArray()[aMgra][0])
                {
                    ...
                    if (debug)
                    {
                        uec.logAnswersArray(logger, "Drive-Transit-Walk Utility");
        ...
        for (int pTap : mgraManager.getMgraWlkTapsDistArray()[pMgra][0])
        {
            ...
            for (int aTap : tazManager.getParkRideOrKissRideTapsForZone(aTaz, accMode))
            {
                        ...
                        || (accMode == AccessMode.KISS_N_RIDE))
                {
                    ...
                    // Set DMU values
                    float driveTime = tazManager.getTapTime(aTaz, aPos, accMode);
                    dmu.setAccessMode(accMode.ordinal());
                    dmu.setDriveDistToTap(tazManager.getTapDist(aTaz, aPos, accMode));
                    dmu.setDriveTimeToTap(driveTime);
                    dmu.setCarToStationWalkTime(0f);
                    ...
                    if (debug)
                    {
                        uec.logAnswersArray(logger, "Walk-Transit-Drive Utility");
                        uec.logResultsArray(logger, pTap, aTap);
                    }
    ...
    /**
     * This method calculates the utilities for a given Tap pair. It is called from the method @link {@link #findBestDriveAccessTaps(int, int)}. The
     * walk times in the dmu must be set separately, or set to 0 for a set of utilities that are mgra-independent.
     * 
     * @param pTap
    ...
    /**
     * This method calculates the drive-access portion of the utility for a given TAZ to a given tap.
     * 
     * @param taz
     *            The TAZ
     * @param tapPosition
     ...
     */
    public double calculateDriveAccessUtilityForTaz(int taz, int tapPosition, AccessMode aMode,
            boolean trace)
    {
    ...
    /**
     * @return the TAP to destination MGRA drive time stored for the best TAP-TAP pair.
     */
