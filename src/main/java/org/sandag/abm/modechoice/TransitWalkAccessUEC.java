 ...
 */
public class TransitWalkAccessUEC extends TransitPathUEC {
	 ...
	 */
	public TransitWalkAccessUEC(HashMap<String, String> rbMap,
	 ...
	 *            The sheet (0-indexed) containing the data specification.
	 */
	public void createWalkAccessUEC(HashMap<String, String> rbMap,
			String UECFileName, int modelSheet, int dataSheet) {
		UECFile = new File(UECFileName);
		walkAccessUEC = new UtilityExpressionCalculator(UECFile, modelSheet,
				dataSheet, rbMap, dmu);
	...
	public void findBestWalkTransitWalkTaps(int pMgra, int aMgra, boolean debug) {
		...
		for (int pTap : pMgraSet) {
			...
			for (int aTap : aMgraSet) {
				...
				double[] results = calculateUtilitiesForTapPair(pTap, aTap,
						writeCalculations);
				...
				// is
				// the best.
				boolean foundNewBestPath = super.comparePaths(results, pTap,
						aTap, writeCalculations);
	 ...
	 */
	public double calculateWalkAccessUtilityForMgra(int mgra, int tapPosition,
			boolean trace) {
