 ...
 *          Edited JEF May 2009
 */
public final class MgraDataManager implements Serializable {
	...
	private static final String LOG_MGRA_FILE = LOG_MGRA + "debug";
	...
	public void readMgraWlkTaps(HashMap<String, String> rbMap) {
		File mgraWlkTapCorresFile = new File(
				Util.getStringValueFromPropertyMap(rbMap, "scenario.path")
						+ Util.getStringValueFromPropertyMap(rbMap,
								"mgra.wlkacc.taps.and.distance.file"));
		...
		for (int i = 0; i < mgraWlkTapList.length; i++) { // elements in the
															// array of
															// arraylists
			...
			// Distance
			// ArrayList
			if (aMgraTapList != null) { // if the list isn't null
				...
				for (int j = 0; j < aMgraTapList.size(); j++) {
					mgraWlkTapsDistArray[i][0][j] = (Integer) aMgraTapList
							.get(j);
					mgraWlkTapsDistArray[i][1][j] = (Integer) aMgraDistList
							.get(j);
				}
				nMgrasWithWlkTaps++;
			}
	...
	public void readMgraWlkDist(HashMap<String, String> rbMap) {
		File mgraWlkDistFile = new File(Util.getStringValueFromPropertyMap(
				rbMap, "scenario.path")
				+ Util.getStringValueFromPropertyMap(rbMap,
						"mgra.walkdistance.file"));
	...
	public int[] getMgrasWithinWalkDistanceFrom(int mgra) {
		...
		if (oMgraWalkDistance[mgra] == null)
			return null;
	...
	public int[] getMgrasWithinWalkDistanceTo(int mgra) {
		...
		if (dMgraWalkDistance[mgra] == null)
			return null;
		...
		if (mgraWlkTapsDistArray[mgra] != null) {
			if (mgraWlkTapsDistArray[mgra][0] != null) {
				for (int i = 0; i < mgraWlkTapsDistArray[mgra][0].length; ++i)
					if (mgraWlkTapsDistArray[mgra][0][i] == tap)
						return i;
			}
		}
	...
	public float getMgraToTapWalkTime(int mgra, int pos) {
		return ((float) mgraWlkTapsDistArray[mgra][1][pos])
				* Constants.walkMinutesPerFoot;
	}
	...
	public float getMgraToMgraWalkTime(int oMgra, int dMgra) {
		...
		if (oMgraWalkDistance[oMgra] == null)
			...
			return ((float) oMgraWalkDistance[oMgra].get(dMgra))
					* Constants.walkMinutesPerFoot;
	...
	public void mapDriveAccessTapsToMgras(TazDataManager tazDataManager) {
		...
		for (int mgra = 1; mgra <= maxMgra; mgra++) {
			...
			// to determine, for a given mgra, if a tap is contained in the set.
			int[] tapItems = tazDataManager.getParkRideOrKissRideTapsForZone(
					taz, AccessMode.PARK_N_RIDE);
	...
	public void process4ddensityData(HashMap<String, String> rbMap) {
		...
		try {
			...
			// 0-based double[]
			double[] duDenField = mgraTableDataSet
					.getColumnAsDouble(MGRA_4DDENSITY_DU_DEN_FIELD);
			double[] empDenField = mgraTableDataSet
					.getColumnAsDouble(MGRA_4DDENSITY_EMP_DEN_FIELD);
			double[] totIntField = mgraTableDataSet
					.getColumnAsDouble(MGRA_4DDENSITY_TOT_INT_FIELD);
	...
	private void readMgraTableData(HashMap<String, String> rbMap) {
		...
		try {
			...
			logger.error(
					"problem reading mgra data table for MgraDataManager.", e);
			System.exit(1);
		}
	...
	public double getMgraGradeSchoolEnrollment(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return mgraTableDataSet.getValueAt(row,
				MGRA_GRADE_SCHOOL_ENROLLMENT_FIELD_NAME);
	}
	...
	public double getMgraHighSchoolEnrollment(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return mgraTableDataSet.getValueAt(row,
				MGRA_HIGH_SCHOOL_ENROLLMENT_FIELD_NAME);
	}
	...
	public double getMgraUniversityEnrollment(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return mgraTableDataSet.getValueAt(row,
				MGRA_UNIVERSITY_ENROLLMENT_FIELD_NAME);
	}
	...
	public double getMgraOtherCollegeEnrollment(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return mgraTableDataSet.getValueAt(row,
				MGRA_OTHER_COLLEGE_ENROLLMENT_FIELD_NAME);
	}
	...
	public double getMgraAdultSchoolEnrollment(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return mgraTableDataSet.getValueAt(row,
				MGRA_ADULT_SCHOOL_ENROLLMENT_FIELD_NAME);
	}
	...
	public int getMgraGradeSchoolDistrict(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return (int) mgraTableDataSet.getValueAt(row,
				MGRA_GRADE_SCHOOL_DISTRICT_FIELD_NAME);
	}
	...
	public int getMgraHighSchoolDistrict(int mgra) {
		int row = mgraDataTableMgraRowMap.get(mgra);
		return (int) mgraTableDataSet.getValueAt(row,
				MGRA_HIGH_SCHOOL_DISTRICT_FIELD_NAME);
	}
	...
	private void calculateMgraAvgParkingCosts(
		...
		String projectPath = propertyMap
				.get(CtrampApplication.PROPERTIES_PROJECT_DIRECTORY);
		...
		try {
			...
			System.out.println("Exception caught trying to create file "
					+ outFile);
			e.printStackTrace();
			throw new RuntimeException();
		}
		...
		if (LOG_MGRA > 0) {
			try {
				outM = new PrintWriter(new BufferedWriter(new FileWriter(
						new File(projectPath + "output/" + LOG_MGRA_FILE
								+ "M.csv"))));
				outD = new PrintWriter(new BufferedWriter(new FileWriter(
						new File(projectPath + "output/" + LOG_MGRA_FILE
								+ "D.csv"))));
				outH = new PrintWriter(new BufferedWriter(new FileWriter(
						new File(projectPath + "output/" + LOG_MGRA_FILE
								+ "H.csv"))));
			} catch (IOException e) {
		...
		float workDistCoeff = Float.parseFloat(propertyMap
				.get(MGRA_DISTANCE_COEFF_WORK));
		float otherDistCoeff = Float.parseFloat(propertyMap
				.get(MGRA_DISTANCE_COEFF_OTHER));

		int[] mgraField = mgraTableDataSet.getColumnAsInt(MGRA_FIELD_NAME);
		int[] mgraParkAreaField = mgraTableDataSet
				.getColumnAsInt(MGRA_PARKAREA_FIELD);
		int[] hstallsothField = mgraTableDataSet
				.getColumnAsInt(MGRA_HSTALLSOTH_FIELD);
		int[] hstallssamField = mgraTableDataSet
				.getColumnAsInt(MGRA_HSTALLSSAM_FIELD);
		float[] hparkcostField = mgraTableDataSet
				.getColumnAsFloat(MGRA_HPARKCOST_FIELD);
		int[] numfreehrsField = mgraTableDataSet
				.getColumnAsInt(MGRA_NUMFREEHRS_FIELD);
		int[] dstallsothField = mgraTableDataSet
				.getColumnAsInt(MGRA_DSTALLSOTH_FIELD);
		int[] dstallssamField = mgraTableDataSet
				.getColumnAsInt(MGRA_DSTALLSSAM_FIELD);
		float[] dparkcostField = mgraTableDataSet
				.getColumnAsFloat(MGRA_DPARKCOST_FIELD);
		int[] mstallsothField = mgraTableDataSet
				.getColumnAsInt(MGRA_MSTALLSOTH_FIELD);
		int[] mstallssamField = mgraTableDataSet
				.getColumnAsInt(MGRA_MSTALLSSAM_FIELD);
		float[] mparkcostField = mgraTableDataSet
				.getColumnAsFloat(MGRA_MPARKCOST_FIELD);
		...
		for (int k = 0; k < maxMgra; k++) {
			...
			if (mgraParkArea[mgra] == PARK_AREA_ONE) {
				...
				double numeratorM = mstallssam[mgra]
						* Math.exp(workDistCoeff * dist) * mparkcost[mgra];
				double denominatorM = mstallssam[mgra]
						* Math.exp(workDistCoeff * dist);

				double numeratorD = dstallssam[mgra]
						* Math.exp(workDistCoeff * dist) * dparkcost[mgra];
				double denominatorD = dstallssam[mgra]
						* Math.exp(workDistCoeff * dist);

				double discountFactor = Math.max(1 - (numfreehrs[mgra] / 4), 0);
				double numeratorH = hstallssam[mgra]
						* Math.exp(workDistCoeff * dist) * discountFactor
						* hparkcost[mgra];
				double denominatorH = hstallssam[mgra]
						* Math.exp(workDistCoeff * dist);

				if (mgra == LOG_MGRA) {
					// log the file header
					outM.println("wMgra" + "," + "mgraParkArea" + ","
							+ "workDistCoeff*dist" + ","
							+ "exp(workDistCoeff*dist)" + "," + "mstallsoth"
							+ "," + "mparkcost" + "," + "numeratorM" + ","
							+ "denominatorM");
					outD.println("wMgra" + "," + "mgraParkArea" + ","
							+ "otherDistCoeff*dist" + ","
							+ "exp(otherDistCoeff*dist)" + "," + "dstallsoth"
							+ "," + "dparkcost" + "," + "numeratorD" + ","
							+ "denominatorD");
					outH.println("wMgra" + "," + "mgraParkArea" + ","
							+ "otherDistCoeff*dist" + ","
							+ "exp(otherDistCoeff*dist)" + ","
							+ "discountFactor" + "," + "hstallsoth" + ","
							+ "hparkcost" + "," + "numeratorH" + ","
							+ "denominatorH");

					outM.println(mgra + "," + mgraParkArea[mgra] + ","
							+ workDistCoeff * dist + ","
							+ Math.exp(workDistCoeff * dist) + ","
							+ mstallsoth[mgra] + "," + mparkcost[mgra] + ","
							+ numeratorM + "," + denominatorM);
					outD.println(mgra + "," + mgraParkArea[mgra] + ","
							+ workDistCoeff * dist + ","
							+ Math.exp(workDistCoeff * dist) + ","
							+ dstallsoth[mgra] + "," + dparkcost[mgra] + ","
							+ numeratorD + "," + denominatorD);
					outH.println(mgra + "," + mgraParkArea[mgra] + ","
							+ workDistCoeff * dist + ","
							+ Math.exp(workDistCoeff * dist) + ","
							+ discountFactor + "," + hstallsoth[mgra] + ","
							+ hparkcost[mgra] + "," + numeratorH + ","
							+ denominatorH);
				}
				...
				if (walkMgras != null) {
					...
					for (int wMgra : walkMgras) {
						...
						if (wMgra != mgra) {
							...
							numeratorM += mstallsoth[wMgra]
									* Math.exp(workDistCoeff * dist)
									* mparkcost[wMgra];
							denominatorM += mstallsoth[wMgra]
									* Math.exp(workDistCoeff * dist);

							numeratorD += dstallsoth[wMgra]
									* Math.exp(otherDistCoeff * dist)
									* dparkcost[wMgra];
							denominatorD += dstallsoth[wMgra]
									* Math.exp(otherDistCoeff * dist);

							discountFactor = Math.max(
									1 - (numfreehrs[wMgra] / 4), 0);
							numeratorH += hstallsoth[wMgra]
									* Math.exp(otherDistCoeff * dist)
									* discountFactor * hparkcost[wMgra];
							denominatorH += hstallsoth[wMgra]
									* Math.exp(otherDistCoeff * dist);
			...
			out.println(mgra + "," + mgraParkArea[mgra] + ","
					+ lsWgtAvgCostM[mgra] + "," + lsWgtAvgCostD[mgra] + ","
					+ lsWgtAvgCostH[mgra]);
		}
