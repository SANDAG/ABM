        ...
        implements Serializable
{
    ...
    /**
     * Get a static instance of the Mgra data manager. Creates one if it is not initialized.
     * 
     * @param rb
    ...
    /**
     * This method should only be used after the getInstance(ResourceBundle rb) method has been called since the rb is needed to read in all the data
     * and populate the object. This method will return the instance that has already been populated.
     * 
     * @return instance
     ...
     * 
     * @param rb
     *            The resourcebundle with the scenario.path and mgra.wlkacc.taps.and.distance.file properties.
     */
     ...
     * 
     * @param rb
     *            ResourceBundle with scenario.path and mgra.walkdistance.file property.
     */
     ...
     * @param mgra
     *            The mgra to look up
     * @return The mgras within walking distance. Null is returned if no mgras are within walk distance.
     */
     ...
     * @param tap
     *            The tap to lookup
     * @return The position of the tap in the mgra array. -1 is returned if it is an invalid tap for the mgra, or if the tap is not within walking
     *         distance.
     */
    ...
    /**
     * Get the walk distance from an MGRA to an MGRA. Return 0 if not within walking distance.
     * 
     * @param oMgra
    ...
    /**
     * Get the walk time from an MGRA to an MGRA. Return 0 if not within walking distance.
     * 
     * @param oMgra
     *            The number of the production/origin MGRA.
     * @param dMgra
    ...
    /**
     * Get the bike time from an MGRA to an MGRA. Return 0 if not within walking distance.
     * 
     * @param oMgra
     *            The number of the production/origin MGRA.
     * @param dMgra
    ...
    /**
     * get arrays of drive accessible TAPS for each MGRA and populate an array of sets so that later one can determine, for a given mgra, if a tap is
    ...
    /**
     * @param mgra
     *            for which we want to know if TAP can be reached by drive access
     * @param tap
     *            for which we want to know if the mgra can reach it by drive access
     ...
     * @param tap
     *            for which we want to know if the mgra can reach it by walk access
     * @return true if reachable; false otherwise
     */
    ...
    /**
     * Process the 4D density land use data file and store the selected fields as arrays indexed by the mgra value. The data fields are in the mgra
     * TableDataSet read from the MGRA csv file.
     * 
     * @param rbMap
     *            is a HashMap for the resource bundle generated from the properties file.
     */
