...
public class VisitorTourDestChoiceModel {
	...
	private double[][][] mgraProbabilities; // by purpose, tazNumber, mgra index
											// (sequential, 0-based)
	private Matrix[] tazProbabilities; // by purpose, origin TAZ, destination
										// TAZ
	...
	private HashMap<Integer, Integer> frequencyChosen; // by mgra, number of
														// times chosen
	...
	public VisitorTourDestChoiceModel(HashMap<String, String> rbMap,
		...
		for (int i = 0; i < soaModel.length; ++i) {
			...
			String purpose = VisitorModelStructure.VISITOR_PURPOSES[i]
					.toLowerCase();
			...
			// page and data page.
			soaModel[i] = new ChoiceModelApplication(visitorDCSoaFileName,
		...
		sizeTermUEC = new UtilityExpressionCalculator(new File(
				visitorDCSoaFileName), sizePage, dataPage, rbMap,
				(VariableTable) dcDmu);

		// create the full model UECs
		// read the model pages from the property file, create one choice model
		// for each full model
		String visitorDCFileName = Util.getStringValueFromPropertyMap(rbMap,
				"visitor.dc.uec.file");
		...
		for (int i = 0; i < destModel.length; ++i) {
			...
			// create a ChoiceModelApplication object for the filename, model
			// page and data page.
			destModel[i] = new ChoiceModelApplication(visitorDCFileName,
					destModelPage, dataPage, rbMap, (VariableTable) dcDmu);
			if (i == 0)
		...
		tourModeChoiceModel = new VisitorTourModeChoiceModel(rbMap,
	...
	public void calculateSizeTerms(VisitorDmuFactoryIf dmuFactory) {
		...
		VisitorTourDestChoiceDMU aDmu = dmuFactory
	...
	public void calculateTazProbabilities(VisitorDmuFactoryIf dmuFactory) {
		...
		for (int purpose = 0; purpose < soaModel.length; ++purpose) {
			...
			tazProbabilities[purpose] = new Matrix("Prob_Matrix",
					"Probability Matrix", altData.getRowCount() + 1,
					altData.getRowCount() + 1);
			...
			for (int taz = 0; taz < tazs.length; ++taz) {
				...
				dcDmu.setDmuIndexValues(originTaz, originTaz, originTaz,
						originTaz, false);

				dcDmu.setPurpose(purpose);

				// Calculate utilities & probabilities
				soaModel[purpose].computeUtilities(dcDmu,
						dcDmu.getDmuIndexValues());

				// Store probabilities (by purpose)
				double[] probabilities = soaModel[purpose]
				...
				for (int i = 0; i < probabilities.length; ++i) {
					...
					tazProbabilities[purpose].setValueAt(originTaz, destTaz,
							(float) cumProb);
				}
			}
		}
			...
			for (int i = 0; i < tazCumProb.getColumnCount(); ++i) {
				...
				if (tazCumProb.getValueAt(originTaz, destinationTaz) > random) {
					...
					if (i != 0) {
						...
						altProb = tazCumProb.getValueAt(originTaz,
								destinationTaz)
								- tazCumProb.getValueAt(originTaz, tazCumProb
										.getExternalColumnNumber(i - 1));
					} else {
						altProb = tazCumProb.getValueAt(originTaz,
								destinationTaz);
					}
	...
	private void calculateLogsumsForSample(VisitorTour tour) {
		...
		for (int sample = 1; sample <= sampleRate; ++sample) {
			...
			if (sampleMgras[sample] > 0) {
				...
				double logsum = tourModeChoiceModel.getModeChoiceLogsum(tour,
						logger, "Sample logsum " + sample,
						"tour " + tour.getID() + " dest " + destinationMgra);
	...
	public void chooseDestination(VisitorTour tour) {
		...
		if (tour.getDebugChoiceModels()) {
			destModel[purpose].logUECResults(logger,
					"Visitor tour destination model");
		}
