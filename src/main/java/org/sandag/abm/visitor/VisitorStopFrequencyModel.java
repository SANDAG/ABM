...
public class VisitorStopFrequencyModel {
	...
	public void calculateStopFrequency(VisitorTour tour) {
		...
		if (!modelStructure.getTourModeIsSovOrHov(tourMode)
				&& !modelStructure.getTourModeIsTaxi(tourMode))
			return;

		if (tour.getDebugChoiceModels()) {
			logger.info("Choosing stop frequency for purpose "
					+ modelStructure.VISITOR_PURPOSES[purpose]
		...
		for (int i = 0; i < cumProbability[purpose].length; ++i) {
			...
			if (tour.getDebugChoiceModels()) {
				logger.info("lower bound "
						+ lowerBoundDurationHours[purpose][i] + " upper bound "
						+ upperBoundDurationHours[purpose][i]);
			}
	 ...
	 */
	private boolean tourIsInRange(VisitorTour tour, int lowerBound,
			int upperBound) {
		...
		if ((tourDurationInHours >= (float) lowerBound)
				&& (tourDurationInHours <= (float) upperBound))
			return true;
	 ...
	 */
	private VisitorStop[] generateOutboundStops(VisitorTour tour,
			int numberOfStops) {
	 ...
	 */
	private VisitorStop[] generateInboundStops(VisitorTour tour,
			int numberOfStops) {
