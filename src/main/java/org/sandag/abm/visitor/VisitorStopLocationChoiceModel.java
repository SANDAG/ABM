...
public class VisitorStopLocationChoiceModel {
	...
	private void setupStopLocationChoiceModel(HashMap<String, String> rbMap,
		...
		logger.info(String
				.format("setting up visitor stop location choice model."));
		...
		String visitorStopLocationSoaFileName = Util
				.getStringValueFromPropertyMap(rbMap,
						"visitor.slc.soa.uec.file");
		visitorStopLocationSoaFileName = uecFileDirectory
				+ visitorStopLocationSoaFileName;

		int soaDataPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "visitor.slc.soa.data.page"));
		int soaModelPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "visitor.slc.soa.model.page"));

		String visitorStopLocationFileName = Util
				.getStringValueFromPropertyMap(rbMap, "visitor.slc.uec.file");
		visitorStopLocationFileName = uecFileDirectory
				+ visitorStopLocationFileName;

		int dataPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "visitor.slc.data.page"));
		int modelPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "visitor.slc.model.page"));

		// create a ChoiceModelApplication object for the SOA model.
		soaModel = new ChoiceModelApplication(visitorStopLocationSoaFileName,
				soaModelPage, soaDataPage, rbMap, (VariableTable) dmu);

		// create a ChoiceModelApplication object for the full model.
		destModel = new ChoiceModelApplication(visitorStopLocationFileName,
	...
	private void createSample(VisitorTour tour, VisitorStop stop) {
		...
		soaTourOrigToStopDistanceAlt = logsumHelper.getAnmSkimCalculator()
				.getTazDistanceFromTaz(origTaz, period);
		soaStopToTourDestDistanceAlt = logsumHelper.getAnmSkimCalculator()
				.getTazDistanceToTaz(destTaz, period);
	...
	public void chooseStopLocation(VisitorTour tour, VisitorStop stop) {
		...
		if (tour.getDebugChoiceModels()) {
			String decisionMakerLabel = "Tour ID " + tour.getID() + " stop id "
					+ stop.getId() + " purpose "
					+ modelStructure.VISITOR_PURPOSES[stop.getPurpose()];
	...
	public void setTripModeChoiceModel(
			VisitorTripModeChoiceModel tripModeChoiceModel) {
