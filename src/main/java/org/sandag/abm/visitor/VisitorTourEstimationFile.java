...
public class VisitorTourEstimationFile {
	...
	public void createEstimationFile() {
		...
		tourModeChoiceModel = new VisitorTourModeChoiceModel(rbMap,
				myModelStructure, dmuFactory, logsumsCalculator);
	...
	public void calculateTourMCLogsums() {
		...
		float[][] sampleLogsum = new float[SAMPLE_SIZE][estimationData
				.getRowCount()];
		...
		for (int i = 0; i < estimationData.getRowCount(); ++i) {
			...
			byte segment = (byte) estimationData
					.getValueAt(i + 1, "PersonType");
			byte purpose = (byte) estimationData.getValueAt(i + 1, "PURPOSE");
			byte income = (byte) estimationData.getValueAt(i + 1, "HHIncome");
			byte autoAvailable = (byte) estimationData.getValueAt(i + 1,
					"AutoAvail");
			byte participants = (byte) (estimationData.getValueAt(i + 1,
					"Persons") + 1);
			int departTime = (int) estimationData.getValueAt(i + 1,
					"PeriodDepart");
			int arriveTime = (int) estimationData.getValueAt(i + 1,
					"PeriodArrive");
			int originMGRA = (int) estimationData.getValueAt(i + 1,
					"originMGRA");
			int destinationMGRA = (int) estimationData.getValueAt(i + 1,
					"destMGRA");
			...
			for (int j = 0; j < SAMPLE_SIZE; ++j) {
				...
				if (sample[j] > 0 && originMGRA > 0) {
					logsum = tourModeChoiceModel.getModeChoiceLogsum(tour,
							logger, "DCEstimationFileLogsum", "ID " + ID);
	...
	public static void main(String[] args) {
		...
		VisitorTourEstimationFile visitorEstimationFile = new VisitorTourEstimationFile(
				pMap);
