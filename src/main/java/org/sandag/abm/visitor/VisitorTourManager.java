...
public class VisitorTourManager {
	...
	public VisitorTourManager(HashMap<String, String> rbMap) {
		...
		String directory = Util.getStringValueFromPropertyMap(rbMap,
				"Project.Directory");
		String mgraFile = Util.getStringValueFromPropertyMap(rbMap,
				"mgra.socec.file");
		...
		businessHotelPercent = new Float(Util.getStringValueFromPropertyMap(
				rbMap, "visitor.hotel.businessPercent"));
		businessHouseholdPercent = new Float(
	...
	public void generateVisitorTours() {
		...
		for (int i = 1; i <= rows; ++i) {
			...
			int businessParties = Math.round(hotelVisitorParties
					* businessHotelPercent);
			int personalParties = Math.round(hotelVisitorParties
					* (1.0f - businessHotelPercent));

			businessParties += Math.round(householdVisitorParties
					* businessHouseholdPercent);
	...
	private void calculateSize(VisitorTour tour) {
		...
		if (size == -1) {
			logger.error("Error attempting to choose party size for visitor tour "
					+ tour.getID());
			throw new RuntimeException();
		}
	...
	private void writeTour(VisitorTour tour, PrintWriter writer) {
		String record = new String(tour.getID() + "," + tour.getSegment() + ","
				+ tour.getPurpose() + "," + tour.getAutoAvailable() + ","
				+ tour.getNumberOfParticipants() + "," + tour.getIncome() + ","
				+ tour.getDepartTime() + "," + tour.getArriveTime() + ","
				+ tour.getOriginMGRA() + "," + tour.getDestinationMGRA() + ","
				+ tour.getTourMode() + "," + tour.getNumberOutboundStops()
	 ...
	 * @param writer
	 */
	private void writeTrip(VisitorTour tour, VisitorTrip trip, int tripNumber,
			PrintWriter writer) {
