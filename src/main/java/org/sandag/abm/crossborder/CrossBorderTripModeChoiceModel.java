...
public class CrossBorderTripModeChoiceModel {
	...
	private void setupTripModeChoiceModel(HashMap<String, String> propertyMap,
		...
		tripModeChoiceModel = new ChoiceModelApplication(tripModeUecFile,
				modelPage, dataPage, propertyMap, (VariableTable) dmu);
	...
	public double computeUtilities(CrossBorderTour tour, CrossBorderTrip trip) {
		...
		if (tour.getDebugChoiceModels()) {
			...
			tripModeChoiceModel.logUECResults(logger,
					"Cross border trip mode choice model");
	...
	public void setDmuAttributes(CrossBorderTour tour, CrossBorderTrip trip) {
		...
		dmu.setDmuIndexValues(tripOriginTaz, tripDestinationTaz, tripOriginTaz,
				tripDestinationTaz, tour.getDebugChoiceModels());
		...
		if (tour.getTourMode() == modelStructure.DRIVEALONE)
			dmu.setTourModeIsDA(1);
		else
			dmu.setTourModeIsDA(0);

		if (tour.getTourMode() == modelStructure.SHARED2)
			dmu.setTourModeIsS2(1);
		else
			dmu.setTourModeIsS2(0);

		if (tour.getTourMode() == modelStructure.SHARED3)
			dmu.setTourModeIsS3(1);
		else
			dmu.setTourModeIsS3(0);

		if (tour.getTourMode() == modelStructure.WALK)
			dmu.setTourModeIsWalk(1);
		else
			dmu.setTourModeIsWalk(0);

		if (tour.isSentriAvailable())
			dmu.setTourCrossingIsSentri(1);
		else
			dmu.setTourCrossingIsSentri(0);
