...
public class CrossBorderStopLocationChoiceModel {
	...
	private TableDataSet alternativeData; // the alternatives, with a "dest" -
											// indicating the destination TAZ in
											// San Diego County
	...
	private double[][] sampledSizeTerms; // by purpose, alternative (taz or
											// sampled mgra)
	private double[] correctionFactors; // by alternative (sampled mgra, for
										// full model only)
	...
	private int originMgra; // the origin MGRA of the stop (originMgra ->
	 ...
	 */
	public CrossBorderStopLocationChoiceModel(
			HashMap<String, String> propertyMap,
			CrossBorderModelStructure myModelStructure,
	...
	private void setupStopLocationChoiceModel(HashMap<String, String> rbMap,
		...
		logger.info(String
				.format("setting up cross border stop location choice model."));
		...
		String crossBorderStopLocationSoaFileName = Util
				.getStringValueFromPropertyMap(rbMap,
						"crossBorder.slc.soa.uec.file");
		crossBorderStopLocationSoaFileName = uecFileDirectory
				+ crossBorderStopLocationSoaFileName;

		int soaDataPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.slc.soa.data.page"));
		int soaModelPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.slc.soa.model.page"));

		String crossBorderStopLocationFileName = Util
				.getStringValueFromPropertyMap(rbMap,
						"crossBorder.slc.uec.file");
		crossBorderStopLocationFileName = uecFileDirectory
				+ crossBorderStopLocationFileName;

		int dataPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.slc.data.page"));
		int modelPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.slc.model.page"));

		// create a ChoiceModelApplication object for the SOA model.
		soaModel = new ChoiceModelApplication(
				crossBorderStopLocationSoaFileName, soaModelPage, soaDataPage,
				rbMap, (VariableTable) dmu);

		// create a ChoiceModelApplication object for the full model.
		destModel = new ChoiceModelApplication(crossBorderStopLocationFileName,
				modelPage, dataPage, rbMap, (VariableTable) dmu);
	...
	private void createSample(CrossBorderTour tour, CrossBorderStop stop) {
		...
		// distances
		soaTourOrigToStopDistanceAlt = logsumHelper.getAnmSkimCalculator()
				.getTazDistanceFromTaz(originTAZ, period);
		soaStopToTourDestDistanceAlt = logsumHelper.getAnmSkimCalculator()
				.getTazDistanceToTaz(destinationTAZ, period);
		...
		for (int sample = 1; sample <= sampleRate; ++sample) {
			...
			for (int i = 0; i < alts.length; ++i) {
				...
				if (random < cumProb) {
					sampledTaz = (int) alternativeData
							.getValueAt(i + 1, "dest");
			...
			if (mgraCumProb == null) {
				logger.error("Error: mgraCumProb array is null for purpose "
						+ purpose + " sampledTaz " + sampledTaz + " hhID "
						+ tour.getID());
		...
		for (int sample = 1; sample <= sampleRate; ++sample) {
			...
			correctionFactors[sample] = (float) Math.log((double) freq
	...
	public void chooseStopLocation(CrossBorderTour tour, CrossBorderStop stop) {
		...
		if (tour.getDebugChoiceModels()) {
			String decisionMakerLabel = "Tour ID " + tour.getID() + " stop id "
					+ stop.getId() + " purpose "
					+ modelStructure.CROSSBORDER_PURPOSES[stop.getPurpose()];
		...
		if (tour.getDebugChoiceModels()) {
			...
			logger.info("Chose alternative " + alt + " mgra " + destMgra
					+ " with random number " + random);
			logger.info("");
			logger.info("");
		}
	 ...
	 */
	public void setTripModeChoiceModel(
			CrossBorderTripModeChoiceModel tripModeChoiceModel) {
