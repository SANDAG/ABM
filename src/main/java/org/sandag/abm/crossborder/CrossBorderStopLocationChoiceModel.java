...
public class CrossBorderStopLocationChoiceModel {
	 ...
	 */
	public CrossBorderStopLocationChoiceModel(HashMap<String, String> propertyMap, CrossBorderModelStructure myModelStructure, CrossBorderDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper){
	 ...
	 * @param dmuFactory
	 */
    private void setupStopLocationChoiceModel( HashMap<String, String> rbMap, CrossBorderDmuFactoryIf dmuFactory  ) {
        ...
        String crossBorderStopLocationSoaFileName = Util.getStringValueFromPropertyMap(rbMap, "crossBorder.slc.soa.uec.file");
        crossBorderStopLocationSoaFileName = uecFileDirectory + crossBorderStopLocationSoaFileName;
        
        int soaDataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "crossBorder.slc.soa.data.page" ) );
        int soaModelPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,  "crossBorder.slc.soa.model.page" ) );
        
        String crossBorderStopLocationFileName = Util.getStringValueFromPropertyMap(rbMap, "crossBorder.slc.uec.file");
        crossBorderStopLocationFileName = uecFileDirectory + crossBorderStopLocationFileName;
        
        int dataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "crossBorder.slc.data.page" ) );
        int modelPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,  "crossBorder.slc.model.page" ) );
        
        // create a ChoiceModelApplication object for the SOA model.
        soaModel = new ChoiceModelApplication( crossBorderStopLocationSoaFileName, soaModelPage, soaDataPage, rbMap, (VariableTable)dmu );
       
        // create a ChoiceModelApplication object for the full model.
        destModel = new ChoiceModelApplication( crossBorderStopLocationFileName, modelPage, dataPage, rbMap, (VariableTable)dmu );
    ...
    private void createSample(CrossBorderTour tour, CrossBorderStop stop){
    	...
    	//distances
    	soaTourOrigToStopDistanceAlt = logsumHelper.getAnmSkimCalculator().getTazDistanceFromTaz(originTAZ, period);
    	soaStopToTourDestDistanceAlt = logsumHelper.getAnmSkimCalculator().getTazDistanceToTaz(destinationTAZ, period);
     ...
     * 
     * @param tour  The cross border tour.
     * @param stop  The cross border stop.
     */
    public void chooseStopLocation(CrossBorderTour tour, CrossBorderStop stop){
        ...
        {
        	String decisionMakerLabel = "Tour ID "+tour.getID()+" stop id "+stop.getId()+" purpose "+modelStructure.CROSSBORDER_PURPOSES[stop.getPurpose()];
