...
public class CrossBorderStopTimeOfDayChoiceModel {
	...
	private double[][] outboundCumProbability; // by alternative: outbound
												// cumulative probability
												// distribution
	private int[] outboundOffsets; // by alternative: offsets for outbound stop
									// duration choice

	private double[][] inboundCumProbability; // by alternative: inbound
												// cumulative probability
												// distribution
	private int[] inboundOffsets; // by alternative: offsets for inbound stop
									// duration choice
	private CrossBorderModelStructure modelStructure;

	private HashMap<Integer, Integer> outboundElementMap; // Hashmap used to get
															// the element
															// number of the
															// cumProbability
															// array based on
	...
	private HashMap<Integer, Integer> inboundElementMap; // Hashmap used to get
															// the element
															// number of the
															// cumProbability
															// array based on
	...
	public void readOutboundFile(String fileName) {
		...
		for (int row = 1; row <= rows; ++row) {
			...
			int lowerBound = (int) outboundTable
					.getValueAt(row, "RemainingLow");
			int upperBound = (int) outboundTable.getValueAt(row,
					"RemainingHigh");
	...
	public void readInboundFile(String fileName) {
		...
		for (int row = 1; row <= rows; ++row) {
			...
			int upperBound = (int) inboundTable
	...
	public void chooseTOD(CrossBorderTour tour, CrossBorderStop stop) {
		...
		if (!inbound) {
			...
			if (tour.getDebugChoiceModels()) {
				logger.info("Stop TOD Choice Model for tour " + tour.getID()
						+ " outbound stop " + stop.getId()
						+ " periods remaining " + periodsRemaining);
			...
			for (int i = 0; i < cumProb.length; ++i) {
				if (random < cumProb[i]) {
					...
					if (tour.getDebugChoiceModels()) {
						logger.info("***");
						logger.info("Chose alt " + i + " offset " + offset
								+ " from depart period " + departPeriod);
			...
			if (stop.getId() == 0) {
				...
				if (tour.getOutboundStops() != null) {
					...
					departPeriod = outboundStops[outboundStops.length - 1]
							.getStopPeriod();
				} else {
					// no outbound stops
					departPeriod = tour.getDepartTime();
			...
			for (int i = 0; i < cumProb.length; ++i) {
				if (random < cumProb[i]) {
					...
					if (tour.getDebugChoiceModels()) {
						logger.info("***");
						logger.info("Chose alt " + i + " offset " + offset
								+ " from arrive period " + arrivePeriod);
						logger.info("Stop period is " + stop.getStopPeriod());
