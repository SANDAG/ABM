...
public class CrossBorderTripTables {
     ...
     public CrossBorderTripTables(HashMap<String,String> rbMap){
		...
		for(int i=1;i<modeIndex.length;++i){
			if(modelStructure.getTripModeIsSovOrHov(i)){
				...
				++nmotModes;
			}else if(modelStructure.getTripModeIsWalkTransit(i)|| modelStructure.getTripModeIsPnrTransit(i)|| modelStructure.getTripModeIsKnrTransit(i)){
				modeIndex[i] = 2;
	...
	public void processTrips(int timePeriod, TableDataSet tripData){
      ...
      //iterate through the trip data and save trips in arrays
        for(int i = 1; i <= tripData.getRowCount(); ++i){
        	...
        	if(modelStructure.getTripModeIsWalkTransit(tripMode)||modelStructure.getTripModeIsPnrTransit(tripMode)||modelStructure.getTripModeIsKnrTransit(tripMode)){
	...
	public void writeTrips(int period, MatrixType mt){
		...
		fileName[0] =directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.results.autoTripMatrix")+end;
		fileName[1] =directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.results.nMotTripMatrix")+end;
		fileName[2] =directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.results.tranTripMatrix")+end; 
		fileName[3] =directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.results.othrTripMatrix")+end; 
	...
	public static void main(String[] args) {
		...
		logger.info(String.format("SANDAG Visitor Model Trip Table Generation Program using CT-RAMP version %s",
