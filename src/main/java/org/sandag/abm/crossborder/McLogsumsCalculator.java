...
public class McLogsumsCalculator implements Serializable {
	...
	private transient Logger autoSkimLogger = Logger
			.getLogger("crossBorderModel");
	private transient Logger wtwSkimLogger = Logger
			.getLogger("crossBorderModel");
	private transient Logger wtdSkimLogger = Logger
			.getLogger("crossBorderModel");
	private transient Logger dtwSkimLogger = Logger
			.getLogger("crossBorderModel");
	...
	public void setTazDistanceSkimArrays(
		...
		anm.get().setTazDistanceSkimArrays(storedFromTazDistanceSkims,
	 ...
	 * @param mcDmuObject
	 */
	public void setTripMcDmuSkimAttributes(CrossBorderTour tour,
			CrossBorderTrip trip, CrossBorderTripModeChoiceDMU mcDmuObject) {
		...
		if (trip.getBestWtwTapPairs() == null) {
			...
			bestTapPairs = wtw.get().getBestTapPairs(origMgra, destMgra,
	...
	private void setNmTripMcDmuAttributes(
			CrossBorderTripModeChoiceDMU tripMcDmuObject, int origMgra,
		...
		nmSkims = anm.get().getNonMotorizedSkims(origMgra, destMgra,
				departPeriod, loggingEnabled, autoSkimLogger);
		if (loggingEnabled)
			anm.get().logReturnedSkims(origMgra, destMgra, departPeriod,
					nmSkims, "non-motorized trip mode choice skims",
					autoSkimLogger);
	...
	private void setWtwTripMcDmuAttributes(
		...
		if (bestTapPairs == null) {
			crSkims = wtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("CR"));
			lrSkims = wtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("LR"));
			brSkims = wtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("BRT"));
			ebSkims = wtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("EB"));
			lbSkims = wtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("LB"));
			return;
		}
		...
		if (bestTapPairs[i] != null) {
			...
			crSkims = wtw.get().getWalkTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			lrSkims = wtw.get().getWalkTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			brSkims = wtw.get().getWalkTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			ebSkims = wtw.get().getWalkTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			lbSkims = wtw.get().getWalkTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
	...
	private void setWtdTripMcDmuAttributes(
		...
		if (bestTapPairs == null) {
			crSkims = wtd.get().getNullTransitSkims(
					Modes.getTransitModeIndex("CR"));
			lrSkims = wtd.get().getNullTransitSkims(
					Modes.getTransitModeIndex("LR"));
			brSkims = wtd.get().getNullTransitSkims(
					Modes.getTransitModeIndex("BRT"));
			ebSkims = wtd.get().getNullTransitSkims(
					Modes.getTransitModeIndex("EB"));
			lbSkims = wtd.get().getNullTransitSkims(
					Modes.getTransitModeIndex("LB"));
			return;
		}
		...
		if (bestTapPairs[i] != null) {
			...
			crSkims = wtd.get().getWalkTransitDriveSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findDriveTransitEgressTime(destMgra, bestTapPairs[i][1]);
			lrSkims = wtd.get().getWalkTransitDriveSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findDriveTransitEgressTime(destMgra, bestTapPairs[i][1]);
			brSkims = wtd.get().getWalkTransitDriveSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findDriveTransitEgressTime(destMgra, bestTapPairs[i][1]);
			ebSkims = wtd.get().getWalkTransitDriveSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findDriveTransitEgressTime(destMgra, bestTapPairs[i][1]);
			lbSkims = wtd.get().getWalkTransitDriveSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
	...
	private void setDtwTripMcDmuAttributes(
		...
		if (bestTapPairs == null) {
			crSkims = dtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("CR"));
			lrSkims = dtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("LR"));
			brSkims = dtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("BRT"));
			ebSkims = dtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("EB"));
			lbSkims = dtw.get().getNullTransitSkims(
					Modes.getTransitModeIndex("LB"));
			return;
		}
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			crSkims = dtw.get().getDriveTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			lrSkims = dtw.get().getDriveTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			brSkims = dtw.get().getDriveTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			ebSkims = dtw.get().getDriveTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
		...
		if (bestTapPairs[i] != null) {
			...
			double aWalkTime = BestTransitPathCalculator
					.findWalkTransitEgressTime(destMgra, bestTapPairs[i][1]);
			lbSkims = dtw.get().getDriveTransitWalkSkims(i, pWalkTime,
					aWalkTime, bestTapPairs[i][0], bestTapPairs[i][1],
					departPeriod, loggingEnabled);
		} else {
