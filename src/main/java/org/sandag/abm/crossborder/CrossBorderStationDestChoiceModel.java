 ...
 * 
 * The model first calculates a set of station-level logsums which represent the attractiveness of each station based upon
 * the accessibility to Mexico populations (taking into account population size of Colonias and distance between each station
 * and each Colonia.  The model then creates a sample of alternatives where each alternative is a pair of border crossing station (entry MGRA) and
 * destination MGRA in San Diego County, by tour purpose.  This is sampled from for each tour, and a mode choice logsum is calculated for each station-MGRA
 * pair.  The full destination choice model is run on the sample with the mode choice logsums influencing station - destination choice, and a 
 * station-MGRA pair is chosen for each tour.
 *  
 * @author Freedman
 *
 */
public class CrossBorderStationDestChoiceModel {
	...
	private class KeyClass{
        ...
        public boolean equals(Object obj) {         
            if (obj instanceof KeyClass) {             
                return station == (((KeyClass)obj).station) &&
				   mgra == (((KeyClass)obj).mgra);
				   }         
			   return false;
          }      
	...
	private TableDataSet alternativeData;    //the alternatives, with the following fields: 
                                             ...
                                             //  "dest" - indicating the destination TAZ in San Diego County  	
     ...
     * @param dmuFactory   Factory object for creation of cross border model DMUs
     */
	public CrossBorderStationDestChoiceModel(HashMap<String,String> rbMap, CrossBorderModelStructure myStructure,
			CrossBorderDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper){
        ...
        String crossBorderDCSoaFileName = Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.soa.uec.file");
        crossBorderDCSoaFileName = uecFileDirectory + crossBorderDCSoaFileName;
        
        int soaDataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.soa.data.page" ) );
        int soaSizePage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.soa.size.page" ) );
        int soaModelPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,  "crossBorder.dc.soa.model.page" ) );
        
        String crossBorderDCFileName = Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.uec.file");
        crossBorderDCFileName = uecFileDirectory + crossBorderDCFileName;
        
        int dataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.data.page" ) );
        int modelPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,  "crossBorder.dc.model.page" ) );
        ...
        soaModel = new ChoiceModelApplication( crossBorderDCSoaFileName, soaModelPage, soaDataPage, rbMap, (VariableTable)dcDmu );
       
        // create a ChoiceModelApplication object for the full model.
        destModel = new ChoiceModelApplication( crossBorderDCFileName, modelPage, dataPage, rbMap, (VariableTable)dcDmu );
        ...
        sizeTermUEC = new UtilityExpressionCalculator(new File(crossBorderDCSoaFileName), soaSizePage, soaDataPage, rbMap, dmuFactory.getCrossBorderStationChoiceDMU());
        ...
        String coloniaDistanceFile = Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.colonia.file");
        ...
        float distanceParam = new Float(Util.getStringValueFromPropertyMap(rbMap, "crossBorder.dc.colonia.distance.parameter"));
    	...
    	tourModeChoiceModel = new CrossBorderTourModeChoiceModel(rbMap, myStructure, dmuFactory, myLogsumHelper);
	...
	/**
	 * Calculate the station logsum.  Station logsums are based on distance from supercolonia to station
	 * and population of supercolonia, as follows:
	 * 
	 ...
	 * 
	 *  Population  Population of supercolonia
	 *  Distance_MGRANumber where MGRANumber is the number of the MGRA corresponding to the entry station, with
	 *     one field for each possible entry station.
	 *     
	 * @param fileName  Name of file containing supercolonia population and distance
	 * @param distanceParameter  Parameter for distance.
	 */
	...
	public void calculateTazProbabilities(CrossBorderDmuFactoryIf dmuFactory){
        ...
        for(int purpose = 0; purpose < purposes; ++ purpose){
            ...
            tazStationProbabilities[purpose] = Arrays.copyOf(soaModel.getCumulativeProbabilities(),soaModel.getCumulativeProbabilities().length);
        }
	 ...
	 * @param tour CrossBorderTour with purpose and Random
	...
	private void chooseStationMgraSample(CrossBorderTour tour){
		...
		for(int sample = 1;sample<=sampleRate;++sample){
			...
			int freq = frequencyChosen.get(key);
			sampledCorrectionFactors[sample] = (float) Math.log((double) freq / sampledCorrectionFactors[sample]);
	 ...
	 * 
	 * @param tour  The tour attributes used are tour purpose, depart and arrive periods,
	 * and sentri availability.
	 */
	private void calculateLogsumsForSample(CrossBorderTour tour){
		...
		for(int sample = 1; sample <=sampleRate;++sample){
			...
			if(sampledEntryMgra[sample]>0){
				...
				double logsum = tourModeChoiceModel.getLogsum(tour, logger, "Sample logsum "+sample, "tour "+tour.getID());
				tourModeChoiceLogsums[sample]=logsum;
			}
			else
				tourModeChoiceLogsums[sample]=0;
	 ...
	 * 
	 * @param tour A cross border tour with a tour purpose and departure\arrival time and SENTRI availability members.
	 */
	public void chooseStationAndDestination(CrossBorderTour tour){
        ...
        if(tour.getDebugChoiceModels()){
        	...
        	//log the sample
        	logSample();
        	destModel.choiceModelUtilityTraceLoggerHeading("Station-destination model",
                    "tour "+tour.getID());
        }
	...
	public void logSample(){
		...
		for(int i = 1; i<=sampleRate;++i){
			logger.info(i+" "+sampledStations[i]+" "+sampledEntryMgra[i]+" "+ sampledDestinationMgra[i]);
		}
		logger.info("");
	}
