...
public class CrossBorderStationDestChoiceModel {
	...
	private double[] stationLogsums; // by entry station, logsum from colonia to
										// station
	private double[] soaStationLogsums; // by station-TAZ alternative, station
										// logsums
	private double[][] soaSizeTerms; // by purpose, station-TAZ alternative,
										// size terms for tazs
	private int[] soaOriginTazs; // by station-TAZ alternative, origin Taz
	private int[] soaDestinationTazs; // by station-TAZ alternative, destination
										// Taz

	private int[] sampledDestinationMgra; // destination mgra for each of n
											// samples
	private int[] sampledEntryMgra; // entry mgra for each of n samples
	private double[][] sampledSizeTerms; // size term for each of n samples (1st
											// dimension is purpose)
	private double[] sampledStationLogsums; // station logsum for each of n
											// samples
	private int[] sampledStations; // POE for each of n samples
	private int[] sampledOriginTazs; // Origin Taz for each of n samples
	private int[] sampledDestinationTazs; // Destination Taz for each of n
											// samples

	private double[] sampledCorrectionFactors; // correction factor for each of
												// n samples
	private double[] tourModeChoiceLogsums; // mode choice logsum for each of n
											// samples
	...
	private HashMap<KeyClass, Integer> frequencyChosen; // by alternative,
														// number of times
														// chosen

	private TableDataSet alternativeData; // the alternatives, with the
											// following fields:
											// "EntryMGRA" - indicating border
											// crossing entry MGRA
											// "dest" - indicating the
											// destination TAZ in San Diego
											// County
	...
	public CrossBorderStationDestChoiceModel(HashMap<String, String> rbMap,
			CrossBorderModelStructure myStructure,
		...
		String crossBorderDCSoaFileName = Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.soa.uec.file");
		crossBorderDCSoaFileName = uecFileDirectory + crossBorderDCSoaFileName;

		int soaDataPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.soa.data.page"));
		int soaSizePage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.soa.size.page"));
		int soaModelPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.soa.model.page"));

		String crossBorderDCFileName = Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.uec.file");
		...
		int dataPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.data.page"));
		int modelPage = Integer.parseInt(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.model.page"));

		// read the model pages from the property file, create one choice model
		// for each
		CrossBorderStationDestChoiceDMU dcDmu = dmuFactory
				.getCrossBorderStationChoiceDMU();

		// create a ChoiceModelApplication object for the SOA model.
		soaModel = new ChoiceModelApplication(crossBorderDCSoaFileName,
				soaModelPage, soaDataPage, rbMap, (VariableTable) dcDmu);

		// create a ChoiceModelApplication object for the full model.
		destModel = new ChoiceModelApplication(crossBorderDCFileName,
				modelPage, dataPage, rbMap, (VariableTable) dcDmu);
		...
		sizeTermUEC = new UtilityExpressionCalculator(new File(
				crossBorderDCSoaFileName), soaSizePage, soaDataPage, rbMap,
				dmuFactory.getCrossBorderStationChoiceDMU());
		...
		float distanceParam = new Float(Util.getStringValueFromPropertyMap(
				rbMap, "crossBorder.dc.colonia.distance.parameter"));
		...
		// probabilities
		tourModeChoiceModel = new CrossBorderTourModeChoiceModel(rbMap,
				myStructure, dmuFactory, myLogsumHelper);
	...
	private void calculateStationLogsum(String fileName, float distanceParameter) {
		...
		for (int i = 0; i < colonias; ++i) {
			...
			for (int j = 0; j < stations; ++j) {
				...
				float distance = coloniaTable.getValueAt(i + 1, "Distance_poe"
						+ j);
				if (population > 0)
					stationLogsums[j] += Math.exp(distanceParameter * distance)
							* population;
			}
		}
	...
	public void calculateSizeTerms(CrossBorderDmuFactoryIf dmuFactory) {
		...
		CrossBorderStationDestChoiceDMU aDmu = dmuFactory
				.getCrossBorderStationChoiceDMU();
		...
		for (int purpose = 0; purpose < purposes; ++purpose) {
			for (int taz = 0; taz < tazs.length; ++taz) {
				...
				if (tazSizeTerms[purpose][tazNumber] > 0.0
						&& Math.abs(lastProb - 1.0) > 0.000001)
					logger.info("Error: purpose " + purpose + " taz "
		...
		for (int purpose = 0; purpose < purposes; ++purpose) {
			for (int taz = 0; taz < tazSizeTerms[purpose].length; ++taz)
				if (tazSizeTerms[purpose][taz] > 0.0)
					tazSizeTerms[purpose][taz] = Math
							.log(tazSizeTerms[purpose][taz] + 1.0);

			for (int mgra = 0; mgra < mgraSizeTerms[purpose].length; ++mgra)
				if (mgraSizeTerms[purpose][mgra] > 0.0)
					mgraSizeTerms[purpose][mgra] = Math
							.log(mgraSizeTerms[purpose][mgra] + 1.0);
	...
	public void calculateTazProbabilities(CrossBorderDmuFactoryIf dmuFactory) {
		...
		for (int purpose = 0; purpose < purposes; ++purpose) {
			...
			tazStationProbabilities[purpose] = Arrays.copyOf(
	...
	private void chooseStationMgraSample(CrossBorderTour tour) {
		...
		for (int sample = 1; sample <= sampleRate; ++sample) {
			...
			// that taz
			int destinationTaz = (int) alternativeData.getValueAt(alt + 1,
					"dest");
			int poe = (int) alternativeData.getValueAt(alt + 1, "poe");
			int entryMgra = (int) alternativeData.getValueAt(alt + 1,
					"mgra_entry");
			sampledEntryMgra[sample] = entryMgra;
			int[] mgraArray = tazManager.getMgraArray(destinationTaz);

			// set the origin taz
			sampledOriginTazs[sample] = (int) alternativeData.getValueAt(
					alt + 1, "poe_taz");
	...
	private void calculateLogsumsForSample(CrossBorderTour tour) {
		...
		for (int sample = 1; sample <= sampleRate; ++sample) {
			...
			if (sampledEntryMgra[sample] > 0) {
				...
				double logsum = tourModeChoiceModel.getLogsum(tour, logger,
						"Sample logsum " + sample, "tour " + tour.getID());
	...
	public void chooseStationAndDestination(CrossBorderTour tour) {
		...
		if (tour.getDebugChoiceModels()) {
			...
			destModel.choiceModelUtilityTraceLoggerHeading(
					"Station-destination model", "tour " + tour.getID());
		}
	...
	public void logSample() {
		...
		for (int i = 1; i <= sampleRate; ++i) {
			logger.info(i + " " + sampledStations[i] + " "
					+ sampledEntryMgra[i] + " " + sampledDestinationMgra[i]);
		}
		logger.info("");
	}
