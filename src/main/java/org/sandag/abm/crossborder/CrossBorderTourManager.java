...
public class CrossBorderTourManager {
    ...
    public CrossBorderTourManager(HashMap<String,String> rbMap){
        ...
        String nonSentriPurposeFile = directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.purpose.nonsentri.file");
        String sentriPurposeFile = directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.purpose.sentri.file");
        
        //the share of cross-border tours that are sentri is an input
        sentriShare =  new Double(Util.getStringValueFromPropertyMap(rbMap, "crossBorder.sentriShare"));
                
        //Read the distributions
        sentriPurposeDistribution = setPurposeDistribution(sentriPurposeFile, sentriPurposeDistribution);
        nonSentriPurposeDistribution = setPurposeDistribution(nonSentriPurposeFile, nonSentriPurposeDistribution);
        totalTours = new Integer(Util.getStringValueFromPropertyMap(rbMap,"crossBorder.tours").replace(",", ""));
	...
	public void writeOutputFile(HashMap<String,String> rbMap){
		...
		String directory = Util.getStringValueFromPropertyMap(rbMap,"Project.Directory");
        String tourFileName = directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.tour.output.file");
        String tripFileName = directory+Util.getStringValueFromPropertyMap(rbMap,"crossBorder.trip.output.file");
		...
		String tourHeaderString = new String("id,purpose,sentri,poe,departTime,arriveTime,originMGRA,destinationMGRA,originTAZ,destinationTAZ,tourMode\n");
		...
		String tripHeaderString = new String("tourID,tripID,originPurp,destPurp,originMGRA,destinationMGRA,originTAZ,destinationTAZ,inbound,originIsTourDestination,destinationIsTourDestination,period,tripMode,boardingTap,alightingTap\n");
	 ...
	 * @param writer
	 */
	private void writeTrip(CrossBorderTour tour, CrossBorderTrip trip, int tripNumber, PrintWriter writer){
