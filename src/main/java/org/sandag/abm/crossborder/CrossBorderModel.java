...
public class CrossBorderModel {
	...
	public CrossBorderModel(HashMap<String, String> rbMap) {
		...
		synchronized (INITIALIZATION_LOCK) { // lock to make sure only one of
		...
		seek = Boolean.valueOf(Util.getStringValueFromPropertyMap(rbMap,
				"crossBorder.seek"));
		traceId = Integer.valueOf(Util.getStringValueFromPropertyMap(rbMap,
				"crossBorder.trace"));
	...
	private final AtomicBoolean calculatorsInitialized = new AtomicBoolean(
			false);
	...
	private void runModel(CrossBorderTour[] tours, int start, int end) {
		...
		CrossBorderDmuFactoryIf dmuFactory = new CrossBorderDmuFactory(
				modelStructure);
		...
		CrossBorderStopFrequencyModel stopFrequencyModel = new CrossBorderStopFrequencyModel(
				rbMap);
		CrossBorderStopPurposeModel stopPurposeModel = new CrossBorderStopPurposeModel(
				rbMap);
		...
		for (int i = start; i < end; i++) {
			...
			CrossBorderTrip[] trips = new CrossBorderTrip[outboundStops
					+ inboundStops + 2];
			int tripNumber = 0;
			...
			if (outboundStops > 0) {
				...
				trips[tripNumber] = new CrossBorderTrip(tour,
	...
	private class CrossBorderModelAction extends DnCRecursiveAction {
		...
		private CrossBorderModelAction(HashMap<String, String> rbMap,
				CrossBorderTour[] tours) {
		...
		private CrossBorderModelAction(HashMap<String, String> rbMap,
				CrossBorderTour[] tours, long start, long length,
				DnCRecursiveAction next) {
		...
		@Override
		protected DnCRecursiveAction getNextAction(long start, long length,
				DnCRecursiveAction next) {
	...
	public void runModel() {
		...
		boolean concurrent = rbMap
				.containsKey(RUN_MODEL_CONCURRENT_PROPERTY_KEY)
				&& Boolean.valueOf(Util.getStringValueFromPropertyMap(rbMap,
						RUN_MODEL_CONCURRENT_PROPERTY_KEY));
		int parallelism = rbMap
				.containsKey(CONCURRENT_PARALLELISM_PROPERTY_KEY) ? Integer
				.valueOf(Util.getStringValueFromPropertyMap(rbMap,
						CONCURRENT_PARALLELISM_PROPERTY_KEY)) : Runtime
				.getRuntime().availableProcessors();
		if (concurrent) { // use fork-join
			CrossBorderModelAction action = new CrossBorderModelAction(rbMap,
					tours);
		...
		try {
			...
			if (!matrixServerAddress.equalsIgnoreCase("none")) {
				...
				if (matrixServerAddress.equalsIgnoreCase("localhost")) {
					matrixServer = crossBorderModel.startMatrixServerProcess(
							matrixServerAddress, serverPort, mt);
					crossBorderModel.ms = matrixServer;
				} else {
					crossBorderModel.ms = new MatrixDataServerRmi(
