...
public class CrossBorderModel {
    ...
    public CrossBorderModel(HashMap<String,String> rbMap){
        ...
        synchronized (INITIALIZATION_LOCK) { //lock to make sure only one of these actually initializes things so we don't cross threads
     ...
     * @param start The starting index of the tours to process.
     * @param end The (exclusive) ending index of the tours to process.
     */
    private void runModel(CrossBorderTour[] tours, int start, int end) {
		...
		CrossBorderTourTimeOfDayChoiceModel todChoiceModel = new CrossBorderTourTimeOfDayChoiceModel(rbMap);
        CrossBorderStationDestChoiceModel destChoiceModel = new CrossBorderStationDestChoiceModel(rbMap, modelStructure, dmuFactory,logsumsCalculator );
		CrossBorderTourModeChoiceModel tourModeChoiceModel = destChoiceModel.getTourModeChoiceModel();
        CrossBorderTripModeChoiceModel tripModeChoiceModel = tourModeChoiceModel.getTripModeChoiceModel();
        ...
        CrossBorderStopTimeOfDayChoiceModel stopTodChoiceModel = new CrossBorderStopTimeOfDayChoiceModel(rbMap);
        CrossBorderStopLocationChoiceModel stopLocationChoiceModel = new CrossBorderStopLocationChoiceModel(rbMap, modelStructure, dmuFactory,logsumsCalculator );
        ...
        for (int i = start; i < end; i++) {
            ...
            if (tourCount % 1000 == 0)
        		logger.info("Processing tour " + tourCount);
    ...
    /**
     * This class is the divide-and-conquer action (void return task) for running the cross-border model using the fork-join
     * framework. The divisible problem is an array of tours, and the actual work is the {@link CrossBorderModel#runModel(CrossBorderTour[],int,int)}
     * method, applied to a section of the array.
     */
    private class CrossBorderModelAction extends DnCRecursiveAction {
        ...
        private CrossBorderModelAction(HashMap<String,String> rbMap, CrossBorderTour[] tours, long start, long length, DnCRecursiveAction next) {
	...
	public void runModel(){
        ...
        boolean concurrent = rbMap.containsKey(RUN_MODEL_CONCURRENT_PROPERTY_KEY) &&
                             Boolean.valueOf(Util.getStringValueFromPropertyMap(rbMap,RUN_MODEL_CONCURRENT_PROPERTY_KEY));
        int parallelism = rbMap.containsKey(CONCURRENT_PARALLELISM_PROPERTY_KEY) ?
                               Integer.valueOf(Util.getStringValueFromPropertyMap(rbMap,CONCURRENT_PARALLELISM_PROPERTY_KEY)) :
                               Runtime.getRuntime().availableProcessors();
        if (concurrent) { //use fork-join
        ...
        } else { // single-threaded: call the model runner in this thread
	...
	private MatrixDataServerRmi startMatrixServerProcess( String serverAddress, int serverPort, MatrixType mt ) throws IOException
	    {
	...
	public static void main(String[] args) {
        ...
        try
        {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
            {
                ...
                if (matrixServerAddress.equalsIgnoreCase("localhost")) {
                    matrixServer = crossBorderModel.startMatrixServerProcess(matrixServerAddress, serverPort, mt);
                    crossBorderModel.ms = matrixServer;
                }
                else {
                	crossBorderModel.ms = new MatrixDataServerRmi( matrixServerAddress, serverPort, MatrixDataServer.MATRIX_DATA_SERVER_NAME );
                	crossBorderModel.ms.testRemote(Thread.currentThread().getName());
                	crossBorderModel.ms.start32BitMatrixIoServer( mt );
        ...
        if (matrixServerAddress.equalsIgnoreCase("localhost")) {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
                crossBorderModel.ms.stop32BitMatrixIoServer();
        }
