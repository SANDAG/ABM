...
/**
 * This class is the stop frequency model for cross border tours.  It is currently based on a static probability distribution
...
public class CrossBorderStopFrequencyModel {
	...
	public CrossBorderStopFrequencyModel(HashMap<String,String> rbMap ){
        ...
        String stopFrequencyFile = Util.getStringValueFromPropertyMap(rbMap,"crossBorder.stop.frequency.file");
	...
	public void calculateStopFrequency(CrossBorderTour tour){
		...
		if(tour.getDebugChoiceModels()){
			logger.info("Choosing stop frequency for purpose "+modelStructure.CROSSBORDER_PURPOSES[purpose]+" using random number "+random);
	...
	private boolean tourIsInRange(CrossBorderTour tour, int lowerBound, int upperBound){
		...
		if(tourDurationInHours>=lowerBound && tourDurationInHours<=upperBound)
			return true;
