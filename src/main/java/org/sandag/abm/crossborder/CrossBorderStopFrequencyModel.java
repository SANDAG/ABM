...
public class CrossBorderStopFrequencyModel {
	...
	private double[][] cumProbability; // by purpose, alternative: cumulative
										// probability distribution
	private int[][] lowerBoundDurationHours; // by purpose, alternative: lower
												// bound in hours
	private int[][] upperBoundDurationHours; // by purpose, alternative: upper
												// bound in hours
	private int[][] outboundStops; // by purpose, alternative: number of
									// outbound stops
	private int[][] inboundStops; // by purpose, alternative: number of inbound
									// stops
	...
	private void readStopFrequencyFile(String fileName) {
		...
		for (int row = 1; row <= rowCount; ++row) {
			...
			int purpose = (int) probabilityTable.getValueAt(row, "Purpose");
			int lowerBound = (int) probabilityTable.getValueAt(row,
					"DurationLo");
			int upperBound = (int) probabilityTable.getValueAt(row,
					"DurationHi");
			...
			if (purpose != lastPurpose || lowerBound != lastLowerBound) {
				...
				logger.info("Cumulative probability for purpose " + purpose
						+ " lower bound " + lowerBound + " is " + cumProb);
				cumProb = 0;
			}
		...
		for (int purp = 0; purp < purposes; ++purp) {
			for (int a = 0; a < cumProbability[purp].length; ++a) {
				logger.info("Purpose " + purp + " lower "
						+ lowerBoundDurationHours[purp][a] + " upper "
						+ upperBoundDurationHours[purp][a] + " cumProb "
	...
	public void calculateStopFrequency(CrossBorderTour tour) {
		...
		if (tour.getDebugChoiceModels()) {
			logger.info("Choosing stop frequency for purpose "
					+ modelStructure.CROSSBORDER_PURPOSES[purpose]
		...
		for (int i = 0; i < cumProbability[purpose].length; ++i) {
			...
			if (random < cumProbability[purpose][i]) {
				...
				if (outStops > 0) {
					CrossBorderStop[] stops = generateOutboundStops(tour,
							outStops);
					tour.setOutboundStops(stops);
				}

				if (inbStops > 0) {
					CrossBorderStop[] stops = generateInboundStops(tour,
							inbStops);
					tour.setInboundStops(stops);
				}
				if (tour.getDebugChoiceModels()) {
					logger.info("");
					logger.info("Chose " + outStops + " outbound stops and  "
							+ inbStops + " inbound stops");
					logger.info("");
				}
				break;
			}
	 ...
	 */
	private boolean tourIsInRange(CrossBorderTour tour, int lowerBound,
			int upperBound) {
		...
		if (tourDurationInHours >= lowerBound
				&& tourDurationInHours <= upperBound)
			return true;
	 ...
	 */
	private CrossBorderStop[] generateOutboundStops(CrossBorderTour tour,
			int numberOfStops) {
	 ...
	 */
	private CrossBorderStop[] generateInboundStops(CrossBorderTour tour,
			int numberOfStops) {
