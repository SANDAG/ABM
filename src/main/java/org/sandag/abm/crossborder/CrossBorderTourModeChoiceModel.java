        ...
        implements Serializable
{
    ...
    private ChoiceModelApplication[]   mcModel; //by segment - mandatory vs non-mandatory (each has different nesting coefficients)
     ...
     */
    public CrossBorderTourModeChoiceModel(HashMap<String, String> propertyMap, CrossBorderModelStructure myModelStructure,
            CrossBorderDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper)
    {
        ...
        tripModeChoiceModel = new CrossBorderTripModeChoiceModel(propertyMap, myModelStructure, dmuFactory, myLogsumHelper);
        ...
        String waitTimeFile = Util.getStringValueFromPropertyMap(propertyMap,PROPERTIES_POE_WAITTIMES);
    ...
    private void setupModeChoiceModelApplicationArray(HashMap<String, String> propertyMap)
    {
        ...
        logger.info(String.format("Setting up cross border tour (border crossing) mode choice model."));
        ...
        int mandatoryModelPage = new Integer(Util.getStringValueFromPropertyMap(propertyMap,PROPERTIES_UEC_MANDATORY_MODEL_SHEET));
        int nonmandatoryModelPage = new Integer(Util.getStringValueFromPropertyMap(propertyMap,PROPERTIES_UEC_NONMANDATORY_MODEL_SHEET));
       ...
       // default is to not save the tour mode choice utils and probs for each tour
        ...
        mcModel[0] = new ChoiceModelApplication(mcUecFile, mandatoryModelPage, dataPage, propertyMap, (VariableTable) mcDmuObject);
        mcModel[1] = new ChoiceModelApplication(mcUecFile, nonmandatoryModelPage, dataPage, propertyMap, (VariableTable) mcDmuObject);
     ...
     * @return
     */
    public double getLogsum(CrossBorderTour tour,
     ...
     * @param tour  The tour with an origin MGRA and departure time period.
     */
     ...
     * @param tour  The tour with other attributes such as origin, destination, purpose coded.
     */
     ...
     */
    public double getModeChoiceLogsum(CrossBorderTour tour,
            Logger modelLogger, String choiceModelDescription, String decisionMakerLabel)
    {
    	setDmuAttributes(tour);
     ...
     * 
     * @param tour  The cross border tour whose mode to choose.
    ...
    public int getModeChoice(CrossBorderTour tour)
    {
        ...
        if (tour.getDebugChoiceModels())
        {
        	...
        	choiceModelDescription	 = String.format(
                    "%s Tour Mode Choice Model for: Purpose=%s, Origin=%d, Dest=%d",
                    tourCategory, purposeName, tour.getOriginMGRA(), tour.getDestinationMGRA());
       ...
       double rn = tour.getRandom();
        ...
        if (tour.getDebugChoiceModels())
        {
            ...
            modelLogger.info(String.format("Choice: %s, with rn=%.8f", altString,
                    rn));
   ...
   //         tour.setTourModalProbabilities(probs);
	 ...
	 * 
	 * @param fileName  Name of file containing station, beginPeriod, endPeriod and wait time 
	 * for standard, SENTRI, and pedestrians.
	 */
