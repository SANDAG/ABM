...
public class CrossBorderTourModeChoiceModel implements Serializable {
	...
	private ChoiceModelApplication[] mcModel; // by segment - mandatory vs
												// non-mandatory (each has
												// different nesting
												// coefficients)
	...
	public CrossBorderTourModeChoiceModel(HashMap<String, String> propertyMap,
		...
		tripModeChoiceModel = new CrossBorderTripModeChoiceModel(propertyMap,
				myModelStructure, dmuFactory, myLogsumHelper);

		tour = new CrossBorderTour(0);
		trip = new CrossBorderTrip();

		String directory = Util.getStringValueFromPropertyMap(propertyMap,
				"Project.Directory");
	...
	private void setupModeChoiceModelApplicationArray(
		...
		mcModel[0] = new ChoiceModelApplication(mcUecFile, mandatoryModelPage,
				dataPage, propertyMap, (VariableTable) mcDmuObject);
		mcModel[1] = new ChoiceModelApplication(mcUecFile,
				nonmandatoryModelPage, dataPage, propertyMap,
	...
	public double getLogsum(CrossBorderTour tour, Logger modelLogger,
		...
		return getModeChoiceLogsum(tour, modelLogger, choiceModelDescription,
				decisionMakerLabel);
	...
	public int getModelIndex(CrossBorderTour tour) {
		int modelIndex = 1;
		if (tour.getPurpose() == modelStructure.WORK
				|| tour.getPurpose() == modelStructure.SCHOOL)
			modelIndex = 0;
		return modelIndex;
	}
	...
	public double getModeChoiceLogsum(CrossBorderTour tour, Logger modelLogger,
		...
		mcModel[modelIndex].computeUtilities(mcDmuObject,
	...
	public int getModeChoice(CrossBorderTour tour) {
		...
		if (tour.getDebugChoiceModels()) {
			String purposeName = modelStructure.CROSSBORDER_PURPOSES[tour
					.getPurpose()];
			choiceModelDescription = String
					.format("%s Tour Mode Choice Model for: Purpose=%s, Origin=%d, Dest=%d",
		...
		if (tour.getDebugChoiceModels()) {
			...
			String altString = String.format("%-3d  %s", chosen,
					altNames[chosen - 1]);
			modelLogger.info(String.format("Choice: %s, with rn=%.8f",
					altString, rn));
	...
	protected void readWaitTimeFile(String fileName) {
		...
		for (int row = 1; row <= rowCount; ++row) {
			...
			wait.beginPeriod[index] = (int) waitTimeTable.getValueAt(row,
					"StartPeriod");
			wait.endPeriod[index] = (int) waitTimeTable.getValueAt(row,
					"EndPeriod");
			wait.StandardWait[index] = waitTimeTable.getValueAt(row,
					"StandardWait");
			wait.SENTRIWait[index] = waitTimeTable
					.getValueAt(row, "SENTRIWait");
			wait.PedestrianWait[index] = waitTimeTable.getValueAt(row,
					"PedestrianWait");
