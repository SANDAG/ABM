...
public class DestChoiceModelManager implements Serializable {
	...
	private static transient Logger logger = Logger
			.getLogger(DestChoiceModelManager.class);
	...
	public synchronized void managerSetup(HashMap<String, String> propertyMap,
			ModelStructure modelStructure, MatrixDataServerIf ms,
			String dcUecFileName, String soaUecFileName, int soaSampleSize,
			CtrampDmuFactoryIf dmuFactory, String restartModelString) {
		...
		System.out.println(String.format(
				"initializing DC ModelManager: thread=%s.", Thread
		...
		if (ms == null) {
			...
			logger.info(String
					.format(Thread.currentThread().getName()
							+ ": DestChoiceModelManager connecting to remote MatrixDataServer."));
			logger.info(String.format("MatrixDataServer connection test: %s",
					testString));
		...
		if (!aggAcc.getAccessibilitiesAreBuilt()) {
			...
			String accFileName = projectDirectory
					+ Util.getStringValueFromPropertyMap(propertyMap,
							"acc.output.file");
			boolean accFileReadFlag = Util.getBooleanValueFromPropertyMap(
	...
	public synchronized void returnWorkLocModelObject(
			WorkLocationChoiceModel dcModel, int taskIndex, int startIndex,
			int endIndex) {
		...
		logger.info(String
				.format("returned workLocationChoice[%d,%d] to workQueueLoc, task=%d, thread=%s, completedHouseholds=%d.",
						currentIteration, dcModel.getModelIndex(), taskIndex,
						Thread.currentThread().getName(),
						completedHouseholdsWork));
	}

	public synchronized void returnDcWorkModelObject(
			MandatoryDestChoiceModel dcModel, int taskIndex, int startIndex,
			int endIndex) {
	...
	public synchronized void returnSchoolLocModelObject(
			SchoolLocationChoiceModel dcModel, int taskIndex, int startIndex,
			int endIndex) {
		...
		logger.info(String
				.format("returned schoolLocationChoice[%d,%d] to schoolQueueLoc, task=%d, thread=%s, completedHouseholds=%d.",
						currentIteration, dcModel.getModelIndex(), taskIndex,
						Thread.currentThread().getName(),
						completedHouseholdsSchool));
	}

	public synchronized void returnDcSchoolModelObject(
			MandatoryDestChoiceModel dcModel, int taskIndex, int startIndex,
			int endIndex) {
	...
	public synchronized WorkLocationChoiceModel getWorkLocModelObject(
			int taskIndex, int iteration, DestChoiceSize dcSizeObj,
			int[] uecIndices, int[] soaUecIndices) {
		...
		if (!modelQueueWorkLoc.isEmpty()) {
			...
			if (iteration > currentIteration) {
				...
				updateWorkSoaProbabilities(workLocSoaDistProbsObject,
						dcSizeObj, workSizeProbs, workTazDistProbs, dcSizeArray);
			...
			logger.info(String
					.format("removed workLocationChoice[%d,%d] from workQueueLoc, task=%d, thread=%s.",
							currentIteration, dcModel.getModelIndex(),
			...
			if (modelIndexWork == 0 && iteration == 0) {
				...
				workLocSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
						...
						PROPERTIES_WORK_DC_SOA_UEC_FILE,
						PROPERTIES_WORK_DC_SOA_UEC_MODEL_PAGE,
						PROPERTIES_WORK_DC_SOA_UEC_DATA_PAGE);
				double[][] dcSizeArray = dcSizeObj.getDcSizeArray();
			...
			logsumHelper.setTazDistanceSkimArrays(tazDistanceCalculator
					.getStoredFromTazToAllTazsDistanceSkims(),
					tazDistanceCalculator
							.getStoredToTazFromAllTazsDistanceSkims());

			// pass null in instead if modelStructure, since it's not available
			// and won't be needed for logsum calculation.
			TourModeChoiceModel immcModel = new TourModeChoiceModel(
					propertyMap, null,
					TourModeChoiceModel.MANDATORY_MODEL_INDICATOR, dmuFactory,
					logsumHelper);

			dcModel = new WorkLocationChoiceModel(modelIndexWork, propertyMap,
					dcSizeObj, aggAcc, dcUecFileName, soaUecFileName,
					soaSampleSize, modeChoiceUecFileName, dmuFactory,
					immcModel, workSizeProbs, workTazDistProbs);

			dcModel.setupWorkSegments(uecIndices, soaUecIndices);
			dcModel.setupDestChoiceModelArrays(propertyMap, dcUecFileName,
					soaUecFileName, soaSampleSize);

			logger.info(String.format(
					"created workLocationChoice[%d,%d], task=%d, thread=%s.",
					currentIteration, dcModel.getModelIndex(), taskIndex,
					Thread.currentThread().getName()));
	...
	public synchronized MandatoryDestChoiceModel getDcWorkModelObject(
			int taskIndex, int iteration, DestChoiceSize dcSizeObj,
			int[] uecIndices, int[] soaUecIndices) {
		...
		if (!modelQueueWork.isEmpty()) {
			...
			logger.info(String
					.format("removed dcModelWork[%d,%d] from workQueue, task=%d, thread=%s.",
							currentIteration, dcModel.getModelIndex(),
							taskIndex, Thread.currentThread().getName()));
			...
			dcModel = new MandatoryDestChoiceModel(modelIndexWork, propertyMap,
					dcSizeObj, aggAcc, mgraManager, dcUecFileName,
					soaUecFileName, soaSampleSize, modeChoiceUecFileName,
					dmuFactory, immcModel);
			...
			logger.info(String.format(
					"created dcModelWork[%d,%d], task=%d, thread=%s.",
					currentIteration, dcModel.getModelIndex(), taskIndex,
					Thread.currentThread().getName()));
	...
	public synchronized SchoolLocationChoiceModel getSchoolLocModelObject(
			int taskIndex, int iteration, DestChoiceSize dcSizeObj) {
		...
		if (!modelQueueSchoolLoc.isEmpty()) {
			...
			if (iteration > currentIteration) {
				...
				updateSchoolSoaProbabilities(aggAcc.getPsSegmentNameIndexMap(),
						dcSizeObj, tazDistExpUtils,
						schoolSizeProbs[PRESCHOOL_ALT_INDEX],
				...
				updateSchoolSoaProbabilities(aggAcc.getGsSegmentNameIndexMap(),
						dcSizeObj, tazDistExpUtils,
						schoolSizeProbs[GRADE_SCHOOL_ALT_INDEX],
				...
				updateSchoolSoaProbabilities(aggAcc.getHsSegmentNameIndexMap(),
						dcSizeObj, tazDistExpUtils,
						schoolSizeProbs[HIGH_SCHOOL_ALT_INDEX],
				...
				updateSchoolSoaProbabilities(
						aggAcc.getUnivTypicalSegmentNameIndexMap(), dcSizeObj,
						tazDistExpUtils,
				...
				updateSchoolSoaProbabilities(
						aggAcc.getUnivNonTypicalSegmentNameIndexMap(),
						dcSizeObj, tazDistExpUtils,
			...
			logger.info(String
					.format("removed schoolLocationChoice[%d,%d] from schoolQueueLoc, task=%d, thread=%s.",
							currentIteration, dcModel.getModelIndex(),
							taskIndex, Thread.currentThread().getName()));
				...
				if (schoolSizeProbs == null) {
					...
					for (int taz = 1; taz <= tdm.getMaxTaz(); taz++) {
						...
						if (mgraArray != null) {
							for (int mgra : mgraArray) {
								univEnrollment[taz] = aggAcc
										.getMgraUniversityEnrollment(mgra);
							}
						}
					}
					...
					try {
						...
						psLocSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
								propertyMap, dmuFactory,
								PROPERTIES_PS_DC_SOA_UEC_FILE,
								PROPERTIES_PS_DC_SOA_UEC_MODEL_PAGE,
								PROPERTIES_PS_DC_SOA_UEC_DATA_PAGE);
						tazDistExpUtils = computeTazDistanceExponentiatedUtilities(psLocSoaDistProbsObject);
						updateSchoolSoaProbabilities(
								aggAcc.getPsSegmentNameIndexMap(), dcSizeObj,
								tazDistExpUtils,
								schoolSizeProbs[PRESCHOOL_ALT_INDEX],
								schoolTazDistProbs[PRESCHOOL_ALT_INDEX]);
					} catch (Exception e) {
						logger.error(
								"exception caught updating pre-school SOA probabilities",
								e);
						System.exit(-1);
					}
					...
					try {
						...
						gsLocSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
								propertyMap, dmuFactory,
								PROPERTIES_GS_DC_SOA_UEC_FILE,
								PROPERTIES_GS_DC_SOA_UEC_MODEL_PAGE,
								PROPERTIES_GS_DC_SOA_UEC_DATA_PAGE);
						tazDistExpUtils = computeTazDistanceExponentiatedUtilities(gsLocSoaDistProbsObject);
						updateSchoolSoaProbabilities(
								aggAcc.getGsSegmentNameIndexMap(), dcSizeObj,
								tazDistExpUtils,
								schoolSizeProbs[GRADE_SCHOOL_ALT_INDEX],
								schoolTazDistProbs[GRADE_SCHOOL_ALT_INDEX]);
					} catch (Exception e) {
						logger.error(
								"exception caught updating grade school SOA probabilities",
								e);
						System.exit(-1);
					}
					...
					try {
						...
						hsLocSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
								propertyMap, dmuFactory,
								PROPERTIES_HS_DC_SOA_UEC_FILE,
								PROPERTIES_HS_DC_SOA_UEC_MODEL_PAGE,
								PROPERTIES_HS_DC_SOA_UEC_DATA_PAGE);
						tazDistExpUtils = computeTazDistanceExponentiatedUtilities(hsLocSoaDistProbsObject);
						updateSchoolSoaProbabilities(
								aggAcc.getHsSegmentNameIndexMap(), dcSizeObj,
								tazDistExpUtils,
								schoolSizeProbs[HIGH_SCHOOL_ALT_INDEX],
								schoolTazDistProbs[HIGH_SCHOOL_ALT_INDEX]);
					} catch (Exception e) {
						logger.error(
								"exception caught updating high school SOA probabilities",
								e);
						System.exit(-1);
					}
					...
					try {
						...
						univLocSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
								propertyMap, dmuFactory,
								PROPERTIES_UNIV_DC_SOA_UEC_FILE,
						...
						logger.error(
								"exception caught updating university SOA probabilities",
								e);
						System.exit(-1);
					}
			...
			dcModel = new SchoolLocationChoiceModel(modelIndexSchool,
					propertyMap, dcSizeObj, aggAcc, dcUecFileName,
					soaUecFileName, soaSampleSize, modeChoiceUecFileName,
			...
			logger.info(String.format(
					"created schoolLocationChoice[%d,%d], task=%d, thread=%s.",
					currentIteration, dcModel.getModelIndex(), taskIndex,
					Thread.currentThread().getName()));
	...
	public synchronized MandatoryDestChoiceModel getDcSchoolModelObject(
			int taskIndex, int iteration, DestChoiceSize dcSizeObj) {
		...
		if (!modelQueueSchool.isEmpty()) {
			...
			logger.info(String
					.format("removed dcModelSchool[%d,%d] from schoolQueue, task=%d, thread=%s.",
							currentIteration, dcModel.getModelIndex(),
							taskIndex, Thread.currentThread().getName()));
			...
			dcModel = new MandatoryDestChoiceModel(modelIndexSchool,
					propertyMap, dcSizeObj, aggAcc, mgraManager, dcUecFileName,
					soaUecFileName, soaSampleSize, modeChoiceUecFileName,
					dmuFactory, immcModel);
			...
			logger.info(String.format(
					"created dcModelSchool[%d,%d], task=%d, thread=%s.",
					currentIteration, dcModel.getModelIndex(), taskIndex,
					Thread.currentThread().getName()));
	...
	private void clearWorkLocModels() {
		...
		if (modelQueueWorkLoc != null && !modelQueueWorkLoc.isEmpty()) {
			...
			logger.info(String
					.format("%s:  clearing dc choice models modelQueueWorkLoc, thread=%s.",
		...
		if (modelQueueWork != null && !modelQueueWork.isEmpty()) {
			...
			logger.info(String
					.format("%s:  clearing dc choice models modelQueueWork, thread=%s.",
							new Date(), Thread.currentThread().getName()));
	...
	private void clearSchoolLocModels() {
		...
		if (modelQueueSchoolLoc != null && !modelQueueSchoolLoc.isEmpty()) {
			...
			logger.info(String
					.format("%s:  clearing dc choice models modelQueueSchoolLoc, thread=%s.",
							new Date(), Thread.currentThread().getName()));
		...
		if (modelQueueSchool != null && !modelQueueSchool.isEmpty()) {
			...
			logger.info(String
					.format("%s:  clearing dc choice models modelQueueSchool, thread=%s.",
							new Date(), Thread.currentThread().getName()));
	...
	private void updateWorkSoaProbabilities(
		...
		HashMap<String, Integer> segmentNameIndexMap = dcSizeObj
				.getSegmentNameIndexMap();
		...
		for (String segmentName : segmentNameIndexMap.keySet()) {
			...
			computeSizeSegmentProbabilities(sizeProbs[segmentIndex],
					dcSizeArray[segmentIndex]);

		}

	}

	private void updateSchoolSoaProbabilities(
			HashMap<String, Integer> segmentNameIndexMap,
			DestChoiceSize dcSizeObj, double[][] tazDistExpUtils,
			double[][] sizeProbs, double[][] tazDistProbs) {
		...
		for (String segmentName : segmentNameIndexMap.keySet()) {
			...
			computeSizeSegmentProbabilities(sizeProbs,
					dcSizeArray[segmentIndex]);
	...
	private void computeSizeSegmentProbabilities(double[][] sizeProbs,
			double[] size) {
