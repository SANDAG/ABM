...
public class TourDepartureTimeAndDurationDMU implements Serializable,
		VariableTable {
	...
	public int getPreDrivingAgeChild() {
		return (person.getPersonIsStudentNonDriving() == 1 || person
				.getPersonIsPreschoolChild() == 1) ? 1 : 0;
	}
	...
	public int getAllAdultsFullTimeWorkers() {
		...
		for (int i = 1; i < p.length; i++) {
			if (p[i].getPersonIsAdult() == 1
					&& p[i].getPersonIsFullTimeWorker() == 0) {
	...
	public int getSubtourPurposeIsEatOut() {
		if (tour.getSubTourPurpose().equalsIgnoreCase(
	...
	public int getNumIndivShopTours() {
		...
		for (Tour t : person.getListOfIndividualNonMandatoryTours())
			if (t.getTourPurpose().equalsIgnoreCase(
		...
		if (tour.getTourCategory().equalsIgnoreCase(
			...
			for (int i = 0; i < personNums.length; i++) {
				...
				if (persons[p].getPersonIsPreschoolChild() == 1
						|| persons[p].getPersonIsStudentNonDriving() == 1)
					return 1;
			}
		} else if (tour.getTourCategory().equalsIgnoreCase(
				ModelStructure.INDIVIDUAL_NON_MANDATORY_CATEGORY)) {
	...
	public double getModeChoiceLogsumAlt(int alt) {
		...
		int index = modelStructure.getSkimPeriodCombinationIndex(startPeriod,
				endPeriod);
	...
	public int getAdjWindowAfterThisPeriodAlt(int alt) {
		...
		int numAdjacentPeriodsAvailable = 0;
		for (int i = thisTourEndsPeriod + 1; i < modelStructure
				.getNumberOfTimePeriods(); i++) {
			if (person.isPeriodAvailable(i))
				numAdjacentPeriodsAvailable++;
			else
				break;
	...
	public float getRemainingInmToursToAvailablePeriodsRatioAlt(int alt) {
		...
		if (periodsAvail > 0) {
			float ratio = (float) (person
					.getListOfIndividualNonMandatoryTours().size() - tourNumber)
					/ periodsAvail;
			return ratio;
		} else
			return -999;
	...
	public int getTourCategoryIsJoint() {
		return tour.getTourCategory().equalsIgnoreCase(
