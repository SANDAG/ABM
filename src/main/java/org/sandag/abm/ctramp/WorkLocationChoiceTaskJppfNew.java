...
public class WorkLocationChoiceTaskJppfNew extends JPPFTask {
	...
	public WorkLocationChoiceTaskJppfNew(int taskIndex, int startIndex,
			int endIndex, int iteration) {
	...
	public void run() {
		...
		try {
			...
			this.propertyMap = (HashMap<String, String>) dataProvider
					.getValue("propertyMap");
			this.ms = (MatrixDataServerIf) dataProvider.getValue("ms");
			this.hhDataManager = (HouseholdDataManagerIf) dataProvider
					.getValue("hhDataManager");
			this.modelStructure = (ModelStructure) dataProvider
					.getValue("modelStructure");
			...
			this.dcSizeObj = (DestChoiceSize) dataProvider
					.getValue("dcSizeObj");
			this.dcUecFileName = (String) dataProvider
					.getValue("dcUecFileName");
			this.soaUecFileName = (String) dataProvider
					.getValue("soaUecFileName");
			this.soaSampleSize = (Integer) dataProvider
					.getValue("soaSampleSize");
			this.dmuFactory = (CtrampDmuFactoryIf) dataProvider
					.getValue("dmuFactory");
			this.restartModelString = (String) dataProvider
					.getValue("restartModelString");
		...
		DestChoiceModelManager modelManager = DestChoiceModelManager
				.getInstance();

		// one of tasks needs to initialize the manager object by passing
		// attributes
		// needed to create a destination choice model object.
		modelManager.managerSetup(propertyMap, modelStructure, ms,
				dcUecFileName, soaUecFileName, soaSampleSize, dmuFactory,
				restartModelString);

		// get a dcModel object from manager, which either creates one or
		// returns one for re-use.
		WorkLocationChoiceModel dcModel = modelManager.getWorkLocModelObject(
				taskIndex, iteration, dcSizeObj, uecIndices, soaUecIndices);
		...
		System.out.println(String.format(
				"%s: %s, task=%d run(), thread=%s, start=%d, end=%d.",
				new Date(), VERSION, taskIndex, threadName, startIndex,
				endIndex));

		long setup1 = (System.currentTimeMillis() - startTime) / 1000;

		Household[] householdArray = hhDataManager.getHhArray(startIndex,
				endIndex);
		...
		System.out.println(String.format(
				"%s: task=%d processing households[%d:%d], thread=%s.",
				new Date(), taskIndex, startIndex, endIndex, threadName));

		int i = -1;
		try {

			boolean runDebugHouseholdsOnly = Util
					.getBooleanValueFromPropertyMap(propertyMap,
			...
			for (i = 0; i < householdArray.length; i++) {
				...
				if (runDebugHouseholdsOnly
						&& !householdArray[i].getDebugChoiceModels())
				...
				System.out
						.println(String
								.format("exception caught in taskIndex=%d applying dc model for i=%d, hhId=%d, startIndex=%d.",
										taskIndex, i,
										householdArray[i].getHhId(), startIndex));
			else
				System.out
						.println(String
								.format("exception caught in taskIndex=%d applying dc model for i=%d, startIndex=%d.",
		...
		long getHhs = ((System.currentTimeMillis() - startTime) / 1000)
				- setup1;
		long processHhs = ((System.currentTimeMillis() - startTime) / 1000)
				- setup2 - getHhs;
		// logger.info( String.format(
		// "task=%d finished, thread=%s, getHhs=%d, processHhs=%d.", taskIndex,
		// threadName, getHhs, processHhs ) );
		System.out.println(String.format("%s: task=%d finished, thread=%s.",
				new Date(), taskIndex, threadName));
		...
		String resultString = String
				.format("result for thread=%s, task=%d, startIndex=%d, endIndex=%d, startTime=%s, endTime=%s, setup1=%d, setup2=%d, getHhs=%d, run=%d, total=%d.",
						threadName, taskIndex, startIndex, endIndex, start,
						new Date(), setup1, setup2, getHhs, processHhs, total);
		// logger.info( resultString );
		setResult(resultString);

		modelManager.returnWorkLocModelObject(dcModel, taskIndex, startIndex,
				endIndex);
