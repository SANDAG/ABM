 ...
 * 
 */
public class HouseholdIndividualNonMandatoryTourFrequencyModel implements
		Serializable {
	...
	private static String[] escortTypes = { "", "escort0", "escort1", "escort2" };
	private static String[] shopTypes = { "", "shop0", "shop1", "shop2" };
	private static String[] maintTypes = { "", "maint0", "maint1", "maint2" };
	private static String[] discrTypes = { "", "discr0", "discr1", "discr2" };
	private static String[] eatOutTypes = { "", "eatOut0", "eatOut1", "eatOut2" };
	private static String[] visitTypes = { "", "visit0", "visit1", "visit2" };
	...
	public HouseholdIndividualNonMandatoryTourFrequencyModel(
	...
	private void setUpModels(HashMap<String, String> propertyMap,
		...
		String inmtfUecFile = propertyMap
				.get(PROPERTIES_UEC_INDIV_NON_MANDATORY_TOUR_FREQ);
		...
		int sheet = Integer
				.parseInt(propertyMap.get(UEC_PERSONTYPE_1_PAGE_KEY));
		...
		choiceModelApplication = new ChoiceModelApplication[personTypeIndexToModelIndexMap
				.size() + 1]; // one
		// choice
		// model
		// for
		...
		for (int i : personTypeIndexToModelIndexMap.keySet())
			choiceModelApplication[i] = new ChoiceModelApplication(uecFileName,
					personTypeIndexToModelIndexMap.get(i), dataSheet,
		...
		alternativesTable = choiceModelApplication[ftIndex].getUEC()
				.getAlternativeData();
		...
		purposeIndexToNameMap
				.put(1, ModelStructure.ESCORT_PRIMARY_PURPOSE_NAME);
		purposeIndexToNameMap.put(2,
				ModelStructure.SHOPPING_PRIMARY_PURPOSE_NAME);
		purposeIndexToNameMap.put(3,
				ModelStructure.OTH_MAINT_PRIMARY_PURPOSE_NAME);
		purposeIndexToNameMap.put(4,
				ModelStructure.EAT_OUT_PRIMARY_PURPOSE_NAME);
		purposeIndexToNameMap.put(5,
				ModelStructure.VISITING_PRIMARY_PURPOSE_NAME);
		purposeIndexToNameMap.put(6,
				ModelStructure.OTH_DISCR_PRIMARY_PURPOSE_NAME);

		if (!fieldNames[0].equalsIgnoreCase("a")
				&& !fieldNames[0].equalsIgnoreCase("alt")) {
		...
		loadIndividualNonMandatoryIncreaseModelData(uecPath
				+ propertyMap
						.get(PROPERTIES_TOUR_FREQUENCY_EXTENSION_PROBABILITIES_FILE));
	...
	public void applyModel(Household household) {
		...
		if (household.getDebugChoiceModels())
			household.logHouseholdObject(
					"Pre Individual Non-Mandatory Tour Frequency Choice HHID="
			...
			try {
				...
				if (!activity.equalsIgnoreCase(HOME_ACTIVITY)) {
					...
					if (workMgra > 0
						...
						double[] accessibilities = mandAcc
								.calculateAccessibilitiesForMgraPair(
					...
					dmuObject.setDmuIndexValues(household.getHhId(),
							household.getHhTaz(), household.getHhTaz(), -1);
					...
					if (household.getDebugChoiceModels()) {
						...
						decisionMakerLabel = String
								.format("HH=%d, PersonNum=%d, PersonType=%s",
						...
						modelLogger.info(" ");
						String loggerString = choiceModelDescription + " for "
								+ decisionMakerLabel + ".";
					...
					tours = runIndividualNonMandatoryToursIncreaseModel(person,
							choice);
					...
					if (household.getDebugChoiceModels()) {
						...
						modelLogger.info("Person num: " + personNum
						...
						for (int k = 0; k < alternativeNames.length; k++) {
							...
							String altString = String.format("%-3d %-66s",
									k + 1, getAlternativeNameFromChoice(k + 1));
							modelLogger.info(String.format(
									"%-72s%18.6e%18.6e%18.6e", altString,
						...
						modelLogger
								.info(String
										.format("Original Choice: %s, with rn=%.8f, randomCount=%d",
						...
						modelLogger
								.info(String.format(
										"Revised Choice After Increase: %s",
										altString));
				...
				logger.error(String.format(
						"Exception caught for j=%d, activity=%s, HHID=%d", j,
	...
	private void loadIndividualNonMandatoryIncreaseModelData(String filePath) {
		...
		try {
			probabilityTable = (new CSVFileReader()
		...
		for (int i = 1; i <= probabilityTable.getRowCount(); i++) {
			int key = getTourIncreaseTableKey(
					(int) probabilityTable.getValueAt(i,
							nonMandatoryTourTypeColumn),
					(int) probabilityTable.getValueAt(i, personTypeColumnName),
					((int) probabilityTable.getValueAt(i,
							mandatoryTourParticipationColumnName)) == 1,
					((int) probabilityTable.getValueAt(i,
							jointTourParticipationColumnName)) == 1);
			tourFrequencyIncreaseProbabilityMap.put(
					key,
					new float[] {
							probabilityTable.getValueAt(i,
									zeroAdditionalToursColumnName),
							probabilityTable.getValueAt(i,
									oneAdditionalToursColumnName),
							probabilityTable.getValueAt(i,
									twoAdditionalToursColumnName) });
		}
	}

	private float[] runIndividualNonMandatoryToursIncreaseModel(Person person,
			int choice) {
		// use the 1s based choice value as the table row number
		// rowValues is a 0s based indexed array, but the first field is the
		// alternative number,
		...
		boolean participatedInMandatoryTour = person.getListOfWorkTours()
				.size() > 0 || person.getListOfSchoolTours().size() > 0;
		...
		while (notDone) {
			...
			for (int i = 1; i < rowValues.length; i++) {
				...
				int newChoice = -1;
				int key = getTourIncreaseTableKey(i, personType,
						participatedInMandatoryTour, participatedInJointTour);
				float[] probabilities = tourFrequencyIncreaseProbabilityMap
						.get(key);
				Random random = household.getHhRandom();
				int randomCount = household.getHhRandomCount();
				double rn = random.nextDouble();
				...
				if (household.getDebugChoiceModels()) {
					...
					for (int k = 1; k < alternativeNames.length; k++) {
						...
						String altString = String.format("%-3d %-15s", k + 1,
								alternativeNames[k]);
						modelLogger.info(String.format("%-20s%18.6e%18.6e",
								altString, prob, cumProb));
					}
	...
	private int getTourIncreaseTableKey(int nonMandatoryTourType,
			int personType, boolean participatedInMandatoryTour,
	...
	private void createIndividualNonMandatoryTours_new(Person person,
			float[] tours) {
		...
		for (int i = 1; i < tours.length; i++) {
			...
			if (numTours > 0)
				person.createIndividualNonMandatoryTours(numTours,
						purposeIndexToNameMap.get(i));
		}
	}
