...
public class WorkLocationChoiceModel implements Serializable {
	...
	public WorkLocationChoiceModel(int index,
			HashMap<String, String> propertyMap, DestChoiceSize dcSizeObj,
			BuildAccessibilities aggAcc, String dcUecFileName,
	...
	public void setupDestChoiceModelArrays(HashMap<String, String> propertyMap,
		...
		worksAtHomeModel = new ChoiceModelApplication(dcUecFileName,
				...
				(VariableTable) dcTwoStageDmuObject);
	...
	public void applyWorkLocationChoice(Household hh) {
		...
		for (i = 1; i < persons.length; i++) {
			...
			int worksAtHomeChoice = selectWorksAtHomeChoice(
					dcTwoStageDmuObject, hh, p);
			...
			try {
				...
				dcTwoStageDmuObject.setDmuIndexValues(hh.getHhId(), homeMgra,
						origMgra, 0);
				...
				logger.fatal(String
						.format("Exception caught in dcModel selecting location for i=%d, hh.hhid=%d, person i=%d, in work location choice, modelIndex=%d, occup=%d, segmentIndex=%d, segmentName=%s",
								i, hh.getHhId(), i, modelIndex, occup,
								occupSegmentIndex, occupSegmentName));
				logger.fatal("Exception caught:", e);
				logger.fatal("Throwing new RuntimeException() to terminate.");
				throw new RuntimeException();
			}
	...
	private double[] selectLocationFromSampleOfAlternatives(String segmentType,
		...
		// the same values.
		dcTwoStageModelObject.chooseSample(household.getHhTaz(),
				sizeSegmentIndex, sizeSegmentIndex, soaSampleSize,
		...
		locationChoiceModel.computeUtilities(dcTwoStageDmuObject,
				dcTwoStageDmuObject.getDmuIndexValues(), dcModelAltsAvailable,
				dcModelAltsSample);
		...
		if (locationChoiceModel.getAvailabilityCount() > 0) {
			...
			logger.error(String
					.format("Exception caught for HHID=%d, PersonNum=%d, no available %s destination choice alternatives to choose from in choiceModelApplication.",
							...
							dcTwoStageDmuObject.getPersonObject()
									.getPersonNum(), segmentName));
		}
			...
			if (chosen < 0) {
				logger.error(String
						.format("Exception caught for HHID=%d, PersonNum=%d, workSegment=%d, no available %s destination choice alternatives to choose from in ChoiceModelApplication.",
								dcTwoStageDmuObject.getHouseholdObject()
										.getHhId(), dcTwoStageDmuObject
	...
	private int selectWorksAtHomeChoice(
			DestChoiceTwoStageModelDMU dcTwoStageDmuObject,
		...
		dcTwoStageDmuObject.setDmuIndexValues(household.getHhId(),
