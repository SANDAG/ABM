...
public class StopLocationDMU implements Serializable, VariableTable {
	...
	public void setDmuIndexValues(int hhid, int homeTaz, int origTaz,
			int destTaz) {
	...
	public int getStopsOnHalfTour() {
		return stop.isInboundStop() ? tour.getInboundStops().length : tour
				.getOutboundStops().length;
	}
	...
	public int getTourModeIsWalk() {
		boolean tourModeIsWalk = modelStructure
				.getTourModeIsWalk(tourModeIndex);
		return tourModeIsWalk ? 1 : 0;
	}

	public int getTourModeIsBike() {
		boolean tourModeIsBike = modelStructure
				.getTourModeIsBike(tourModeIndex);
		return tourModeIsBike ? 1 : 0;
	}

	public int getTourModeIsWalkTransit() {
		boolean tourModeIsWalkLocal = modelStructure
				.getTourModeIsWalkLocal(tourModeIndex);
		boolean tourModeIsWalkPremium = modelStructure
				.getTourModeIsWalkPremium(tourModeIndex);
