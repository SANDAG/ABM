...
public class Tour implements Serializable {
	...
	private static final String[] RIDE_MODE_LABELS = { "CR", "LRT", "BRT",
			"EB", "LB" };
	...
	public Tour(Household hhObj, String tourPurpose, String category,
			int primaryIndex) {
		this.hhObj = hhObj;
	...
	// creation
	public Tour(int id, Household hhObj, Person persObj, String tourPurpose,
			String category, int primaryIndex) {
		this.hhObj = hhObj;
		this.perObj = persObj;
		tourId = id;
		this.tourPurpose = tourPurpose;
	...
	public void createOutboundStops(String[] stopOrigPurposes,
			String[] stopDestPurposes, int[] stopPurposeIndex) {
		outboundStops = new Stop[stopOrigPurposes.length];
		for (int i = 0; i < stopOrigPurposes.length; i++)
			outboundStops[i] = new Stop(this, stopOrigPurposes[i],
					stopDestPurposes[i], i, false, stopPurposeIndex[i]);
	}

	public void createInboundStops(String[] stopOrigPurposes,
			String[] stopDestPurposes, int[] stopPurposeIndex) {
		// needs outbound stops to be created first to get id numbering correct

		inboundStops = new Stop[stopOrigPurposes.length];
		for (int i = 0; i < stopOrigPurposes.length; i++)
			inboundStops[i] = new Stop(this, stopOrigPurposes[i],
					stopDestPurposes[i], i, true, stopPurposeIndex[i]);
	}
	 ...
	 */
	public Stop createStop(ModelStructure modelStructure, String origPurp,
		...
		if (inbound) {
			...
			outboundStops[0] = new Stop(this, origPurp, destPurp, id, inbound,
					0);
	...
	public String getTourWindow(String purposeAbbreviation) {
		...
		for (int i = 1; i < windows.length; i++) {
			String tempString = String.format("%s", i >= tourDepartPeriod
					&& i <= tourArrivePeriod ? purposeAbbreviation : "    ");
	...
	public void logTourObject(Logger logger, int totalChars) {
		...
		if (personNumArray != null) {
			...
			for (int i = 1; i < personNumArray.length; i++)
				personNumArrayString += String
						.format(", %d", personNumArray[i]);
			personNumArrayString += " ]";
		}
		...
		Household.logHelper(logger, "tourPurposeIndex: ",
				tourPrimaryPurposeIndex, totalChars);
		Household.logHelper(logger, "personNumArray: ", personNumArrayString,
				totalChars);
		Household.logHelper(logger, "jointTourComposition: ",
				jointTourComposition, totalChars);
		...
		Household.logHelper(logger, "tourOrigWalkSubzone: ",
				tourOrigWalkSubzone, totalChars);
		Household.logHelper(logger, "tourDestWalkSubzone: ",
				tourDestWalkSubzone, totalChars);
		Household.logHelper(logger, "tourDepartPeriod: ", tourDepartPeriod,
				totalChars);
		Household.logHelper(logger, "tourArrivePeriod: ", tourArrivePeriod,
				totalChars);
		...
		String tempString = String.format(
				"outboundStops[%s]:",
				outboundStops == null ? "" : String
						.valueOf(outboundStops.length));
		logger.info(tempString);

		tempString = String
				.format("inboundStops[%s]:",
						inboundStops == null ? "" : String
		...
		if (bestWtwTapPairsOut == null) {
			...
			if (bestWtwTapPairsOut[0] == null)
				tempString = "bestWtwTapPairsOut: " + RIDE_MODE_LABELS[0] + "["
						+ "none" + "," + "none" + "]";
			else
				tempString = "bestWtwTapPairsOut: " + RIDE_MODE_LABELS[0] + "["
						+ bestWtwTapPairsOut[0][0] + ","
						+ bestWtwTapPairsOut[0][1] + "]";
			for (int i = 1; i < bestWtwTapPairsOut.length; i++)
				if (bestWtwTapPairsOut[i] == null)
					tempString += ", " + RIDE_MODE_LABELS[i] + "[" + "none"
							+ "," + "none" + "]";
				else
					tempString += ", " + RIDE_MODE_LABELS[i] + "["
							+ bestWtwTapPairsOut[i][0] + ","
		...
		if (bestWtwTapPairsIn == null) {
			...
			if (bestWtwTapPairsIn[0] == null)
				tempString = "bestWtwTapPairsIn: " + RIDE_MODE_LABELS[0] + "["
						+ "none" + "," + "none" + "]";
			else
				tempString = "bestWtwTapPairsIn: " + RIDE_MODE_LABELS[0] + "["
						+ bestWtwTapPairsIn[0][0] + ","
						+ bestWtwTapPairsIn[0][1] + "]";
			for (int i = 1; i < bestWtwTapPairsIn.length; i++)
				if (bestWtwTapPairsIn[i] == null)
					tempString += ", " + RIDE_MODE_LABELS[0] + "[" + "none"
							+ "," + "none" + "]";
				else
					tempString += ", " + RIDE_MODE_LABELS[i] + "["
							+ bestWtwTapPairsIn[i][0] + ","
		...
		if (bestWtdTapPairsOut == null) {
			...
			if (bestWtdTapPairsOut[0] == null)
				tempString = "bestWtdTapPairsOut: " + RIDE_MODE_LABELS[0] + "["
						+ "none" + "," + "none" + "]";
			else
				tempString = "bestWtdTapPairsOut: " + RIDE_MODE_LABELS[0] + "["
						+ bestWtdTapPairsOut[0][0] + ","
						+ bestWtdTapPairsOut[0][1] + "]";
		...
		if (bestWtdTapPairsIn == null) {
			...
			if (bestWtdTapPairsIn[0] == null)
				tempString = "bestWtdTapPairsIn: " + RIDE_MODE_LABELS[0] + "["
						+ "none" + "," + "none" + "]";
			else
				tempString = "bestWtdTapPairsIn: " + RIDE_MODE_LABELS[0] + "["
						+ bestWtdTapPairsIn[0][0] + ","
						+ bestWtdTapPairsIn[0][1] + "]";
		...
		if (bestDtwTapPairsOut == null) {
			...
			if (bestDtwTapPairsOut[0] == null)
				tempString = "bestDtwTapPairsOut: " + RIDE_MODE_LABELS[0] + "["
						+ "none" + "," + "none" + "]";
			else
				tempString = "bestDtwTapPairsOut: " + RIDE_MODE_LABELS[0] + "["
						+ bestDtwTapPairsOut[0][0] + ","
						+ bestDtwTapPairsOut[0][1] + "]";
		...
		if (bestDtwTapPairsIn == null) {
			...
			if (bestDtwTapPairsIn[0] == null)
				tempString = "bestDtwTapPairsIn: " + RIDE_MODE_LABELS[0] + "["
						+ "none" + "," + "none" + "]";
			else
				tempString = "bestDtwTapPairsIn: " + RIDE_MODE_LABELS[0] + "["
						+ bestDtwTapPairsIn[0][0] + ","
						+ bestDtwTapPairsIn[0][1] + "]";
