        ...
        implements HouseholdDataManagerIf, Serializable
{
     ...
     * 
     * @param sampleRate
     *            , proportion of total households for which to run the model [0.0, 1.0].
     */
    ...
    public Household[] getHhArray(int first, int last)
    {
        ...
        // long endTime = System.currentTimeMillis();
        // logger.info(String.format( "end getHhArray for first=%d, last=%d, endTime=%d, elapsed=%d millisecs.", first, last, endTime, (endTime -
        // startTime)));
        return tempHhs;
    }
            ...
            for (int p = 1; p < persons.length; p++)
            {
                        ...
                        || person.getPersonIsUniversityStudent() == 1)
                {
                    ...
                    try
                    {
                        ...
                        // if person type is a student but segment index is -1, the person is not enrolled; assume home schooled and don't add to sum
                        // by home mgra
        ...
        for (int r = 0; r < getNumHouseholds(); r++)
        {
            ...
            for (int p = 1; p < persons.length; p++)
            {
                ...
                try
                {
                    ...
                    // if person type is a student but segment index is -1, the person is not enrolled; assume home schooled and don't add to sum by
                    // home mgra
    ...
    /**
     * Assigns each individual person their own value of time, drawing from a lognormal distribution as a function of income.
     */
