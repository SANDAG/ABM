 ...
 */
public abstract class HouseholdDataManager implements HouseholdDataManagerIf,
		Serializable {

	protected transient Logger logger = Logger
			.getLogger(HouseholdDataManager.class);
	...
	public String testRemote() {
		...
		return String.format("testRemote() method in %s called.", this
	...
	public void setDebugHhIdsFromHashmap() {
		...
		String householdTraceStringList = propertyMap
				.get(PROPERTIES_HOUSEHOLD_TRACE_LIST);

		if (householdTraceStringList != null) {
			StringTokenizer householdTokenizer = new StringTokenizer(
					householdTraceStringList, ",");
	...
	public void readPopulationFiles(String inputHouseholdFileName,
	...
	public void setupHouseholdDataManager(ModelStructure modelStructure,
		...
		inputRandomSeed = Integer.parseInt(propertyMap
				.get(HouseholdDataManager.RANDOM_SEED_NAME));
		...
		String readPreAoResultsString = propertyMap
				.get(READ_PRE_AO_RESULTS_FILE);
	...
	public void setPropertyFileValues(HashMap<String, String> propertyMap) {
		...
		this.projectDirectory = propertyMap
		...
		outputPersonFileName = propertyMap
				.get(CtrampApplication.PROPERTIES_OUTPUT_PERSON_FILE);
		...
		propertyValue = propertyMap
				.get(CtrampApplication.PROPERTIES_SCHEDULING_FIRST_TIME_PERIOD);
		if (propertyValue == null)
			firstPeriod = 0;
		else
	...
	public void logPersonSummary() {
			...
			for (int j = 1; j < personArray.length; ++j) {
				...
				if (summaryResults.containsKey(personType)) {
					...
					if (summaryResults.get(personType).containsKey(
						...
						summaryResults.get(personType).put(studentStatus,
								new int[7]);
						summaryResults.get(personType).get(studentStatus)[ageCategory] += 1;
					}
				} else {
					...
					summaryResults
							.put(personType, new HashMap<String, int[]>());
					summaryResults.get(personType).put(studentStatus,
							new int[7]);
					summaryResults.get(personType).get(studentStatus)[ageCategory] += 1;
					summaryResults.get(personType).put(employmentStatus,
		...
		String[] ageCategories = { "0-5", "6-15", "16-18", "19-24", "25-44",
				"45-64", "65+" };
		...
		for (String personType : summaryResults.keySet()) {
			...
			HashMap<String, int[]> personTypeSummary = summaryResults
					.get(personType);
			...
			for (int j = 0; j < ageCategories.length; ++j) {
				...
				for (String empCategory : Person.employmentCategoryNameArray) {
					if (personTypeSummary.containsKey(empCategory)) {
						row += String.format("%16d\t",
								personTypeSummary.get(empCategory)[j]);
					} else
						row += String.format("%16d\t", 0);
				}
				for (String stuCategory : Person.studentCategoryNameArray) {
					if (personTypeSummary.containsKey(stuCategory)) {
						row += String.format("%16d\t",
								personTypeSummary.get(stuCategory)[j]);
					} else
						row += String.format("%16d\t", 0);
				}
				logger.info(row);
	...
	public void computeTransponderChoiceTazPercentArrays() {
		...
		for (int i = 0; i <= maxTaz; i++) {
			...
			if (numHhs[i] > 0) {
				...
				out.println("," + percentHhsIncome100Kplus[i] + ","
						+ percentHhsMultipleAutos[i]);
			} else {
				out.println("," + 0.0 + "," + 0.0);
			}
					...
					try {
						...
						logger.error(
								String.format(
										...
										r, person.getPersonNum(), homeMgra,
										occup, segmentIndex), e);
				...
				try {
					...
					logger.error(
							String.format(
									...
									r, person.getPersonNum(), destMgra, occup,
									segmentIndex), e);
					throw new RuntimeException();
				}
				...
				try {
					...
					logger.error(
							String.format(
									...
									r, person.getPersonNum(), destMgra,
									segmentIndex), e);
					throw new RuntimeException();
				}
	...
	public void readWorkSchoolLocationResults() {
		...
		String[] headings = { "HHID", "HomeMGRA", "Income", "PersonID",
				"PersonNum", "PersonType", "PersonAge", "EmploymentCategory",
				"StudentCategory", "WorkSegment", "SchoolSegment",
				"WorkLocation", "WorkLocationDistance", "WorkLocationLogsum",
				"SchoolLocation", "SchoolLocationDistance",
				"SchoolLocationLogsum" };

		String wsLocResultsFileName = propertyMap
				.get(READ_UWSL_RESULTS_FILENAME);
					...
					if (indexHeadingMap.containsKey(col)) {
						...
						if (fieldName.equalsIgnoreCase("HHID")) {
							...
							person.setSchoolLoc(schoolLocation);
								...
								.equalsIgnoreCase("SchoolLocationDistance")) {
							float distance = Float.parseFloat(fieldValue);
							person.setSchoolLocDistance(distance);
						} else if (fieldName
								.equalsIgnoreCase("SchoolLocationLogsum")) {
							float logsum = Float.parseFloat(fieldValue);
	...
	public void readPreAoResults() {
		...
		String preAoResultsFileName = propertyMap
				.get(READ_PRE_AO_RESULTS_FILENAME);
	...
	protected void setDistributedValuesOfTime() {
		...
		for (int i = 0; i < valueOfTimeDistribution.length; i++) {
			double mu = Math.log(meanValueOfTime[i]
					* meanValueOfTimeMultiplierBeforeLogForMu);
			valueOfTimeDistribution[i] = new LognormalDist(mu,
					valueOfTimeLognormalSigma);
		}
		...
		for (int i = 0; i < hhs.length; ++i) {
			...
			int incomeCategory = getIncomeIndexForValueOfTime(household
					.getIncomeInDollars());
			double hhValueOfTime = valueOfTimeDistribution[incomeCategory - 1]
					.inverseF(rnum);

			// constrain to logical min and max values
			if (hhValueOfTime < minValueOfTime)
				hhValueOfTime = minValueOfTime;
			if (hhValueOfTime > maxValueOfTime)
				hhValueOfTime = maxValueOfTime;
	...
	private void setValueOfTimePropertyFileValues() {
		...
		if (propertyValue == null) {
			logger.error("property file key missing: "
					+ PROPERTIES_MIN_VALUE_OF_TIME_KEY
		...
		if (propertyValue == null) {
			logger.error("property file key missing: "
					+ PROPERTIES_MAX_VALUE_OF_TIME_KEY
		...
		if (meanValueOfTimesPropertyValue == null) {
			logger.error("property file key missing: "
					+ PROPERTIES_MEAN_VALUE_OF_TIME_VALUES_KEY
					+ ", not able to set mean value of time values.");
			errorFlag = true;
		} else {

			ArrayList<Float> valueList = new ArrayList<Float>();
			StringTokenizer valueTokenizer = new StringTokenizer(
					meanValueOfTimesPropertyValue, ",");
		...
		if (valueOfTimeIncomesPropertyValue == null) {
			...
			StringTokenizer valueTokenizer = new StringTokenizer(
					valueOfTimeIncomesPropertyValue, ",");
			while (valueTokenizer.hasMoreTokens()) {
				String listValue = valueTokenizer.nextToken();
			...
			if (numIncomeValues != (numIncomeCategories - 1)) {
				...
				logger.error("the mean value of time values property="
						+ meanValueOfTimesPropertyValue + " specifies "
						+ numIncomeCategories + " mean values, thus "
						+ numIncomeCategories + " income ranges.");
				logger.error("the value of time income range values property="
						+ valueOfTimeIncomesPropertyValue + " specifies "
						+ numIncomeValues + " income range limit values.");
				logger.error("there should be exactly "
						+ (numIncomeCategories - 1)
						+ " income range limit values for "
						+ numIncomeCategories + " mean value of time values.",
						e);
				System.exit(-1);
			}
		...
		propertyValue = propertyMap
				.get(PROPERTIES_HH_VALUE_OF_TIME_MULTIPLIER_FOR_UNDER_18_KEY);
		if (propertyValue == null) {
			logger.error("property file key missing: "
			...
			hhValueOfTimeMultiplierForPersonUnder18 = Float
					.parseFloat(propertyValue);

		propertyValue = propertyMap
				.get(PROPERTIES_MEAN_VALUE_OF_TIME_MULTIPLIER_FOR_MU_KEY);
		if (propertyValue == null) {
			logger.error("property file key missing: "
			...
			meanValueOfTimeMultiplierBeforeLogForMu = Float
					.parseFloat(propertyValue);

		propertyValue = propertyMap
				.get(PROPERTIES_VALUE_OF_TIME_LOGNORMAL_SIGMA_KEY);
		if (propertyValue == null) {
			logger.error("property file key missing: "
