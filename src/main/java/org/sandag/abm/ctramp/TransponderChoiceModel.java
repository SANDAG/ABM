...
public class TransponderChoiceModel implements Serializable {
	...
	public TransponderChoiceModel(HashMap<String, String> propertyMap,
			...
			double[] transpDist, double[] pctDetour) {
	...
	private void setupTransponderChoiceModelApplication(
		...
		tpModel = new ChoiceModelApplication(tpUecFile, modelSheet, dataSheet,
	...
	public void applyModel(Household hhObject) {
		...
		tpDmuObject.setDmuIndexValues(hhObject.getHhId(), hhObject.getHhTaz(),
				hhObject.getHhTaz(), 0);
		...
		float accessibility = accTable.getAggregateAccessibility("transit",
				hhObject.getHhMgra());
		...
		if (hhObject.getDebugChoiceModels()) {
			String decisionMaker = String.format("HHID=%d", hhObject.getHhId());
			tpModel.logAlternativesInfo("Transponder Choice", decisionMaker,
					logger);
