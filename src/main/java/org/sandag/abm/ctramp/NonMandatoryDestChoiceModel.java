...
public class NonMandatoryDestChoiceModel implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(NonMandatoryDestChoiceModel.class);
	...
	private static final int[] tourPurposePriorities = { 0, 0, 0, 0, 1, 3, 2,
			6, 4, 5 };
	...
	private final int[] sizeSheetIndices = { BuildAccessibilities.ESCORT_INDEX,
	...
	public NonMandatoryDestChoiceModel(HashMap<String, String> propertyMap,
		...
		if (useNewSoaMethod)
			dcSoaTwoStageObject = new DestChoiceTwoStageModel(propertyMap,
					soaSampleSize);
	...
	private void setupDestChoiceModelArrays(
		...
		if (useNewSoaMethod) {
			...
			dcDistSoaDmuObject.setAccTable(aggAcc
					.getAccessibilitiesTableObject());
		}
		...
		purposeNameIndexMap = new HashMap<String, Integer>(
				tourPurposeNames.length);

		int i = 0;
		for (String purposeName : tourPurposeNames) {
			int uecIndex = Util.getIntegerValueFromPropertyMap(propertyMap,
					dcModelSheetKeys[i]);
		...
		for (int k = 0; k < tourPurposeNames.length; k++) {
			...
			nonMandatorySizeSegmentNameIndexMap.put(tourPurposeNames[k],
					sizeSheetIndices[k]);
		}
		...
		if (useNewSoaMethod) {
			...
			dcSoaModel = new DestinationSampleOfAlternativesModel(
					soaUecFileName, soaSampleSize, propertyMap, mgraManager,
					dcSizeArray, dcSoaDmuObject, soaUecSheetIndices);
		}
		...
		for (int uecIndex : modelIndexMap.keySet()) {
			...
			try {
				dcModel[i] = new ChoiceModelApplication(dcUecFileName,
						uecIndex, dcModelDataSheet, propertyMap,
						(VariableTable) dcDmuObject);

				if (useNewSoaMethod) {
					dcModel2[i] = new ChoiceModelApplication(dcUecFileName2,
							uecIndex, dcModelDataSheet, propertyMap,
							(VariableTable) dcDistSoaDmuObject);
				}
		...
		for (i = 1; i < persons.length; i++) {
			...
			for (Tour tour : tourList) {
				...
				try {
					...
					mcDmuObject.setDmuIndexValues(hh.getHhId(), homeTaz,
							origMgra, 0, hh.getDebugChoiceModels());
					...
					dcDmuObject.setDmuIndexValues(hh.getHhId(), homeTaz,
							origMgra, 0);
					...
					if (useNewSoaMethod) {
						...
						dcDistSoaDmuObject.setDmuIndexValues(hh.getHhId(),
								homeTaz, origMgra, 0);
					}
					...
					if (useNewSoaMethod) {
						chosen = selectLocationFromTwoStageSampleOfAlternatives(
								tour, mcDmuObject);
						soaRunTime += dcSoaTwoStageObject.getSoaRunTime();
					} else {
						chosen = selectLocationFromSampleOfAlternatives(tour,
								dcDmuObject, dcSoaDmuObject, mcDmuObject);
	...
	public void applyJointModel(Household hh) {
		...
		for (Tour tour : tourList) {
			...
			try {
				...
				mcDmuObject.setTourObject(tour);
				mcDmuObject.setDmuIndexValues(hh.getHhId(), homeTaz, origMgra,
						0, hh.getDebugChoiceModels());

				// update the DC dmuObject for this person
				dcDmuObject.setHouseholdObject(hh);
				dcDmuObject.setPersonObject(null);
				dcDmuObject.setTourObject(tour);
				dcDmuObject.setDmuIndexValues(hh.getHhId(), homeTaz, origMgra,
						0);
				...
				if (useNewSoaMethod) {
					...
					dcDistSoaDmuObject.setTourObject(tour);
					dcDistSoaDmuObject.setDmuIndexValues(hh.getHhId(), homeTaz,
							origMgra, 0);
				}
	 ...
	 * @return chosen mgra.
	 */
	private int selectLocationFromSampleOfAlternatives(Tour tour,
			DestChoiceDMU dcDmuObject, DcSoaDMU dcSoaDmuObject,
		...
		int sizeIndex = nonMandatorySizeSegmentNameIndexMap
				.get(tourPurposeName);
		...
		mcModel.getAnmSkimCalculator().getOpSkimDistancesFromMgra(
				household.getHhMgra(), mgraDistanceArray);
		...
		dcSoaModel.computeDestinationSampleOfAlternatives(dcSoaDmuObject, tour,
				person, tourPurposeName, tourPurposeIndex,
				household.getHhMgra());
		...
		for (int i = 1; i < finalSample.length; i++) {
			...
			// zone and subzone.
			double logsum = calculateSimpleTODChoiceLogsum(person, tour,
					destMgra, i);
		...
		if (household.getDebugChoiceModels()) {
			...
			choiceModelDescription = String.format(
					"Non-Mandatory Location Choice Model for: tour purpose=%s",
					tourPurposeName);
			...
			modelLogger
					.info("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
			modelLogger
					.info("Non-Mandatory Location Choice Model for tour purpose="
							+ tourPurposeName
							+ ", Person Num: "
							+ person.getPersonNum()
							+ ", Person Type: "
		...
		if (household.getDebugChoiceModels() || chosen <= 0) {
			...
			for (int j = 1; j < finalSample.length; j++) {
				...
				modelLogger.info(String.format("%-21s%15s%18.6e%18.6e%18.6e",
						altString, availabilities[alt], utilities[alt - 1],
						probabilities[alt - 1], cumProb));
			}

			modelLogger.info(" ");
			String altString = String.format("j=%d, mgra=%d", selectedIndex,
					chosen);
			...
			dcModel[m].logAlternativesInfo(choiceModelDescription,
					decisionMakerLabel);
			dcModel[m].logSelectionInfo(choiceModelDescription,
	...
	private int selectLocationFromTwoStageSampleOfAlternatives(Tour tour,
		...
		dcSoaTwoStageObject.chooseSample(
				mgraManager.getTaz(tour.getTourOrigMgra()), tourPurposeIndex,
				tourPurposeIndex, soaSampleSize, household.getHhRandom(),
		...
		int numUniqueAlts = dcSoaTwoStageObject
		...
		if (household.getDebugChoiceModels()) {
			...
			dcModel2[m].choiceModelUtilityTraceLoggerHeading(
					choiceModelDescription, decisionMakerLabel);

		}

		// compute destination choice proportions and choose alternative
		dcModel2[m].computeUtilities(dcDistSoaDmuObject,
				dcDistSoaDmuObject.getDmuIndexValues(), dcModel2AltsAvailable,
				dcModel2AltsSample);
		...
		if (dcModel2[m].getAvailabilityCount() > 0) {
			try {
				...
				logger.error(String
						.format("Exception caught for HHID=%d, PersonNum=%d, tourId=%d, in %s destination choice.",
								dcDistSoaDmuObject.getHouseholdObject()
										.getHhId(), dcDistSoaDmuObject
										.getPersonObject().getPersonNum(), tour
										.getTourId(), tourPurposeName));
				throw new RuntimeException();
			}
		}
		...
		if (household.getDebugChoiceModels() || chosen <= 0) {
			...
			dcModel2[m].logAlternativesInfo(choiceModelDescription,
					decisionMakerLabel);
			dcModel2[m].logSelectionInfo(choiceModelDescription,
					decisionMakerLabel, rn, chosen);
	...
	private void setModeChoiceDmuAttributes(Household household, Person person,
			Tour t, int startPeriod, int endPeriod, int sampleDestMgra) {
		...
		mcDmuObject.setDmuIndexValues(household.getHhId(), t.getTourOrigMgra(),
				t.getTourOrigMgra(), sampleDestMgra,
				household.getDebugChoiceModels());
	...
	private double calculateSimpleTODChoiceLogsum(Person person, Tour tour,
			int sampleDestMgra, int sampleNum) {
		...
		if (household.getDebugChoiceModels()) {
			...
			decisionMakerLabel = String
					.format("HH=%d, PersonNum=%d, PersonType=%s, tourId=%d of %d non-mand tours",
		...
		for (int[] combo : periodCombinations[tourPurposeIndex]) {
			...
			if (needToComputeLogsum[index]) {
				...
				String periodString = modelStructure
						.getModelPeriodLabel(modelStructure
				...
				setModeChoiceDmuAttributes(household, person, tour,
						startPeriod, endPeriod, sampleDestMgra);
			...
			if (household.getDebugChoiceModels())
				modelLogger
						.info("i = "
								...
								+ ", exp(MCLS + ASC) = " + expUtil
								+ ", cumExpUtility = " + totalExpUtility);
	...
	public void setNonMandatorySoaProbs(double[][][] soaDistProbs,
			double[][][] soaSizeProbs) {
		if (useNewSoaMethod) {
	...
	public static void main(String[] args) {
		...
		String matrixServerAddress = (String) propertyMap
				.get("RunModel.MatrixServerAddress");
		String matrixServerPort = (String) propertyMap
				.get("RunModel.MatrixServerPort");

		MatrixDataServerIf ms = new MatrixDataServerRmi(matrixServerAddress,
				Integer.parseInt(matrixServerPort),
		...
		if (!aggAcc.getAccessibilitiesAreBuilt()) {
			...
			boolean readAccessibilities = Util.getBooleanValueFromPropertyMap(
					propertyMap, CtrampApplication.READ_ACCESSIBILITIES);
		...
		NonTransitUtilities ntUtilities = new NonTransitUtilities(propertyMap,
				sovExpUtilities, hovExpUtilities, nMotorExpUtilities);

		MandatoryAccessibilitiesCalculator mandAcc = new MandatoryAccessibilitiesCalculator(
				propertyMap, ntUtilities, expConstants,
		...
		HouseholdIndividualNonMandatoryTourFrequencyModel inmtfModel = new HouseholdIndividualNonMandatoryTourFrequencyModel(
				...
				aggAcc.getAccessibilitiesTableObject(), mandAcc);

		TourModeChoiceModel inmmcModel = new TourModeChoiceModel(propertyMap,
				modelStructure, "Non-Mandatory", dmuFactory, logsumHelper);

		NonMandatoryDestChoiceModel testObject = new NonMandatoryDestChoiceModel(
				propertyMap, modelStructure, aggAcc, dmuFactory, inmmcModel);

		String hhHandlerAddress = (String) propertyMap
				.get("RunModel.HouseholdServerAddress");
		int hhServerPort = Integer.parseInt((String) propertyMap
				.get("RunModel.HouseholdServerPort"));

		HouseholdDataManagerIf householdDataManager = new HouseholdDataManagerRmi(
				hhHandlerAddress, hhServerPort,
		...
		try {
			...
			// stf, stl
			String restartModel = (String) propertyMap
					.get("RunModel.RestartWithHhServer");
			if (restartModel.equalsIgnoreCase("none"))
				restartHhServer = true;
					...
					|| restartModel.equalsIgnoreCase("stl"))
