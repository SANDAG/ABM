...
public class JointTourModels implements Serializable {
	...
	public static final String[] JOINT_TOUR_COMPOSITION_NAMES = { "", "adult",
			"child", "mixed" };
	...
	public JointTourModels(HashMap<String, String> propertyMap,
			AccessibilitiesTable myAccTable, ModelStructure modelStructure,
	...
	public void setUpModels(HashMap<String, String> propertyMap,
		...
		int freqCompSheet = Integer.parseInt(propertyMap
				.get(UEC_JOINT_TOUR_FREQ_COMP_MODEL_PAGE));
		int particSheet = Integer.parseInt(propertyMap
				.get(UEC_JOINT_TOUR_PARTIC_MODEL_PAGE));

		// set up the models
		jointTourFrequencyModel = new ChoiceModelApplication(uecFileName,
				freqCompSheet, dataSheet, propertyMap,
				(VariableTable) dmuObject);
		jointModelsAltsTable = jointTourFrequencyModel.getUEC()
				.getAlternativeData();
		modelStructure.setJtfAltLabels(jointTourFrequencyModel
				.getAlternativeNames());

		jointTourParticipation = new ChoiceModelApplication(uecFileName,
				particSheet, dataSheet, propertyMap, (VariableTable) dmuObject);
	}
		...
		try {
			...
			if (household.getDebugChoiceModels())
				household.logHouseholdObject(
						"Pre Joint Tour Frequency Choice HHID="
			...
			int validIndex = household
					.getValidHouseholdForJointTourFrequencyModel();
			if (validIndex != 1) {
				...
				case 3:
					...
					break;
				case 4:
					household.setJointTourFreqResult(-4,
							"-4_only preschool travel");
					break;
				}
				return;
			}
			...
			if (household.getAutoSufficiency() == 1) {
				dmuObject.setShopHOVAccessibility(accTable
						.getAggregateAccessibility("shopHov0",
								household.getHhMgra()));
				dmuObject.setMaintHOVAccessibility(accTable
						.getAggregateAccessibility("maintHov0",
								household.getHhMgra()));
				dmuObject.setDiscrHOVAccessibility(accTable
						.getAggregateAccessibility("discrHov0",
								household.getHhMgra()));
			} else if (household.getAutoSufficiency() == 2) {
				dmuObject.setShopHOVAccessibility(accTable
						.getAggregateAccessibility("shopHov1",
								household.getHhMgra()));
				dmuObject.setMaintHOVAccessibility(accTable
						.getAggregateAccessibility("maintHov1",
								household.getHhMgra()));
				dmuObject.setDiscrHOVAccessibility(accTable
						.getAggregateAccessibility("discrHov1",
								household.getHhMgra()));
			} else if (household.getAutoSufficiency() == 3) {
				dmuObject.setShopHOVAccessibility(accTable
						.getAggregateAccessibility("shopHov2",
								household.getHhMgra()));
				dmuObject.setMaintHOVAccessibility(accTable
						.getAggregateAccessibility("maintHov2",
								household.getHhMgra()));
				dmuObject.setDiscrHOVAccessibility(accTable
						.getAggregateAccessibility("discrHov2",
								household.getHhMgra()));
			}
			...
			if (household.getDebugChoiceModels()) {
				...
				decisionMakerLabel = String.format("HH=%d, hhSize=%d.",
						household.getHhId(), household.getHhSize());
			...
			if (household.getDebugChoiceModels()) {
				...
				double[] probabilities = jointTourFrequencyModel
						.getProbabilities();
				...
				for (int k = 0; k < altNames.length; k++) {
					...
					String altString = String.format("%-3d %10s", k + 1,
							altNames[k]);
					modelLogger
							.info(String.format("%-15s%18.6e%18.6e%18.6e",
									altString, utilities[k], probabilities[k],
									cumProb));
				}
				...
				jointTourFrequencyModel.logAlternativesInfo(
						choiceModelDescription, decisionMakerLabel);
				jointTourFrequencyModel.logSelectionInfo(
			...
			logger.error(String.format(
					"error joint tour choices model for hhId=%d.",
					household.getHhId()));
			throw new RuntimeException();
		}
	...
	private void jointTourParticipation(Tour jointTour) {
		...
		while (!validParty) {
			...
			for (int p = 1; p < persons.length; p++) {
				...
				if (household.getDebugChoiceModels() || loopCount == 1000) {
					decisionMakerLabel = String
							.format("HH=%d, hhSize=%d, PersonNum=%d, PersonType=%s, tourId=%d.",
									household.getHhId(), household.getHhSize(),
									person.getPersonNum(),
				...
				case 1:
					if (persons[p].getPersonIsAdult() == 1) {
						...
						if (household.getDebugChoiceModels()
							...
							String[] altNames = jointTourParticipation
									.getAlternativeNames();

							double[] utilities = jointTourParticipation
									.getUtilities();
							double[] probabilities = jointTourParticipation
									.getProbabilities();
							...
							for (int k = 0; k < altNames.length; k++) {
								cumProb += probabilities[k];
								String altString = String.format("%-3d %13s",
										k + 1, altNames[k]);
								modelLogger.info(String
										.format("%-18s%18.6e%18.6e%18.6e",
												altString, utilities[k],
												probabilities[k], cumProb));
							}

							modelLogger.info(" ");
							String altString = String.format("%-3d %13s",
									chosen, altNames[chosen - 1]);
							modelLogger.info(String.format(
							...
							// file
							jointTourParticipation.logAlternativesInfo(
									choiceModelDescription, decisionMakerLabel);
							jointTourParticipation.logSelectionInfo(
									choiceModelDescription, decisionMakerLabel,
									rn, chosen);

							// write UEC calculation results to separate model
							// specific log file
							jointTourParticipation.logUECResults(modelLogger,
									loggingHeader);
							...
							if (loopCount == 1000) {
								jointTourFrequencyModel
										.choiceModelUtilityTraceLoggerHeading(
												choiceModelDescription,
												decisionMakerLabel);
								jointTourFrequencyModel.computeUtilities(
										dmuObject,
										dmuObject.getDmuIndexValues());
								jointTourFrequencyModel.logUECResults(
				...
				case 2:
					if (persons[p].getPersonIsAdult() == 0) {
						...
						if (household.getDebugChoiceModels()
							...
							modelLogger.info("HHID: " + household.getHhId()
									+ ", HHSize: " + household.getHhSize()
									+ ", tourId: " + jointTour.getTourId());
							modelLogger
									.info("Alternative                 Utility       Probability           CumProb");
							modelLogger
									.info("------------------   --------------    --------------    --------------");
					...
					if (household.getDebugChoiceModels() || loopCount == 1000) {
						...
						for (int k = 0; k < altNames.length; k++) {
							...
							String altString = String.format("%-3d %13s",
									k + 1, altNames[k]);
							modelLogger.info(String.format(
									"%-18s%18.6e%18.6e%18.6e", altString,
			...
			if (loopCount > 1000) {
				logger.warn("loop count in joint tour participation model is "
						+ loopCount);
				if (loopCount > 2000) {
	...
	private void createJointTours(Household household, int chosenAlt) {
		...
		int purposeIndex1 = (int) jointModelsAltsTable.getValueAt(chosenAlt,
				PURPOSE_1_FIELD);
		int purposeIndex2 = (int) jointModelsAltsTable.getValueAt(chosenAlt,
				PURPOSE_2_FIELD);

		if (purposeIndex1 > 0 && purposeIndex2 > 0) {

			Tour t1 = new Tour(household,
					(String) purposeIndexNameMap.get(purposeIndex1),
					ModelStructure.JOINT_NON_MANDATORY_CATEGORY, purposeIndex1);
			int party1 = (int) jointModelsAltsTable.getValueAt(chosenAlt,
					PARTY_1_FIELD);
			t1.setJointTourComposition(party1);

			Tour t2 = new Tour(household,
					(String) purposeIndexNameMap.get(purposeIndex2),
					ModelStructure.JOINT_NON_MANDATORY_CATEGORY, purposeIndex2);
			int party2 = (int) jointModelsAltsTable.getValueAt(chosenAlt,
					PARTY_2_FIELD);
	...
	public static void main(String[] args) {
		...
		JointTourModels jtfModel = new JointTourModels(propertyMap, accTable,
