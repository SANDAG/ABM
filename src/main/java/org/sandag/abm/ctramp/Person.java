...
public class Person implements java.io.Serializable {
	...
	public static final String[] employmentCategoryNameArray = {
			EMPLOYMENT_CATEGORY_FULL_TIME_WORKER_NAME,
			EMPLOYMENT_CATEGORY_PART_TIME_WORKER_NAME,
			EMPLOYMENT_CATEGORY_NOT_EMPLOYED_NAME,
			EMPLOYMENT_CATEGORY_UNDER_AGE_16_NAME };
	...
	public boolean isPreviousArrival(int period) {
		...
		if (windows[period] == 3 || windows[period] == 4)
			return true;
		else
			return false;
	...
	public boolean isPreviousDeparture(int period) {
		...
		if (windows[period] == 2 || windows[period] == 4)
			return true;
		else
			return false;
	...
	public void createIndividualNonMandatoryTours(int numberOfTours,
		...
		int primaryIndex = modelStructure.getPrimaryPurposeNameIndexMap().get(
				primaryPurposeName);
	...
	public void createWorkTours(int numberOfTours, int startId,
			String tourPurpose, int tourPurposeIndex) {
		...
		for (int i = 0; i < numberOfTours; i++) {
			...
			if (workLoc == ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR)
				...
				tempTour.setTourDestMgra(workLoc);
	...
	public void createAtWorkSubtour(int id, int choice, int workMgra,
			String subtourPurpose) {
	...
	public void createSchoolTours(int numberOfTours, int startId,
			String tourPurpose, int tourPurposeIndex) {
	...
	public int getPersonIsChildUnder16WithHomeOrNonMandatoryActivity() {
		...
		if (persIsStudentNonDrivingAge() == 1 || persIsPreschoolChild() == 1) {
			...
			if (cdapActivity.equalsIgnoreCase(ModelStructure.HOME_PATTERN))
				return (1);

			if (cdapActivity.equalsIgnoreCase(ModelStructure.MANDATORY_PATTERN))
				return (1);
	...
	public int getCdapIndex() {
		...
		if (cdapActivity.equalsIgnoreCase(ModelStructure.NONMANDATORY_PATTERN))
			return 2;
	...
	public int getNumIndividualEscortTours() {
		...
		for (Tour tour : getListOfIndividualNonMandatoryTours())
			if (tour.getTourPurpose().equalsIgnoreCase(
					modelStructure.ESCORT_PURPOSE_NAME))
				num++;
		return num;
	}
	...
	public int getNumIndividualShoppingTours() {
		...
		for (Tour tour : getListOfIndividualNonMandatoryTours())
			if (tour.getTourPurpose().equalsIgnoreCase(
					modelStructure.SHOPPING_PURPOSE_NAME))
				num++;
		return num;
	}
	...
	public int getNumIndividualEatOutTours() {
		...
		for (Tour tour : getListOfIndividualNonMandatoryTours())
			if (tour.getTourPurpose().equalsIgnoreCase(
					modelStructure.EAT_OUT_PURPOSE_NAME))
				num++;
		return num;
	}
	...
	public int getNumIndividualOthMaintTours() {
		...
		for (Tour tour : getListOfIndividualNonMandatoryTours())
			if (tour.getTourPurpose().equalsIgnoreCase(
					modelStructure.OTH_MAINT_PURPOSE_NAME))
				num++;
		return num;
	}
	...
	public int getNumIndividualSocialTours() {
		...
		for (Tour tour : getListOfIndividualNonMandatoryTours())
			if (tour.getTourPurpose().equalsIgnoreCase(
					modelStructure.SOCIAL_PURPOSE_NAME))
				num++;
		return num;
	}
	...
	public int getNumMandatoryTours() {
		...
		ArrayList<Tour> workTours = getListOfWorkTours();
		if (workTours != null)
			numTours += workTours.size();

		ArrayList<Tour> schoolTours = getListOfSchoolTours();
		if (schoolTours != null)
			numTours += schoolTours.size();
	...
	public int getNumNonMandatoryTours() {
		...
		if (nonMandTours == null)
			return 0;
		else
			return nonMandTours.size();
	}
	...
	public int getNumSubtours() {
		...
		if (subtours == null)
			...
			return subtours.size();
	}

	public int getNumTotalIndivTours() {
		return getNumMandatoryTours() + getNumNonMandatoryTours()
				+ getNumSubtours();
	}
	...
	public void computeIdapResidualWindows() {
		...
		if (firstTourStart > modelStructure.getNumberOfTimePeriods() - 1
				&& lastTourEnd < 0) {
			...
			windowAfterLastMandJointTour = modelStructure
					.getNumberOfTimePeriods() - lastTourEnd;
	...
	public int getPersonIsAdult() {
		if (persIsPreschoolChild() == 1 || getPersonIsStudentNonDriving() == 1)
			return 0;
		else
			return 1;
	}
	...
	private int persIsHomemaker() {
		if (persAge >= MIN_ADULT_AGE
				&& persEmploymentCategory == EmployStatus.NOT_EMPLOYED
						.ordinal())
			return 1;
		else
			return 0;
	...
	public int notEmployed() {
		if (persEmploymentCategory == EmployStatus.NOT_EMPLOYED.ordinal())
			return 1;
		else
			return 0;
	}
	...
	private int persIsWorker() {
		if (persEmploymentCategory == EmployStatus.FULL_TIME.ordinal()
				|| persEmploymentCategory == EmployStatus.PART_TIME.ordinal())
			return 1;
		else
			return 0;
	}
	...
	private int persIsStudent() {
		if (persStudentCategory == StudentStatus.STUDENT_HIGH_SCHOOL_OR_LESS
				.ordinal()
				|| persStudentCategory == StudentStatus.STUDENT_COLLEGE_OR_HIGHER
						.ordinal()) {
			return 1;
		} else {
			return 0;
	...
	private int persIsStudentDrivingAge() {
		if (personType == PersonType.Student_age_16_19_not_FT_wrkr_or_univ_stud
				.ordinal())
			return 1;
		else
			return 0;
	...
	public void logPersonObject(Logger logger, int totalChars) {
		...
		Household.logHelper(logger, "persEmploymentCategory: ",
				persEmploymentCategory, totalChars);
		Household.logHelper(logger, "persStudentCategory: ",
				persStudentCategory, totalChars);
		...
		Household.logHelper(logger, "workLocSegmentIndex: ",
				workLocSegmentIndex, totalChars);
		Household.logHelper(logger, "schoolLocSegmentIndex: ",
				schoolLocSegmentIndex, totalChars);
		Household.logHelper(logger, "freeParkingAvailable: ",
				freeParkingAvailable, totalChars);
		...
		Household.logHelper(logger, "maxAdultOverlaps: ", maxAdultOverlaps,
				totalChars);
		Household.logHelper(logger, "maxChildOverlaps: ", maxChildOverlaps,
				totalChars);
		...
		Household.logHelper(logger, "windowAfterLastMandJointTour: ",
				windowAfterLastMandJointTour, totalChars);
		...
		for (int i = 1; i < windows.length; i++) {
			...
			header2 += String.format("%4s|",
					modelStructure.getTimePeriodLabel(i));
			switch (windows[i]) {
			case 0:
				periodString = "    ";
				break;
		...
		if (atWorkSubtourArrayList.size() > 0) {
			for (Tour tour : atWorkSubtourArrayList) {
				...
				if (purposeName
						.equalsIgnoreCase(modelStructure.AT_WORK_BUSINESS_PURPOSE_NAME))
					alias = "aB";
				else if (purposeName
						.equalsIgnoreCase(modelStructure.AT_WORK_EAT_PURPOSE_NAME))
					alias = "aE";
				else if (purposeName
						.equalsIgnoreCase(modelStructure.AT_WORK_MAINT_PURPOSE_NAME))
					alias = "aM";
		...
		if (hhObj.getJointTourArray() != null
				&& hhObj.getJointTourArray().length > 0) {
					...
					for (int num : persNumArray) {
						if (num == persNum) {
							...
							if (tour.getTourPurpose()
								...
								alias = "jD";
							logger.info(tour.getTourWindow(String.format(
									"%s%d", alias, id)));
						}
					}
				}
	...
	public void logEntirePersonObject(Logger logger) {
		...
		String windowString = "     Window:     |";
		for (int i = 1; i < windows.length; i++) {
			header += String.format("%4s|",
					modelStructure.getTimePeriodLabel(i));
			windowString += String.format("%4s|", windows[i] == 0 ? "    "
					: "XXXX");
		}
		...
		if (schoolTourArrayList.size() > 0) {
			for (Tour tour : schoolTourArrayList) {
				logger.info(tour.getTourWindow(tour.getTourPurpose()
						.equalsIgnoreCase("university") ? "U" : "S"));
			}
		}
		if (indNonManTourArrayList.size() > 0) {
			for (Tour tour : indNonManTourArrayList) {
