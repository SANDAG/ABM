...
public class AtWorkSubtourFrequencyDMU implements Serializable, VariableTable {
	...
	protected transient Logger logger = Logger
			.getLogger(AtWorkSubtourFrequencyDMU.class);
	...
	public int getNumIndivEatOutTours() {
		int numTours = 0;
		ArrayList<Tour> tourList = person
	...
	public int getNumTotalTours() {
		...
		if (wTourList != null)
			numTours += wTourList.size();

		ArrayList<Tour> sTourList = person.getListOfSchoolTours();
		if (sTourList != null)
			numTours += sTourList.size();

		ArrayList<Tour> nmTourList = person
				.getListOfIndividualNonMandatoryTours();
		if (nmTourList != null)
			numTours += nmTourList.size();
