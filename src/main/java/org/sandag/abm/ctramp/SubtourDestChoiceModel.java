...
public class SubtourDestChoiceModel implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(SubtourDestChoiceModel.class);
	...
	private static final double[][] periodCombinationCoefficients = {
			{ -3.1453, -0.1029, -2.9056 }, { -3.1453, -0.1029, -2.9056 },
	...
	public SubtourDestChoiceModel(HashMap<String, String> propertyMap,
		...
		logger.info(String.format(
				"creating %s subtour dest choice mode instance", tourCategory));
	...
	private void setupDestChoiceModelArrays(
		...
		dcSoaModel = new DestinationSampleOfAlternativesModel(soaUecFileName,
				soaSampleSize, propertyMap, mgraManager, dcSizeArray,
				dcSoaDmuObject, soaUecSheetIndices);
	...
	public void applyModel(Household hh) {
		...
		if (hh.getDebugChoiceModels())
			hh.logHouseholdObject(
					"Pre Subtour Location Choice Household " + hh.getHhId()
							+ " Object", modelLogger);
		...
		for (i = 1; i < persons.length; i++) {
			...
			for (Tour tour : tourList) {
				...
				Tour workTour = null;
				int workTourIndex = 0;
				workTourIndex = tour.getWorkTourIndexFromSubtourId(tour
						.getTourId());
				...
				try {
					...
					mcDmuObject.setDmuIndexValues(hh.getHhId(), homeMgra,
					...
					dcDmuObject.setDmuIndexValues(hh.getHhId(), homeMgra,
							origMgra, 0);
					...
					logger.fatal(String
							.format("exception caught selecting %s tour destination choice for hh.hhid=%d, personNum=%d, tourId=%d, purposeName=%s",
									tourCategory, hh.getHhId(),
									p.getPersonNum(), tour.getTourId(),
	...
	private int selectLocationFromSampleOfAlternatives(Tour tour,
		...
		mcModel.getAnmSkimCalculator().getOpSkimDistancesFromMgra(
				tour.getTourOrigMgra(), mgraDistanceArray);
		...
		if (household.getDebugChoiceModels()) {
			...
			choiceModelDescription = String
					.format("At-work Subtour Location Choice Model for: tour purpose=%s",
							tourPurposeName);
			...
			modelLogger
					.info("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
			modelLogger
					.info("At-work Subtour Location Choice Model for tour purpose="
							+ tourPurposeName
							+ ", Person Num: "
							+ person.getPersonNum()
							+ ", Person Type: "
	...
	private void setModeChoiceDmuAttributes(Household household, Person person,
		...
		mcDmuObject.setDmuIndexValues(household.getHhId(),
				household.getHhMgra(), t.getTourOrigMgra(), sampleDestMgra,
				household.getDebugChoiceModels());
			...
			decisionMakerLabel = String
					.format("HH=%d, PersonNum=%d, PersonType=%s, tourId=%d of %d non-mand tours",
							household.getHhId(), person.getPersonNum(), person
									...
									.getListOfAtWorkSubtours().size());
			loggingHeader = String.format("%s    %s", choiceModelDescription,
					decisionMakerLabel);
		}

		int i = 0;
		int tourPurposeIndex = purposeNameIndexMap
				.get(tour.getSubTourPurpose());
	...
	public static void main(String[] args) {
		...
		TourModeChoiceModel awmcModel = new TourModeChoiceModel(propertyMap,
				modelStructure, ModelStructure.AT_WORK_CATEGORY, dmuFactory,
				logsumHelper);

		SubtourDestChoiceModel testObject = new SubtourDestChoiceModel(
				propertyMap, modelStructure, aggAcc, dmuFactory, awmcModel);
