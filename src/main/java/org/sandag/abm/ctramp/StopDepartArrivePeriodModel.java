...
public class StopDepartArrivePeriodModel implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(StopDepartArrivePeriodModel.class);
	...
	private void setupModels(HashMap<String, String> propertyMap) {
		...
		logger.info(String
				.format("setting up stop depart/arrive choice model."));

		String uecPath = propertyMap.get(CtrampApplication.PROPERTIES_UEC_PATH);
		String propsFile = uecPath
				+ propertyMap.get(PROPERTIES_STOP_TOD_LOOKUP_FILE);
	...
	private void readLookupProportions(String propsLookupFilename) {
		...
		for (int i = 0; i < propsLookupTable.getRowCount(); i++) {
			...
			int tourPrimaryPurposeIndex = (int) propsLookupTable.getValueAt(
					i + 1, TOUR_PURPOSE_INDEX_COLUMN_HEADING);
			...
			int todInterval = (int) propsLookupTable.getValueAt(i + 1,
					TOUR_TOD_PERIOD_HEADING);

			// get the trip number (1-4)
			int tripNumber = (int) propsLookupTable.getValueAt(i + 1,
	...
	public double[] getStopTodIntervalProportions(int tourPrimaryPurposeIndex,
			int direction, int prevTripTodInterval, int tripNumber) {
