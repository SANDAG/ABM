...
public class HouseholdChoiceModelRunner {
	...
	public HouseholdChoiceModelRunner(HashMap<String, String> propertyMap,
			JPPFClient jppfClient, String restartModelString,
			HouseholdDataManagerIf hhDataManager, MatrixDataServerIf ms,
			ModelStructure modelStructure, CtrampDmuFactoryIf dmuFactory) {
		setupHouseholdChoiceModelRunner(propertyMap, jppfClient,
				restartModelString, hhDataManager, ms, modelStructure,
				dmuFactory);
	}

	private void setupHouseholdChoiceModelRunner(
			HashMap<String, String> propertyMap, JPPFClient jppfClient,
			String restartModelString, HouseholdDataManagerIf hhDataManager,
			MatrixDataServerIf ms, ModelStructure modelStructure,
	...
	public void runHouseholdChoiceModels() {
		...
		logger.info(String.format(
				"household model runner finished %d households in %d minutes.",
	...
	private void submitTasks() {
		...
		try {
			...
			for (int[] startEndIndices : startEndTaskIndicesList) {
				...
				HouseholdChoiceModelsTaskJppf task = new HouseholdChoiceModelsTaskJppf(
			...
			for (JPPFTask task : results) {
				...
				try {
					logger.info(String.format(
							"HH TASK: %s returned: %s, maxAlts: %d.",
