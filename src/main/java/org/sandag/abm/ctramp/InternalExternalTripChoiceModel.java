        ...
        implements Serializable
{
    ...
    public InternalExternalTripChoiceModel( HashMap<String, String> propertyMap, CtrampDmuFactoryIf dmuFactory  )
    {
        ...
        ieModel = new ChoiceModelApplication(ieUecFile, modelSheet, dataSheet, propertyMap, (VariableTable)ieDmuObject);
    ...
    public void applyModel(Household hhObject, double[] distanceToCordonsLogsums){
        ...
        double vehiclesPerHouseholdMember = hhObject.getAutoOwnershipModelResult() / hhObject.getHhSize();
        ...
        for (int i=1; i<person.length; i++) {
            ...
            if ( ieModel.getAvailabilityCount() > 0) {
                ...
                String decisionMaker = String.format("HHID=%d, PersonNum=%d",  hhObject.getHhId(), person[i].getPersonNum() );
                String errorMessage = String.format("Exception caught for %s, no available internal-external trip choice alternatives to choose from in choiceModelApplication.", decisionMaker );
