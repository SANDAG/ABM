...
public class InternalExternalTripChoiceModel implements Serializable {
	...
	public InternalExternalTripChoiceModel(HashMap<String, String> propertyMap,
		...
		String ieUecFile = uecFileDirectory
				+ propertyMap.get(IE_CONTROL_FILE_TARGET);
		...
		int modelSheet = Util.getIntegerValueFromPropertyMap(propertyMap,
				IE_MODEL_SHEET_TARGET);

		// create the choice model DMU object.
		ieDmuObject = dmuFactory.getInternalExternalTripChoiceDMU();

		// create the choice model object
		ieModel = new ChoiceModelApplication(ieUecFile, modelSheet, dataSheet,
				propertyMap, (VariableTable) ieDmuObject);
	...
	public void applyModel(Household hhObject, double[] distanceToCordonsLogsums) {
		...
		ieDmuObject
				.setDistanceToCordonsLogsum(distanceToCordonsLogsums[homeTaz]);

		ieDmuObject.setHouseholdObject(hhObject);
		double vehiclesPerHouseholdMember = hhObject
				.getAutoOwnershipModelResult() / hhObject.getHhSize();
		...
		for (int i = 1; i < person.length; i++) {
			...
			ieDmuObject.setDmuIndexValues(hhObject.getHhId(),
					hhObject.getHhTaz());

			double randomNumber = hhRandom.nextDouble();

			// compute utilities and choose alternative.
			ieModel.computeUtilities(ieDmuObject,
					ieDmuObject.getDmuIndexValues());
			...
			if (ieModel.getAvailabilityCount() > 0) {
				...
				String decisionMaker = String.format("HHID=%d, PersonNum=%d",
						hhObject.getHhId(), person[i].getPersonNum());
			...
			if (hhObject.getDebugChoiceModels()) {
				String decisionMaker = String.format("HHID=%d, PersonNum=%d",
						hhObject.getHhId(), person[i].getPersonNum());
				ieModel.logAlternativesInfo("Internal-External Trip Choice",
						decisionMaker, logger);
				logger.info(String.format(
						"%s result chosen for %s is %d with rn %.8f",
						"Internal-External Trip Choice", decisionMaker,
						chosenAlt, randomNumber));
				ieModel.logUECResults(logger, decisionMaker);
			}
