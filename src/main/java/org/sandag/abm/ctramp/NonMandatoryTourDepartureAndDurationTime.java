...
public class NonMandatoryTourDepartureAndDurationTime implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(NonMandatoryTourDepartureAndDurationTime.class);
	...
	private static final String[] dcModelSheetKeys = {
			...
			IMTOD_UEC_MAINT_MODEL_TARGET, IMTOD_UEC_EAT_MODEL_TARGET,
			IMTOD_UEC_VISIT_MODEL_TARGET, IMTOD_UEC_DISCR_MODEL_TARGET };
	...
	public NonMandatoryTourDepartureAndDurationTime(
	...
	private void setupTodChoiceModels(HashMap<String, String> propertyMap,
		...
		int todModelDataSheet = Util.getIntegerValueFromPropertyMap(
				propertyMap, IMTOD_UEC_DATA_TARGET);

		for (int uecIndex : modelIndexMap.keySet()) {
			int modelIndex = modelIndexMap.get(uecIndex);
			try {
				todModels[modelIndex] = new ChoiceModelApplication(
						todUecFileName, uecIndex, todModelDataSheet,
						propertyMap, (VariableTable) todDmuObject);
		...
		int numDepartureTimeChoiceAlternatives = todModels[0]
	...
	public void applyIndivModel(Household hh, boolean runModeChoice) {
		...
		for (int p = 1; p < persons.length; p++) {
			...
			getPriorityOrderedTourList(person
			...
			for (int i = 0; i < tourPurposeIndexOrder.length; i++) {
				...
				for (Tour t : purposeTourLists[tourPurposeIndex]) {
					...
					try {
						...
						m = todModelIndices[purposeNameIndexMap
								.get(tourPurposeName)];
						...
						if (hh.getDebugChoiceModels()) {
							...
							decisionMakerLabel = String
									.format("HH=%d, PersonNum=%d, PersonType=%s, tourId=%d, num=%d of %d %s tours",
											hh.getHhId(),
											person.getPersonNum(), person
													.getPersonType(), t
													.getTourId(), tourPurpNum,
											purposeTourLists[tourPurposeIndex]
													.size(), tourPurposeName);
						...
						if (purposeTourLists[tourPurposeIndex].size() == 1) {
							...
							if (tourPurpNum == 1) {
								...
								int otherTourEndHour = purposeTourLists[tourPurposeIndex]
										.get(tourPurpNum - 2)
										.getTourArrivePeriod();
								todDmuObject
										.setEndOfPreviousScheduledTour(otherTourEndHour);
							}
						}
						...
						if (departureTimeChoiceAvailability.length != tourDepartureTimeChoiceSample.length) {
							logger.error(String
									.format("error in individual non-mandatory departure time choice model for hhId=%d, personNum=%d, tour purpose index=%d, tour ArrayList index=%d.",
											hh.getHhId(),
											person.getPersonNum(),
											tourPurposeIndex, tourPurpNum - 1));
							logger.error(String
						...
						if (alternativeAvailable < 0) {
							...
							person.scheduleWindow(chosenStartPeriod,
									chosenEndPeriod);
							...
							if (runModeChoice) {
								...
								setModeChoiceDmuAttributes(hh, person, t,
								...
								// for this work tour.
								int chosenMode = mcModel.getModeChoice(
										mcDmuObject, t.getTourPrimaryPurpose());
							...
							setTourModeChoiceLogsumsForDepartureTimeAndDurationAlternatives(
									t, departureTimeChoiceAvailability);

							if (hh.getDebugChoiceModels()) {
								hh.logTourObject(loggingHeader, modelLogger,
										person, t);
							}

							todDmuObject.setOriginZone(mgraManager.getTaz(t
									.getTourOrigMgra()));
							todDmuObject.setDestinationZone(mgraManager
									.getTaz(t.getTourDestMgra()));

							todModels[m].computeUtilities(todDmuObject,
									todDmuObject.getIndexValues(),
							...
							if (todModels[m].getAvailabilityCount() > 0) {
								...
								if (hh.getDebugChoiceModels()) {
									...
									double[] utilities = todModels[m]
											.getUtilities();
									double[] probabilities = todModels[m]
											.getProbabilities();
									boolean[] availabilities = todModels[m]
											.getAvailabilities();

									String personTypeString = person
											.getPersonType();
									...
									for (int k = 0; k < todModels[m]
										...
										modelLogger.info(String.format(
												...
												altString,
												availabilities[k + 1],
												utilities[k], probabilities[k],
												cumProb));
									}

									modelLogger.info(" ");
									String altString = String.format(
											"%-3d out=%-3d, in=%-3d", chosen,
											altStarts[chosen - 1],
											altEnds[chosen - 1]);
									modelLogger
											.info(String
													.format("Choice: %s, with rn=%.8f, randomCount=%d",
															altString, rn,
															randomCount));
									...
									// debug log file
									todModels[m].logAlternativesInfo(
											choiceModelDescription,
											decisionMakerLabel);
									todModels[m].logSelectionInfo(
											choiceModelDescription,
									...
									loggingHeader = String.format("%s for %s",
											choiceModelDescription,
											decisionMakerLabel);
									todModels[m].logUECResults(modelLogger,
											loggingHeader);
						...
						String decisionMakerLabel = String
								.format("Final Individual Non-Mandatory Departure Time Person Object: HH=%d, PersonNum=%d, PersonType=%s",
										hh.getHhId(), person.getPersonNum(),
										person.getPersonType());
						hh.logPersonObject(decisionMakerLabel, modelLogger,
								person);
				...
				try {
					...
					if (hh.getDebugChoiceModels()) {
						...
						decisionMakerLabel = String.format(
								"HH=%d, tourId=%d, %s, num=%d of %d %s tours",
								hh.getHhId(), t.getTourId(),
								personNumsInJointTour, tourPurpNum,
								purposeTourLists[tourPurposeIndex].size(),
								tourPurposeName);
					...
					if (purposeTourLists[tourPurposeIndex].size() == 1) {
						...
						if (tourPurpNum == 1) {
							...
							// get the right index
							int otherTourEndHour = purposeTourLists[tourPurposeIndex]
									.get(tourPurpNum - 2).getTourArrivePeriod();
							todDmuObject
									.setEndOfPreviousScheduledTour(otherTourEndHour);
						}
					}
					...
					boolean[] departureTimeChoiceAvailability = hh
							.getAvailableJointTourTimeWindows(t, altStarts,
									altEnds);
					Arrays.fill(tourDepartureTimeChoiceSample, 1);
					...
					if (alternativeAvailable < 0) {
						...
						// schedule the chosen alternative
						hh.scheduleJointTourTimeWindows(t, chosenStartPeriod,
								chosenEndPeriod);
						t.setTourDepartPeriod(chosenStartPeriod);
						t.setTourArrivePeriod(chosenEndPeriod);
						previouslySelectedDepartPeriod = chosenStartPeriod;
						...
						if (runModeChoice) {
							...
							// set the mode choice attributes needed by
							// @variables in the UEC spreadsheets
							setModeChoiceDmuAttributes(hh, null, t,
									chosenStartPeriod, chosenEndPeriod);
						...
						todDmuObject.setOriginZone(mgraManager.getTaz(t
								.getTourOrigMgra()));
						todDmuObject.setDestinationZone(mgraManager.getTaz(t
								.getTourDestMgra()));
						...
						if (todModels[m].getAvailabilityCount() > 0) {
							...
							if (hh.getDebugChoiceModels()) {
								...
								for (int k = 0; k < todModels[m]
									...
									String altString = String.format(
											"%-3d out=%-3d, in=%-3d", k + 1,
											altStarts[k], altEnds[k]);
									modelLogger.info(String.format(
											"%-20s%15s%18.6e%18.6e%18.6e",
											altString, availabilities[k + 1],
											utilities[k], probabilities[k],
											cumProb));
								}
	...
	private void setTourModeChoiceLogsumsForDepartureTimeAndDurationAlternatives(
		...
		String choiceModelDescription = String
				.format("NonMandatory Tour Mode Choice Logsum calculation for %s Departure Time Choice",
						tour.getTourPurpose());
	...
	public static void main(String[] args) {
		...
		TourModeChoiceModel inmmcModel = new TourModeChoiceModel(propertyMap,
				modelStructure,
				ModelStructure.INDIVIDUAL_NON_MANDATORY_CATEGORY, dmuFactory,
				logsumHelper);
