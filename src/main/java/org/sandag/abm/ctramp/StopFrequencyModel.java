...
public class StopFrequencyModel implements Serializable {
	...
	private static String[] shopTypes = { "", "shopSov0", "shopSov1",
			"shopSov2" };
	private static String[] maintTypes = { "", "maintSov0", "maintSov1",
			"maintSov2" };
	private static String[] discrTypes = { "", "discrSov0", "discrSov1",
			"discrSov2" };
	 ...
	 */
	public StopFrequencyModel(HashMap<String, String> propertyMap,
			CtrampDmuFactoryIf dmuFactory, ModelStructure myModelStructure,
	...
	private void setupModels(HashMap<String, String> propertyMap,
		...
		tourPrimaryPurposeIndexNameMap = modelStructure
				.getIndexPrimaryPurposeNameMap();

		tourPurposeModelIndexMap = dmuObject
				.getTourPurposeChoiceModelIndexMap();
		...
		for (int i = 0; i < modelSheetsArray.length; i++)
			choiceModelApplication[i] = new ChoiceModelApplication(uecFileName,
					modelSheetsArray[i], UEC_DATA_PAGE, propertyMap,
	...
	public void applyModel(Household household) {
		...
		if (household.getDebugChoiceModels())
			household.logHouseholdObject("Pre Stop Frequency Choice: HH="
					+ household.getHhId(), stopFreqLogger);
		...
		dmuObject.setMaintenanceAccessibility(accTable
				.getAggregateAccessibility(maintTypes[autoSufficiency],
						household.getHhMgra()));
		dmuObject.setDiscretionaryAccessibility(accTable
				.getAggregateAccessibility(discrTypes[autoSufficiency],
						household.getHhMgra()));
		...
		if (jt != null) {
			...
			for (Tour tour : tourList) {
				...
				try {
					...
					int modelIndex = tourPurposeModelIndexMap.get(tour
					...
					if (household.getDebugChoiceModels()) {
						...
						decisionMakerLabel = String
								.format("HH=%d, TourType=%s, TourId=%d, TourPurpose=%s.",
										household.getHhId(),
					...
					dmuObject.setDmuIndexValues(household.getHhId(),
							household.getHhTaz(),
							mgraManager.getTaz(tour.getTourOrigMgra()),
							mgraManager.getTaz(tour.getTourDestMgra()));

					// compute the utilities
					choiceModelApplication[modelIndex].computeUtilities(
							dmuObject, dmuObject.getDmuIndexValues());
					...
					int choice = -1;
					if (choiceModelApplication[modelIndex]
							.getAvailabilityCount() > 0)
						choice = choiceModelApplication[modelIndex]
								.getChoiceResult(rn);
					else {
						logger.error(String
					...
					if (household.getDebugChoiceModels()) {
						...
						double[] utilities = choiceModelApplication[modelIndex]
								.getUtilities();
						...
						for (int k = 0; k < altNames.length; k++) {
							...
							String altString = String.format("%-3d %15s",
									k + 1, altNames[k]);
							modelLogger.info(String.format(
									"%-20s%18.6e%18.6e%18.6e", altString,
						...
						String altString = String.format("%-3d  %s", choice,
								altNames[choice - 1]);
						modelLogger.info(String.format(
								"Choice: %s, with rn=%.8f, randomCount=%d",
						...
						choiceModelApplication[modelIndex].logSelectionInfo(
								choiceModelDescription, decisionMakerLabel, rn,
								choice);

						// write UEC calculation results to separate model
						// specific log file
						choiceModelApplication[modelIndex].logUECResults(
					...
					if (household.getDebugChoiceModels()) {
						...
						decisionMakerLabel = String
								.format("HH=%d, PersonNum=%d, PersonType=%s, TourType=%s, TourId=%d, TourPurpose=%s, modelIndex=%d.",
										...
										tour.getTourCategory(),
										tour.getTourId(),
										tour.getTourPurpose(), modelIndex);
	...
	private int setStopFreqChoice(Tour tour, int stopFreqChoice) {
		...
		String tourPrimaryPurpose = tourPrimaryPurposeIndexNameMap
				.get(tourPrimaryPurposeIndex);
		...
		if (numObStops > 0) {
			...
			for (int i = 0; i < numObStops; i++) {
				...
				obStopPurposeIndices[i] = getStopPurpose(hh,
						OUTBOUND_DIRECTION_NAME, tourDepartPeriod,
						tourPrimaryPurpose, personType);
				obStopDestPurposes[i] = indexPurposeMap
						.get(obStopPurposeIndices[i]);
			}
			...
			if (tour.getOutboundStops() != null) {
				...
				logger.error(
						"outbound stops array for hhid=" + tour.getHhId()
								+ ", person="
								+ tour.getPersonObject().getPersonNum()
								+ ", tour=" + tour.getTourId() + ", purpose="
								+ tour.getTourPurpose(), e);
				try {
					throw e;
				} catch (Exception e1) {
			...
			tour.createOutboundStops(obStopOrigPurposes, obStopDestPurposes,
					obStopPurposeIndices);
		}
		...
		if (numIbStops > 0) {
			...
			for (int i = 0; i < numIbStops; i++) {
				...
				ibStopPurposeIndices[i] = getStopPurpose(hh,
						INBOUND_DIRECTION_NAME, tourArrivePeriod,
						tourPrimaryPurpose, personType);
				ibStopDestPurposes[i] = indexPurposeMap
						.get(ibStopPurposeIndices[i]);
			}
			ibStopOrigPurposes[numIbStops] = ibStopDestPurposes[numIbStops - 1];
			ibStopDestPurposes[numIbStops] = tour.getTourCategory()
					.equalsIgnoreCase(ModelStructure.AT_WORK_CATEGORY) ? "Work"
					: "Home";
			...
			if (tour.getInboundStops() != null) {
				Exception e = new RuntimeException();
				logger.error(
						"inbound stops array for hhid=" + tour.getHhId()
								+ ", person="
								+ tour.getPersonObject().getPersonNum()
								+ ", tour=" + tour.getTourId() + ", purpose="
								+ tour.getTourPurpose(), e);
			...
			tour.createInboundStops(ibStopOrigPurposes, ibStopDestPurposes,
					ibStopPurposeIndices);
		}
	...
	private void readPurposeLookupProportionsTable(String purposeLookupFilename) {
		...
		try {
			...
			purposeLookupTable = reader
					.readFile(new File(purposeLookupFilename));
		} catch (Exception e) {
			logger.error(
					String.format(
		...
		personTypeMap.put(FT_WORKER_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_FULL_TIME_WORKER_NAME);
		personTypeMap.put(PT_WORKER_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_PART_TIME_WORKER_NAME);
		personTypeMap.put(UNIVERSITY_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_UNIVERSITY_STUDENT_NAME);
		personTypeMap.put(NONWORKER_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_NON_WORKER_NAME);
		personTypeMap.put(RETIRED_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_RETIRED_NAME);
		personTypeMap.put(DRIVING_STUDENT_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_STUDENT_DRIVING_NAME);
		personTypeMap.put(NONDRIVING_STUDENT_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_STUDENT_NON_DRIVING_NAME);
		personTypeMap.put(PRESCHOOL_PERSON_TYPE_NAME,
				Person.PERSON_TYPE_PRE_SCHOOL_CHILD_NAME);
		...
		for (int i = 0; i < purposeLookupTable.getRowCount(); i++) {
			...
			String tourPrimPurp = primaryPurposeMap.get(purposeLookupTable
					.getStringValueAt(i + 1,
			...
			int departPeriodRangeStart = (int) purposeLookupTable.getValueAt(
					i + 1, TOUR_DEPARTURE_START_RANGE_COLUMN_HEADING);

			// get the end of the range of departure hours
			int arriveperiodRangeEnd = (int) purposeLookupTable.getValueAt(
					i + 1, TOUR_DEPARTURE_END_RANGE_COLUMN_HEADING);

			int startRange = modelStructure
					.getTimePeriodIndexForTime(departPeriodRangeStart);
			int endRange = modelStructure
					.getTimePeriodIndexForTime(arriveperiodRangeEnd);

			// get the person type
			String personType = personTypeMap.get(purposeLookupTable
					.getStringValueAt(i + 1, PERSON_TYPE_COLUMN_HEADING));

			// columns following person type are proportions by stop purpose.
			// Get the
			// index of the first stop purpose proportion.
			int firstPropColumn = purposeLookupTable
					.getColumnPosition(PERSON_TYPE_COLUMN_HEADING) + 1;
			...
			for (int j = 0; j < props.length; j++) {
				props[j] = purposeLookupTable.getValueAt(i + 1, firstPropColumn
						+ j);
			}
				...
				for (String ptype : personTypeMap.values()) {
					...
					if (direction.equalsIgnoreCase(OUTBOUND_DIRECTION_NAME)) {
						for (int k = startRange; k <= endRange; k++) {
							...
							.equalsIgnoreCase(INBOUND_DIRECTION_NAME)) {
						for (int k = startRange; k <= endRange; k++)
	...
	private int getStopPurpose(Household household, String halfTourDirection,
		...
		try {
			...
			int choice = ChoiceModelApplication.getMonteCarloSelection(props,
					rn);
			...
			logger.error("key=" + key + ", tourPrimaryPurpose="
					+ tourPrimaryPurpose + ", personType=" + personType
					+ ", halfTourDirection=" + halfTourDirection
