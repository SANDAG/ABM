...
public class SubtourDepartureAndDurationTime implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(SubtourDepartureAndDurationTime.class);
	...
	private void setupTodChoiceModels(HashMap<String, String> propertyMap,
		...
		int todModelDataSheet = Util.getIntegerValueFromPropertyMap(
				propertyMap, TOD_UEC_DATA_TARGET);
		...
		for (int p = 1; p < persons.length; p++) {
			...
			for (Tour t : subtourList) {
				...
				try {
					...
					workTourIndex = t.getWorkTourIndexFromSubtourId(t
							.getTourId());
					...
					if (subtourNumForWorkTours[workTourIndex] == 1) {
						...
						person.scheduleWindow(0,
								workTour.getTourDepartPeriod() - 1);
						person.scheduleWindow(
								workTour.getTourArrivePeriod() + 1,
						...
						logger.error("too many subtours for a work tour.  workTourIndex="
								...
								+ ", subtourNumForWorkTours[workTourIndex]"
								+ subtourNumForWorkTours[workTourIndex]);
						logger.error("hhid=" + hh.getHhId() + ", persNum="
								+ person.getPersonNum());
						throw new RuntimeException();
					}
					...
					if (hh.getDebugChoiceModels()) {
						...
						choiceModelDescription = String
								.format("AtWork Subtour Departure Time Choice Model for: Purpose=%s",
										tourPurposeName);
	...
	private void setModeChoiceDmuAttributes(Household household, Person person,
		...
		mcDmuObject.setDmuIndexValues(household.getHhId(),
				household.getHhMgra(), t.getTourOrigMgra(),
	...
	private void setTourModeChoiceLogsumsForDepartureTimeAndDurationAlternatives(
		...
		String choiceModelDescription = String
				.format("At-work Subtour Mode Choice Logsum calculation for %s Departure Time Choice",
						tour.getTourPurpose());
	...
	public static void main(String[] args) {
		...
		System.out
				.println("SubtourDepartureAndDurationTime object creation passed.");
