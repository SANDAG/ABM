 ...
 * 
 */
public class HouseholdIndividualMandatoryTourFrequencyModel implements
		Serializable {
	...
	public static final String[] choiceResults = { "1 Work", "2 Work",
			"1 School", "2 School", "Wrk & Schl", "Worker Works At Home",
			"Student Works At Home", "Worker School At Home",
			"Student School At Home" };
	 ...
	 */
	public HouseholdIndividualMandatoryTourFrequencyModel(
			HashMap<String, String> propertyMap, ModelStructure modelStructure,
			CtrampDmuFactoryIf dmuFactory, AccessibilitiesTable accTable,
			MandatoryAccessibilitiesCalculator myMandAcc) {

		setupHouseholdIndividualMandatoryTourFrequencyModel(propertyMap,
				modelStructure, dmuFactory, accTable, myMandAcc);
	...
	private void setupHouseholdIndividualMandatoryTourFrequencyModel(
		...
		int dataPage = Util.getIntegerValueFromPropertyMap(propertyMap,
				IMTF_DATA_SHEET_TARGET);
		int modelPage = Util.getIntegerValueFromPropertyMap(propertyMap,
		...
		// set up the model
		choiceModelApplication = new ChoiceModelApplication(imtfUecFile,
				modelPage, dataPage, propertyMap, (VariableTable) imtfDmuObject);
	...
	public void applyModel(Household household) {
		...
		if (household.getDebugChoiceModels())
			household.logHouseholdObject(
					"Pre Individual Mandatory Tour Frequency Choice HHID="
		...
		float accessibility = accTable.getAggregateAccessibility(
				types[autoSufficiency], household.getHhMgra());
				...
				if (person.getPersonIsPreschoolChild() == 0
					...
					if (person.getPersonIsWorker() == 1) {
						...
						if (workMgra != ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR) {
							...
							// wt time
							if (accessibilities[2] > 0.0
									&& accessibilities[2] < time)
								time = accessibilities[2];
							// dt time
							if (accessibilities[3] > 0.0
									&& accessibilities[3] < time)
					...
					choiceModelApplication.computeUtilities(imtfDmuObject,
							index);
					...
					if (household.getDebugChoiceModels()) {
						...
						double[] utilities = choiceModelApplication
								.getUtilities();
						double[] probabilities = choiceModelApplication
								.getProbabilities();
						...
						for (int k = 0; k < probabilities.length; ++k) {
							...
							String altString = String.format("%-3d %10s",
									k + 1, choiceResults[k]);
							modelLogger.info(String.format(
									"%-15s%18.6e%18.6e%18.6e", altString,
						...
						choiceModelApplication.logAlternativesInfo(
								choiceModelDescription, decisionMakerLabel);
						...
						// specific
						// log file
						choiceModelApplication.logUECResults(modelLogger,
								loggingHeader);
