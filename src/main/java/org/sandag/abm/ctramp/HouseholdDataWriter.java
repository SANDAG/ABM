...
public class HouseholdDataWriter {
	...
	private transient Logger logger = Logger
			.getLogger(HouseholdDataWriter.class);
	...
	public HouseholdDataWriter(HashMap<String, String> rbMap,
			ModelStructure modelStructure, int iteration) {
		...
		int autoSkimPage = Integer.parseInt(rbMap
				.get("acc.mandatory.auto.page"));
		...
		purposeIndexNameMap = this.modelStructure
	...
	public void writeDataToDatabase(HouseholdDataManagerIf householdData,
			String dbFileName) {
		...
		logger.info("Finished writing data structures to database (" + delta
				+ " minutes).");
	}
	...
	private void writeData(HouseholdDataManagerIf householdDataManager,
			DataWriter writer) {
		int hhid = 0;
		int persNum = 0;
		int tourid = 0;
		try {
			...
			logger.error(String.format("error writing hh=%d, persNum=%d", hhid,
					persNum), e);
	...
	private List<String> formIndivTourDataEntry(Tour t) {
		...
		data.add(string(calculateDistancesForAllMgras(t.getTourOrigMgra(),
				t.getTourDestMgra())));
		data.add(string(t.getSubtourFreqChoice()));
		data.add(string(t.getNumOutboundStops() == 0 ? 0 : t
				.getNumOutboundStops() - 1));
		data.add(string(t.getNumInboundStops() == 0 ? 0 : t
	...
	private String formTourParticipationEntry(Tour t) {
		...
		if (persons == null)
			throw new RuntimeException(
					"null Person[] object for joint tour, hhid=" + t.getHhId());
		if (persons.length == 1)
			throw new RuntimeException(
					"Person[] object has length=1 for joint tour, hhid="
							+ t.getHhId());
	...
	private List<String> formIndivTripDataEntry(Stop s) {
		...
		if (s.getStopId() == 0) {
			if (s.isInboundStop()) {
				...
				if (t.getTourCategory().equalsIgnoreCase(
		...
		} else if (s.isInboundStop()
				&& s.getStopId() == t.getNumInboundStops() - 1) {
			...
			data.add(string(t.getTourOrigMgra()));
				...
				&& s.getStopId() == t.getNumOutboundStops() - 1) {
	...
	private List<String> formTourAsIndivTripDataEntry(Tour t, boolean inbound) {
		...
		data.add(string(inbound ? t.getTourArrivePeriod() : t
				.getTourDepartPeriod()));
		data.add(string(t.getTourModeChoice()));
		data.add(string(t.getTourModeChoice()));
	...
	private class DatabaseDataWriter implements DataWriter {
		private final String householdTable = rbMap
				.get(PROPERTIES_HOUSEHOLD_TABLE);
		private final String personTable = rbMap.get(PROPERTIES_PERSON_TABLE);
		private final String indivTourTable = rbMap
				.get(PROPERTIES_INDIV_TOUR_TABLE);
		private final String jointTourTable = rbMap
				.get(PROPERTIES_JOINT_TOUR_TABLE);
		private final String indivTripTable = rbMap
				.get(PROPERTIES_INDIV_TRIP_TABLE);
		private final String jointTripTable = rbMap
				.get(PROPERTIES_JOINT_TRIP_TABLE);
		...
		private void initializeTables(String dbFileName) {
			...
			try {
				...
				s.addBatch(getTableInitializationString(householdTable,
						formHouseholdColumnNames(), formHouseholdColumnTypes()));
				s.addBatch(getTableInitializationString(personTable,
						formPersonColumnNames(), formPersonColumnTypes()));
				s.addBatch(getTableInitializationString(indivTourTable,
						formIndivTourColumnNames(), formIndivTourColumnTypes()));
				s.addBatch(getTableInitializationString(jointTourTable,
						formJointTourColumnNames(), formJointTourColumnTypes()));
				s.addBatch(getTableInitializationString(indivTripTable,
						formIndivTripColumnNames(), formIndivTripColumnTypes()));
				s.addBatch(getTableInitializationString(jointTripTable,
						formJointTripColumnNames(), formJointTripColumnTypes()));
		...
		private void setupPreparedStatements() {
			...
			jtp2.append(");");
			try {
				hhPreparedStatement = connection.prepareStatement(hhp
						.toString());
				personPreparedStatement = connection.prepareStatement(pp
						.toString());
				indivTourPreparedStatement = connection.prepareStatement(itp
						.toString());
				jointTourPreparedStatement = connection.prepareStatement(jtp
						.toString());
				indivTripPreparedStatement = connection.prepareStatement(itp2
						.toString());
				jointTripPreparedStatement = connection.prepareStatement(jtp2
						.toString());
		...
		private String getTableInitializationString(String table,
				List<String> columns, List<SqliteDataTypes> types) {
			...
			while (cols.hasNext())
				sb.append(",").append(cols.next()).append(" ")
						.append(tps.next().name());
			sb.append(");");
			return sb.toString();
		}
	...
	private class FileDataWriter implements DataWriter {
		...
		public FileDataWriter() {
			String baseDir = rbMap
					.get(CtrampApplication.PROPERTIES_PROJECT_DIRECTORY);

			String hhFile = formFileName(
					rbMap.get(PROPERTIES_HOUSEHOLD_DATA_FILE), iteration);
			String personFile = formFileName(
					rbMap.get(PROPERTIES_PERSON_DATA_FILE), iteration);
			String indivTourFile = formFileName(
					rbMap.get(PROPERTIES_INDIV_TOUR_DATA_FILE), iteration);
			String jointTourFile = formFileName(
					rbMap.get(PROPERTIES_JOINT_TOUR_DATA_FILE), iteration);
			String indivTripFile = formFileName(
					rbMap.get(PROPERTIES_INDIV_TRIP_DATA_FILE), iteration);
			String jointTripFile = formFileName(
					rbMap.get(PROPERTIES_JOINT_TRIP_DATA_FILE), iteration);

			try {
				hhWriter = new PrintWriter(new File(baseDir + hhFile));
				personWriter = new PrintWriter(new File(baseDir + personFile));
				indivTourWriter = new PrintWriter(new File(baseDir
						+ indivTourFile));
				jointTourWriter = new PrintWriter(new File(baseDir
						+ jointTourFile));
				indivTripWriter = new PrintWriter(new File(baseDir
						+ indivTripFile));
				jointTripWriter = new PrintWriter(new File(baseDir
						+ jointTripFile));
			} catch (IOException e) {
				throw new RuntimeException(e);
			}
		...
		private String formFileName(String originalFileName, int iteration) {
			...
			if (lastDot > 0) {
				...
				returnString = String.format("%s_%d.csv", originalFileName,
						iteration);
			}
