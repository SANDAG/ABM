...
public class Household implements java.io.Serializable {
	...
	public int getAutoSufficiency() {
		if (aoModelAutos < hhWorkers)
			return 1;
		else if (aoModelAutos == hhWorkers)
			return 2;
		else
			return 3;
	}
	...
	public int getJointTourFreqChosenAlt() {
		...
		if (jtfModelPattern == null) {
			...
			returnValue = Integer.parseInt(jtfModelPattern.substring(0,
					endIndex));
		}
		return returnValue;
	}
	...
	public int getIsNonFamilyHousehold() {
		...
		if (hhType == HouseholdType.NON_FAMILY_MALE_ALONE.ordinal())
			return (1);
		if (hhType == HouseholdType.NON_FAMILY_MALE_NOT_ALONE.ordinal())
			return (1);
		if (hhType == HouseholdType.NON_FAMILY_FEMALE_ALONE.ordinal())
			return (1);
		if (hhType == HouseholdType.NON_FAMILY_FEMALE_NOT_ALONE.ordinal())
			return (1);
	...
	public int getNumberOfPreDrivingWithNonHomeActivity() {
		...
		for (int i = 1; i < persons.length; ++i) {
			...
			if (persons[i].getPersonIsStudentDriving() == 1) {
				...
				if (!persons[i].getCdapActivity().equalsIgnoreCase(
						ModelStructure.HOME_PATTERN))
					count++;
			}
		}
	...
	public int getNumberOfDrivingAgedStudentsWithDcPurposeIndex(int segmentIndex) {
		...
		for (int j = 1; j < persons.length; j++) {
			if (persons[j].getPersonIsStudentDriving() == 1
					&& persons[j].getSchoolLocationSegmentIndex() == segmentIndex)
				num++;
		}
		return num;
	}

	public int getNumberOfNonDrivingAgedStudentsWithDcPurposeIndex(
			int segmentIndex) {
		int num = 0;
		for (int j = 1; j < persons.length; j++) {
	...
	public int getNumPersons6to15() {
		...
		for (int i = 1; i < persons.length; i++) {
			if (persons[i].getAge() >= 6 && persons[i].getAge() <= 15)
				numPersons6to15++;
		}
	...
	public int getNumPersons65to79() {
		...
		for (int i = 1; i < persons.length; i++) {
			if (persons[i].getAge() >= 65 && persons[i].getAge() <= 79)
				numPersons65to79++;
		}
	...
	public void calculateTimeWindowOverlaps() {
		...
		for (int p = 1; p < persons.length; p++) {
			...
			for (int q = 1; q < persons.length; q++) {
				...
				int maxWindow = persons[p]
						.getMaximumContinuousPairwiseAvailableWindow(persons[q]
								.getTimeWindows());
	...
	public boolean[] getAvailableJointTourTimeWindows(Tour t, int[] altStarts,
			int[] altEnds) {
		int[] participatingPersonIndices = t.getPersonNumArray();
		...
		if (jointTours != null) {
			for (Tour t : jointTours) {
				for (int i : t.getPersonNumArray())
					persons[i].scheduleWindow(t.getTourDepartPeriod(),
							t.getTourArrivePeriod());
				t.clearStopModelResults();
			}
		}
	...
	public void logHouseholdObject(String titleString, Logger logger) {
		...
		Household.logHelper(logger, "debugChoiceModels: ",
				debugChoiceModels ? "True" : "False", totalChars);
		Household.logHelper(logger, "hhIncome: ", hhIncome, totalChars);
		Household.logHelper(logger, "hhIncomeInDollars: ", hhIncomeInDollars,
				totalChars);
		...
		Household.logHelper(logger, "homeWalkSubzone: ", homeWalkSubzone,
				totalChars);
		Household.logHelper(logger, "aoModelAutos: ", aoModelAutos, totalChars);
		Household.logHelper(logger, "cdapModelPattern: ", cdapModelPattern,
				totalChars);
		Household.logHelper(logger, "imtfModelPattern: ", imtfModelPattern,
				totalChars);
		Household.logHelper(logger, "jtfModelPattern: ", jtfModelPattern,
				totalChars);
		...
		Household.logHelper(logger, "aoRandomCount: ", aoRandomCount,
				totalChars);
		Household.logHelper(logger, "tpRandomCount: ", tpRandomCount,
				totalChars);
		Household.logHelper(logger, "fpRandomCount: ", fpRandomCount,
				totalChars);
		Household.logHelper(logger, "ieRandomCount: ", ieRandomCount,
				totalChars);
		Household.logHelper(logger, "cdapRandomCount: ", cdapRandomCount,
				totalChars);
		Household.logHelper(logger, "imtfRandomCount: ", imtfRandomCount,
				totalChars);
		Household.logHelper(logger, "imtodRandomCount: ", imtodRandomCount,
				totalChars);
		Household.logHelper(logger, "awfRandomCount: ", awfRandomCount,
				totalChars);
		Household.logHelper(logger, "awlRandomCount: ", awlRandomCount,
				totalChars);
		Household.logHelper(logger, "awtodRandomCount: ", awtodRandomCount,
				totalChars);
		Household.logHelper(logger, "jtfRandomCount: ", jtfRandomCount,
				totalChars);
		Household.logHelper(logger, "jtlRandomCount: ", jtlRandomCount,
				totalChars);
		Household.logHelper(logger, "jtodRandomCount: ", jtodRandomCount,
				totalChars);
		Household.logHelper(logger, "inmtfRandomCount: ", inmtfRandomCount,
				totalChars);
		Household.logHelper(logger, "inmtlRandomCount: ", inmtlRandomCount,
				totalChars);
		Household.logHelper(logger, "inmtodRandomCount: ", inmtodRandomCount,
				totalChars);
		Household.logHelper(logger, "stfRandomCount: ", stfRandomCount,
				totalChars);
		Household.logHelper(logger, "stlRandomCount: ", stlRandomCount,
				totalChars);
		Household.logHelper(logger, "maxAdultOverlaps: ", maxAdultOverlaps,
				totalChars);
		Household.logHelper(logger, "maxChildOverlaps: ", maxChildOverlaps,
				totalChars);
		Household.logHelper(logger, "maxMixedOverlaps: ", maxMixedOverlaps,
				totalChars);
	...
	public void logTourObject(String titleString, Logger logger, Person person,
			Tour tour) {
	...
	public static void logHelper(Logger logger, String label, int value,
			int totalChars) {
		int labelChars = label.length() + 2;
		int remainingChars = totalChars - labelChars - 4;
		String formatString = String.format("     %%%ds %%%dd", label.length(),
				remainingChars);
		String logString = String.format(formatString, label, value);
		logger.info(logString);
	}

	public static void logHelper(Logger logger, String label, String value,
			int totalChars) {
		int labelChars = label.length() + 2;
		int remainingChars = totalChars - labelChars - 4;
		String formatString = String.format("     %%%ds %%%ds", label.length(),
				remainingChars);
		String logString = String.format(formatString, label, value);
		logger.info(logString);
	}
