...
public class HouseholdAutoOwnershipModel implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(HouseholdAutoOwnershipModel.class);
	...
	public HouseholdAutoOwnershipModel(HashMap<String, String> rbMap,
		...
		int modelPage = Util.getIntegerValueFromPropertyMap(rbMap,
				AO_MODEL_SHEET_TARGET);
		...
		aoModel = new ChoiceModelApplication(autoOwnershipUecFile, modelPage,
				dataPage, rbMap, (VariableTable) aoDmuObject);
	...
	public void applyModel(Household hhObj, boolean preAutoOwnership) {
		...
		aoDmuObject.setDmuIndexValues(hhObj.getHhId(), hhObj.getHhMgra(),
				hhObj.getHhMgra(), 0);

		// set the non-mandatory accessibility values for the home MGRA.
		// values used by both pre-ao and ao models.
		aoDmuObject.setHomeTazAutoAccessibility(accTable
				.getAggregateAccessibility("auto", hhObj.getHhMgra()));
		aoDmuObject.setHomeTazTransitAccessibility(accTable
				.getAggregateAccessibility("transit", hhObj.getHhMgra()));
		aoDmuObject.setHomeTazNonMotorizedAccessibility(accTable
				.getAggregateAccessibility("nonmotor", hhObj.getHhMgra()));
			...
			for (int i = 1; i < persons.length; i++) {
				...
				if (persons[i].getPersonIsWorker() == 1) {
					...
					if (workMgra != ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR) {
						...
						double nmFactor = 0.5 * (Math
								.min(Math.max(
										persons[i].getWorkLocationDistance(),
										1.0), 3.0)) - 0.5;
						...
						double[] workerAccessibilities = mandAcc
								.calculateWorkerMandatoryAccessibilities(
										hhObj.getHhMgra(), workMgra);
				...
				if (persons[i].getPersonIsUniversityStudent() == 1
					...
					if (schoolMgra != ModelStructure.NOT_ENROLLED_SEGMENT_INDEX) {
						...
						double[] studentAccessibilities = mandAcc
								.calculateStudentMandatoryAccessibilities(
										hhObj.getHhMgra(), schoolMgra);
		...
		if (aoModel.getAvailabilityCount() > 0) {
			try {
				...
				logger.error(String
						.format("exception caught for HHID=%d in choiceModelApplication.",
								hhObj.getHhId()));
			}
		} else {
			logger.error(String
		...
		if (hhObj.getDebugChoiceModels() || chosenAlt < 0) {
			...
			String loggerString = (preAutoOwnership ? "Pre-AO without"
					: "AO with")
					+ " accessibilities, Household "
					+ hhObj.getHhId() + " Object";
			...
			for (int k = 0; k < aoModel.getNumberOfAlternatives(); k++) {
				...
				aoLogger.info(String.format("%-20s%18.6e%18.6e%18.6e", k
						+ " autos", utilities[k], probabilities[k], cumProb));
			}

			aoLogger.info(" ");
			aoLogger.info(String.format(
					"Choice: %s, with rn=%.8f, randomCount=%d", chosenAlt, rn,
					randomCount));
			...
			aoModel.logUECResults(aoLogger, String.format(
					"Household Auto Ownership Choice, HH_%d", hhObj.getHhId()));
		}

		if (preAutoOwnership)
			hhObj.setPreAoRandomCount(hhObj.getHhRandomCount());
		else
