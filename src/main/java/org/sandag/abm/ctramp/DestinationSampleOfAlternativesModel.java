...
public class DestinationSampleOfAlternativesModel implements Serializable {
	...
	public DestinationSampleOfAlternativesModel(String soaUecFile,
			int sampleSize, HashMap<String, String> propertyMap,
			MgraDataManager mgraManager, double[][] dcSizeArray,
			DcSoaDMU dcSoaDmuObject, int[] soaUecIndices) {
		...
		// objects
		// for each purpose
		setupSampleOfAlternativesChoiceModelArrays(propertyMap, dcSizeArray,
				dcSoaDmuObject, soaUecIndices);

	}

	private void setupSampleOfAlternativesChoiceModelArrays(
			HashMap<String, String> propertyMap, double[][] dcSizeArray,
			DcSoaDMU dcSoaDmuObject, int[] soaUecIndices) {
		...
		for (int uecIndex : soaUecIndices) {
			...
			if (!modelIndexMap.containsKey(uecIndex)) {
				...
				dcSoaModelIndices[sizeSegmentIndex] = modelIndexMap
						.get(uecIndex);
			}
		...
		for (int uecIndex : modelIndexMap.keySet()) {
			...
			try {
				...
				choiceModel[modelIndex] = new ChoiceModelApplication(
						dcSoaUecFileName, uecIndex, DC_SOA_DATA_SHEET,
						propertyMap, (VariableTable) dcSoaDmuObject);
				i++;
	...
	public void computeDestinationSampleOfAlternatives(DcSoaDMU dcSoaDmuObject,
			Tour tour, Person person, String segmentName, int segmentIndex,
			int origMgra) {
		...
		if (tour != null
				&& tour.getTourCategory() == ModelStructure.AT_WORK_CATEGORY) {
			...
			if (subtourProbabilitiesCache[segmentIndex] == null) {
				computeSampleOfAlternativesChoiceProbabilities(dcSoaDmuObject,
						tour, person, segmentName, segmentIndex, origMgra);
		...
		for (int i = 0; i < sampleSize; i++) {
			...
			if (tour != null
					&& tour.getTourCategory() == ModelStructure.AT_WORK_CATEGORY)
				chosenAlt = Util.binarySearchDouble(
						subtourCumProbabilitiesCache[segmentIndex], rn) + 1;
			else
				chosenAlt = Util.binarySearchDouble(
						cumProbabilitiesCache[segmentIndex], rn) + 1;
				...
				choiceModel[modelIndex]
						.logSelectionInfo(
								String.format(
										...
										segmentName, segmentIndex, modelIndex,
										origMgra), String.format(
										"HHID=%d, rn=%.8f, rnCount=%d",
										hhObj.getHhId(), rn, (rnCount + i)),
								rn, chosenAlt);
			}
	 ...
	 */
	private void computeSampleOfAlternativesChoiceProbabilities(
			DcSoaDMU dcSoaDmuObject, Tour tour, Person person,
			String segmentName, int segmentIndex, int origMgra) {
		...
		dcSoaDmuObject.setDmuIndexValues(hhObj.getHhId(), hhObj.getHhMgra(),
				origMgra, 0);
		...
		if (hhObj.getDebugChoiceModels()) {
			...
			if (tour == null) {
				...
				choiceModelDescription = String
						.format("Usual Location Sample of Alternatives Choice Model for: Segment=%s",
								segmentName);
				...
				choiceModelDescription = String.format(
						"Destination Choice Model for: Segment=%s, TourId=%d",
						segmentName, tour.getTourId());
				decisionMakerLabel = String.format(
						"HH=%d, PersonNum=%d, PersonType=%s", person
								.getHouseholdObject().getHhId(), person
								.getPersonNum(), person.getPersonType());
		...
		try {
			choiceModel[choiceModelIndex].computeUtilities(dcSoaDmuObject,
					dcSoaDmuObject.getDmuIndexValues(),
		...
		if (choiceModel[choiceModelIndex].getAvailabilityCount() == 0) {
			...
			choiceModel[choiceModelIndex].logUECResultsSpecificAlts(
					dcSoaLogger, "debugging", debugAlts);
		}
		...
		if (tour != null
			...
			if (hhObj.getDebugChoiceModels()) {
				...
				try {
					...
					out.println("choiceModelDescription:"
					...
					for (int i = 0; i < probabilitiesCache[segmentIndex].length; i++) {
						out.println((i + 1) + ","
								+ probabilitiesCache[segmentIndex][i]);
					}
				} catch (IOException e) {
		...
		if (hhObj.getDebugChoiceModels() && ALLOW_DEBUG) {
			...
			choiceModel[choiceModelIndex].logAlternativesInfo(String.format(
					"%s Sample Of Alternatives Choice for origTaz=%d",
					segmentName, origMgra), String.format("HHID=%d",
					hhObj.getHhId()), dcSoaLogger);
			choiceModel[choiceModelIndex].logUECResultsSpecificAlts(
					dcSoaLogger, choiceModelDescription + ", "
			...
			for (int i = 1; i < altsToLog.length; i++)
				dcSoaLogger
						.info("alt=" + (altsToLog[i] + 1) + ", util="
								+ utils[altsToLog[i]] + ", prob="
								+ probs[altsToLog[i]]);
