...
/**
 * This class will be used for determining the trip departure time for outbound stops, trip arrival time for inbound stops, location of stops, and
 * trip mode for trips between stops on individual mandatory, individual non-mandatory and joint tours.
 * 
 * @author Jim Hicks
 * @version Oct 2010
 */
        ...
        implements Serializable
{
            ...
            CtrampDmuFactoryIf dmuFactory)
    {
        ...
        // decalre the arrays for storing stop location choice ik and kj segment mode choice probability arrays
        mcCumProbsSegmentIk = new double[sampleSize + 1][];
        mcCumProbsSegmentKj = new double[sampleSize + 1][];

        // decalre the arrays for storing stop location choice ik and kj segment best tap pair arrays
    ...
    private double[] getLnSlcSizeTermsForStopPurpose(int stopPurpose, Household hh)
    {
        ...
        // the procedure to generate availabilty for transit tours overwrites the arrays used by the UECs,
    ...
    private void setupStopLocationChoiceAlternativeArrays(int numAltsInSample, Stop s)
    {
        ...
        for (int i = 1; i <= numAltsInSample; i++)
        {
            ...
            // for walk-transit tours - if half-tour direction is outbound and stop alternative is in the walk shed, walk and walk-transit should be
            // allowed for ik segments
            ...
            // for drive-transit tours - if half-tour direction is outbound and stop alternative is in the drive shed, auto should be allowed for ik
            // segments
            ...
            Household household, int[] loggingSample)
    {
        ...
        {
            // // if inbound, final half-tour destination is the tour origin
            // anm.getDistancesToMgra( tour.getTourOrigMgra(), distanceToFinalDestFromAllMgras, modelStructure.getTourModeIsSovOrHov(
            // tour.getTourModeChoice() ) );
            // stopLocDmuObj.setDistancesToDestMgra( distanceToFinalDestFromAllMgras );
            //
            // // set the distance from the stop origin to the final half-tour destination
        ...
        {
            // // if outbound, final half-tour destination is the tour destination
            // anm.getDistancesToMgra( tour.getTourDestMgra(), distanceToFinalDestFromAllMgras, modelStructure.getTourModeIsSovOrHov(
            // tour.getTourModeChoice() ) );
            // stopLocDmuObj.setDistancesToDestMgra( distanceToFinalDestFromAllMgras );
            //
            // // set the distance from the stop origin to the final half-tour destination
            ...
            Household household, int[] loggingSample)
    {
        ...
        // stopLocDmuObj.setDistancesFromOrigMgra( distanceFromStopOrigToAllMgras );
        //
        // // if tour mode is transit, set availablity of location alternatives based on transit accessibility relative to best transit TAP pair for
        // tour
        // if ( modelStructure.getTourModeIsTransit( tour.getTourModeChoice() ) ) {
        // int numAvailableAlternatives = setSoaAvailabilityForTransitTour(s, tour);
        // if ( numAvailableAlternatives == 0 ) {
        ...
        // if the sample was determined for a transit tour, the sample and availability arrays for the full set of SOA alternatives need to be
        // restored.
        if (modelStructure.getTourModeIsTransit(tour.getTourModeChoice()))
        {
            for (int i = 0; i < soaSample.length; i++)
    ...
    /**
     * determine if each indexed mgra has transit access to the best tap pairs for the tour create an array with 1 if the mgra indexed has at least
     * one TAP within walk egress distance of the mgra or zero if no walk TAPS exist for the mgra.
     */
    private int setSoaAvailabilityForTransitTour(Stop s, Tour t)
    {
        ...
        if (s.isInboundStop())
        {
            ...
            for (int alt : mgras)
            {
                ...
                for (int[] tapPair : bestTaps)
                {
                    ...
                    {
                        // if alternative location mgra is accessible by walk to any of the best inbound boarding taps, AND it's accessible by walk to
                        // the stop origin, it's available.
                        ...
                        // if alternative location mgra is accessible by walk to any of the best inbound alighting taps, AND it's accessible by walk
                        // to the tour origin, it's available.
                    ...
                    {
                        // if alternative location mgra is accessible by walk to any of the best origin taps, AND it's accessible by walk to the stop
                        // origin, it's available.
                        if (mgraManager.getTapIsWalkAccessibleFromMgra(alt, tapPair[0])
                                && mgraManager.getMgrasAreWithinWalkDistance(s.getOrig(), alt)
                                && earlierTripWasLocatedInAlightingTapShed == false)
                        {
        ...
        } else
        {
            ...
            for (int alt : mgras)
            {
                ...
                // check whether any of the outbound dtw boarding taps or best wtw alighting taps are in the set of walk accessible TAPs for the
                // alternative mgra.
                ...
                for (int[] tapPair : bestTaps)
                {
                    ...
                    if (modelStructure.getTourModeIsWalkTransit(t.getTourModeChoice()))
                    {
                        ...
                        // if alternative location mgra is accessible by walk to any of the best destination taps, AND it's accessible by walk to the
                        // tour primary destination, it's available.
                        else if (mgraManager.getTapIsWalkAccessibleFromMgra(alt, tapPair[1])
                                && mgraManager.getMgrasAreWithinWalkDistance(alt,
                                        t.getTourDestMgra()))
                        {
    ...
    /**
     * create an array with 1 if the mgra indexed has at least one TAP within walk egress distance of the mgra or zero if no walk TAPS exist for the
     * mgra. private void createWalkTransitAvailableArray() {
     * 
     * ArrayList<Integer> mgras = mgraManager.getMgras(); int maxMgra = mgraManager.getMaxMgra();
     * 
     * walkTransitAvailable = new int[maxMgra+1];
     * 
     * // loop through mgras and determine if they have walk egress for (int alt : mgras) {
     * 
     * // get the TAP set within walk egress distance of the stop location alternative. int[] aMgraSet =
     * mgraManager.getMgraWlkTapsDistArray()[alt][0];
     * 
     * // set to 1 if the list of TAPS with walk accessible egress to alt is not empty; 0 otherwise if ( aMgraSet != null && aMgraSet.length > 0 )
    ...
    /**
     * Do a monte carlo selection from the array of stored mode choice cumulative probabilities (0 based array). The probabilities were saved at the
     * time the stop location alternative segment mode choice logsums were calculated. If the stop is not the last stop for the half-tour, the IK
     * segment probabilities are passed in. If the stop is the last stop, the KJ probabilities are passeed in.
     * 
     * @param household
     *            object frim which to get the Random object.
     * @param props
     *            is the array of stored mode choice probabilities - 0s based array.
     * 
    ...
    /**
     * This method is taken from the setupStopLocationChoiceAlternativeArrays(), except that the stop location choice dmu attributes are not set and
     * the logsum calculation setup is done only for the selected stop location alternative.
     * 
     * @param stop
                ...
                } else
                {
                    ...
                    } else
                    {
                        ...
                        while (validTripDepartPeriodSet == false)
                        {
                            ...
                            // check that this stop depart time departs at same time or later than the stop object preceding this one in the stop
                            // sequence.
                ...
                } else
                {
                    ...
                    } else
                    {
                        ...
                        while (validTripArrivePeriodSet == false)
                        {
                            ...
                            // check that this stop arrival time arrives at same time or earlier than the stop object following this one in the stop
                            // sequence.
