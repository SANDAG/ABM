...
public class IntermediateStopChoiceModels implements Serializable {
	...
	private transient Logger logger = Logger
			.getLogger(IntermediateStopChoiceModels.class);
	...
	private transient Logger tripDepartLogger = Logger
			.getLogger("tripDepartLog");
	...
	public static final int[] MC_PURPOSE_SHEET_INDICES = { -1, WORK_SHEET,
			UNIVERSITY_SHEET, SCHOOL_SHEET, MAINTENANCE_SHEET,
			MAINTENANCE_SHEET, MAINTENANCE_SHEET, DISCRETIONARY_SHEET,
			DISCRETIONARY_SHEET, DISCRETIONARY_SHEET, SUBTOUR_SHEET };
	...
	public static final int[] PURPOSE_CATEGORIES = { -1, WORK_CATEGORY,
			UNIVERSITY_CATEGORY, SCHOOL_CATEGORY, MAINTENANCE_CATEGORY,
	...
	public static final String[] SLC_SIZE_SEGMENT_NAMES = { "work", "univ",
			"escort_0", "escort_ps", "escort_gs", "escort_hs", "escort_ps_gs",
			"escort_ps_hs", "escort_gs_hs", "escort_ps_gs_hs", "shop", "maint",
			"eatout", "visit", "discr" };
	...
	private int availAltsToLog = 55; // set larger than number of mode alts to
	...
	private void setupStopLocationChoiceModels(
		...
		slcSoaModel[OTHER_STOP_LOC_SOA_SHEET_INDEX] = new ChoiceModelApplication(
				slcSoaUecFile, OTHER_STOP_LOC_SOA_SHEET_INDEX, 0, propertyMap,
				(VariableTable) stopLocDmuObj);
		slcSoaModel[WALK_STOP_LOC_SOA_SHEET_INDEX] = new ChoiceModelApplication(
				slcSoaUecFile, WALK_STOP_LOC_SOA_SHEET_INDEX, 0, propertyMap,
				(VariableTable) stopLocDmuObj);
		slcSoaModel[BIKE_STOP_LOC_SOA_SHEET_INDEX] = new ChoiceModelApplication(
				slcSoaUecFile, BIKE_STOP_LOC_SOA_SHEET_INDEX, 0, propertyMap,
				(VariableTable) stopLocDmuObj);
		...
		for (int k = 0; k < SLC_SIZE_SEGMENT_INDICES.length; k++) {
			...
			sizeSegmentNameIndexMap.put(SLC_SIZE_SEGMENT_NAMES[k],
					SLC_SIZE_SEGMENT_INDICES[k]);
		}
		...
		mcDmuObject.setParkingCostInfo(mgraParkArea, lsWgtAvgCostM,
				lsWgtAvgCostD, lsWgtAvgCostH);

		mcModelArray = new ChoiceModelApplication[5 + 1];
		mcModelArray[WORK_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				WORK_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
		mcModelArray[UNIVERSITY_CATEGORY] = new ChoiceModelApplication(
				mcUecFile, UNIVERSITY_SHEET, 0, propertyMap,
				(VariableTable) mcDmuObject);
		mcModelArray[SCHOOL_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				SCHOOL_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
		mcModelArray[MAINTENANCE_CATEGORY] = new ChoiceModelApplication(
				mcUecFile, MAINTENANCE_SHEET, 0, propertyMap,
				(VariableTable) mcDmuObject);
		mcModelArray[DISCRETIONARY_CATEGORY] = new ChoiceModelApplication(
				mcUecFile, DISCRETIONARY_SHEET, 0, propertyMap,
				(VariableTable) mcDmuObject);
		mcModelArray[SUBTOUR_CATEGORY] = new ChoiceModelApplication(mcUecFile,
				SUBTOUR_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);

		// set up the stop location choice model object
		int slcDataPage = Integer.parseInt(propertyMap
				.get(PROPERTIES_UEC_SLC_DATA_PAGE));
		int slcMandModelPage = Integer.parseInt(propertyMap
				.get(PROPERTIES_UEC_MAND_SLC_MODEL_PAGE));
		...
		slcModelArray[MAND_SLC_MODEL_INDEX] = new ChoiceModelApplication(
				slcUecFile, slcMandModelPage, slcDataPage, propertyMap,
				(VariableTable) stopLocDmuObj);
		slcModelArray[MAINT_SLC_MODEL_INDEX] = new ChoiceModelApplication(
				slcUecFile, slcMaintModelPage, slcDataPage, propertyMap,
				(VariableTable) stopLocDmuObj);
		slcModelArray[DISCR_SLC_MODEL_INDEX] = new ChoiceModelApplication(
				slcUecFile, slcDiscrModelPage, slcDataPage, propertyMap,
				(VariableTable) stopLocDmuObj);

		sampleSize = slcModelArray[MAND_SLC_MODEL_INDEX]
				.getNumberOfAlternatives();
	...
	public void setupSlcDistanceBaseSoaModel(
			HashMap<String, String> propertyMap, double[][] soaExpUtils,
			double[][][] soaSizeProbs, double[][] soaTazSize) {

		String uecPath = propertyMap.get(CtrampApplication.PROPERTIES_UEC_PATH);
		String slcSoaDistanceUecFile = propertyMap
				.get(PROPERTIES_UEC_SLC_SOA_DISTANCE_UTILITY);
		slcSoaDistanceUecFile = uecPath + slcSoaDistanceUecFile;

		dcTwoStageModelObject = new DestChoiceTwoStageModel(propertyMap,
				sampleSize);
		dcTwoStageModelObject.setSlcSoaProbsAndUtils(soaExpUtils, soaSizeProbs,
				soaTazSize);
	...
	private void setupTripDepartTimeModel(HashMap<String, String> propertyMap,
		...
		stopTodModel = new StopDepartArrivePeriodModel(propertyMap,
				modelStructure);
	}
	...
	private void setupParkingLocationModel(HashMap<String, String> propertyMap,
		...
		// locate the UEC
		String uecPath = propertyMap.get(CtrampApplication.PROPERTIES_UEC_PATH);
		String plcUecFile = propertyMap
				.get(PROPERTIES_UEC_PARKING_LOCATION_CHOICE);
		plcUecFile = uecPath + plcUecFile;

		int plcDataPage = Integer.parseInt(propertyMap
				.get(PROPERTIES_UEC_PLC_DATA_PAGE));
		int plcModelPage = Integer.parseInt(propertyMap
				.get(PROPERTIES_UEC_PLC_MODEL_PAGE));
		...
		plcModel = new ChoiceModelApplication(plcUecFile, plcModelPage,
				plcDataPage, propertyMap, (VariableTable) parkingChoiceDmuObj);

		// read the parking choice alternatives data file to get alternatives
		// names
		String plcAltsFile = propertyMap
				.get(PROPERTIES_UEC_PARKING_LOCATION_CHOICE_ALTERNATIVES);
		...
		try {
			...
			logger.error(
					"problem reading table of cbd zones for parking location choice model.",
					e);
			System.exit(1);
		}
	...
	private double[][] calculateLnSlcSizeTerms(HashMap<String, String> rbMap,
			int logMgra) {
		...
		int slcSizeUecData = Integer.parseInt(rbMap
				.get(PROPERTIES_UEC_STOP_LOCATION_SIZE_DATA));
		int slcSizeUecModel = Integer.parseInt(rbMap
				.get(PROPERTIES_UEC_STOP_LOCATION_SIZE_MODEL));

		IndexValues iv = new IndexValues();
		UtilityExpressionCalculator slcSizeUec = new UtilityExpressionCalculator(
				new File(slcSizeUecFile), slcSizeUecModel, slcSizeUecData,
				rbMap, null);
		...
		for (int mgra : mgras) {
			...
			if (mgra == logMgra)
				slcSizeUec.logAnswersArray(slcSoaLogger,
						"Stop Location SOA Size Terms, MGRA = " + mgra);
	...
	private double[] getLnSlcSizeTermsForStopPurpose(int stopPurpose,
			Household hh) {
	...
	public void applyModel(Household household, boolean withTiming) {
		...
		if (household.getDebugChoiceModels()) {
			slcLogger
					.info("applying SLC model for hhid=" + household.getHhId());
		}
	...
	private void applyForOutboundStops(Tour tour, Person person,
			Household household) {
	...
	private void applyForOutboundStopsWithTiming(Tour tour, Person person,
			Household household) {
		...
		applySlcModelWithTiming(household, person, tour, stops, origMgra,
				destMgra, false);

	}

	private void applyForInboundStops(Tour tour, Person person,
			Household household) {
	...
	private void applyForInboundStopsWithTiming(Tour tour, Person person,
			Household household) {
	...
	private void applySlcModel(Household household, Person person, Tour tour,
		...
		if (stops != null) {
			...
			for (int i = 0; i < stops.length; i++) {
				...
				stopLocDmuObj.setDmuIndexValues(household.getHhId(),
						household.getHhMgra(), newOrig, destMgra);
				...
				if (i < stops.length - 1) {
					...
					try {
						...
						if (household.getDebugChoiceModels()) {
							smcLogger
									.info("Monte Carlo selection for determining Mode Choice from IK Probabilities for "
											+ (stop.isInboundStop() ? "INBOUND"
													: "OUTBOUND") + " stop.");
							smcLogger.info("HHID=" + household.getHhId()
									+ ", persNum=" + person.getPersonNum()
									+ ", tourPurpose="
									+ tour.getTourPrimaryPurpose()
									+ ", tourId=" + tour.getTourId()
									+ ", tourMode=" + tour.getTourModeChoice());
							smcLogger.info("StopID=" + (stop.getStopId() + 1)
									+ " of " + (stops.length - 1)
									+ " stops, inbound=" + stop.isInboundStop()
									+ ", stopPurpose=" + stop.getDestPurpose()
									+ ", stopDepart=" + stop.getStopPeriod()
									+ ", stopOrig=" + stop.getOrig()
						...
						if (modeAlt < 0) {
							logger.info("error getting trip mode choice for IK proportions, i="
									+ i);
							logger.info("HHID=" + household.getHhId()
									+ ", persNum=" + person.getPersonNum()
									+ ", tourPurpose="
									+ tour.getTourPrimaryPurpose()
									+ ", tourId=" + tour.getTourId()
									+ ", tourMode=" + tour.getTourModeChoice());
							logger.info("StopID=" + (stop.getStopId() + 1)
									+ " of " + (stops.length - 1)
									+ " stops, inbound=" + stop.isInboundStop()
									+ ", stopPurpose=" + stop.getDestPurpose()
									+ ", stopDepart=" + stop.getStopPeriod()
					...
					if (modelStructure.getTripModeIsWalkTransit(modeAlt)
						...
						if (segmentIkBestTapPairs[selectedIndex] == null) {
							...
							int rideMode = modelStructure
									.getRideModeIndexForTripMode(modeAlt);
					...
					if (household.getDebugChoiceModels()) {
						...
						smcLogger.info("StopID=End of "
								+ (stop.isInboundStop() ? "INBOUND"
										: "OUTBOUND")
								+ " half-tour, stopPurpose="
								+ stop.getDestPurpose() + ", stopDepart="
								+ stop.getStopPeriod() + ", stopOrig="
								+ stop.getOrig() + ", stopDest="
								+ stop.getDest());
					}
					...
					if (modeAlt < 0) {
						logger.error("error getting trip mode choice for KJ proportions, i="
								+ i);
						logger.error("HHID=" + household.getHhId()
								+ ", persNum=" + person.getPersonNum()
								+ ", tourPurpose="
								+ tour.getTourPrimaryPurpose() + ", tourId="
								+ tour.getTourId() + ", tourMode="
					...
					if (directionIsInbound) {
						if (tour.getTourCategory().equalsIgnoreCase(
							...
							if (modelStructure.getTripModeIsSovOrHov(modeAlt)) {
								park = selectParkingLocation(household, tour,
										stop);
								stop.setPark(park);
							}
						}
		...
		// for the half-tour with no stops.
			...
			Stop stop = null;
			try {
				stop = tour.createStop(
						modelStructure,
						origStopPurpose,
						destStopPurpose,
						directionIsInbound,
						tour.getTourCategory().equalsIgnoreCase(
								ModelStructure.AT_WORK_CATEGORY));
			} catch (Exception e) {
			...
			if (modeAlt < 0) {
				...
				logger.info("HHID=" + household.getHhId() + ", persNum="
						...
						+ tour.getTourId());
				logger.info("StopID=" + (stop.getStopId() + 1)
						+ " of no stops, inbound=" + stop.isInboundStop()
						+ ", stopPurpose=" + stop.getDestPurpose()
						+ ", stopDepart=" + stop.getStopPeriod()
						+ ", stopOrig=" + stop.getOrig() + ", stopDest="
						+ stop.getDest());
				throw new RuntimeException();
			}
	...
	private void applySlcModelWithTiming(Household household, Person person,
			Tour tour, Stop[] stops, int origMgra, int destMgra,
			boolean directionIsInbound) {
			...
			for (int i = 0; i < stops.length; i++) {
				...
				if (i < stops.length - 1) {
					...
					try {
						...
						logger.error(String
								.format("Exception caught processing %s stop location choice model.",
										(stopLocDmuObj.getInboundStop() == 1 ? "inbound"
												: "outbound")));
						logger.error("HHID=" + household.getHhId()
								+ ", persNum=" + person.getPersonNum()
								+ ", tour category=" + tour.getTourCategory()
								+ ", tourPurpose="
								+ tour.getTourPrimaryPurpose() + ", tourId="
								+ tour.getTourId() + ", tourMode="
								+ tour.getTourModeChoice());
						logger.error("StopID=" + (stop.getStopId() + 1)
								+ " of " + (stops.length - 1)
								+ " stops, inbound=" + stop.isInboundStop()
								+ ", stopPurpose=" + stop.getDestPurpose()
								+ ", stopDepart=" + stop.getStopPeriod()
						...
						logger.error(String
								.format("origMgra=%d, destMgra=%d, newOrig=%d, lastDest=%d, modeAlt=%d, selectedIndex=%d, choice=%d.",
										origMgra, destMgra, newOrig, lastDest,
										modeAlt, selectedIndex, choice));
						throw new RuntimeException(e);
					}
	...
	private int selectDestination(Stop s) {
		...
		if (household.getDebugChoiceModels()) {
			household.logHouseholdObject(
					"Pre Stop Location Choice for trip: HH_"
							+ household.getHhId() + ", Pers_"
							+ tour.getPersonObject().getPersonNum()
							+ ", Tour Purpose_" + tour.getTourPurpose()
							+ ", Tour_" + tour.getTourId() + ", Tour Purpose_"
							+ tour.getTourPurpose() + ", Stop_"
							+ (s.getStopId() + 1), modelLogger);
			household.logPersonObject("Pre Stop Location Choice for person "
					+ tour.getPersonObject().getPersonNum(), modelLogger,
					tour.getPersonObject());
			household.logTourObject(
					"Pre Stop Location Choice for tour " + tour.getTourId(),
					modelLogger, tour.getPersonObject(), tour);
			household.logStopObject(
					"Pre Stop Location Choice for stop " + (s.getStopId() + 1),
		...
		stopLocDmuObj.setLogSize(getLnSlcSizeTermsForStopPurpose(
				s.getStopPurposeIndex(), household));
		...
		if (s.isInboundStop()) {
			...
			anm.getDistancesToMgra(tour.getTourOrigMgra(),
					distanceToFinalDestFromAllMgras, modelStructure
							.getTourModeIsSovOrHov(tour.getTourModeChoice()));
			stopLocDmuObj
					.setDistancesToDestMgra(distanceToFinalDestFromAllMgras);

			// set the distance from the stop origin to the final half-tour
			// destination
			stopLocDmuObj
					.setOrigDestDistance(distanceFromStopOrigToAllMgras[tour
							.getTourOrigMgra()]);
		} else {
		...
		if (useNewSoaMethod) {
			if (modelStructure.getTourModeIsSovOrHov(tour.getTourModeChoice()))
				selectSampleOfAlternativesAutoTourNew(s, tour, person,
						household, loggingSample);
			else
				selectSampleOfAlternativesOther(s, tour, person, household,
						loggingSample);

			numAltsInSample = dcTwoStageModelObject
					.getNumberofUniqueMgrasInSample();
		} else {
			if (modelStructure.getTourModeIsSovOrHov(tour.getTourModeChoice()))
				selectSampleOfAlternativesAutoTour(s, tour, person, household,
						loggingSample);
			else
				selectSampleOfAlternativesOther(s, tour, person, household,
						loggingSample);
		...
		int slcModelIndex = -1;
		if (tour.getTourCategory().equalsIgnoreCase(
				ModelStructure.MANDATORY_CATEGORY))
		...
		slcModelArray[slcModelIndex]
				.computeUtilities(stopLocDmuObj,
						stopLocDmuObj.getDmuIndexValues(), sampleAvailability,
						inSample);
		...
		if (household.getDebugChoiceModels() || chosen < 0) {
			...
			if (chosen < 0) {
				...
				modelLogger
						.error(String
								.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopId=%d, StopOrig=%d",
										...
										tour.getTourPurpose(),
										tour.getTourModeChoice(),
										tour.getTourId(), s.getDestPurpose(),
				...
				double[] utilities = slcModelArray[slcModelIndex]
						.getUtilities();
				double[] probabilities = slcModelArray[slcModelIndex]
						.getProbabilities();

				// availabilities is 1 based.
				boolean[] availabilities = slcModelArray[slcModelIndex]
						.getAvailabilities();

				String personTypeString = person.getPersonType();
				int personNum = person.getPersonNum();

				modelLogger.error("Person num: " + personNum
				...
				for (int j = 1; j <= numAltsInSample; j++) {
					...
					modelLogger.error(String.format(
							"%-21s%15s%18.6e%18.6e%18.6e", altString,
							availabilities[j], utilities[j - 1],
							probabilities[j - 1], cumProb));
				}
				...
				slcModelArray[slcModelIndex].logAlternativesInfo(
						choiceModelDescription, decisionMakerLabel);
				slcModelArray[slcModelIndex].logSelectionInfo(
						choiceModelDescription, decisionMakerLabel, rn, chosen);

				// write UEC calculation results to separate model specific log
				// file
				slcModelArray[slcModelIndex].logUECResults(modelLogger,
						loggingHeader);
				...
				logger.error(String
						.format("Error for HHID=%d, PersonNum=%d, no available %s stop destination choice alternatives to choose from in choiceModelApplication.",
								tour.getHhId(), tour.getPersonObject()
										.getPersonNum(), tour.getTourPurpose()));
				throw new RuntimeException();
			...
			for (int j = 1; j <= numAltsInSample; j++) {
				...
				modelLogger.info(String.format("%-21s%15s%18.6e%18.6e%18.6e",
						altString, availabilities[j], utilities[j - 1],
						probabilities[j - 1], cumProb));
			}

			modelLogger.info(" ");
			String altString = String.format("%-3d %5d", selectedIndex,
					finalSample[selectedIndex]);
	...
	private void setupStopLocationChoiceAlternativeArrays(int numAltsInSample,
			Stop s) {
		...
		int category = PURPOSE_CATEGORIES[s.getTour()
				.getTourPrimaryPurposeIndex()];
		ChoiceModelApplication mcModel = mcModelArray[category];

		int halfTourFinalDest = s.isInboundStop() ? s.getTour()
				.getTourOrigMgra() : s.getTour().getTourDestMgra();
		...
		for (int i = 1; i <= numAltsInSample; i++) {
			...
			mcDmuObject
					.setATazTerminalTime(tazs
							.getDestinationTazTerminalTime(mgraManager
									.getTaz(altMgra)));
			...
			// tours
			if (modelStructure.getTourModeIsDriveTransit(s.getTour()
					.getTourModeChoice())) {
				...
				if (!s.isInboundStop()) {
					...
					if (sampleMgraInBoardingTapShed[altMgra]) {
						logsumHelper
								.setWalkTransitSkimsUnavailable(mcDmuObject);
						logsumHelper.setDriveTransitSkimsUnavailable(
								mcDmuObject, s.isInboundStop());
					}
					...
					if (sampleMgraInAlightingTapShed[altMgra]) {
						logsumHelper
								.setWalkTransitSkimsUnavailable(mcDmuObject);
						logsumHelper.setDtwTripMcDmuAttributesForBestTapPairs(
								mcDmuObject, s.getOrig(), altMgra, s
										.getStopPeriod(), s.getTour()
										.getBestDtwTapPairsOut(), s.getTour()
										.getPersonObject().getHouseholdObject()
					...
					if (sampleMgraInAlightingTapShed[s.getOrig()]
						...
						logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject,
								s.getOrig(), altMgra, s.getStopPeriod(), s
										.getTour().getPersonObject()
										.getHouseholdObject()
										.getDebugChoiceModels());
						logsumHelper.setDriveTransitSkimsUnavailable(
								mcDmuObject, s.isInboundStop());
					...
					if (sampleMgraInAlightingTapShed[altMgra]) {
						logsumHelper
								.setWalkTransitSkimsUnavailable(mcDmuObject);
						logsumHelper.setWtdTripMcDmuAttributesForBestTapPairs(
								mcDmuObject, s.getOrig(), altMgra, s
										.getStopPeriod(), s.getTour()
										.getBestWtdTapPairsIn(), s.getTour()
										.getPersonObject().getHouseholdObject()
										.getDebugChoiceModels());
					}
			...
			else if (modelStructure.getTourModeIsWalkTransit(s.getTour()
					.getTourModeChoice())) {

				logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject,
						s.getOrig(), altMgra, s.getStopPeriod(), s.getTour()
								.getPersonObject().getHouseholdObject()
								.getDebugChoiceModels());

			} else {
				logsumHelper.setWalkTransitSkimsUnavailable(mcDmuObject);
				logsumHelper.setDriveTransitSkimsUnavailable(mcDmuObject,
						s.isInboundStop());
			}
			ikSegment = logsumHelper
					.calculateTripMcLogsum(s.getOrig(), altMgra,
			...
			if (ikSegment < -900) {
							...
							.error(String
									.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumIBStops=%d, StopOrig=%d, AltStopLoc=%d",
											s.getTour().getPersonObject()
													...
													.getPersonObject()
													.getPersonType(),
											s.getTour().getTourPurpose(), s
													.getTour()
													.getTourModeChoice(), s
													.getTour().getTourId(), s
													.getDestPurpose(),
											"inbound", (s.getStopId() + 1), s
													.getTour()
													.getNumInboundStops() - 1,
											s.getOrig(), altMgra));
				else
					slcLogger
							.error(String
									.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumOBStops=%d, StopOrig=%d, AltStopLoc=%d",
											...
											s.getTour().getTourPurpose(), s
													...
													.getTour().getTourId(), s
													.getDestPurpose(),
											"outbound", (s.getStopId() + 1), s
													.getTour()
													.getNumOutboundStops() - 1,
											s.getOrig(), altMgra));
				...
				mcDmuObject.getDmuIndexValues().setHHIndex(
						s.getTour().getPersonObject().getHouseholdObject()
								.getHhId());
				ikSegment = logsumHelper.calculateTripMcLogsum(s.getOrig(),
						altMgra, s.getStopPeriod(), mcModel, mcDmuObject,
						slcLogger);
			...
			mcCumProbsSegmentIk[i] = logsumHelper
					.getStoredSegmentCumulativeProbabilities();
				...
				if (!s.isInboundStop()) {
					...
					if (sampleMgraInBoardingTapShed[altMgra]) {
						logsumHelper
								.setWalkTransitSkimsUnavailable(mcDmuObject);
						logsumHelper.setDtwTripMcDmuAttributesForBestTapPairs(
								mcDmuObject, altMgra, halfTourFinalDest, s
										.getStopPeriod(), s.getTour()
										.getBestDtwTapPairsOut(), s.getTour()
										.getPersonObject().getHouseholdObject()
										.getDebugChoiceModels());
					...
					if (sampleMgraInAlightingTapShed[altMgra]) {
						logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject,
								altMgra, halfTourFinalDest, s.getStopPeriod(),
								s.getTour().getPersonObject()
										.getHouseholdObject()
										.getDebugChoiceModels());
						logsumHelper.setDriveTransitSkimsUnavailable(
			...
			kjSegment = logsumHelper.calculateTripMcLogsum(altMgra,
					halfTourFinalDest, s.getStopPeriod(), mcModel, mcDmuObject,
					slcLogger);
			...
			if (kjSegment < -900) {
				...
				mcDmuObject.getDmuIndexValues().setHHIndex(
						s.getTour().getPersonObject().getHouseholdObject()
								.getHhId());
				kjSegment = logsumHelper.calculateTripMcLogsum(altMgra,
						halfTourFinalDest, s.getStopPeriod(), mcModel,
						mcDmuObject, slcLogger);
				mcDmuObject.getDmuIndexValues().setDebug(false);
			}

			// store the mode choice probabilities for the segment
			mcCumProbsSegmentKj[i] = logsumHelper
					.getStoredSegmentCumulativeProbabilities();
	...
	private void selectSampleOfAlternativesAutoTourNew(Stop s, Tour tour,
			Person person, Household household, int[] loggingSample) {
		...
		// alternate method
		dcTwoStageModelObject.chooseSlcSample(slcOrigTaz, slcDestTaz,
				slcSegment, sampleSize, household.getHhRandom(),
				household.getDebugChoiceModels());
		int[] tempSample = dcTwoStageModelObject.getUniqueSampleMgras();
		double[] tempFactors = dcTwoStageModelObject
				.getUniqueSampleMgraCorrectionFactors();
		int numUniqueAlts = dcTwoStageModelObject
				.getNumberofUniqueMgrasInSample();
	...
	private void selectSampleOfAlternativesAutoTour(Stop s, Tour tour,
			Person person, Household household, int[] loggingSample) {
		...
		if (household.getDebugChoiceModels()) {
			choiceModelDescription = String
					.format("Stop Location SOA Choice Model for: stop purpose=%s, direction=%s, stopId=%d, stopOrig=%d",
							s.getDestPurpose(), s.isInboundStop() ? "inbound"
									: "outbound", (s.getStopId() + 1), s
									.getOrig());
			decisionMakerLabel = String
					.format("HH=%d, persNum=%d, persType=%s, tourId=%d, tourPurpose=%s, tourOrig=%d, tourDest=%d, tourMode=%d",
							...
							tour.getTourModeChoice());
			cm.choiceModelUtilityTraceLoggerHeading(choiceModelDescription,
					decisionMakerLabel);
		}
		...
		if (household.getDebugChoiceModels()) {
			...
			if (loggingSample == null) {
				cm.logUECResultsSpecificAlts(soaLogger, loggingHeader,
						new int[] { 0, s.getOrig(), tour.getTourOrigMgra(),
		...
		for (int i = 0; i < sampleSize; i++) {
			...
			// write choice model alternative info to log file
			if (household.getDebugChoiceModels()) {
				cm.logSelectionInfo(loggingHeader,
						String.format("rnCount=%d", rnCount), rn, chosenAlt);
			}
	...
	private void selectSampleOfAlternativesOther(Stop s, Tour tour,
			Person person, Household household, int[] loggingSample) {
		...
		if (cm.getAvailabilityCount() == 0) {
			...
			household.logHouseholdObject("Stop Location Choice for trip: HH_"
					+ household.getHhId() + ", Pers_"
			...
			household.logPersonObject("Stop Location Choice for person "
					+ tour.getPersonObject().getPersonNum(), badSlcLogger,
					tour.getPersonObject());
			household.logTourObject(
					"Stop Location Choice for tour " + tour.getTourId(),
					badSlcLogger, tour.getPersonObject(), tour);
			household.logStopObject(
					"Stop Location Choice for stop " + (s.getStopId() + 1),
					badSlcLogger, s, modelStructure);

			badSlcLogger.info(decisionMakerLabel
					+ " has no available alternatives for "
					+ choiceModelDescription + ".");
			badSlcLogger
					.info("Logging StopLocation SOA Choice utility calculations for: stopOrig="
							+ s.getOrig()
							+ ", tourOrig="
			...
			for (int j = 0; j < probabilitiesList.length; j++) {
				...
				if (sum != cumProbabilitiesList[j] || sum > 1.0) {
					badSlcLogger.info("error condition found!  sum=" + sum
							+ ", j=" + j + ", cumProbabilitiesList[j]="
							+ cumProbabilitiesList[j]);
	...
	private void setupLogsumCalculation(Stop s) {
		...
		mcDmuObject.setOutboundStops(t.getOutboundStops() == null ? 0 : t
				.getOutboundStops().length - 1);
		mcDmuObject.setInboundStops(t.getInboundStops() == null ? 0 : t
				.getInboundStops().length - 1);

		mcDmuObject.setTripOrigIsTourDest(s.isInboundStop()
				&& s.getStopId() == 0 ? 1 : 0);
		mcDmuObject.setTripDestIsTourDest(!s.isInboundStop()
				&& ((s.getStopId() + 1) == (t.getNumOutboundStops() - 1)) ? 1
				: 0);
		...
		if (s.isInboundStop()) {
			...
			mcDmuObject.setLastTrip(((s.getStopId() + 1) == (t
					.getNumInboundStops() - 1)) ? 1 : 0);
		} else {
		...
		mcDmuObject
				.setTourModeIsDA(modelStructure.getTourModeIsSov(tourMode) ? 1
						: 0);
		mcDmuObject
				.setTourModeIsS2(modelStructure.getTourModeIsS2(tourMode) ? 1
						: 0);
		mcDmuObject
				.setTourModeIsS3(modelStructure.getTourModeIsS3(tourMode) ? 1
						: 0);
		mcDmuObject.setTourModeIsWalk(modelStructure
				.getTourModeIsWalk(tourMode) ? 1 : 0);
		mcDmuObject.setTourModeIsBike(modelStructure
				.getTourModeIsBike(tourMode) ? 1 : 0);
		mcDmuObject.setTourModeIsWTran(modelStructure
				.getTourModeIsWalkLocal(tourMode)
				|| modelStructure.getTourModeIsWalkPremium(tourMode) ? 1 : 0);
		mcDmuObject
				.setTourModeIsPnr(modelStructure.getTourModeIsPnr(tourMode) ? 1
						: 0);
		mcDmuObject
				.setTourModeIsKnr(modelStructure.getTourModeIsKnr(tourMode) ? 1
						: 0);
		mcDmuObject.setTourModeIsSchBus(modelStructure
				.getTourModeIsSchoolBus(tourMode) ? 1 : 0);
		...
		double reimbursePct = mcDmuObject.getPersonObject()
				.getParkingReimbursement();
		...
		if (s.isInboundStop()) {
			...
			for (int alt : mgras) {
				...
				for (int[] tapPair : bestTaps) {
					...
					if (modelStructure.getTourModeIsWalkTransit(t
							.getTourModeChoice())) {
						...
						// available.
						if (mgraManager.getTapIsWalkAccessibleFromMgra(alt,
								tapPair[0])
								&& mgraManager.getMgrasAreWithinWalkDistance(
										s.getOrig(), alt)
								&& earlierTripWasLocatedInAlightingTapShed == false) {
							accessible = true;
						...
						// available.
						else if (mgraManager.getTapIsWalkAccessibleFromMgra(
								alt, tapPair[1])
								&& mgraManager.getMgrasAreWithinWalkDistance(
						...
						else if (mgraManager.getTapIsDriveAccessibleFromMgra(
								alt, tapPair[1])) {
							accessible = true;
							sampleMgraInAlightingTapShed[alt] = true;
						}
			...
			for (int alt : mgras) {
				...
				for (int[] tapPair : bestTaps) {
					...
					if (modelStructure.getTourModeIsWalkTransit(t
						...
						if (mgraManager.getTapIsDriveAccessibleFromMgra(alt,
								tapPair[0])
								&& earlierTripWasLocatedInAlightingTapShed == false) {
							accessible = true;
							sampleMgraInBoardingTapShed[alt] = true;
	...
	private int selectModeFromProbabilities(Stop s, double[] cumProbs) {
		...
		Household household = s.getTour().getPersonObject()
				.getHouseholdObject();
		...
		if (household.getDebugChoiceModels() || selectedModeAlt < 0
			...
			if (numAvailAlts >= availAltsToLog) {
				...
				smcLogger.info("StopID="
						+ (s.getStopId() + 1)
						+ " of "
						+ (s.isInboundStop() ? tour.getNumInboundStops() - 1
								: tour.getNumOutboundStops() - 1)
						+ " stops, inbound=" + s.isInboundStop()
						+ ", stopPurpose=" + s.getDestPurpose()
						+ ", stopDepart=" + s.getStopPeriod() + ", stopOrig="
						+ s.getOrig() + ", stopDest=" + s.getDest());
				availAltsToLog = 9999;
			}
			...
			for (int j = 0; j < cumProbs.length; j++) {
				...
				smcLogger.info(String.format("%-30s%15s%18s%18.6e%18.6e",
						altString, "", "", tempProb, cumProbs[j]));
				sumProb += tempProb;
			}
	...
	private int getHalfTourModeChoice(Stop s) {
		...
		mcDmuObject.setATazTerminalTime(tazs
				.getDestinationTazTerminalTime(mgraManager.getTaz(altMgra)));
		...
		if (hh.getDebugChoiceModels()) {
			smcLogger
					.info("LOGSUM calculation for determining Mode Choice Probabilities for "
							+ (s.isInboundStop() ? "INBOUND" : "OUTBOUND")
							+ " half-tour with no stops.");
			smcLogger.info("HHID=" + hh.getHhId() + ", persNum="
					+ s.getTour().getPersonObject().getPersonNum()
					+ ", tourPurpose=" + s.getTour().getTourPrimaryPurpose()
					+ ", tourId=" + s.getTour().getTourId() + ", tourMode="
					+ s.getTour().getTourModeChoice());
			smcLogger.info("StopID=" + (s.getStopId() + 1) + ", inbound="
		...
		if (modelStructure.getTourModeIsDriveTransit(s.getTour()
			...
			if (s.isInboundStop())
				logsumHelper.setWtdTripMcDmuAttributesForBestTapPairs(
						mcDmuObject, s.getOrig(), altMgra, s.getStopPeriod(), s
								.getTour().getBestWtdTapPairsIn(), s.getTour()
								.getPersonObject().getHouseholdObject()
								.getDebugChoiceModels());
			else
				logsumHelper.setDtwTripMcDmuAttributesForBestTapPairs(
						mcDmuObject, s.getOrig(), altMgra, s.getStopPeriod(), s
								.getTour().getBestDtwTapPairsOut(), s.getTour()
								.getPersonObject().getHouseholdObject()
								.getDebugChoiceModels());
			...
			if (s.isInboundStop())
				logsumHelper.setWtwTripMcDmuAttributesForBestTapPairs(
						mcDmuObject, s.getOrig(), altMgra, s.getStopPeriod(), s
								.getTour().getBestWtwTapPairsIn(), s.getTour()
								.getPersonObject().getHouseholdObject()
								.getDebugChoiceModels());
			else
				logsumHelper.setWtwTripMcDmuAttributesForBestTapPairs(
						mcDmuObject, s.getOrig(), altMgra, s.getStopPeriod(), s
								.getTour().getBestWtwTapPairsOut(), s.getTour()
								.getPersonObject().getHouseholdObject()
								.getDebugChoiceModels());

		}
		double logsum = logsumHelper.calculateTripMcLogsum(s.getOrig(),
				altMgra, s.getStopPeriod(), mcModel, mcDmuObject, smcLogger);
		...
		if (hh.getDebugChoiceModels() || selectedModeAlt < 0
			...
			for (int j = 0; j < utilities.length; j++) {
				...
				String altString = String.format("%-3d %-25s", j + 1,
						altNames[j]);
				smcLogger.info(String.format("%-30s%15s%18.6e%18.6e%18.6e",
			...
			if (logsum < -900 || selectedModeAlt < 0) {
					...
					smcLogger
							.error("ERROR calculating trip mode choice logsum for "
									+ (s.isInboundStop() ? "inbound"
											: "outbound")
									+ " half-tour with no stops - ikLogsum = "
									+ logsum + ".");
				else
					smcLogger
							.error("No half-tour mode choice alternatives available "
									+ (s.isInboundStop() ? "inbound"
											: "outbound")
									+ " half-tour with no stops - ikLogsum = "
									+ logsum + ".");
				...
				mcDmuObject.getDmuIndexValues().setHHIndex(
						s.getTour().getPersonObject().getHouseholdObject()
								.getHhId());
				logsum = logsumHelper.calculateTripMcLogsum(s.getOrig(),
						altMgra, s.getStopPeriod(), mcModel, mcDmuObject,
						smcLogger);
				mcDmuObject.getDmuIndexValues().setDebug(false);
			...
			if (tour.getTourArrivePeriod() == tour.getTourDepartPeriod()) {
				...
				if (tripIndex == 1) {
					...
					if (prevTripPeriod == tour.getTourArrivePeriod()) {
						...
						int tourPrimaryPurposeIndex = tour
								.getTourPrimaryPurposeIndex();

						double[] proportions = stopTodModel
								.getStopTodIntervalProportions(
										tourPrimaryPurposeIndex,
										halfTourDirection, prevTripPeriod,
										tripIndex);
						...
						while (validTripDepartPeriodSet == false) {
							...
							if (choice >= prevTripPeriod
								...
								if (household.getDebugChoiceModels()) {
									...
									for (int p = 1; p < proportions.length; p++) {
										...
										tripDepartLogger.info(String.format(
												"%-4d  %-8s  %10.8f  %10.8f",
												p, timeString, proportions[p],
												cumProb));
									}
									tripDepartLogger.info(loggerSeparator);
									tripDepartLogger.info("rn=" + rn
											+ ", choice=" + choice + ", try="
											+ invalidCount);
									tripDepartLogger.info("");
								}
							...
							if (invalidCount > MAX_INVALID_FIRST_ARRIVAL_COUNT) {
								tripDepartLogger
										.error("Error in Outbound Trip Depart Time Model.");
								tripDepartLogger
								...
								tripDepartLogger
										.error(String
												.format("prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d, last choice=%d",
														...
														tour.getTourArrivePeriod(),
														choice));
								tripDepartLogger.error("=" + invalidCount
										+ " times.");

								System.out
										.println("Error in Outbound Trip Depart Time Model.");
								System.out
	...
	private void setInboundTripDepartTimes(Stop[] stops,
			int lastOutboundTripDeparts) {
				...
				if (tripIndex == 1) {
					...
					if (prevTripPeriod == tour.getTourArrivePeriod()) {
						...
						while (validTripArrivePeriodSet == false) {
							...
							if (invalidCount > MAX_INVALID_FIRST_ARRIVAL_COUNT) {
								tripDepartLogger
										.error("Error in Inbound Trip Arrival Time Model.");
								tripDepartLogger
								...
								System.out
										.println("Error in Inbound Trip Arrival Time Model.");
								System.out
	 ...
	 * @return
	 */
	private int getMonteCarloSelection(double[] probabilities,
			double randomNumber) {
	...
	private int selectParkingLocation(Household household, Tour tour, Stop stop) {
		...
		if (mgraAltLocationIndex.containsKey(stop.getDest()) == false
				|| mgraAltParkArea.get(stop.getDest()) != 1)
			return -1;
		...
		parkingChoiceDmuObj.setDmuIndexValues(household.getHhId(),
				stop.getOrig(), stop.getDest(),
				household.getDebugChoiceModels());

		parkingChoiceDmuObj.setPersonType(tour.getPersonObject()
				.getPersonTypeNumber());
		...
		if (stops.length == 1) {
			activityIntervals = tour.getTourArrivePeriod()
					- tour.getTourDepartPeriod();
		} else {
			...
			if (stopId == stops.length - 1)
				...
				activityIntervals = stops[stopId + 1].getStopPeriod()
						- stop.getStopPeriod();
		}
		...
		parkingChoiceDmuObj.setReimbPct(tour.getPersonObject()
				.getParkingReimbursement());

		int[] sampleIndices = setupParkLocationChoiceAlternativeArrays(
				stop.getOrig(), stop.getDest());
		...
		if (household.getDebugChoiceModels()) {
			household.logHouseholdObject(
					"Pre Parking Location Choice for trip: HH_"
							+ household.getHhId() + ", Pers_"
							+ tour.getPersonObject().getPersonNum()
							+ ", Tour Purpose_" + tour.getTourPurpose()
							+ ", Tour_" + tour.getTourId() + ", Tour Purpose_"
			...
			household.logPersonObject("Pre Parking Location Choice for person "
					+ tour.getPersonObject().getPersonNum(), modelLogger,
					tour.getPersonObject());
			household.logTourObject("Pre Parking Location Choice for tour "
					+ tour.getTourId(), modelLogger, tour.getPersonObject(),
					tour);
			household.logStopObject("Pre Parking Location Choice for stop "
					+ stop.getStopId(), modelLogger, stop, modelStructure);
		}
		...
		if (household.getDebugChoiceModels()) {
			...
			decisionMakerLabel = String
					.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourId=%d, StopPurpose=%s, StopId=%d",
							household.getHhId(), person.getPersonNum(),
							person.getPersonType(), tour.getTourPurpose(),
							tour.getTourId(), tour.getTourPurpose(),
							stop.getStopId());

			modelLogger.info(" ");
			loggerString = choiceModelDescription + " for "
					+ decisionMakerLabel + ".";
			...
			plcModel.choiceModelUtilityTraceLoggerHeading(
					choiceModelDescription, decisionMakerLabel);

		}

		plcModel.computeUtilities(parkingChoiceDmuObj,
				parkingChoiceDmuObj.getDmuIndexValues(), altParkAvail,
				altParkSample);
		...
		if (household.getDebugChoiceModels() || chosen < 0) {
			...
			for (int k = 1; k <= numAltsInSample; k++) {
				...
				String altString = String.format("k=%d, index=%d, altMgra=%d",
						k, index, altMgra);
				modelLogger.info(String.format("%-35s%18.6e%18.6e%18.6e",
						altString, utilities[k - 1], probabilities[k - 1],
						cumProb));
			}

			modelLogger.info(" ");
			if (chosen < 0) {
				modelLogger.info(String
						.format("No Alternatives Available For Choice !!!"));
			} else {
				String altString = String.format(
						"chosen=%d, chosenIndex=%d, chosenMgra=%d", chosen,
						chosenIndex, parkMgra);
				modelLogger.info(String.format(
						"Choice: %s, with rn=%.8f, randomCount=%d", altString,
						rn, randomCount));
			}
			...
			plcModel.logAlternativesInfo(choiceModelDescription,
					decisionMakerLabel);
			plcModel.logSelectionInfo(choiceModelDescription,
	...
	private int[] setupParkLocationChoiceAlternativeArrays(int tripOrigMgra,
			int tripDestMgra) {

		// get the array of mgras within walking distance of the trip
		// destination
		int[] walkMgras = mgraManager
				.getMgrasWithinWalkDistanceTo(tripDestMgra);
		...
		if (walkMgras != null) {
			...
			for (int wMgra : walkMgras) {
				...
				if (mgraAltLocationIndex.containsKey(wMgra)) {
					...
					double curWalkDist = mgraManager.getMgraToMgraWalkDistTo(
							wMgra, tripDestMgra) / 5280.0;
