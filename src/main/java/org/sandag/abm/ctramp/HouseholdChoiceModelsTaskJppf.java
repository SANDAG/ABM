...
public class HouseholdChoiceModelsTaskJppf extends JPPFTask {
	...
	public HouseholdChoiceModelsTaskJppf(int taskIndex, int startIndex,
			int endIndex) {
	...
	public void run() {
		...
		try {
			...
			propertyMap = (HashMap<String, String>) dataProvider
					.getValue("propertyMap");
			ms = (MatrixDataServerIf) dataProvider.getValue("ms");
			hhDataManager = (HouseholdDataManagerIf) dataProvider
					.getValue("hhDataManager");
			modelStructure = (ModelStructure) dataProvider
					.getValue("modelStructure");
			dmuFactory = (CtrampDmuFactoryIf) dataProvider
					.getValue("dmuFactory");
			restartModelString = (String) dataProvider
		...
		HouseholdChoiceModelsManager modelManager = HouseholdChoiceModelsManager
				.getInstance();
		modelManager.managerSetup(ms, hhDataManager, propertyMap,
				restartModelString, modelStructure, dmuFactory);

		HouseholdChoiceModels hhModel = modelManager
				.getHouseholdChoiceModelsObject(taskIndex);
		...
		boolean runDebugHouseholdsOnly = Util.getBooleanValueFromPropertyMap(
				propertyMap, HouseholdDataManager.DEBUG_HHS_ONLY_KEY);
			...
			try {
				...
				logger.fatal(String
						.format("exception caught in taskIndex=%d hhModel index=%d applying hh model for i=%d, hhId=%d.",
								taskIndex, hhModel.getModelIndex(), i,
		...
		logger.info(String
				.format("end of household choice model thread=%s, task[%d], hhModel[%d], startIndex=%d, endIndex=%d",
						threadName, taskIndex, hhModel.getModelIndex(),
						startIndex, endIndex));
		...
		logger.info("task=" + taskIndex + ", setup=" + setup1 + ", getHhs="
				+ (setup2 - setup1) + ", processHhs=" + (setup3 - setup2)
				+ ", putHhs=" + (setup4 - setup3) + ", return model="
				+ (setup5 - setup4) + ".");
		...
		// tasks of this type
		modelManager.returnHouseholdChoiceModelsObject(hhModel, startIndex,
				endIndex);

	}

	private void logModelComponentTimes(long[] componentTimes,
			long[] partialStopTimes, Logger logger, int modelIndex) {

		String[] label1 = { "AO", "FP", "IE", "CDAP", "IMTF", "IMTOD", "IMMC",
				"JTF", "JTDC", "JTTOD", "JTMC", "INMTF", "INMTDCSOA",
				"INMTDCTOT", "INMTTOD", "INMTMC", "AWTF", "AWTDCSOA",
				"AWTDCTOT", "AWTTOD", "AWTMC", "STF", "STDTM" };
		...
		for (int i = 0; i < componentTimes.length; i++) {
			...
			logger.info(String.format("%-6d%30s:%15.1f", (i + 1), label1[i],
					time));
			total += time;
		}
		logger.info(String.format("%-6s%30s:%10.1f", "Total",
				"Total all components", total));
		logger.info("");

		String[] label2 = { "SLC SOA AUTO", "SLC SOA OTHER", "SLC LS",
				"SLC DIST", "SLC", "SLC TOT", "S TOD", "S MC", "TOTAL" };
		...
		for (int i = 0; i < partialStopTimes.length; i++) {
			...
			logger.info(String.format("%-6d%30s:%15.1f", (i + 1), label2[i],
					time));
		}
