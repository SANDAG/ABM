...
public class IndividualNonMandatoryTourFrequencyDMU implements Serializable,
		VariableTable {

	protected transient Logger logger = Logger
			.getLogger(IndividualNonMandatoryTourFrequencyDMU.class);
	...
	public int getWorksAtHome() {
		if (person.getPersonIsWorker() == 1
				&& person.getUsualWorkLocation() == ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR)
			return 1;
		else
			return 0;
	}
	...
	public int getPreDrivingAtHome() {
		...
		for (int i = 1; i < persons.length; i++) {
			if (persons[i].getPersonIsStudentNonDriving() == 1
					&& persons[i].getCdapActivity().equalsIgnoreCase(
