        ...
        implements Serializable
{
    ...
    {
        //logger.info( "beginning of HouseholdChoiceModelsManager() - objInstance address = " + objInstance );
        if (objInstance == null) {
            objInstance = new HouseholdChoiceModelsManager();
            //logger.info( "after new HouseholdChoiceModelsManager() - objInstance address = " + objInstance );
            return objInstance;
        }
        else {
            //logger.info( "returning current HouseholdChoiceModelsManager() - objInstance address = " + objInstance );
            return objInstance;
        }
    }
            ...
            ModelStructure modelStructure, CtrampDmuFactoryIf dmuFactory )
    {
        ...
        // get the DestChoiceModelManager instance and clear the objects that hold large memory references
        ...
        if ( ! aggAcc.getAccessibilitiesAreBuilt() )
        {
            ...
            if ( accFileReadFlag && (new File(accFileName)).canRead() )
            {
                ...
                logger.info("filling Accessibilities Object in HouseholdChoiceModelManager by reading file: " + accFileName + ".");
    ...
    public synchronized HouseholdChoiceModels getHouseholdChoiceModelsObject(int taskIndex)
    {
        ...
        if (modelQueue.isEmpty()) {
            ...
            // calculate array of distanceToExternalCordon logsums by taz for use by internal-external model
            ...
            hhChoiceModels = new HouseholdChoiceModels(++modelIndex, restartModelString, propertyMap, modelStructure, dmuFactory, aggAcc, logsumHelper, mandAcc,
                    ...
                    subTourTazDistProbs, subTourSizeProbs, distanceToCordonsLogsums );
    ...
    public synchronized void clearHhModels()
    {
        ...
        logger.info( String.format( "%s:  clearing household choice models modelQueue, thread=%s.", new Date(), Thread.currentThread().getName()) );
    ...
    private double[] computeTazDistanceToExternalCordonLogsums() {
        ...
        // get parameters used to develop distance to cordon logsums for IE model
        ...
        double[][][] periodDistanceMatrices = tazDistanceCalculator.getStoredFromTazToAllTazsDistanceSkims();
