...
public class HouseholdChoiceModelsManager implements Serializable {
	...
	private static transient Logger logger = Logger
			.getLogger(HouseholdChoiceModelsManager.class);
	...
	public synchronized void managerSetup(MatrixDataServerIf ms,
			HouseholdDataManagerIf hhDataManager,
	 ...
	 * 
	 */
	public synchronized HouseholdChoiceModels getHouseholdChoiceModelsObject(
			int taskIndex) {
		...
		if (modelQueue.isEmpty()) {
			...
			NonTransitUtilities ntUtilities = new NonTransitUtilities(
					propertyMap, sovExpUtilities, hovExpUtilities,
					nMotorExpUtilities);
			...
			hhChoiceModels = new HouseholdChoiceModels(++modelIndex,
					restartModelString, propertyMap, modelStructure,
					dmuFactory, aggAcc, logsumHelper, mandAcc, pctHighIncome,
					pctMultipleAutos, avgtts, transpDist, pctDetour,
					nonMandatoryTazDistProbs, nonMandatorySizeProbs,
					subTourTazDistProbs, subTourSizeProbs,
					distanceToCordonsLogsums);

			message = String.format(
					"created hhChoiceModels=%d, task=%d, thread=%s.",
					modelIndex, taskIndex, Thread.currentThread().getName());

		} else {
			hhChoiceModels = modelQueue.remove();
			message = String
					.format("removed hhChoiceModels=%d from queue, task=%d, thread=%s.",
							hhChoiceModels.getModelIndex(), taskIndex, Thread
	 ...
	 * @param hhModels
	 */
	public void returnHouseholdChoiceModelsObject(
			HouseholdChoiceModels hhModels, int startIndex, int endIndex) {
		modelQueue.add(hhModels);
		completedHouseholds += (endIndex - startIndex + 1);
		logger.info("returned hhChoiceModels=" + hhModels.getModelIndex()
				+ " to queue: thread=" + Thread.currentThread().getName()
	...
	public synchronized void clearHhModels() {
		...
		logger.info(String.format(
				"%s:  clearing household choice models modelQueue, thread=%s.",
	...
	private void readTpChoiceAvgTtsFile() {
		...
		double[] transpDistField = table
				.getColumnAsDouble(TRANSP_DIST_COLUMN_NAME);
		double[] pctDetourField = table
				.getColumnAsDouble(PCT_DETOUR_COLUMN_NAME);
	...
	private void computeNonMandatorySegmentSizeArrays(
		...
		DestChoiceTwoStageSoaProbabilitiesCalculator nonManSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
				...
				PROPERTIES_NON_MANDATORY_DC_SOA_UEC_FILE,
		...
		for (String tourPurpose : tourPurposeNames) {
			...
			int purposeSizeIndex = nonMandatorySizeSegmentNameIndexMap
					.get(tourPurpose);
	...
	private void computeSubtourSegmentSizeArrays(ModelStructure modelStructure,
		...
		for (int k = 0; k < tourPurposeNames.length; k++) {
			...
			sizeSegmentNameIndexMap.put(tourPurposeNames[k],
					sizeSheetIndices[k]);
		}
		...
		DestChoiceTwoStageSoaProbabilitiesCalculator subTourSoaDistProbsObject = new DestChoiceTwoStageSoaProbabilitiesCalculator(
				propertyMap, dmuFactory,
				PROPERTIES_NON_MANDATORY_DC_SOA_UEC_FILE,
				PROPERTIES_ATWORK_DC_SOA_UEC_MODEL_PAGE,
	...
	private double[] computeTazDistanceToExternalCordonLogsums() {
		...
		String altFileName = uecPath
				+ propertyMap.get("internalExternal.dc.uec.alts.file");
