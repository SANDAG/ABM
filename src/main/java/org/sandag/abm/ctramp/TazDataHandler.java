...
public class TazDataHandler implements TazDataIf, Serializable {
	...
	public TazDataHandler(ResourceBundle rb, String projectDirectory) {
		...
		tazDataFileName = projectDirectory
				+ ResourceUtil.getProperty(rb, PROPERTIES_TAZ_DATA);
		...
		zonalAccessibilityFileName = projectDirectory
				+ ResourceUtil.getProperty(rb,
						PROPERTIES_ZONAL_ACCESSIBILITIESS_DATA);
		...
		if (fileName != null) {
			...
			try {
				...
				if (tazPosition <= 0) {
					logger.fatal(String
							.format("expected zone field name=%s was not a field in the walk access file: %s.",
									WALK_PERCENTAGE_FILE_ZONE_FIELD_NAME,
									fileName));
					throw new RuntimeException();
				}
				...
				if (shrtPosition <= 0) {
					logger.fatal(String
							.format("expected short field name=%s was not a field in the walk access file: %s.",
									WALK_PERCENTAGE_FILE_SHORT_FIELD_NAME,
									fileName));
					throw new RuntimeException();
				}
				...
				if (longPosition <= 0) {
					logger.fatal(String
							.format("expected long field name=%s was not a field in the walk access file: %s.",
									WALK_PERCENTAGE_FILE_LONG_FIELD_NAME,
									fileName));
					throw new RuntimeException();
				}
				...
				logger.fatal(String.format(
						"exception caught reading walk access file: %s",
						fileName), e);
			}
	...
	private void readZonalAccessibilitiesFile(String fileName) {
		...
		if (fileName != null) {
			...
			try {
				...
				int tazPosition = acc
						.getColumnPosition(ACCESSIBILITIES_FILE_ZONE_FIELD_NAME);
				if (tazPosition <= 0) {
					logger.fatal(String
				...
				if (pkAutoRetailPosition <= 0) {
					logger.fatal(String
							.format("expected field name=%s was not a field in the zonal accessibilities file: %s.",
									ACCESSIBILITIES_PEAK_AUTO_RETAIL_FIELD_NAME,
									fileName));
					throw new RuntimeException();
				}
				...
				if (pkAutoTotalPosition <= 0) {
					logger.fatal(String
							.format("expected field name=%s was not a field in the zonal accessibilities file: %s.",
									ACCESSIBILITIES_PEAK_AUTO_TOTAL_FIELD_NAME,
									fileName));
					throw new RuntimeException();
				}
				...
				if (pkTransitTotalPosition <= 0) {
					logger.fatal(String
							.format("expected field name=%s was not a field in the zonal accessibilities file: %s.",
									ACCESSIBILITIES_PEAK_TRANSIT_TOTAL_FIELD_NAME,
									fileName));
					throw new RuntimeException();
				}
				...
				for (int i = 1; i <= acc.getRowCount(); i++) {
					...
					pkTransitRetail[taz] = acc.getValueAt(i,
							pkTransitRetailPosition);
					pkTransitTotal[taz] = acc.getValueAt(i,
							pkTransitTotalPosition);
					opAutoRetail[taz] = acc.getValueAt(i, opAutoRetailPosition);
					opAutoTotal[taz] = acc.getValueAt(i, opAutoTotalPosition);
					opTransitRetail[taz] = acc.getValueAt(i,
							opTransitRetailPosition);
					opTransitTotal[taz] = acc.getValueAt(i,
							opTransitTotalPosition);
					nonMotorizedRetail[taz] = acc.getValueAt(i,
							nonMotorizedRetailPosition);
					nonMotorizedTotal[taz] = acc.getValueAt(i,
							nonMotorizedTotalPosition);
				}

			} catch (IOException e) {
				logger.fatal(String.format(
						"exception caught reading accessibilities file: %s",
						fileName), e);
			}
	...
	private int getZoneFromAlt(int alt) {
		...
		if (zone < 1 || zone > NUM_ZONES) {
			logger.fatal(String
					.format("invalid value for zone index = %d, determined for alt = %d.",
							zone, alt));
			logger.fatal(String.format("NUM_ZONES = %d, NUM_SUBZONES = %d.",
					NUM_ZONES, NUM_SUBZONES));
			throw new RuntimeException();
		}
		return zone;
	}
	...
	public int getTazNumber(int tableRowNumber) {
		return (int) zoneDataTable.getValueAt(tableRowNumber,
				tazDataZoneFieldName);
	}
	...
	public int[] getZonalAreaType() {
		int atFieldPosition = zoneDataTable
				.getColumnPosition(tazDataAtFieldName);
	...
	public int[] getZonalDistrict() {
		int districtFieldPosition = zoneDataTable
				.getColumnPosition(tazDataDistFieldName);
	...
	public int[] getZonalCounty() {
		int countyFieldPosition = zoneDataTable
				.getColumnPosition(tazDataCountyFieldName);
	...
	public int[] getZonalParkTot() {
		int parkTotFieldPosition = zoneDataTable
				.getColumnPosition(parkTotFieldName);
	...
	public int[] getZonalParkLong() {
		int parkLongFieldPosition = zoneDataTable
				.getColumnPosition(parkLongFieldName);
	...
	public float[] getZonalPropFree() {
		int propFreeFieldPosition = zoneDataTable
				.getColumnPosition(propFreeFieldName);
	...
	public float[] getZonalParkRate() {
		int parkRateFieldPosition = zoneDataTable
				.getColumnPosition(parkRateFieldName);
