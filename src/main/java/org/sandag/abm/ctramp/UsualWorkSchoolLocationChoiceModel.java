...
public class UsualWorkSchoolLocationChoiceModel implements Serializable {
	...
	private static final int[] WORK_LOC_SOA_SEGMENT_TO_UEC_SHEET_INDEX = { 1,
			1, 1, 1, 1, 1 };
	...
	public UsualWorkSchoolLocationChoiceModel(ResourceBundle resourceBundle,
			String restartModelString, JPPFClient jppfClient,
		...
		if (useNewSoaMethod) {
			usualWorkLocationUecFileName = ResourceUtil.getProperty(
					resourceBundle, PROPERTIES_UEC_USUAL_LOCATION_NEW);
			usualSchoolLocationUecFileName = ResourceUtil.getProperty(
					resourceBundle, PROPERTIES_UEC_USUAL_LOCATION_NEW);
		} else {
			usualWorkLocationUecFileName = ResourceUtil.getProperty(
					resourceBundle, PROPERTIES_UEC_USUAL_LOCATION);
			usualSchoolLocationUecFileName = ResourceUtil.getProperty(
					resourceBundle, PROPERTIES_UEC_USUAL_LOCATION);
		}
		...
		String usualLocationSoaUecFileName = ResourceUtil.getProperty(
				resourceBundle, PROPERTIES_UEC_USUAL_LOCATION_SOA);
	...
	public void runWorkLocationChoiceModel(
		...
		HashMap<Integer, Integer> occupValueIndexMap = aggAcc
				.getWorkOccupValueIndexMap();

		HashMap<Integer, String> workSegmentIndexNameMap = aggAcc
				.getWorkSegmentIndexNameMap();
		HashMap<String, Integer> workSegmentNameIndexMap = aggAcc
		...
		workerDcSizeObj = new DestChoiceSize(propertyMap,
				workSegmentIndexNameMap, workSegmentNameIndexMap,
				workerSizeTerms, maxShadowPriceIterations);
		...
		if (fileName != null) {
			if (fileName.length() > 2) {
				String projectDirectory = ResourceUtil.getProperty(
						resourceBundle,
						CtrampApplication.PROPERTIES_PROJECT_DIRECTORY);
				workerDcSizeObj.restoreShadowPricingInfo(projectDirectory
						+ fileName);
				int underScoreIndex = fileName.lastIndexOf('_');
				int dotIndex = fileName.lastIndexOf('.');
				currentIter = Integer.parseInt(fileName.substring(
		...
		for (int iter = 0; iter < workerDcSizeObj.getMaxShadowPriceIterations(); iter++) {
			...
			try {
				...
				dataProvider.setValue("uecIndices",
						WORK_LOC_SEGMENT_TO_UEC_SHEET_INDEX);
				dataProvider.setValue("soaUecIndices",
						WORK_LOC_SOA_SEGMENT_TO_UEC_SHEET_INDEX);
				dataProvider.setValue("tourCategory",
				...
				for (int[] startEndIndices : startEndTaskIndicesList) {
					...
					if (useNewSoaMethod) {
						myTaskNew = new WorkLocationChoiceTaskJppfNew(
								taskIndex, startIndex, endIndex, iter);
						job.addTask(myTaskNew);
					} else {
						myTask = new WorkLocationChoiceTaskJppf(taskIndex,
								startIndex, endIndex, iter);
						job.addTask(myTask);
					}
					taskIndex++;
				}
			...
			for (int i = 0; i < numChosenDests.length; i++) {
				...
				logger.info(String.format("\t%-8d%-15s = %10d", i + 1,
						segmentString, numChosenDests[i]));
				total += numChosenDests[i];
			}
			logger.info(String.format("\t%-8s%-15s = %10.0f", "total", "",
					total));

			// apply the shadow price adjustments
			workerDcSizeObj.reportMaxDiff(iter,
					finalModeledDestChoiceLocationsByDestMgra);
			workerDcSizeObj.saveWorkMaxDiffValues(iter,
					finalModeledDestChoiceLocationsByDestMgra);
			workerDcSizeObj
					.updateShadowPrices(finalModeledDestChoiceLocationsByDestMgra);
			workerDcSizeObj.updateSizeVariables();
			workerDcSizeObj.updateShadowPricingInfo(currentIter,
					originLocationsByHomeMgra,
	...
	public void runSchoolLocationChoiceModel(
			HouseholdDataManagerIf householdDataManager,
		...
		// size
		HashMap<Integer, String> schoolSegmentIndexNameMap = aggAcc
				.getSchoolSegmentIndexNameMap();
		HashMap<String, Integer> schoolSegmentNameIndexMap = aggAcc
		...
		// terms
		// and managing shadow price calculations.
		schoolDcSizeObj = new DestChoiceSize(propertyMap,
				schoolSegmentIndexNameMap, schoolSegmentNameIndexMap,
				schoolSizeTerms, maxShadowPriceIterations);
		...
		schoolDcSizeObj
				.setNoShadowPriceSchoolSegmentIndices(skipSegmentIndexSet);
		...
		householdDataManager.setSchoolDistrictMappings(
				schoolSegmentNameIndexMap, aggAcc.getMgraGsDistrict(),
				aggAcc.getMgraHsDistrict(), aggAcc.getGsDistrictIndexMap(),
				aggAcc.getHsDistrictIndexMap());

		int[][] originLocationsByHomeMgra = householdDataManager
				.getStudentsByHomeMgra();
			...
			if (fileName.length() > 2) {
				{
					String projectDirectory = ResourceUtil.getProperty(
							resourceBundle,
							CtrampApplication.PROPERTIES_PROJECT_DIRECTORY);
					schoolDcSizeObj.restoreShadowPricingInfo(projectDirectory
							+ fileName);
					int underScoreIndex = fileName.lastIndexOf('_');
					int dotIndex = fileName.lastIndexOf('.');
					currentIter = Integer.parseInt(fileName.substring(
		...
		for (int iter = 0; iter < schoolDcSizeObj.getMaxShadowPriceIterations(); iter++) {
			...
			try {
				...
				for (int[] startEndIndices : startEndTaskIndicesList) {
					...
					if (useNewSoaMethod) {
						myTaskNew = new SchoolLocationChoiceTaskJppfNew(
								taskIndex, startIndex, endIndex, iter);
						job.addTask(myTaskNew);
					} else {
						myTask = new SchoolLocationChoiceTaskJppf(taskIndex,
								startIndex, endIndex, iter);
						job.addTask(myTask);
					}
					taskIndex++;
			...
			logger.info(String
					.format("Usual school location choice tasks completed for shadow price iteration %d.",
							iter));
			logger.info(String.format("Chosen dests by segment:"));
			double total = 0;
			for (int i = 0; i < numChosenDests.length; i++) {
				String segmentString = schoolSegmentIndexNameMap.get(i);
				logger.info(String.format("\t%-8d%-20s = %10d", i + 1,
						segmentString, numChosenDests[i]));
				total += numChosenDests[i];
			}
			logger.info(String.format("\t%-8s%-20s = %10.0f", "total", "",
					total));
			...
			// apply the shadow price adjustments
			schoolDcSizeObj.reportMaxDiff(iter,
					finalModeledDestChoiceLocationsByDestMgra);
			schoolDcSizeObj.saveSchoolMaxDiffValues(iter,
					finalModeledDestChoiceLocationsByDestMgra);
			schoolDcSizeObj
					.updateShadowPrices(finalModeledDestChoiceLocationsByDestMgra);
			schoolDcSizeObj.updateSizeVariables();
			schoolDcSizeObj.updateShadowPricingInfo(currentIter,
					originLocationsByHomeMgra,
	 ...
	 */
	public void saveResults(HouseholdDataManagerIf householdDataManager,
			String projectDirectory, int globalIteration) {
		...
		if (wsLocResultsFileName != null) {
			...
			if (dotIndex < 0) {
				wsLocResultsFileName = String.format("%s_%d",
						wsLocResultsFileName, globalIteration);
			} else {
				String base = wsLocResultsFileName.substring(0, dotIndex);
				String extension = wsLocResultsFileName.substring(dotIndex);
				wsLocResultsFileName = String.format("%s_%d%s", base,
						globalIteration, extension);
			}
			...
			try {
				...
				logger.fatal(String.format(
						"Exception occurred opening wsLoc results file: %s.",
			...
			for (int[] startEndIndices : startEndTaskIndicesList) {
				...
				for (int i = 0; i < householdArray.length; ++i) {
					...
					for (int j = 1; j < personArray.length; ++j) {
						...
						int employmentCategory = person
								.getPersonEmploymentCategoryIndex();
						int studentCategory = person
								.getPersonStudentCategoryIndex();

						int schoolSegmentIndex = person
								.getSchoolLocationSegmentIndex();
						int workSegmentIndex = person
								.getWorkLocationSegmentIndex();
						...
						outStream
								.println(String
										.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.5e,%.5e,%d,%.5e,%.5e",
												hhId,
												homeMgra,
												income,
												personId,
												...
												schoolSegmentIndex,
												workLocation,
												person.getWorkLocationDistance(),
												person.getWorkLocationLogsum(),
												schoolLocation,
		...
		try {
			...
			logger.fatal(String
					.format("Exception occurred opening work/school segment definitions file: %s.",
							fileName));
			throw new RuntimeException(e);
		}
		...
		outStream
				.println(String.format("%-15s %-20s", "Index", "Segment Name"));
		...
		for (int i = 0; i < names.length; i++) {
			if (names[i] != null)
				outStream.println(String.format("%-15d %-20s", i, names[i]));
		}
