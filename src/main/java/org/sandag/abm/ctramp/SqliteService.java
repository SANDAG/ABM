...
public class SqliteService {
	...
	public void connect(String fileName, String tableName) throws DAOException {
		...
		try {
			...
			s.execute("CREATE TABLE IF NOT EXISTS " + tableName + " ("
					+ "   id INTEGER, " + "   numProcessed INTEGER, "
					+ "   totalToProcess INTEGER, " + "   startUp INTEGER, "
					+ "   runTime INTEGER, " + "   shutDown INTEGER " + ")");
		...
		try {
			...
			while (rs.next()) {
				System.out.println(rs.getInt("id") + ", "
						+ rs.getInt("numProcessed") + ", "
						+ rs.getInt("totalToProcess") + ", "
						+ rs.getInt("startUp") + ", " + rs.getInt("runTime")
						+ ", " + rs.getInt("shutDown"));
			}
	...
	public void insertRecord(String tableName, int id, int numProcessed,
			int totalToProcess, int startUp, int runTime, int shutDown)
			throws DAOException {
		...
		try {
			...
			String query = String
					.format("INSERT INTO %s (id, numProcessed, totalToProcess, startUp, runTime, shutDown) VALUES (%d, %d, %d, %d, %d, %d)",
							tableName, id, numProcessed, totalToProcess,
	...
	public void updateRecord(String tableName, int id, int numProcessed,
			int totalToProcess, int startUp, int runTime, int shutDown)
			throws DAOException {
		...
		try {
			...
			String query = String
					.format("UPDATE %s SET numProcessed=%d, totalToProcess=%d, startUp=%d, runTime=%d, shutDown=%d WHERE id=%d",
							tableName, numProcessed, totalToProcess, startUp,
							runTime, shutDown, id);
			s.execute(query);
