        ...
        implements Serializable
{
    ...
    /**
     * This method maintains the sequencing of the various AB Model choice model components
     * 
     * @param iteration
     *            is the global iteration number in the sequence of AB Model runs during feedback
     * @param householdDataManager
     *            is the handle to the household object data manager
     * @param dmuFactory
     *            is the factory object for creating DMU objects used in choice models
     */
    ...
    /**
     * Build the mandatory accessibilities object used by the usual work and school location choice models
     * 
     * @return BuildAccessibilities object containing mandatory size term and logsum information private void buildMandatoryAccessibilities() {
     * 
     ...
     * 
     *         MatrixDataManager mdm = MatrixDataManager.getInstance(); mdm.setMatrixDataServerObject( ms );
     * 
     *         aggAcc.setupBuildAccessibilities( propertyMap ); aggAcc.calculateConstants();
     * 
     *         // do this in dest choice model //aggAcc.buildAccessibilityComponents( propertyMap );
     * 
     *         }
     */

    /**
     * Build the non-mandatory accessibilities object used by the auto ownership model
     * 
     * @return BuildAccessibilities object containing non-mandatory size term and logsum information
     */
    ...
    /*
     * method used in original ARC implementation private void runIteration( int iteration, HouseholdDataManagerIf householdDataManager,
     * CtrampDmuFactoryIf dmuFactory ) { String restartModel = ""; if ( hhDiskObjectKey != null && ! hhDiskObjectKey.equalsIgnoreCase("none") ) {
     * String doFileName = hhDiskObjectFile + "_" + hhDiskObjectKey; householdDataManager.createHhArrayFromSerializedObjectInFile( doFileName,
     * hhDiskObjectKey ); restartModel = hhDiskObjectKey; restartModels ( householdDataManager ); } else { restartModel = ResourceUtil.getProperty(
     * resourceBundle, PROPERTIES_RESTART_WITH_HOUSEHOLD_SERVER ); if ( restartModel == null ) restartModel = "none"; if ( !
     * restartModel.equalsIgnoreCase("none") ) restartModels ( householdDataManager ); } JPPFClient jppfClient = new JPPFClient(); boolean
     * runUsualWorkSchoolChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_WORKSCHOOL_CHOICE);
     * if(runUsualWorkSchoolChoiceModel){ // create an object for calculating destination choice attraction size terms and managing shadow price
     * calculations. DestChoiceSize dcSizeObj = new DestChoiceSize( modelStructure, tazDataManager ); // new the usual school and location choice
     * model object UsualWorkSchoolLocationChoiceModel usualWorkSchoolLocationChoiceModel = new UsualWorkSchoolLocationChoiceModel(resourceBundle,
     * restartModel, jppfClient, modelStructure, ms, tazDataManager, dcSizeObj, dmuFactory ); // run the model logger.info (
     * "starting usual work and school location choice."); usualWorkSchoolLocationChoiceModel .runSchoolAndLocationChoiceModel(householdDataManager);
     * logger.info ( "finished with usual work and school location choice."); logger.info (
     * "writing work/school location choice results file; may take a few minutes ..." ); usualWorkSchoolLocationChoiceModel.saveResults(
     * householdDataManager, projectDirectory, iteration ); logger.info ( String.format("finished writing results file.") );
     * usualWorkSchoolLocationChoiceModel = null; dcSizeObj = null; System.gc(); // write a disk object fle for the householdDataManager, in case we
     * want to restart from the next step. if ( hhDiskObjectFile != null ) { logger.info (
     * "writing household disk object file after work/school location choice; may take a long time ..." ); String hhFileName = String.format(
     * "%s_%d_ao", hhDiskObjectFile, iteration ); householdDataManager.createSerializedHhArrayInFileFromObject( hhFileName, "ao" ); logger.info
     * (String.format( "finished writing household disk object file = %s after uwsl; continuing to household choice models ..." , hhFileName) ); } }
     * boolean runAutoOwnershipChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_AUTO_OWNERSHIP ); boolean
     * runFreeParkingChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_FREE_PARKING_AVAILABLE ); boolean
     * runCoordinatedDailyActivityPatternChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_DAILY_ACTIVITY_PATTERN );
     * boolean runMandatoryTourFreqChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_INDIV_MANDATORY_TOUR_FREQ ); boolean
     * runMandatoryTourTimeOfDayChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_MAND_TOUR_DEP_TIME_AND_DUR ); boolean
     * runMandatoryTourModeChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_MAND_TOUR_MODE_CHOICE ); boolean
     * runJointTourFrequencyModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_JOINT_TOUR_FREQ ); boolean
     * runJointTourLocationChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_JOINT_LOCATION_CHOICE ); boolean
     * runJointTourModeChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_JOINT_TOUR_MODE_CHOICE ); boolean
     * runJointTourDepartureTimeAndDurationModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_JOINT_TOUR_DEP_TIME_AND_DUR );
     ...
     * boolean runIndivNonManTourLocationChoiceModel = ResourceUtil.getBooleanProperty(resourceBundle,
     * PROPERTIES_RUN_INDIV_NON_MANDATORY_LOCATION_CHOICE ); boolean runIndivNonManTourModeChoiceModel =
     * ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_INDIV_NON_MANDATORY_TOUR_MODE_CHOICE ); boolean
     * runIndivNonManTourDepartureTimeAndDurationModel = ResourceUtil.getBooleanProperty(resourceBundle,
     * PROPERTIES_RUN_INDIV_NON_MANDATORY_TOUR_DEP_TIME_AND_DUR ); boolean runAtWorkSubTourFrequencyModel =
     * ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_AT_WORK_SUBTOUR_FREQ ); boolean runAtWorkSubtourLocationChoiceModel =
     * ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_AT_WORK_SUBTOUR_LOCATION_CHOICE ); boolean runAtWorkSubtourModeChoiceModel =
     * ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_AT_WORK_SUBTOUR_MODE_CHOICE ); boolean
     * runAtWorkSubtourDepartureTimeAndDurationModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_AT_WORK_SUBTOUR_DEP_TIME_AND_DUR
     * ); boolean runStopFrequencyModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_STOP_FREQUENCY ); boolean
     * runStopLocationModel = ResourceUtil.getBooleanProperty(resourceBundle, PROPERTIES_RUN_STOP_LOCATION ); boolean runHouseholdModels = false; if (
     * runAutoOwnershipChoiceModel || runFreeParkingChoiceModel || runCoordinatedDailyActivityPatternChoiceModel || runMandatoryTourFreqChoiceModel ||
     * runMandatoryTourModeChoiceModel || runMandatoryTourTimeOfDayChoiceModel || runJointTourFrequencyModel || runJointTourLocationChoiceModel ||
     * runJointTourModeChoiceModel || runJointTourDepartureTimeAndDurationModel || runIndivNonManTourFrequencyModel ||
     * runIndivNonManTourLocationChoiceModel || runIndivNonManTourModeChoiceModel || runIndivNonManTourDepartureTimeAndDurationModel ||
     * runAtWorkSubTourFrequencyModel || runAtWorkSubtourLocationChoiceModel || runAtWorkSubtourModeChoiceModel ||
     * runAtWorkSubtourDepartureTimeAndDurationModel || runStopFrequencyModel || runStopLocationModel ) runHouseholdModels = true; // disk object file
     * is labeled with the next component eligible to be run if model restarted String lastComponent = "uwsl"; String nextComponent = "ao"; if(
     * runHouseholdModels ) { logger.info ( "starting HouseholdChoiceModelRunner." ); HashMap<String, String> propertyMap =
     * ResourceUtil.changeResourceBundleIntoHashMap(resourceBundle); HouseholdChoiceModelRunner runner = new HouseholdChoiceModelRunner( propertyMap,
     * jppfClient, restartModel, householdDataManager, ms, modelStructure, tazDataManager, dmuFactory ); runner.runHouseholdChoiceModels(); if(
     * runAutoOwnershipChoiceModel ){ saveAoResults( householdDataManager, projectDirectory ); logAoResults( householdDataManager ); lastComponent =
     * "ao"; nextComponent = "fp"; } if( runFreeParkingChoiceModel ){ logFpResults( householdDataManager ); lastComponent = "fp"; nextComponent =
     * "cdap"; } if( runCoordinatedDailyActivityPatternChoiceModel ){ saveCdapResults( householdDataManager, projectDirectory ); logCdapResults(
     * householdDataManager ); lastComponent = "cdap"; nextComponent = "imtf"; } if( runMandatoryTourFreqChoiceModel ){ logImtfResults(
     * householdDataManager ); lastComponent = "imtf"; nextComponent = "imtod"; } if( runMandatoryTourTimeOfDayChoiceModel ||
     * runMandatoryTourModeChoiceModel ){ lastComponent = "imtod"; nextComponent = "jtf"; } if( runJointTourFrequencyModel ){ logJointModelResults(
     * householdDataManager ); lastComponent = "jtf"; nextComponent = "jtl"; } if( runJointTourLocationChoiceModel ){ lastComponent = "jtl";
     * nextComponent = "jtod"; } if( runJointTourDepartureTimeAndDurationModel || runJointTourModeChoiceModel ){ lastComponent = "jtod"; nextComponent
     * = "inmtf"; } if( runIndivNonManTourFrequencyModel ){ lastComponent = "inmtf"; nextComponent = "inmtl"; } if(
     * runIndivNonManTourLocationChoiceModel ){ lastComponent = "inmtl"; nextComponent = "inmtod"; } if(
     * runIndivNonManTourDepartureTimeAndDurationModel || runIndivNonManTourModeChoiceModel ){ lastComponent = "inmtod"; nextComponent = "awf"; } if(
     * runAtWorkSubTourFrequencyModel ){ logAtWorkSubtourFreqResults( householdDataManager ); lastComponent = "awf"; nextComponent = "awl"; } if(
     * runAtWorkSubtourLocationChoiceModel ){ lastComponent = "awl"; nextComponent = "awtod"; } if( runAtWorkSubtourDepartureTimeAndDurationModel ||
     * runAtWorkSubtourModeChoiceModel ){ lastComponent = "awtod"; nextComponent = "stf"; } if( runStopFrequencyModel ){ lastComponent = "stf";
     * nextComponent = "stl"; } if( runStopLocationModel ){ lastComponent = "stl"; nextComponent = "done"; } // write a disk object fle for the
     * householdDataManager, in case we want to restart from the next step. if ( hhDiskObjectFile != null && ! lastComponent.equalsIgnoreCase("uwsl")
     * ) { logger.info (String.format( "writing household disk object file after %s choice model; may take a long time ..." , lastComponent) ); String
     * hhFileName = hhDiskObjectFile + "_" + nextComponent; householdDataManager.createSerializedHhArrayInFileFromObject( hhFileName, nextComponent );
     * logger.info ( String.format("finished writing household disk object file = %s.", hhFileName) ); } logger.info (
     ...
     * try{ DataFile dataFile = new DataFile( serializedObjectFileName, 1 ); DataWriter dw = new DataWriter( serializedObjectKey ); dw.writeObject(
     * objectToSerialize ); dataFile.insertRecord( dw ); dataFile.close(); } catch(NotSerializableException e) { logger.error(
     * String.format("NotSerializableException for %s.  Trying to create serialized object with key=%s, in filename=%s." ,
     * objectToSerialize.getClass().getName(), serializedObjectKey, serializedObjectFileName ), e ); throw new RuntimeException(); } catch(IOException
     * e) { logger.error( String.format( "IOException trying to write disk object file=%s, with key=%s for writing.", serializedObjectFileName,
     * serializedObjectKey ), e ); throw new RuntimeException(); } }
     * 
     * 
     * private Object createObjectFromSerializedObjectInFile( Object newObject, String serializedObjectFileName, String serializedObjectKey ){ try{
     * DataFile dataFile = new DataFile( serializedObjectFileName, "r" ); DataReader dr = dataFile.readRecord( serializedObjectKey ); newObject =
     * dr.readObject(); dataFile.close(); return newObject; } catch(IOException e) { logger.error(
     * String.format("IOException trying to read disk object file=%s, with key=%s.", serializedObjectFileName, serializedObjectKey ), e ); throw new
     * RuntimeException(); } catch(ClassNotFoundException e) { logger.error( String.format
     * ("could not instantiate %s object, with key=%s from filename=%s.", newObject.getClass().getName(), serializedObjectFileName,
     * serializedObjectKey ), e ); throw new RuntimeException(); } }
     **/
    /**
     * Loops through the households in the HouseholdDataManager, gets the auto ownership result for each household, and writes a text file with hhid
     * and auto ownership.
     * 
     * @param householdDataManager
    ...
    /**
     * Records the coordinated daily activity pattern model results to the logger. A household-level summary simply records each pattern type and a
     * person-level summary summarizes the activity choice by person type (full-time worker, university student, etc).
     * 
     */
    ...
    /**
     * Loops through the households in the HouseholdDataManager, gets the coordinated daily activity pattern for each person in the household, and
