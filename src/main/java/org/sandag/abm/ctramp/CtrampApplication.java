...
public class CtrampApplication implements Serializable {
	...
	public void runModels(HouseholdDataManagerIf householdDataManager,
			CtrampDmuFactoryIf dmuFactory, int globalIterationNumber,
			float iterationSampleRate) {
		...
		try {
			...
			matrixServerAddress = Util.getStringValueFromPropertyMap(
					propertyMap, "RunModel.MatrixServerAddress");
			try {
		...
		if (mt != MatrixType.BINARY) {
			...
			try {
				if (matrixServerAddress.equalsIgnoreCase("localhost")) {
					...
					try {
						...
						logger.error(
								"RuntimeException caught starting 32 bit Matrix IO Process.",
								e);
					...
					ms = new MatrixDataServerRmi(matrixServerAddress,
							serverPort,
				...
				logger.error(
						String.format("exception caught starting MatrixDataServer -- exiting."),
						e);
			...
			try {
				clearMatrixMgr = Util.getBooleanValueFromPropertyMap(
						propertyMap, PROPERTIES_CLEAR_MATRIX_MANAGER_ON_START);
			} catch (RuntimeException e) {
			...
			// need to clear.
			if (clearMatrixMgr
					&& !matrixServerAddress.equalsIgnoreCase("localhost"))
				ms.clear();

		}

		// run core activity based model for the specified iteration
		runModelSequence(globalIterationNumber, householdDataManager,
				dmuFactory);
	 ...
	 *            models
	 */
	private void runModelSequence(int iteration,
		...
		boolean logResults = Util.getStringValueFromPropertyMap(propertyMap,
				"RunModel.LogResults").equalsIgnoreCase("true");
		...
		boolean runPreAutoOwnershipChoiceModel = ResourceUtil
		...
		if (runPreAutoOwnershipChoiceModel) {
			...
			HouseholdAutoOwnershipModel aoModel = new HouseholdAutoOwnershipModel(
					propertyMap, dmuFactory,
					aggAcc.getAccessibilitiesTableObject(), null);
		...
		logger.info("flag to run pre-AO was set to: "
				+ runPreAutoOwnershipChoiceModel);
		logAoResults(householdDataManager, true);

		boolean runUsualWorkSchoolChoiceModel = ResourceUtil
				.getBooleanProperty(resourceBundle,
		...
		if (runUsualWorkSchoolChoiceModel) {
			...
			try {
				String stringValue = resourceBundle
						.getString(PROPERTIES_RUN_WORK_LOC_CHOICE_KEY);
			...
			logger.info("flag to run work location choice was set to: "
					+ runWorkLocationChoice);

			boolean runSchoolLocationChoice = false;
			try {
				String stringValue = resourceBundle
						.getString(PROPERTIES_RUN_SCHOOL_LOC_CHOICE_KEY);
				runSchoolLocationChoice = stringValue.equalsIgnoreCase("true");
			} catch (MissingResourceException e) {
				// default value is true if property was not defined
				runSchoolLocationChoice = true;
			}
			logger.info("flag to run school location choice was set to: "
			...
			try {
				...
				writeLocationChoiceResultsFile = stringValue
						.equalsIgnoreCase("true");
			} catch (MissingResourceException e) {
				// default value is true if property was not defined
				writeLocationChoiceResultsFile = true;
			}
			logger.info("flag to write uwsl result was set to: "
					+ writeLocationChoiceResultsFile);
			...
			UsualWorkSchoolLocationChoiceModel usualWorkSchoolLocationChoiceModel = new UsualWorkSchoolLocationChoiceModel(
					resourceBundle, restartModel, jppfClient, modelStructure,
					ms, dmuFactory, aggAcc);
			...
			if (runWorkLocationChoice) {
				...
				usualWorkSchoolLocationChoiceModel.runWorkLocationChoiceModel(
						householdDataManager, workerSizeTerms);
			...
			if (runSchoolLocationChoice) {
				...
				double[][] schoolFactors = aggAcc
						.calculateSchoolSegmentFactors();
				...
				usualWorkSchoolLocationChoiceModel
						.runSchoolLocationChoiceModel(householdDataManager,
								schoolSizeTerms, schoolFactors);
			...
			if (writeLocationChoiceResultsFile) {
				...
				usualWorkSchoolLocationChoiceModel.saveResults(
						householdDataManager, projectDirectory, iteration);
				logger.info(String
		...
		logger.info("flag to run UWSL was set to: "
				+ runUsualWorkSchoolChoiceModel);

		boolean runAutoOwnershipChoiceModel = ResourceUtil.getBooleanProperty(
				resourceBundle, PROPERTIES_RUN_AUTO_OWNERSHIP);
		boolean runTransponderChoiceModel = ResourceUtil.getBooleanProperty(
				resourceBundle, PROPERTIES_RUN_TRANSPONDER_CHOICE);
		boolean runFreeParkingChoiceModel = ResourceUtil.getBooleanProperty(
				resourceBundle, PROPERTIES_RUN_FREE_PARKING_AVAILABLE);
		boolean runInternalExternalTripChoiceModel = ResourceUtil
				.getBooleanProperty(resourceBundle,
						PROPERTIES_RUN_INTERNAL_EXTERNAL_TRIP);
		boolean runCoordinatedDailyActivityPatternChoiceModel = ResourceUtil
				.getBooleanProperty(resourceBundle,
						PROPERTIES_RUN_DAILY_ACTIVITY_PATTERN);
		boolean runMandatoryTourFreqChoiceModel = ResourceUtil
				.getBooleanProperty(resourceBundle,
						PROPERTIES_RUN_INDIV_MANDATORY_TOUR_FREQ);
		boolean runJointTourFreqChoiceModel = ResourceUtil.getBooleanProperty(
				resourceBundle, PROPERTIES_RUN_JOINT_TOUR_FREQ);
		boolean runIndivNonManTourFrequencyModel = ResourceUtil
				.getBooleanProperty(resourceBundle,
		...
		if (runAutoOwnershipChoiceModel || runTransponderChoiceModel
			...
			HouseholdChoiceModelRunner runner = new HouseholdChoiceModelRunner(
					propertyMap, jppfClient, restartModel,
					householdDataManager, ms, modelStructure, dmuFactory);
			...
			if (runInternalExternalTripChoiceModel) {
				if (logResults)
					logIeResults(householdDataManager);
			}
		...
		if (writeTextFileFlag || writeSqliteFlag) {
			dataWriter = new HouseholdDataWriter(propertyMap, modelStructure,
					iteration);
			...
			if (writeSqliteFlag) {
				String dbFilename = "";
				try {
					String baseDir = resourceBundle
							.getString(PROPERTIES_PROJECT_DIRECTORY);
					dbFilename = baseDir
							+ resourceBundle
									.getString(SQLITE_DATABASE_FILENAME) + "_"
							+ iteration;
					dataWriter.writeDataToDatabase(householdDataManager,
							dbFilename);
				} catch (MissingResourceException e) {
	...
	private void buildNonMandatoryAccessibilities(
		...
		boolean readAccessibilities = ResourceUtil.getBooleanProperty(
				resourceBundle, READ_ACCESSIBILITIES);
		if (readAccessibilities) {

			// output data
			String accFileName = projectDirectory
					+ Util.getStringValueFromPropertyMap(propertyMap,
							"acc.output.file");
	...
	public void restartModels(HouseholdDataManagerIf householdDataManager) {
		...
		if (fileName != null) {
			...
			restartIter = Integer.parseInt(fileName.substring(
					underScoreIndex + 1, dotIndex));
		}

		boolean runPreAutoOwnershipModel = ResourceUtil.getBooleanProperty(
				resourceBundle, PROPERTIES_RUN_PRE_AUTO_OWNERSHIP);
		if (runPreAutoOwnershipModel) {
			...
			if (runUsualWorkSchoolChoiceModel) {
				...
				boolean runAutoOwnershipModel = ResourceUtil
						.getBooleanProperty(resourceBundle,
				...
				if (runAutoOwnershipModel) {
					...
					if (runCoordinatedDailyActivityPatternModel) {
						...
						if (runIndividualMandatoryTourFrequencyModel) {
							...
							// }
							// else {
							boolean runStopFrequencyModel = ResourceUtil
									.getBooleanProperty(resourceBundle,
	...
	private void saveAoResults(HouseholdDataManagerIf householdDataManager,
		...
		try {
			...
			aoResultsFileName = resourceBundle
					.getString(PROPERTIES_RESULTS_AUTO_OWNERSHIP);
		...
		if (aoResultsFileName != null) {
			...
			try {
				...
				logger.fatal(String.format(
						"Exception occurred opening AO results file: %s.",
						aoResultsFileName));
	...
	private void logAoResults(HouseholdDataManagerIf householdDataManager,
		...
		String[] aoRowCategoryLabel = { "0 autos", "1 auto", "2 autos",
				"3 autos", "4 or more autos" };
		...
		for (int[] startEndIndices : startEndTaskIndicesList) {
			...
			for (int i = 0; i < householdArray.length; ++i) {
				...
				Household household = householdArray[i];
				int ao = household.getAutoOwnershipModelResult();
				if (ao > hhsByAutoOwnership.length - 1)
					ao = hhsByAutoOwnership.length - 1;
	...
	private void logTpResults(HouseholdDataManagerIf householdDataManager) {
		...
		logger.info(String.format("%-16s  %20s", "----------",
				"------------------"));
		logger.info(String.format("%-16s  %20d", "Total",
				(numNo + numYes + numOther)));

	}

	private void logFpResults(HouseholdDataManagerIf householdDataManager) {

		String[] fpCategoryLabel = { "No Choice Made", "Free Available",
				"Must Pay", "Reimbursed" };

		logger.info("");
		logger.info("");
		logger.info("Free Parking Choice Model Results");
		logger.info(String.format("%-16s  %20s  %20s  %20s  %20s", "Category",
				"Workers in area 1", "Workers in area 2", "Workers in area 3",
				"Workers in area 4"));
		...
		for (int i = 0; i < workLocationsByFreeParking.length; i++) {
			logger.info(String.format("%-16s  %20d  %20d  %20d  %20d",
					fpCategoryLabel[i], workLocationsByFreeParking[i][0],
					workLocationsByFreeParking[i][1],
					workLocationsByFreeParking[i][2],
					workLocationsByFreeParking[i][3]));
		...
		logger.info(String.format("%-16s  %20s  %20s  %20s  %20s",
				"----------", "------------------", "------------------",
				"------------------", "------------------"));
		logger.info(String.format("%-16s  %20d  %20d  %20d  %20d", "Totals",
	...
	private void logIeResults(HouseholdDataManagerIf householdDataManager) {
		...
		logger.info(String.format("%-30s  %20s  %20s  %20s", "Person Type",
				ieCategoryLabel[0], ieCategoryLabel[1], "Total"));
			...
			for (int i = 0; i < householdArray.length; ++i) {
				...
				for (int p = 1; p < persons.length; p++) {
					...
					try {
						...
						logger.error(
								"hhid=" + household.getHhId() + ", p=" + p
										+ ", ie=" + ie + ", personType="
										+ persons[p].getPersonTypeNumber(), e);
					}
				}
		...
		for (int i = 0; i < personTypeByIeChoice.length; i++) {
			...
			logger.info(String.format("%-30s  %20d  %20d  %20d",
					Person.personTypeNameArray[i], personTypeByIeChoice[i][0],
					personTypeByIeChoice[i][1], total));
			totals[0] += personTypeByIeChoice[i][0];
			totals[1] += personTypeByIeChoice[i][1];
		}
		logger.info(String.format("%-30s  %20s  %20s  %20s", "-------------",
				"-------------", "-------------", "---------"));
		logger.info(String.format("%-30s  %20d  %20d  %20d", "Totals",
	...
	public void logCdapResults(HouseholdDataManagerIf householdDataManager) {
		...
		for (int i = 0; i < Person.personTypeNameArray.length; ++i) {
			...
			if (cdapByPersonTypeAndActivity.containsKey(personType)) {
				...
				for (int j = 0; j < activityNameArray.length; ++j) {
					int count = 0;
					if (cdapByPersonTypeAndActivity.get(personType)
							.containsKey(activityNameArray[j])) {
						count = cdapByPersonTypeAndActivity.get(personType)
								.get(activityNameArray[j]);
					}
		...
		for (int i = 0; i < hhSizeKeyArray.length; ++i) {
			...
			HashMap<String, Integer> patternMap = cdapByHhSizeAndPattern
					.get(hhSizeKeyArray[i]);
			...
			for (int j = 0; j < patternKeyArray.length; ++j) {
				...
				total += count;
				logger.info(String.format("%-18s%10d", patternKeyArray[j],
						count));
			}
			...
			for (int i = 0; i < householdArray.length; ++i) {
				...
				for (int j = 1; j < personArray.length; j++) {
					...
					if (personActivity != null
							&& personArray[j].getCdapActivity()
									.equalsIgnoreCase("M")) {

						String personTypeString = personArray[j]
								.getPersonType();
						...
						if (choice == 0) {
							...
							if (personArray[j]
									.getPersonEmploymentCategoryIndex() < Person.EmployStatus.NOT_EMPLOYED
									.ordinal()
									&& personArray[j]
											.getPersonWorkLocationZone() == ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR)
								choice = 5 + 1;
							else if (personArray[j]
									.getPersonEmploymentCategoryIndex() < Person.EmployStatus.NOT_EMPLOYED
									.ordinal()
									&& personArray[j]
											.getPersonSchoolLocationZone() == ModelStructure.NOT_ENROLLED_SEGMENT_INDEX)
								choice = 5 + 2;
						...
						if (countByPersonType.containsKey(personTypeString)) {
							...
							int[] counterArray = countByPersonType
									.get(personTypeString);
							counterArray[choice - 1]++;
							countByPersonType.put(personTypeString,
									counterArray);
	...
	private void logJointModelResults(
			HouseholdDataManagerIf householdDataManager,
			CtrampDmuFactoryIf dmuFactory) {

		String uecFileDirectory = ResourceUtil.getProperty(resourceBundle,
				PROPERTIES_UEC_PATH);
		...
		ChoiceModelApplication jointTourFrequencyModel = new ChoiceModelApplication(
				uecFileName, freqCompSheet, dataSheet,
				ResourceUtil.changeResourceBundleIntoHashMap(resourceBundle),
			...
			for (int i = 0; i < householdArray.length; ++i) {
				...
				if (jt == null) {
					...
					for (int j = 0; j < jt.length; j++) {
						...
						String key = String.format("%s_%d_%d_%d",
								jt[j].getTourPurpose(), compAlt, adults,
								children);
		...
		logger.info(String.format("%-5s   %-26s   %12s", "Alt", "Alt Name",
				"Households"));

		int rowTotal = jointTourChoiceFreq[0];
		logger.info(String.format("%-5d   %-26s   %12d", 0, "None",
				jointTourChoiceFreq[0]));
		for (int i = 1; i <= altLabels.length; i++) {
			logger.info(String.format("%-5d   %-26s   %12d", i,
					altLabels[i - 1], jointTourChoiceFreq[i]));
		...
		logger.info(String.format("%-5s   %-20s   %-15s   %10s   %10s   %10s",
				"N", "Purpose", "Type", "Adults", "Children", "Freq"));
		...
		for (String key : partySizeFreq.keySet()) {
			...
			logger.info(String.format(
					"%-5d   %-20s   %-15s   %10d   %10d   %10d", count++,
					purpose,
					JointTourModels.JOINT_TOUR_COMPOSITION_NAMES[compIndex],
					adults, children, partySizeFreq.get(key)));
		}
	...
	private void getLogReportSummaries(
			HouseholdDataManagerIf householdDataManager) {
		...
		for (int[] startEndIndices : startEndTaskIndicesList) {
			...
			for (Household hhObject : partialHhArray) {
				...
				for (int j = 1; j < personArray.length; j++) {
					...
					if (cdapByPersonTypeAndActivity
							.containsKey(personTypeString)) {
						...
						cdapByPersonTypeAndActivity.put(personTypeString,
								activityCountMap);
				...
				if ((!cdapByHhSizeAndPattern.isEmpty())
					...
					HashMap<String, Integer> patternCountMap = cdapByHhSizeAndPattern
							.get(pattern.length());
					...
					cdapByHhSizeAndPattern.put(pattern.length(),
							patternCountMap);
	 ...
	 * @param householdDataManager
	 */
	public void saveCdapResults(HouseholdDataManagerIf householdDataManager,
			String projectDirectory) {

		String cdapResultsFileName;
		try {
			cdapResultsFileName = resourceBundle
					.getString(PROPERTIES_RESULTS_CDAP);
		} catch (MissingResourceException e) {
			// if filename not specified in properties file, don't need to write
			// it.
			return;
		...
		if (cdapResultsFileName != null) {
			...
			try {
				...
				logger.fatal(String.format(
						"Exception occurred opening CDAP results file: %s.",
						cdapResultsFileName));
				throw new RuntimeException(e);
			}

			outStream
					.println("HHID,PersonID,PersonNum,PersonType,ActivityString");
			...
			for (int[] startEndIndices : startEndTaskIndicesList) {
				...
				for (int i = 0; i < householdArray.length; ++i) {
					...
					for (int j = 1; j < personArray.length; j++) {
						...
						outStream.println(String.format("%d,%d,%d,%d,%s", hhid,
								persId, persNum, persType, activityString));
	 ...
	 * 
	 */
	public void logAtWorkSubtourFreqResults(
			HouseholdDataManagerIf householdDataManager) {
				...
				for (int j = 1; j < personArray.length; ++j) {
					...
					for (Tour workTour : tourList) {
						...
						if (awfByPersonType.containsKey(personTypeString)) {
							int[] counterArray = awfByPersonType
									.get(personTypeString);
							counterArray[choice - 1]++;
	...
	public void logIndivStfResults(HouseholdDataManagerIf householdDataManager) {
		...
		HashMap<Integer, String> indexPurposeMap = modelStructure
				.getIndexPrimaryPurposeNameMap();
		HashMap<String, Integer> purposeIndexMap = modelStructure
		...
		for (int i = 1; i < chosen.length; ++i) {
			String stringToLog = String.format("%d out, %d in      ",
					obStopsAlt[i], ibStopsAlt[i]);
	...
	private void createSerializedObjectInFileFromObject(
			Object objectToSerialize, String serializedObjectFileName,
