...
public class Util implements Serializable {
	...
	public static boolean getBooleanValueFromPropertyMap(
			HashMap<String, String> rbMap, String key) {
		boolean returnValue;
	...
	public static String getStringValueFromPropertyMap(
			HashMap<String, String> rbMap, String key) {
	...
	public static int getIntegerValueFromPropertyMap(
			HashMap<String, String> rbMap, String key) {
		String value = rbMap.get(key);
		if (value != null) {
	...
	public static int[] getIntegerArrayFromPropertyMap(
			HashMap<String, String> rbMap, String key) {
	...
	public static int binarySearchDouble(double[] cumProbabilities, double entry) {
		...
		if (cumProbabilities[cumProbabilities.length - 1] != 1.0) {
			System.out.println("cumProbabilities[cumProbabilities.length-1] = "
					+ cumProbabilities[cumProbabilities.length - 1]
					+ " must equal 1.0");
			return -1;
		}
		...
		if (mid == 0) {
			if (entry < cumProbabilities[0])
				...
				return 1;
		} else if (entry < cumProbabilities[mid]
				&& entry >= cumProbabilities[mid - 1]) {
			return mid;
		}
		...
		while (true) {
			...
			if (safetyCount++ > cumProbabilities.length) {
				...
				throw new RuntimeException(
						"binary search stuck in the while loop");
			}
	...
	public static int binarySearchDouble(double cumProbabilityLowerBound,
		...
		// if entry is outside the allowed range, return -1
		if (entry < cumProbabilityLowerBound
				|| entry >= cumProbabilities[numIndices - 1]) {
			System.out
					.println("entry = "
							+ entry
							+ " is outside of allowable range of cumulative probabilities.");
			System.out.println("cumProbabilityLowerBound = "
					+ cumProbabilityLowerBound
					+ ", cumProbabilities[numIndices-1] = "
					+ cumProbabilities[numIndices - 1] + ", numIndices = "
					+ numIndices);
			return -1;
		}
