...
public class JointTourModelsDMU implements Serializable, VariableTable {
	...
	public int getActiveCountFullTimeWorkers() {
		...
		for (Person p : hh.getPersons())
			if (p != null && p.getPersonIsFullTimeWorker() == 1)
				if (!p.getCdapActivity().equalsIgnoreCase(
	...
	public int getTourPurposeIsMaint() {
		return tour.getTourPurpose().equalsIgnoreCase(
				ModelStructure.OTH_MAINT_PRIMARY_PURPOSE_NAME) ? 1 : 0;
	}

	public int getTourPurposeIsEat() {
		return tour.getTourPurpose().equalsIgnoreCase(
				ModelStructure.EAT_OUT_PRIMARY_PURPOSE_NAME) ? 1 : 0;
	}

	public int getTourPurposeIsVisit() {
		return tour.getTourPurpose().equalsIgnoreCase(
				ModelStructure.VISITING_PRIMARY_PURPOSE_NAME) ? 1 : 0;
	}

	public int getTourPurposeIsDiscr() {
		return tour.getTourPurpose().equalsIgnoreCase(
				ModelStructure.OTH_DISCR_PRIMARY_PURPOSE_NAME) ? 1 : 0;
	}
