...
public class StopDestChoiceSize implements Serializable {
	...
	public StopDestChoiceSize(HashMap<String, String> propertyMap,
	...
	public double getDcSize(String purpose, String purposeSegment, int zone,
			int subzone) {
		return sizeMap.get(purpose).get(purposeSegment)
				.get(getZoneSubzoneMapping(zone, subzone));
	}
	...
	private void loadSizeCoefficientTableInformation(TableDataSet coefficients) {
		...
		if (!errors) {
			...
			for (int i = 1; i <= coefficients.getRowCount(); i++) {
				String purpose = coefficients
						.getStringValueAt(i, purposeColumn).toLowerCase();
				String segment = coefficients
						.getStringValueAt(i, segmentColumn).toLowerCase();
				if (validPurposes.contains(purpose)) {
					if (!sizeCoefficients.containsKey(purpose))
						sizeCoefficients.put(purpose,
					...
					if (getValidSegments(purpose).contains(segment)) {
						...
						for (String column : sizeTazColumns)
							coefficientMap
									.put(column, (double) coefficients
											.getValueAt(i, column));
						sizeCoefficients.get(purpose).put(segment,
								coefficientMap);
					} else {
						logger.fatal("Invalid segment for purpose "
								+ purpose
		...
		for (String purpose : sizeCoefficients.keySet()) {
			...
			for (String segment : sizeCoefficients.get(purpose).keySet()) {
				...
				for (int i = 1; i <= tazDataManager.getNumberOfZones(); i++) {
					double size = 0.0d;
					Map<String, Double> coefficients = sizeCoefficients.get(
							purpose).get(segment);
					for (String column : sizeCoefficients.get(purpose)
							.get(segment).keySet())
						size += tazDataManager.getZoneTableValue(i, column)
								* coefficients.get(column);
					double[] walkPercentages = tazDataManager
							.getZonalWalkPercentagesForTaz(i);
					for (int j = 0; j < tazDataManager.getNumberOfSubZones(); j++)
						zoneSizeMap.put(getZoneSubzoneMapping(i, j), size
								* walkPercentages[j]);
				}
