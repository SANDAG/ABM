...
public class HouseholdAtWorkSubtourFrequencyModel implements Serializable {
	 ...
	 */
	public HouseholdAtWorkSubtourFrequencyModel(
	...
	private void setUpModels(HashMap<String, String> propertyMap,
		...
		int dataPage = Util.getIntegerValueFromPropertyMap(propertyMap,
				AWTF_DATA_SHEET_TARGET);
		int modelPage = Util.getIntegerValueFromPropertyMap(propertyMap,
				AWTF_MODEL_SHEET_TARGET);

		dmuObject = dmuFactory.getAtWorkSubtourFrequencyDMU();

		// set up the model
		choiceModelApplication = new ChoiceModelApplication(awtfUecFile,
				modelPage, dataPage, propertyMap, (VariableTable) dmuObject);
	...
	public void applyModel(Household household) {
		...
		if (household.getDebugChoiceModels())
			household.logHouseholdObject(
					"Pre AtWork Subtour Frequency Choice HHID="
		...
		for (int j = 1; j < personArray.length; ++j) {
			...
			for (Tour workTour : tourList) {
				...
				try {
					...
					alternativeNames = choiceModelApplication
							.getAlternativeNames();
					...
					if (household.getDebugChoiceModels()) {
						household.logTourObject(loggingHeader, modelLogger,
								person, workTour);
					}
					...
					if (household.getDebugChoiceModels()) {
						...
						for (int k = 0; k < alternativeNames.length; k++) {
							...
							String altString = String.format("%-3d %-16s",
									k + 1, alternativeNames[k]);
					...
					if (choice == ONE_EAT) {
						...
						person.createAtWorkSubtour(id, choice,
								workTour.getTourDestMgra(),
					...
					logger.error(String.format(
							"Exception caught for j=%d, tourNum=%d, HHID=%d.",
