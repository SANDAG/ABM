...
public class HouseholdChoiceModels {
	...
	private transient Logger logger = Logger
			.getLogger(HouseholdChoiceModels.class);
	...
	public HouseholdChoiceModels(int modelIndex, String restartModelString,
			...
			MandatoryAccessibilitiesCalculator mandAcc, double[] pctHighIncome,
			double[] pctMultipleAutos, double[] avgtts, double[] transpDist,
			double[] pctDetour, double[][][] nonManSoaDistProbs,
		...
		globalSeed = Integer.parseInt(propertyMap
				.get(GLOBAL_MODEL_SEED_PROPERTY));
		...
		runCoordinatedDailyActivityPatternModel = Boolean
				.parseBoolean(propertyMap
						.get(CtrampApplication.PROPERTIES_RUN_DAILY_ACTIVITY_PATTERN));
		runIndividualMandatoryTourFrequencyModel = Boolean
				.parseBoolean(propertyMap
		...
		runMandatoryTourDepartureTimeAndDurationModel = Boolean
				.parseBoolean(propertyMap
		...
		runAtWorkSubtourLocationChoiceModel = Boolean
				.parseBoolean(propertyMap
						.get(CtrampApplication.PROPERTIES_RUN_AT_WORK_SUBTOUR_LOCATION_CHOICE));
		runAtWorkSubtourModeChoiceModel = Boolean
				.parseBoolean(propertyMap
						.get(CtrampApplication.PROPERTIES_RUN_AT_WORK_SUBTOUR_MODE_CHOICE));
		runAtWorkSubtourDepartureTimeAndDurationModel = Boolean
				.parseBoolean(propertyMap
		...
		runJointTourDepartureTimeAndDurationModel = Boolean
				.parseBoolean(propertyMap
						.get(CtrampApplication.PROPERTIES_RUN_JOINT_TOUR_DEP_TIME_AND_DUR));
		runIndividualNonMandatoryTourFrequencyModel = Boolean
				.parseBoolean(propertyMap
						.get(CtrampApplication.PROPERTIES_RUN_INDIV_NON_MANDATORY_TOUR_FREQ));
		runIndividualNonMandatoryTourLocationChoiceModel = Boolean
				.parseBoolean(propertyMap
						.get(CtrampApplication.PROPERTIES_RUN_INDIV_NON_MANDATORY_LOCATION_CHOICE));
		runIndividualNonMandatoryTourModeChoiceModel = Boolean
				.parseBoolean(propertyMap
		...
		try {
			useNewSlcSoaMethod = Util.getBooleanValueFromPropertyMap(
					propertyMap, USE_NEW_SLC_SOA_METHOD_PROPERTY_KEY);

			AccessibilitiesTable accTable = aggAcc
					.getAccessibilitiesTableObject();

			// create the auto ownership choice model application object
			if (runAutoOwnershipModel) {
				aoModel = new HouseholdAutoOwnershipModel(propertyMap,
						dmuFactory, accTable, mandAcc);
				if (measureObjectSizes)
					logger.info("AO size:         "
							+ ObjectUtil.sizeOf(aoModel));
			}

			if (runTransponderModel) {
				tcModel = new TransponderChoiceModel(propertyMap, dmuFactory,
						accTable, pctHighIncome, pctMultipleAutos, avgtts,
						transpDist, pctDetour);
				if (measureObjectSizes)
					logger.info("TC size:         "
							+ ObjectUtil.sizeOf(tcModel));
			}
			...
			if (runInternalExternalModel) {
				ieModel = new InternalExternalTripChoiceModel(propertyMap,
						dmuFactory);
				if (measureObjectSizes)
			...
			if (runCoordinatedDailyActivityPatternModel) {
				cdapModel = new HouseholdCoordinatedDailyActivityPatternModel(
						propertyMap, modelStructure, dmuFactory, accTable);
				if (measureObjectSizes)
					logger.info("CDAP size:       "
							+ ObjectUtil.sizeOf(cdapModel));
			}
			...
			if (runIndividualMandatoryTourFrequencyModel) {
				imtfModel = new HouseholdIndividualMandatoryTourFrequencyModel(
						...
						mandAcc);
				if (measureObjectSizes)
					logger.info("IMTF size:       "
							+ ObjectUtil.sizeOf(imtfModel));
			}

			if (runMandatoryTourDepartureTimeAndDurationModel
					|| runMandatoryTourModeChoiceModel) {
				immcModel = new TourModeChoiceModel(propertyMap,
						modelStructure,
						TourModeChoiceModel.MANDATORY_MODEL_INDICATOR,
						dmuFactory, logsumHelper);
				if (measureObjectSizes)
					logger.info("IMMC size:       "
							+ ObjectUtil.sizeOf(immcModel));
			}

			if (runMandatoryTourDepartureTimeAndDurationModel) {
				imtodModel = new HouseholdIndividualMandatoryTourDepartureAndDurationTime(
						propertyMap, modelStructure,
						aggAcc.getWorkSegmentNameList(), dmuFactory, immcModel);
				if (measureObjectSizes)
					logger.info("IMTOD size:      "
							+ ObjectUtil.sizeOf(imtodModel));
			}

			if (runJointTourFrequencyModel) {
				jtfModel = new JointTourModels(propertyMap, accTable,
						modelStructure, dmuFactory);
				if (measureObjectSizes)
					logger.info("JTF size:        "
							+ ObjectUtil.sizeOf(jtfModel));
			}

			if (runIndividualNonMandatoryTourFrequencyModel) {
				inmtfModel = new HouseholdIndividualNonMandatoryTourFrequencyModel(
						propertyMap, dmuFactory, accTable, mandAcc);
				if (measureObjectSizes)
					logger.info("INMTF size:      "
							+ ObjectUtil.sizeOf(inmtfModel));
			}
			...
			if (runIndividualNonMandatoryTourLocationChoiceModel
				...
				nmmcModel = new TourModeChoiceModel(propertyMap,
						modelStructure,
						TourModeChoiceModel.NON_MANDATORY_MODEL_INDICATOR,
						dmuFactory, logsumHelper);
				if (measureObjectSizes)
					logger.info("INMMC size:      "
							+ ObjectUtil.sizeOf(nmmcModel));
			}

			if (runIndividualNonMandatoryTourLocationChoiceModel
					|| runJointTourLocationChoiceModel) {
				nmlcModel = new NonMandatoryDestChoiceModel(propertyMap,
						modelStructure, aggAcc, dmuFactory, nmmcModel);
				nmlcModel.setNonMandatorySoaProbs(nonManSoaDistProbs,
						nonManSoaSizeProbs);
				if (measureObjectSizes)
					logger.info("INMLC size:      "
							+ ObjectUtil.sizeOf(nmlcModel));
			}

			if (runIndividualNonMandatoryTourDepartureTimeAndDurationModel
					|| runJointTourDepartureTimeAndDurationModel) {
				nmtodModel = new NonMandatoryTourDepartureAndDurationTime(
						propertyMap, modelStructure, dmuFactory, nmmcModel);
				if (measureObjectSizes)
					logger.info("INMTOD size:     "
							+ ObjectUtil.sizeOf(nmtodModel));
			}

			if (runAtWorkSubTourFrequencyModel) {
				awfModel = new HouseholdAtWorkSubtourFrequencyModel(
						propertyMap, modelStructure, dmuFactory);
				if (measureObjectSizes)
					logger.info("AWTF size:       "
							+ ObjectUtil.sizeOf(awfModel));
			}

			if (runAtWorkSubtourLocationChoiceModel
					|| runAtWorkSubtourDepartureTimeAndDurationModel
					|| runAtWorkSubtourModeChoiceModel) {
				awmcModel = new TourModeChoiceModel(propertyMap,
						modelStructure,
						TourModeChoiceModel.AT_WORK_SUBTOUR_MODEL_INDICATOR,
						dmuFactory, logsumHelper);
				if (measureObjectSizes)
					logger.info("AWMC size:       "
							+ ObjectUtil.sizeOf(awmcModel));
			}

			if (runAtWorkSubtourLocationChoiceModel) {
				awlcModel = new SubtourDestChoiceModel(propertyMap,
						modelStructure, aggAcc, dmuFactory, awmcModel);
				awlcModel.setNonMandatorySoaProbs(subTourSoaDistProbs,
						subTourSoaSizeProbs);
				if (measureObjectSizes)
					logger.info("AWLC size:       "
							+ ObjectUtil.sizeOf(awlcModel));
			}

			if (runAtWorkSubtourDepartureTimeAndDurationModel) {
				awtodModel = new SubtourDepartureAndDurationTime(propertyMap,
						modelStructure, dmuFactory, awmcModel);
				if (measureObjectSizes)
					logger.info("AWTOD size:      "
							+ ObjectUtil.sizeOf(awtodModel));
			}

			if (runStopFrequencyModel) {
				stfModel = new StopFrequencyModel(propertyMap, dmuFactory,
						modelStructure, accTable);
				if (measureObjectSizes)
					logger.info("STF size:        "
							+ ObjectUtil.sizeOf(stfModel));
			}

			if (runStopLocationModel) {
				stlmcModel = new IntermediateStopChoiceModels(propertyMap,
						modelStructure, dmuFactory, logsumHelper);
				...
				if (useNewSlcSoaMethod && slcSizeProbs == null) {
					...
					computeSlcSoaProbabilities(slcSoaDistProbsObject,
							locChoiceDistSoaDmu,
							stlmcModel.getSizeSegmentNameIndexMap(),
							stlmcModel.getSizeSegmentArray());

					stlmcModel.setupSlcDistanceBaseSoaModel(propertyMap,
							slcTazDistExpUtils, slcSizeProbs, slcTazSize);
				}

				if (measureObjectSizes)
					logger.info("SLMT size:       "
							+ ObjectUtil.sizeOf(stlmcModel));
			}
			...
			if (runInternalExternalModel && ieModel != null)
				lastModel += " ie";
	...
	public void runModels(Household hhObject) {
		...
		if (runIndividualNonMandatoryTourDepartureTimeAndDurationModel)
			nmtodModel.applyIndivModel(hhObject,
					runIndividualNonMandatoryTourModeChoiceModel);
	...
	private void checkRestartModel(Household hhObject) {
		...
		if (restartModelString.equalsIgnoreCase("")
				|| restartModelString.equalsIgnoreCase("none"))
			return;
	...
	private void computeSlcSoaProbabilities(
		...
		slcTazDistExpUtils = computeTazDistanceExponentiatedUtilities(
				locChoiceSoaDistProbsObject, locChoiceDistSoaDmu);
		...
		for (String segmentName : segmentNameIndexMap.keySet()) {
			...
			slcSizeProbs[segmentIndex] = computeSizeSegmentProbabilities(
					dcSizeArray[segmentIndex], slcTazSize[segmentIndex]);
