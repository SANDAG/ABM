...
public class ParkingProvisionModel implements Serializable {
	...
	public ParkingProvisionModel(HashMap<String, String> propertyMap,
			CtrampDmuFactoryIf dmuFactory) {
		mgraManager = MgraDataManager.getInstance(propertyMap);
		setupFreeParkingChoiceModelApplication(propertyMap, dmuFactory);
	}

	private void setupFreeParkingChoiceModelApplication(
		...
		String fpUecFile = uecFileDirectory
				+ propertyMap.get(FP_CONTROL_FILE_TARGET);

		int dataSheet = Util.getIntegerValueFromPropertyMap(propertyMap,
				FP_DATA_SHEET_TARGET);
		int modelSheet = Util.getIntegerValueFromPropertyMap(propertyMap,
				FP_MODEL_SHEET_TARGET);
		...
		fpModel = new ChoiceModelApplication(fpUecFile, modelSheet, dataSheet,
				propertyMap, (VariableTable) fpDmuObject);
	...
	public void applyModel(Household hhObject) {
		...
		for (int i = 1; i < person.length; i++) {
			...
			if (workLoc == ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR) {
				...
				person[i]
						.setFreeParkingAvailableResult(FP_MODEL_NO_REIMBURSEMENT_CHOICE);
				person[i]
						.setParkingReimbursement(FP_MODEL_NO_REIMBURSEMENT_CHOICE);

			} else if (workLoc > 0
					&& mgraParkArea[workLoc] == MgraDataManager.PARK_AREA_ONE) {
				...
				if (chosen == FP_MODEL_REIMB_ALT) {
					double logReimbPct = meanReimb + hhRandom.nextGaussian()
							* stdDevReimb;
	...
	private int getParkingChoice(Person personObj, double randomNumber) {
		...
		fpDmuObject.setMgraParkArea(mgraParkArea[personObj
				.getUsualWorkLocation()]);
		fpDmuObject
				.setNumFreeHours(numfreehrs[personObj.getUsualWorkLocation()]);
		fpDmuObject.setLsWgtAvgCostM(lsWgtAvgCostM[personObj
				.getUsualWorkLocation()]);
		fpDmuObject.setLsWgtAvgCostD(lsWgtAvgCostD[personObj
				.getUsualWorkLocation()]);
		fpDmuObject.setLsWgtAvgCostH(lsWgtAvgCostH[personObj
				.getUsualWorkLocation()]);
		...
		fpDmuObject.setDmuIndexValues(hhObj.getHhId(),
				personObj.getUsualWorkLocation(), hhObj.getHhTaz(),
				personObj.getUsualWorkLocation());
		...
		if (fpModel.getAvailabilityCount() > 0) {
			...
			String decisionMaker = String.format("HHID=%d, PERSID=%d",
					hhObj.getHhId(), personObj.getPersonId());
		...
		if (hhObj.getDebugChoiceModels()) {
			String decisionMaker = String.format("HHID=%d, PERSID=%d",
					hhObj.getHhId(), personObj.getPersonId());
			fpModel.logAlternativesInfo("Free parking Choice", decisionMaker,
					logger);
			logger.info(String.format(
					"%s result chosen for %s is %d with rn %.8f",
					"Free parking Choice", decisionMaker, chosenAlt,
					randomNumber));
			fpModel.logUECResults(logger, decisionMaker);
		}
