 ...
 * 
 */
public class HouseholdCoordinatedDailyActivityPatternModel implements
		Serializable {
	...
	private static final String[] activityNameArray = { MANDATORY_PATTERN,
			NONMANDATORY_PATTERN, HOME_PATTERN };
	...
	private UtilityExpressionCalculator onePersonUec, twoPeopleUec,
			threePeopleUec, allMemberInteractionUec, jointUec;

	public HouseholdCoordinatedDailyActivityPatternModel(
		...
		createLogitModels();
		setupCoordinatedDailyActivityPatternModelApplication(propertyMap,
				dmuFactory);
	...
	private void setupCoordinatedDailyActivityPatternModelApplication(
		...
		// create the uecs
		onePersonUec = new UtilityExpressionCalculator(new File(cdapUecFile),
				onePersonPage, dataPage, propertyMap,
				(VariableTable) cdapDmuObject);
		twoPeopleUec = new UtilityExpressionCalculator(new File(cdapUecFile),
				twoPersonPage, dataPage, propertyMap,
				(VariableTable) cdapDmuObject);
		threePeopleUec = new UtilityExpressionCalculator(new File(cdapUecFile),
				threePersonPage, dataPage, propertyMap,
				(VariableTable) cdapDmuObject);
		allMemberInteractionUec = new UtilityExpressionCalculator(new File(
				cdapUecFile), allPersonPage, dataPage, propertyMap,
				(VariableTable) cdapDmuObject);
		jointUec = new UtilityExpressionCalculator(new File(cdapUecFile),
				jointPage, dataPage, propertyMap, (VariableTable) cdapDmuObject);

		// get the proportions by person type
		double[][] fixedRelativeProportions = modelStructure
				.getCdap6PlusProps();
	...
	public void applyModel(Household hhObject) {
		...
		if (hhObject.getDebugChoiceModels())
			hhObject.logHouseholdObject(
					"Pre CDAP Household " + hhObject.getHhId() + " Object",
					cdapLogger);
		...
		if (hhObject.getDebugChoiceModels()) {
			...
			cdapLogger
					.info("CDAP # Type FT W PT W UNIV NONW RETR SCHD SCHN PRES");
			cdapLogger
					.info("------ ---- ---- ---- ---- ---- ---- ---- ---- ----");
			...
			for (int j = 1; j < personArray.length; ++j) {
				...
				for (int jj = 1; jj < tempPersonArray.length; jj++) {
					if (tempPersonArray[jj].getPersonNum() == personArray[j]
							.getPersonNum())
						persNumMatch = true;
				}
				...
				if (personArray[j].getPersonTypeIsFullTimeWorker() == 1) {
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"FT W", pString, bString, bString, bString,
					...
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"PT W", bString, pString, bString, bString,
							bString, bString, bString, bString);
				}
					...
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"UNIV", bString, bString, pString, bString,
							bString, bString, bString, bString);
				}
					...
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"NONW", bString, bString, bString, pString,
							bString, bString, bString, bString);
				}
					...
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"RETR", bString, bString, bString, bString,
							pString, bString, bString, bString);
				}

				else if (personArray[j].getPersonIsStudentDriving() == 1) {
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"SCHD", bString, bString, bString, bString,
							bString, pString, bString, bString);
				}

				else if (personArray[j].getPersonIsStudentNonDriving() == 1) {
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"SCHN", bString, bString, bString, bString,
							bString, bString, pString, bString);
				}

				else if (personArray[j].getPersonIsPreschoolChild() == 1) {
					stringToLog = String.format(
							"%6s%5s%5s%5s%5s%5s%5s%5s%5s%5s", persNumString,
							"PRES", bString, bString, bString, bString,
							bString, bString, bString, pString);
				}
		...
		for (int i = 0; i < MAX_MODEL_HH_SIZE; ++i) {
			...
			while (numberOfAltsCounter < numberOfAlternatives) {
				...
				for (int j = 0; j < hhSize; ++j) {
					...
					if (!activityNameArray[counterForEachPerson[j]]
							.equalsIgnoreCase(HOME_PATTERN))
						numOutOfHomeActivites++;
				}

				// create the alternative and add it to the model
				if (numOutOfHomeActivites < 2) {
					ConcreteAlternative tempAlt = new ConcreteAlternative(
							alternativeName + "0", totalAltsCounter);
					...
					tempAlt = new ConcreteAlternative(alternativeName + "j",
							totalAltsCounter);
					workingLogitModel.addAlternative(tempAlt);
					totalAltsCounter++;
				...
				for (int j = 0; j < hhSize; ++j) {
					...
					if (counterForEachPerson[j] == activityNameArray.length)
						counterForEachPerson[j] = 0;
					else
						break;
				}
	 ...
	 */
	public String getCoordinatedDailyActivityPatternChoice(
		...
		ArrayList<Alternative> alternativeList = workingLogitModel
				.getAlternatives();
		...
		float retAccess = accTable.getAggregateAccessibility(
				accStrings[householdObject.getAutoSufficiency()],
				householdObject.getHhMgra());
		...
		for (int i = 0; i < modelHhSize; ++i) {
			...
			if (workMgra > 0
				...
				cdapDmuObject.setWorkLocationModeChoiceLogsumA(personA
						.getWorkLocationLogsum());
			else
				cdapDmuObject.setWorkLocationModeChoiceLogsumA(0.0);
			...
			if (studentType > 0 && schoolMgra > 0) {
				cdapDmuObject.setSchoolLocationModeChoiceLogsumA(personA
						.getSchoolLocationLogsum());
			} else {
				cdapDmuObject.setSchoolLocationModeChoiceLogsumA(0.0);
			}

			// compute the single person utilities
			double[] firstPersonUtilities = onePersonUec
					.solve(cdapDmuObject.getIndexValues(), cdapDmuObject,
							availability);

			// log these utilities for trace households
			if (householdObject.getDebugChoiceModels()) {

				String stringToLog = String.format("%-30s%9d%9s%9s",
						"OnePerson", (i + 1), "--", "--");

				for (int j = 0; j < activityNameArray.length; ++j) {
					stringToLog += String.format("%10.4f",
							firstPersonUtilities[j]);
				}
				cdapLogger.info(stringToLog);

				cdapUecLogger.info("PersonA:");
				personA.logEntirePersonObject(cdapUecLogger);
				onePersonUec.logAnswersArray(
						cdapUecLogger,
						"ONE PERSON, personA personNum="
			...
			for (int j = 0; j < alternativeList.size(); ++j) {
				...
				for (int k = 0; k < activityNameArray.length; ++k) {
					...
					if (altNameForPersonA
							.equalsIgnoreCase(activityNameArray[k])) {
						double currentUtility = tempAlt.getUtility();
						tempAlt.setUtility(currentUtility
								+ firstPersonUtilities[k]);
					}
				} // k
			...
			for (int j = 0; j < modelHhSize; ++j) {
				...
				double[] twoPersonUtilities = twoPeopleUec.solve(
						cdapDmuObject.getIndexValues(), cdapDmuObject,
						availability);

				// log these utilities for trace households
				if (householdObject.getDebugChoiceModels()) {

					String stringToLog = String.format("%-30s%9d%9d%9s",
							"TwoPeople", (i + 1), (j + 1), "--");

					for (int k = 0; k < activityNameArray.length; ++k) {
						stringToLog += String.format("%10.4f",
								twoPersonUtilities[k]);
					}
					cdapLogger.info(stringToLog);
					...
					twoPeopleUec.logAnswersArray(
							cdapUecLogger,
							"TWO PERSON, personA personNum="
									+ personA.getPersonNum()
									+ " personB personNum="
				...
				for (int k = 0; k < alternativeList.size(); ++k) {
					...
					for (int l = 0; l < activityNameArray.length; ++l) {
						if (altNameForPersonA
								.equalsIgnoreCase(activityNameArray[l])
								&& altNameForPersonB
										.equalsIgnoreCase(activityNameArray[l])) {
							double currentUtility = tempAlt.getUtility();
							tempAlt.setUtility(currentUtility
									+ twoPersonUtilities[l]);
						}
					} // l
				} // k
				...
				for (int k = 0; k < modelHhSize; ++k) {
					...
					if (householdObject.getDebugChoiceModels()) {
						...
						String stringToLog = String.format("%-30s%9d%9d%9d",
								"ThreePeople", (i + 1), (j + 1), (k + 1));

						for (int l = 0; l < activityNameArray.length; ++l) {
							stringToLog += String.format("%10.4f",
									threePersonUtilities[l]);
						}
						cdapLogger.info(stringToLog);
						...
						threePeopleUec.logAnswersArray(
								cdapUecLogger,
								"THREE PERSON, personA personNum="
										+ personA.getPersonNum()
										+ " personB personNum="
										+ personB.getPersonNum()
										+ " personC personNum="
										+ personC.getPersonNum());
					...
					for (int l = 0; l < alternativeList.size(); ++l) {
						Alternative tempAlt = (Alternative) alternativeList
								.get(l);
						String altName = tempAlt.getName();
						...
						for (int m = 0; m < activityNameArray.length; ++m) {
							if (altNameForPersonA
									.equalsIgnoreCase(activityNameArray[m])
									&& altNameForPersonB
											.equalsIgnoreCase(activityNameArray[m])
									&& altNameForPersonC
											.equalsIgnoreCase(activityNameArray[m])) {
		...
		if (householdObject.getDebugChoiceModels()) {
			...
			for (int i = 0; i < activityNameArray.length; ++i) {
				stringToLog += String.format("%10.4f",
						allMemberInteractionUtilities[i]);
			}
			cdapLogger.info(stringToLog);

			allMemberInteractionUec.logAnswersArray(cdapUecLogger,
		...
		for (int i = 0; i < alternativeList.size(); ++i) {
			...
			for (int j = 0; j < activityNameArray.length; ++j) {
				...
				for (int k = 0; k < modelHhSize; ++k) {
					...
					if (altNameForThisPerson
							.equalsIgnoreCase(activityNameArray[j]))
						continue;
					else {
						samePattern = false;
						break;
				...
				if (samePattern) {
					double currentUtility = tempAlt.getUtility();
					tempAlt.setUtility(currentUtility
							+ allMemberInteractionUtilities[j]);
				}
			} // j
		...
		for (int i = 0; i < alternativeList.size(); ++i) {
			...
			for (int k = 0; k < modelHhSize; ++k) {
				...
				if (altNameForThisPerson.equalsIgnoreCase(MANDATORY_PATTERN)) {
					...
					if (workMgra > 0
						...
						workLocationAccessibilityForWorkers += tempPerson
								.getWorkLocationLogsum();
					}

				} else if (altNameForThisPerson
						.equalsIgnoreCase(NONMANDATORY_PATTERN)) {
					if (isThisCdapPersonAnAdult(k + 1)) {
						adultsWithNonMand++;
						adultsLeaveHome++;
					} else {
			...
			cdapDmuObject
					.setWorkAccessForMandatoryDap(workLocationAccessibilityForWorkers);

			double[] jointUtilities = jointUec
					.solve(cdapDmuObject.getIndexValues(), cdapDmuObject,
							availability);

			// log these utilities for trace households
			if (householdObject.getDebugChoiceModels()) {
				String stringToLog = String.format("%-13s%4d %-12s%9s%9s%9s",
						"Joint", (i + 1), altName, "--", "--", "--");
				stringToLog += String.format("%10.4f",
						(altName.indexOf("j") > 0 ? jointUtilities[0] : 0.0));
				cdapLogger.info(stringToLog);

				jointUec.logAnswersArray(cdapUecLogger,
						"JOINT Utility for CDAP Alt index = " + (i + 1)
								+ ", Alt name = " + altName);
			} // debug trace
		...
		try {
			...
			if (householdObject.getDebugChoiceModels()) {
				...
				cdapLogger
						.info("chosen pattern (5 or fewer hh members): Alt index = "
								+ chosenIndex
								+ ", Alt name = "
								+ firstFiveChosenName);
				cdapLogger.info("");
		...
		if (actualHhSize > MAX_MODEL_HH_SIZE) {
			...
			String allMembersChosenPattern = applyModelForExtraHhMembers(
					householdObject, firstFiveChosenName);
			...
			allMembersChosenPattern = allMembersChosenPattern.substring(0,
					extraCharIndex)
			...
			for (int i = 1; i < cdapPersonArray.length; i++) {
				...
				finalHhPatternActivities[k] = allMembersChosenPattern
						.substring(i - 1, i);
			}
			...
			if (householdObject.getDebugChoiceModels()) {
				cdapLogger.info("final pattern (more than 5 hh members) = "
						+ finalString);
				cdapLogger.info("");
				cdapLogger.info("");
			}
	...
	private String applyModelForExtraHhMembers(Household householdObject,
		...
		for (int i = 1; i < personArray.length; i++) {
			...
			int chosen = ChoiceModelApplication.getMonteCarloSelection(
					fixedCumulativeProportions[personType], householdObject
							.getHhRandom().nextDouble());
	...
	protected Person getCdapPerson(int persNum) {
		if (persNum < 1 || persNum > cdapPersonArray.length - 1) {
			logger.fatal(String.format(
					"persNum value = %d is out of range for hhSize = %d",
	...
	public Person[] getPersonsNotModeledByCdap(int personsModeledByCdap) {
		...
		if (cdapPersonArray.length > personsModeledByCdap + 1)
			personArray = new Person[cdapPersonArray.length
	...
	public boolean isThisCdapPersonAnAdult(int cdapPersonNumber) {
		...
		if (cdapPersonArray[cdapPersonNumber].getPersonIsAdult() == 1)
			return true;
		return false;
	}
	...
	public int getWorkLocationForThisCdapPerson(int cdapPersonNumber) {
		...
		if (loc > 0 && loc != ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR)
			return loc;
		else
			return 0;
	}
