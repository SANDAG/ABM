...
public class AirportModeChoiceModel {
	   private transient Logger logger = Logger.getLogger("airportModel");
		...
		public AirportModeChoiceModel(HashMap<String,String> rbMap, AirportDmuFactoryIf dmuFactory){
	        ...
	        String airportModeUecFileName = Util.getStringValueFromPropertyMap(rbMap, "airport.mc.uec.file");
	        airportModeUecFileName = uecFileDirectory + airportModeUecFileName;
	        
	        int dataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "airport.mc.data.page" ) );
	        int daPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,"airport.mc.da.page" ) );
	        int s2Page = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "airport.mc.s2.page" ) );
	        int s3Page = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,"airport.mc.s3.page" ) );
	        int transitPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "airport.mc.transit.page" ) );
	        int accessPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap,"airport.mc.accessMode.page" ) );
            ...
            driveAloneModel = new ChoiceModelApplication( airportModeUecFileName, daPage, dataPage, rbMap, (VariableTable)dmu );

            // create a ChoiceModelApplication object for shared 2 mode choice
            shared2Model = new ChoiceModelApplication( airportModeUecFileName, s2Page, dataPage, rbMap, (VariableTable)dmu );
            
            // create a ChoiceModelApplication object for shared 3+ mode choice
            shared3Model = new ChoiceModelApplication( airportModeUecFileName, s3Page, dataPage, rbMap, (VariableTable)dmu );

            // create a ChoiceModelApplication object for transit mode choice
            transitModel = new ChoiceModelApplication( airportModeUecFileName, transitPage, dataPage, rbMap, (VariableTable)dmu );

            // create a ChoiceModelApplication object for access mode choice
            accessModel = new ChoiceModelApplication( airportModeUecFileName, accessPage, dataPage, rbMap, (VariableTable)dmu );
		 ...
		 * 
		 * @param party  The travel party
		 * @param dmu    An airport model DMU
		 */
		public void chooseMode(AirportParty party, AirportModelDMU dmu){
			...
			int skimPeriod = AirportModelStructure.getSkimPeriodIndex(period)+1;  // The skims are stored 1-based...don't ask...
			boolean debug = party.getDebugChoiceModels();
			
			// calculate best tap pairs for this airport party
			int[][] walkTransitTapPairs = wtw.getBestTapPairs(origMgra, destMgra, skimPeriod, debug, logger);
			party.setBestWtwTapPairs(walkTransitTapPairs);
			
			//drive transit tap pairs depend on direction; departing parties use drive-transit-walk, else walk-transit-drive is used.
			int[][] driveTransitTapPairs;
			if(party.getDirection()==AirportModelStructure.DEPARTURE){
				driveTransitTapPairs = dtw.getBestTapPairs(origMgra, destMgra, skimPeriod, debug, logger);
				party.setBestDtwTapPairs(driveTransitTapPairs);
			}else{
				driveTransitTapPairs = wtd.getBestTapPairs(origMgra, destMgra, skimPeriod, debug, logger);
			...
			}else if(party.getSize()==2){  // if 2-person party solve for the shared 2 and shared 3+ logsums
				shared2Model.computeUtilities(dmu, dmu.getDmuIndex());
				double shared2Logsum = shared2Model.getLogsum();
				dmu.setShared2Logsum(shared2Logsum);
		 ...
		 * 
		 * @param airportParties  An array of travel parties, with destinations already chosen.
		 * @param dmuFactory      A DMU Factory.
		 */
