...
public class AirportDestChoiceModel {
	...
	private double[][][] mgraProbabilities;  //by segment, tazNumber, mgra index (sequential, 0-based)
	private double[][] tazProbabilities;     //by segment, origin taz 0-based (note since airport model, only 1 taz dimension is needed)
	...
	public AirportDestChoiceModel(HashMap<String,String> rbMap, AirportDmuFactoryIf dmuFactory){
        ...
        String airportDistUecFileName = Util.getStringValueFromPropertyMap(rbMap, "airport.dc.uec.file");
        airportDistUecFileName = uecFileDirectory + airportDistUecFileName;
        
        int dataPage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "airport.dc.data.page" ) );
        int sizePage = Integer.parseInt( Util.getStringValueFromPropertyMap(rbMap, "airport.dc.size.page" ) );
        ...
        for(int i = 0; i < AirportModelStructure.INTERNAL_PURPOSES; ++i){
            ...
            destModel[i] = new ChoiceModelApplication( airportDistUecFileName, destModelPage, dataPage, rbMap, (VariableTable)dcDmu );
        }
        ...
        sizeTermUEC = new UtilityExpressionCalculator(new File(airportDistUecFileName), sizePage, dataPage, rbMap, dmuFactory.getAirportModelDMU());
        	...
        	for(int taz = 0; taz < tazs.length; ++taz){
        		...
        		double lastProb = 0.0;
           		for(int mgra = 0; mgra < mgraArray.length; ++ mgra){
        			...
        			int mgraNumber = mgraArray[mgra];
           			if(sum>0.0)
           				mgraProbabilities[segment][tazNumber][mgra] = lastProb + mgraSizeTerms[segment][mgraNumber]/sum;
           			lastProb = mgraProbabilities[segment][tazNumber][mgra];
           		}
           		if(sum>0.0 && Math.abs(lastProb-1.0)> 0.000001)
           			logger.info("Error: segment " + segment + " taz "+tazNumber+" cum prob adds up to "+lastProb);
        	}
	...
	/**
	 * Calculate the zip codes at a taz level from the mgra data file.
	 * This requires the mgra data to be specified as mgra.socec.file in the properties file.
	 * The mgra file must have four fields:  zone, taz, pop, and zip
	 * The taz zip is coded based upon the highest population mgra within the taz.
	 * 
	...
	public void calculateZipCodes(){
		...
		for(int i = 0; i< tazs.length; ++ i){
			...
			//if iterate through mgra data, and no mgras with pop found, then choose zip of first mgra in taz
			if( zip == 0 ){
				for(int row = 1; row<=mgraTable.getRowCount();++row){
	...
	public void calculateTazProbabilities(AirportDmuFactoryIf dmuFactory){
        ...
        //Note: this is an aggregate model to calculate utilities, but we need income so we need a party object
        ...
        //segments are combinations of 4 purposes and 8 income groups, which apply only to resident purposes
        for(int segment = 0; segment < segments; ++ segment){
        	...
        	// set airport taz as origin.  Destination tazs controlled by alternative file.
            ...
            tazProbabilities[segment] = Arrays.copyOf(destModel[purpose].getCumulativeProbabilities(), destModel[purpose].getCumulativeProbabilities().length);
        }
	...
	/**
	 * Iterate through an array of AirportParty objects, choosing origin MGRAs for each and
	 * setting the result back in the objects.
	 * 
	 * @param airportParties  An array of AirportParty objects
	 */
	public void chooseOrigins(AirportParty[] airportParties){
		...
		for(AirportParty party : airportParties){
			...
			//if this is a departing travel party, the origin is the chosen mgra, and the destination is the airport terminal
