...
public class AirportModelDMU implements Serializable, VariableTable {
	...
	protected String[][] departArriveCombinationLabels = { { "EA", "EA" },
			{ "EA", "AM" }, { "EA", "MD" }, { "EA", "PM" }, { "EA", "EV" },
			{ "AM", "AM" }, { "AM", "MD" }, { "AM", "PM" }, { "AM", "EV" },
			{ "MD", "MD" }, { "MD", "PM" }, { "MD", "EV" }, { "PM", "PM" },
			{ "PM", "EV" }, { "EV", "EV" } };

	protected int[][] departArriveCombinations = { { EA, EA }, { EA, AM },
			{ EA, MD }, { EA, PM }, { EA, EV }, { AM, AM }, { AM, MD },
			{ AM, PM }, { AM, EV }, { MD, MD }, { MD, PM }, { MD, EV },
			{ PM, PM }, { PM, EV }, { EV, EV } };
	...
	public void setDmuSkimCalculators(WalkTransitWalkSkimsCalculator myWtw,
			WalkTransitDriveSkimsCalculator myWtd,
			DriveTransitWalkSkimsCalculator myDtw) {
		wtw = myWtw;
		wtd = myWtd;
		dtw = myDtw;
	}
	 ...
	 */
	protected void setTransitSkim(int accEgr, int lbPrem, int skimIndex,
			double value) {
	 ...
	 *            Debug flag
	 */
	public void setDmuSkimAttributes(int origMgra, int destMgra,
			int departPeriod, boolean isInbound, boolean debugFlag) {
		...
		setWtwDmuAttributes(origMgra, destMgra, departPeriod, bestTapPairs,
				debug);

		// outbound (from origin to airport) sets drive-transit-walk skims
		if (!isInbound) {
			bestTapPairs = airportParty.getBestDtwTapPairs();
			setDtwDmuAttributes(origMgra, destMgra, departPeriod, bestTapPairs,
					debug);
		} else { // return (from airport to origin) sets walk-transit-drive
					// skims
			bestTapPairs = airportParty.getBestWtdTapPairs();
			setWtdDmuAttributes(origMgra, destMgra, departPeriod, bestTapPairs,
					debug);
		}
	 ...
	 *            True to debug
	 */
	protected void setWtwDmuAttributes(int origMgra, int destMgra,
		...
		int skimPeriodIndex = AirportModelStructure
				.getSkimPeriodIndex(departPeriod);
	 ...
	 *            True to debug
	 */
	protected void setWtdDmuAttributes(int origMgra, int destMgra,
			int departPeriod, int[][] bestTapPairs, boolean loggingEnabled) {
	 ...
	 *            True to debug
	 */
	protected void setDtwDmuAttributes(int origMgra, int destMgra,
			int departPeriod, int[][] bestTapPairs, boolean loggingEnabled) {
