...
public class AirportModel {
	...
	public static void main(String[] args) {
        ...
        try
        {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
            {
                ...
                if (matrixServerAddress.equalsIgnoreCase("localhost")) {
                    matrixServer = airportModel.startMatrixServerProcess(matrixServerAddress, serverPort, mt);
                    airportModel.ms = matrixServer;
                }
                else {
                	airportModel.ms = new MatrixDataServerRmi( matrixServerAddress, serverPort, MatrixDataServer.MATRIX_DATA_SERVER_NAME );
                	airportModel.ms.testRemote(Thread.currentThread().getName());
                	airportModel.ms.start32BitMatrixIoServer( mt );
        ...
        if (matrixServerAddress.equalsIgnoreCase("localhost")) {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
                airportModel.ms.stop32BitMatrixIoServer();
        }
