...
public class AirportPartyManager {
    ...
    public AirportPartyManager(HashMap<String,String> rbMap){
        ...
        String purposeFile = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.purpose.file");
        String sizeFile = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.size.file");
        String durationFile = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.duration.file");
        String incomeFile = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.income.file");
        String departFile = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.departureTime.file");
        String arriveFile = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.arrivalTime.file");
        ...
        float enplanements = new Float(Util.getStringValueFromPropertyMap(rbMap,"airport.enplanements").replace(",", ""));
        float connectingPassengers = new Float(Util.getStringValueFromPropertyMap(rbMap,"airport.connecting").replace(",", ""));
        float annualFactor  = new Float(Util.getStringValueFromPropertyMap(rbMap,"airport.annualizationFactor"));
        float averageSize = new Float(Util.getStringValueFromPropertyMap(rbMap,"airport.averageSize"));
    ...
    /**
     * Create parties based upon total parties (calculated in constructor).  Fill parties[] with
     * travel parties, assuming one-half are arriving and one-half are departing.  Simulate
     * party characteristics (income, size, duration, time departing from origin or arriving at airport)
     * from distributions, also read in during constructor.
     * 
     */
	public void generateAirportParties(){
		...
		for(int i=0;i<departures;++i){
			...
			byte period  = (byte) chooseFromDistribution(purpose, departureDistribution, party.getRandom());
		...
		for(int i=0;i<arrivals;++i){
			...
			byte nights  = (byte) chooseFromDistribution(purpose, durationDistribution, party.getRandom());
			byte income  = (byte) chooseFromDistribution(purpose, incomeDistribution, party.getRandom());
			byte period  = (byte) chooseFromDistribution(purpose, arrivalDistribution, party.getRandom());
	...
	/**
	 * Read file containing probabilities by purpose.  Store cumulative distribution
	 * in purposeDistribution.
	 * 
	 * @param fileName  Name of file containing two columns, one row for each purpose.
	 *                  First column has purpose number, second column has probability.
	 */
	 ...
	 * 
	 * This method is used for files that contain 2-dimensional probabilities
	 * by airport purpose and some other dimension, such as income, party size, 
	 * or departure time. 
	 * 
	 * @param probArray  The array to store the cumulative probabilities in
	 * @param fileName   The name of the file containing the probability distribution.
	 */
	protected double[][] setDistribution(double[][] probArray, String fileName){
		...
		if(cols<(purposes+1)){
			logger.fatal("Error:  Number of columns in "+fileName+" is less than "+purposes+1);
	 ...
	 * 
	 * @param purpose  The airport model purpose, used to index into probability array.
	 * @param probs    A 2-dimensional array of cumulative probabilities, where the first
	 * 				   dimension is number of alternative (0-based) and second column is
	 *                 airport model purpose.
	 * @param random   A uniform random number
	...
	public void writeOutputFile(HashMap<String,String> rbMap){
		...
		String directory = Util.getStringValueFromPropertyMap(rbMap,"Project.Directory");
        String fileName = directory+Util.getStringValueFromPropertyMap(rbMap,"airport.output.file");
		...
		String headerString = new String("id,direction,purpose,size,income,nights,departTime,originMGRA,destinationMGRA,tripMode,arrivalMode,boardingTAP,alightingTAP\n");
		...
		for(int i = 0; i < parties.length;++i){
			...
			String record = new String(
				parties[i].getID() + "," +	
				parties[i].getDirection() + "," +
				parties[i].getPurpose() + "," + 
				parties[i].getSize() + "," +
				parties[i].getIncome() + "," +	
				parties[i].getNights() + "," +
				parties[i].getDepartTime() + "," +
				parties[i].getOriginMGRA() + "," +
				parties[i].getDestinationMGRA() + "," +
				parties[i].getMode() + "," +
				parties[i].getArrivalMode() + "," +
				taps[0] + "," + taps[1] + "\n"
			);
			writer.print(record);
		}
	 ...
	 * 
	 * @param party  The airport travel party
	...
	public int[] getTapPair(AirportParty party){
		...
		if(sandagStructure.getTripModeIsWalkTransit(tripMode))
			taps =  party.getWtwTapPair(rideMode);
		else if(sandagStructure.getTripModeIsKnrTransit(tripMode))
			if(party.getDirection()==modelStructure.ARRIVAL)
				taps = party.getWtdTapPair(rideMode);
			else
				taps = party.getDtwTapPair(rideMode);
