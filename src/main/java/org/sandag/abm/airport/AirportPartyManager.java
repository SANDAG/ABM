...
public class AirportPartyManager {
	...
	public AirportPartyManager(HashMap<String, String> rbMap) {
		...
		String purposeFile = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.purpose.file");
		String sizeFile = directory
				+ Util.getStringValueFromPropertyMap(rbMap, "airport.size.file");
		String durationFile = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.duration.file");
		String incomeFile = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.income.file");
		String departFile = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.departureTime.file");
		String arriveFile = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.arrivalTime.file");

		// Read the distributions
		setPurposeDistribution(purposeFile);
		sizeDistribution = setDistribution(sizeDistribution, sizeFile);
		durationDistribution = setDistribution(durationDistribution,
				durationFile);
		incomeDistribution = setDistribution(incomeDistribution, incomeFile);
		departureDistribution = setDistribution(departureDistribution,
				departFile);
		arrivalDistribution = setDistribution(arrivalDistribution, arriveFile);

		// calculate total number of parties
		float enplanements = new Float(Util.getStringValueFromPropertyMap(
				rbMap, "airport.enplanements").replace(",", ""));
		float connectingPassengers = new Float(Util
				.getStringValueFromPropertyMap(rbMap, "airport.connecting")
				.replace(",", ""));
		float annualFactor = new Float(Util.getStringValueFromPropertyMap(
				rbMap, "airport.annualizationFactor"));
		float averageSize = new Float(Util.getStringValueFromPropertyMap(rbMap,
				"airport.averageSize"));

		float directPassengers = (enplanements - connectingPassengers)
				/ annualFactor;
	...
	public void generateAirportParties() {
		...
		for (int i = 0; i < departures; ++i) {
			...
			byte size = (byte) chooseFromDistribution(purpose,
					sizeDistribution, party.getRandom());
			byte nights = (byte) chooseFromDistribution(purpose,
					durationDistribution, party.getRandom());
			byte income = (byte) chooseFromDistribution(purpose,
					incomeDistribution, party.getRandom());
			byte period = (byte) chooseFromDistribution(purpose,
					departureDistribution, party.getRandom());
		...
		for (int i = 0; i < arrivals; ++i) {
			...
			byte income = (byte) chooseFromDistribution(purpose,
					incomeDistribution, party.getRandom());
			byte period = (byte) chooseFromDistribution(purpose,
					arrivalDistribution, party.getRandom());
	...
	protected double[][] setDistribution(double[][] probArray, String fileName) {
		...
		if (cols < (purposes + 1)) {
			logger.fatal("Error:  Number of columns in " + fileName
					+ " is less than " + purposes + 1);
		...
		for (int purp = 0; purp < purposes; ++purp) {
			...
			int col = probabilityTable.getColumnPosition("purp" + purp
					+ "_perc");
			double total_prob = 0.0;
	 ...
	 */
	protected int chooseFromDistribution(int purpose, double[][] probs,
			double random) {
	...
	public void writeOutputFile(HashMap<String, String> rbMap) {
		...
		for (int i = 0; i < parties.length; ++i) {
			...
			String record = new String(parties[i].getID() + ","
					+ parties[i].getDirection() + "," + parties[i].getPurpose()
					+ "," + parties[i].getSize() + "," + parties[i].getIncome()
					+ "," + parties[i].getNights() + ","
					+ parties[i].getDepartTime() + ","
					+ parties[i].getOriginMGRA() + ","
					+ parties[i].getDestinationMGRA() + ","
					+ parties[i].getMode() + "," + parties[i].getArrivalMode()
					+ "," + taps[0] + "," + taps[1] + "\n");
			writer.print(record);
		}
		writer.close();
	...
	public int[] getTapPair(AirportParty party) {
		...
		int rideMode = sandagStructure.getRideModeIndexForTripMode(party
				.getMode());
