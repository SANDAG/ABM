...
public class AirportTripTables {
	...
	public void processTrips(int timePeriod, TableDataSet tripData) {
		...
		for (int i = 1; i <= tripData.getRowCount(); ++i) {
			...
			int destinationMGRA = (int) tripData.getValueAt(i,
					"destinationMgra");
			...
			if (accMode == AirportModelStructure.PUDO_CURB
				...
				if (SandagModelStructure.getTripModeIsPay(tripMode)) // if the
					...
					mat = 0;
				float value = matrix[mode][mat].getValueAt(destinationTAZ,
						originTAZ);
				matrix[mode][mat].setValueAt(destinationTAZ, originTAZ,
						(value + vehicleTrips));
			}
	...
	public void writeTrips(int period, MatrixType mt) {
		...
		fileName[0] = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.results.autoTripMatrix") + end;
		fileName[1] = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.results.nMotTripMatrix") + end;
		fileName[2] = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.results.tranTripMatrix") + end;
		fileName[3] = directory
				+ Util.getStringValueFromPropertyMap(rbMap,
						"airport.results.othrTripMatrix") + end;
	...
	public static void main(String[] args) {
		...
		logger.info(String
				.format("SANDAG Airport Model Trip Table Generation Program using CT-RAMP version %s",
		...
		try {
			...
			if (!matrixServerAddress.equalsIgnoreCase("none")) {
				...
				if (matrixServerAddress.equalsIgnoreCase("localhost")) {
					matrixServer = tripTables.startMatrixServerProcess(
							matrixServerAddress, serverPort, mt);
					tripTables.ms = matrixServer;
				} else {
					tripTables.ms = new MatrixDataServerRmi(
