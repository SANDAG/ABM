...
public class AirportTripTables {
    ...
    private String purposeName[] = {"RES_BUS","RES_PER","VIS_BUS","VIS_PER"};
	...
	public void processTrips(int timePeriod, TableDataSet tripData){
      ...
      //iterate through the trip data and save trips in arrays
        for(int i = 1; i <= tripData.getRowCount(); ++i){
        	...
        	//party size is a variable in output file. it is used for transit trips (person trips)
        	...
        	// generate another drive-alone trip in the opposite direction for pickup/dropoff 
        	if(accMode == AirportModelStructure.PUDO_CURB ||accMode == AirportModelStructure.PUDO_CURB){
        		mode = 0; //auto mode
        		if(SandagModelStructure.getTripModeIsPay(tripMode))  //if the passenger chose pay, assume the driver will also pay
        			mat=1;
        		else
        			mat=0;
	...
	public void writeTrips(int period, MatrixType mt){
		...
		fileName[0] =directory+Util.getStringValueFromPropertyMap(rbMap,"airport.results.autoTripMatrix")+end;
		fileName[1] =directory+Util.getStringValueFromPropertyMap(rbMap,"airport.results.nMotTripMatrix")+end;
		fileName[2] =directory+Util.getStringValueFromPropertyMap(rbMap,"airport.results.tranTripMatrix")+end; 
		fileName[3] =directory+Util.getStringValueFromPropertyMap(rbMap,"airport.results.othrTripMatrix")+end; 
	...
	public static void main(String[] args) {
		...
		logger.info(String.format("SANDAG Airport Model Trip Table Generation Program using CT-RAMP version %s",
        ...
        try
        {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
            {
                ...
                if (matrixServerAddress.equalsIgnoreCase("localhost")) {
                    matrixServer = tripTables.startMatrixServerProcess(matrixServerAddress, serverPort, mt);
                    ...
                    tripTables.ms.testRemote(Thread.currentThread().getName());
                    tripTables.ms.start32BitMatrixIoServer( mt );
