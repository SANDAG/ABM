...
class BuildTransitNetwork(_m.Tool(), gen_utils.Snapshot):
    ...
    dst_emmebank =  _m.Attribute(_m.InstanceType)
    ...
    def __init__(self):
        ...
        self.attributes = [
            "period", "scenario_id", "dst_emmebank", "base_scenario", 
            "skims_only", "transfer_limit", "timed_xfers_table", 
            "scenario_title", "overwrite"]
    ...
    def from_snapshot(self, snapshot):
        ...
        self.dst_emmebank = project_databases.get(self.dst_emmebank, self.dst_emmebank)
        return self

    def page(self):
        pb = _m.ToolPageBuilder(self)
        pb.title = "Build transit network"
        pb.description = """Builds the transit network for the specified period based on exiting base (traffic + transit) scenario."""
        ...
        pb.add_select_database("dst_emmebank", 
    ...
    def run(self):
        ...
        try:
            results = self(
                self.period, self.base_scenario, self.dst_emmebank.core_emmebank, 
    ...
    def __call__(self, period, base_scenario, dst_emmebank, scenario_id, scenario_title="", 
                 timed_xfers_table=None, overwrite=False):
        modeller = _m.Modeller()
        attrs = {
            "period": period, 
            "scenario_id": scenario_id, 
            "dst_emmebank": dst_emmebank.path, 
        ...
        with _m.logbook_trace("Build transit network for period %s" % period, attributes=attrs):
            ...
            if dst_emmebank.scenario(scenario_id):
                if overwrite:
                    dst_emmebank.delete_scenario(scenario_id)
                else:
                    raise Exception("scenario_id: scenario %s already exists" % scenario_id)

            scenario = dst_emmebank.create_scenario(scenario_id)
