...
rem set JAR_LOCATION=%PROJECT_DIRECTORY%/lib
rem set LIB_JAR_PATH=%JAR_LOCATION%\sandag_abm_pb.jar
...
set CLASSPATH=%CONFIG%;%PROJECT_DIRECTORY%;%TRANSCAD_PATH%\GISDK\Matrices\TranscadMatrix.jar;%LIB_JAR_PATH%;%JAR_LOCATION%\*
...
start %JAVA_64_PATH%\bin\java -server -Dname=p%JAVA_32_PORT% -Xms20000m -Xmx20000m -cp "%CLASSPATH%" -Dlog4j.configuration=log4j_mtx.xml -DJAVA_HOME_32=%JAVA_32_PATH% -DJAVA_32_PORT=%JAVA_32_PORT% org.sandag.abm.ctramp.MatrixDataServer -hostname %HOST_IP_ADDRESS% -port %MATRIX_MANAGER_PORT% -ram 1500 -label "SANDAG Matrix Server"
