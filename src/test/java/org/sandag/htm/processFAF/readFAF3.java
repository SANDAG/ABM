...
/**
 * This class reads FAF3 data and stores data in a TableDataSet
 * Author: Rolf Moeckel, PB
 * Date: September 9, 2010
 */

public class readFAF3 {
    Logger logger = Logger.getLogger(readFAF3.class);
    private int factor;
    private String[] valueColumnName;
    private TableDataSet faf3commodityFlows;
    ...
    public void readAllData (ResourceBundle appRb, int year, String unit) {
        ...
        if (ResourceUtil.getBooleanProperty(appRb, "read.in.raw.faf.data", true))
            readAllFAF3dataSets(appRb, unit, year);
        readCommodityList(appRb);
        readFAF3referenceLists(appRb);
    }


    public void readCommodityList(ResourceBundle appRb) {
        // read commodity names
        TableDataSet sctgComList = fafUtils.importTable(ResourceUtil.getProperty(appRb, "faf3.sctg.commodity.list"));
    ...
    public static String getFAFzoneName(int fafInt) {
        ...
        return fafRegionList.getStringValueAt(domRegionIndex[fafInt], "FAF3 Zones -Short Description");
    }
    ...
    public void definePortsOfEntry(ResourceBundle appRb) {
        ...
        for (int row = 1; row <= poe.getRowCount(); row++) {
            int fafID = (int) poe.getValueAt(row, "faf3id");
        ...
        if (appRb.containsKey("marine.ports.of.entry")) {
            ...
            for (int row = 1; row <= mpoe.getRowCount(); row++) {
                int fafID = (int) mpoe.getValueAt(row, "faf3id");
        ...
        if (appRb.containsKey("rail.ports.of.entry")) {
            ...
            for (int row = 1; row <= rpoe.getRowCount(); row++) {
                int fafID = (int) rpoe.getValueAt(row, "faf3id");
        ...
        if (appRb.containsKey("air.ports.of.entry")) {
            ...
            for (int row = 1; row <= apoe.getRowCount(); row++) {
                int fafID = (int) apoe.getValueAt(row, "faf3id");
    ...
    public void readAllFAF3dataSets2007(ResourceBundle appRb, String unit) {
        // read all FAF3 data into TableDataSets in unit (= tons or dollars)

        logger.info ("Reading domestic FAF3 data in " + unit);
        if (unit.equals("tons")) {
            factor = 1000;    // tons are in 1,000s
            valueColumnName = new String[]{"tons_2007"};
        } else if (unit.equals("dollars")) {
            factor = 1000000;  // dollars are in 1,000,000s
            valueColumnName = new String[]{"value_2007"};
        } else {
            logger.fatal ("Wrong token " + unit + " in method readAllFAF3dataSets2007. Use tons or dollars.");
        }
        faf3commodityFlows = readFAF3commodityFlows(appRb, unit);
    }
    ...
    public double[] summarizeFlowByCommodity (ModesFAF fafMode) {
        ...
        for (int row = 1; row <= faf3commodityFlows.getRowCount(); row++) {
            if (faf3commodityFlows.getValueAt(row, "dms_mode") != modeNum) continue;
            int com = sctgStringIndex[(int) faf3commodityFlows.getValueAt(row, "sctg2")];
            if (valueColumnName.length == 1) {
                // use year provided by user
                totalFlows[com] += faf3commodityFlows.getValueAt(row, valueColumnName[0]);
            } else {
                // interpolate between two years
                float val1 = faf3commodityFlows.getValueAt(row, valueColumnName[0]);
                float val2 = faf3commodityFlows.getValueAt(row, valueColumnName[1]);
    ...
    public void readAllFAF3dataSets(ResourceBundle appRb, String unit, int year) {
        // read all FAF3 data into TableDataSets in unit (= tons or dollars)

        logger.info ("  Reading FAF3 data in " + unit);
        switch (unit) {
            case "tons":
            ...
            case "dollars":
                ...
                break;
            default:
                logger.fatal("Wrong token " + unit + " in method readAllFAF3dataSets. Use tons or dollars.");
                System.exit(0);
        }
        int[] availYears = {2007, 2015, 2020, 2025, 2030, 2035, 2040};
        boolean yearInFaf = false;
        for (int y: availYears) if (year == y) yearInFaf = true;
        if (!yearInFaf) {  // interpolate between two years
            logger.info("  Year " + year + " does not exist in FAF3 data.");
            int year1 = 2007;
            int year2 = 2040;
            ...
            logger.info("  FAF3 data are interpolated between " + year1 + " and " + year2 + ".");
    ...
    private TableDataSet readFAF3commodityFlows(ResourceBundle appRb, String unit) {
        // read FAF3 data and return TableDataSet with flow data
        String fileName = ResourceUtil.getProperty(appRb, ("faf3.data"));
    ...
    public void getFlowsByMode(String outFileName, ModesFAF mode, reportFormat repF, HashMap<String, Float> scaler) {
        ...
        for (int row = 1; row <= faf3commodityFlows.getRowCount(); row++) {
            int type = (int) faf3commodityFlows.getValueAt(row, "trade_type");
            double val;
            if (valueColumnName.length == 1) {
                // use year provided by user
                val = faf3commodityFlows.getValueAt(row, valueColumnName[0]);
            } else {
                // interpolate between two years
                float val1 = faf3commodityFlows.getValueAt(row, valueColumnName[0]);
                float val2 = faf3commodityFlows.getValueAt(row, valueColumnName[1]);
            ...
            else logger.info("Invalid trade_type in FAF3 dataset in row " + row + ": " + type);
        }
        outFile.close();
    }
    ...
    public void writeFlowsByModeAndCommodity (String outFileName, ModesFAF mode, reportFormat repF,
        ...
        for (int row = 1; row <= faf3commodityFlows.getRowCount(); row++) {
            ...
            val *= factor * odScaler(row, type, scaler);
            if (val == 0) continue;
            int comIndex = getIndexOfCommodity((int) faf3commodityFlows.getValueAt(row, "sctg2"));
    ...
    public float odScaler (int row, int type, HashMap<String, Float> scaler) {
        ...
        int dest;
        if (type == 1) {
            orig = (int) faf3commodityFlows.getValueAt(row, "dms_orig");
            dest = (int) faf3commodityFlows.getValueAt(row, "dms_dest");
        } else if (type == 2) {
            orig = (int) faf3commodityFlows.getValueAt(row, "fr_orig");
            dest = (int) faf3commodityFlows.getValueAt(row, "dms_dest");
        } else if (type == 3) {
            orig = (int) faf3commodityFlows.getValueAt(row, "dms_orig");
            dest = (int) faf3commodityFlows.getValueAt(row, "fr_dest");
        } else {
            orig = (int) faf3commodityFlows.getValueAt(row, "fr_orig");
            dest = (int) faf3commodityFlows.getValueAt(row, "fr_dest");
        }
    ...
    private int tryGettingThisValue(int row, String token) {
        ...
        int region;
        try {
            region = (int) faf3commodityFlows.getValueAt(row, token);
        } catch (Exception e) {
            region = 0;
        }
        return region;
    ...
    public void writeDomesticFlow (int modeNum, double val, int row, PrintWriter outFile) {
        // internal US flow
        if (faf3commodityFlows.getValueAt(row, "dms_mode") == modeNum) {
            int orig = (int) faf3commodityFlows.getValueAt(row, "dms_orig");
            int dest = (int) faf3commodityFlows.getValueAt(row, "dms_dest");
            int comm = (int) faf3commodityFlows.getValueAt(row, "sctg2");
    ...
    public void writeImportFlow (int modeNum, double val, int row, PrintWriter outFile, reportFormat repF) {
        ...
        int frInMode = (int) faf3commodityFlows.getValueAt(row, "fr_inmode");
        int borderZone = (int) faf3commodityFlows.getValueAt(row, "dms_orig");
        int comm = (int) faf3commodityFlows.getValueAt(row, "sctg2");
    ...
    public void writeExportFlow (int modeNum, double val, int row, PrintWriter outFile, reportFormat repF) {
        ...
        int borderZone = (int) faf3commodityFlows.getValueAt(row, "dms_dest");
        int comm = (int) faf3commodityFlows.getValueAt(row, "sctg2");
    ...
    public void writeThroughFlow(int modeNum, double val, int row, PrintWriter outFile, reportFormat repF) {
        ...
        if ((int) faf3commodityFlows.getValueAt(row, "dms_mode") != modeNum) return;
        int borderInZone = (int) faf3commodityFlows.getValueAt(row, "dms_orig");
        int borderOutZone = (int) faf3commodityFlows.getValueAt(row, "dms_dest");
        logger.warn("Through flows not yet implemented. This flow from " + borderInZone + " to " + borderOutZone + " is lost.");
    }


    public void readFAF3referenceLists(ResourceBundle rb) {
        // read list of regions for FAF3
        String regFileName = rb.getString("faf3.region.list");
    ...
    public TableDataSet getFAF3flows() {
        return faf3commodityFlows;
    }
    ...
    public TableDataSet getFaf3commodityFlows() {
        return faf3commodityFlows;
    }
