...
public class VisitorTripModeChoiceDMUTest {
	...
	public void testVisitorTripModeChoiceDMU() {
		...
		VisitorTripModeChoiceDMU dmu = new VisitorTripModeChoiceDMU(structure,
				CreateFakeLogger());
	}
	...
	private void TestTazParamsExpected(int i, int offset,
			VisitorTripModeChoiceDMU dmu, double expected) {
		double value = dmu.getValueForIndex(i + offset, 0);
		Assert.assertEquals(expected, value);
	...
	public void testBadIndex() {
		...
		SpecialEventTripModeChoiceDMU dmu = new SpecialEventTripModeChoiceDMU(
				structure, logger);
