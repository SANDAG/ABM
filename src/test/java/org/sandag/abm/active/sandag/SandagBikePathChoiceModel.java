...
// note this class is not thread safe! - use a ThreadLocal or something to
// isolate it amongst threads
public class SandagBikePathChoiceModel
{
    ...
    public SandagBikePathChoiceModel(final HashMap<String, String> propertyMap)
    {
        ...
        final int modelSheet = Integer.parseInt(propertyMap
                .get(PATH_CHOICE_MODEL_UEC_MODEL_SHEET_PROPERTY));
        final int dataSheet = Integer.parseInt(propertyMap
                .get(PATH_CHOICE_MODEL_UEC_DATA_SHEET_PROPERTY));
        ...
        model = new ThreadLocal<ChoiceModelApplication>()
        {
            ...
            {
                return new ChoiceModelApplication(uecSpreadsheet, modelSheet, dataSheet,
                        propertyMap, dmu.get());
            }
        };
    ...
    public double[] getPathProbabilities(Person person, SandagBikePathAlternatives paths,
            boolean inboundTrip, Tour tour, boolean debug)
    {
        applyPathChoiceModel(person, paths, inboundTrip, tour);
        double[] probabilities = Arrays
                .copyOf(model.get().getProbabilities(), paths.getPathCount());
        if (debug) debugPathChoiceModel(person, paths, inboundTrip, tour, probabilities);
        return probabilities;

    }

    public double getPathLogsums(Person person, SandagBikePathAlternatives paths,
            boolean inboundTrip, Tour tour, boolean debug)
    {
        applyPathChoiceModel(person, paths, inboundTrip, tour);
        if (debug)
    ...
    private void applyPathChoiceModel(Person person, SandagBikePathAlternatives paths,
            boolean inboundTrip, Tour tour)
    {
        ...
        model.get().computeUtilities(dmu, dmu.getDmuIndexValues(), pathAltsAvailable,
                pathAltsSample);
    }

    private void debugPathChoiceModel(Person person, SandagBikePathAlternatives paths,
            boolean inboundTrip, Tour tour, double[] probabilities)
    {
        // want: person id, tour id, inbound trip, path count, path
        // probabilities
        logger.info(String
                .format("debug of path choice model for person id %s, tour id %s, inbound trip: %s, path alternative count: %s",
