...
public class PathAlternativeList<N extends Node, E extends Edge<N>>
{
    ...
    public PathAlternativeList(NodePair<N> odPair, Network<N, E, ?> network,
            EdgeEvaluator<E> lengthEvaluator)
    {
    ...
    {
        if (!path.getNode(0).equals(odPair.getFromNode())
                || !path.getNode(path.getLength() - 1).equals(odPair.getToNode()))
        {
            throw new IllegalStateException(
                    "OD pair of path does not match that of path alternative list");
        }
        ...
        private void update()
        {
            ...
            for (N node : alternatives.get(index))
            {
                if (previous != null)
                {
                    ...
                    alternatives.setSizeMeasure(index, alternatives.getSizeMeasures().get(index)
                            + edgeLength / nUsingEdge);
                    for (Integer i : incidenceMap.get(edge).subList(0, nUsingEdge - 1))
                    {
                        decrement = edgeLength / lengths.get(i) / (nUsingEdge) / (nUsingEdge - 1);
                        alternatives.setSizeMeasure(i, alternatives.getSizeMeasures().get(i)
                                - decrement);
                    }
                }
                previous = node;
            }
            alternatives.setSizeMeasure(index,
                    alternatives.getSizeMeasures().get(index) / lengths.get(index));
        }
