...
def read_bike_net(
    ...
    edges = edges.assign(
        angle=edges.reset_index()
        .merge(nodes, left_on="fromNode", right_index=True)
        .merge(nodes, left_on="toNode", right_index=True, suffixes=("_from", "_to"))
        .set_index(["fromNode", "toNode"])
        .apply(
            lambda edge: calc_edge_angle(
                edge.x_from, edge.y_from, edge.x_to, edge.y_to
            ),
            axis="columns",
        )
    )
