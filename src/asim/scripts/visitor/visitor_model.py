...
class VisitorModel(TourEnumMixin,
    ...
    def __init__(self):
        # Find asim location
        self.parameters_path = os.path.join(configs, 'settings_visitor.yaml')
        self.processed_data = {}
        ...
        self.tables = load_tables(
            file_path=os.path.join(configs, 'prob_tables'),
            nested_dict=self.parameters['input_data']
        )

        # Add in the land_use table from data
        self.tables['land_use'] = pd.read_csv(
            os.path.join(data_dir, 'land_use.csv')
        )

    def tour_enum(self):
        # Generate tours
        print("Generating visitor tours")
        self.processed_data = self.create_tour_enumeration(self.tables, self.parameters, data_dir)
...
if __name__ == '__main__':
    ...
    if args.asim:
        ...
        # Find asim location
        asim_root = configs.replace('configs/visitor', '')

        # need to pass the configs, data, and output folders to sub process
        config_args = ['-c', configs,
                       '-c', configs.replace('visitor', 'common'),
                       '-d', data_dir,
                       '-d', output_dir.replace('visitor', 'skims'),
                       '-o', output_dir]
