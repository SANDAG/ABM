...
segments = {
    'test_small': {'MAZ': np.arange(MAZ_OFFSET + 492, MAZ_OFFSET + 550)},  # Warning! doesn't include college or gradeschool enrollment
...
def crop_omx(omx_file_name, zones, num_outfiles=1):
    ...
    labels = zones  # TAZ zone_ids in omx index order
...
if check_geography:
    ...
    orphan_households = households[~households.mgra.isin(land_use.MAZ)]
...
for slice_col, slice_values in slicer.items():
    ...
    land_use = land_use[
        land_use[slice_col].isin(slice_values)
        | (land_use['external_MAZ'] == 1)  # keep external stations
        | (land_use['poe_id'] >= 0)  # keep border crossing stations
    ]
...
# do not have MAZ in the taps.csv file, so looking at the maz_tap_walk for xwalk
...
maz_tap_drive = read_csv("maz_tap_drive.csv").sort_values(['MAZ', 'TAP'])
...
maz_tap_drive = maz_tap_drive[maz_tap_drive["MAZ"].isin(land_use["MAZ"]) & maz_tap_drive["TAP"].isin(maz_tap_walk["TAP"])]
# maz_tap_drive = maz_tap_drive[maz_tap_drive["MAZ"].isin(land_use["MAZ"]) & maz_tap_drive["TAP"].isin(taps["TAP"])]

to_csv(maz_tap_walk, "maz_tap_walk.csv")
to_csv(maz_tap_drive, "maz_tap_drive.csv")

taps = read_csv(TAP_MAZ)
taps = taps[['TAP']].sort_values(by='TAP').reset_index(drop=True)
...
maz_maz_bike = read_csv("maz_maz_bike.csv").sort_values(['OMAZ', 'DMAZ'])
...
households = read_csv(HOUSEHOLDS)
households = households[households["mgra"].isin(land_use["MAZ"])]
...
unique_tazs = land_use.sort_values(by='TAZ').TAZ.unique()

# drive skims
for omx_fpath in glob.glob(input_path("traffic_skims_processed*.omx")):
    print(omx_fpath)
    omx_fname = omx_fpath.replace("\\", '/').split('/')[-1].split(".omx")[0]
    crop_omx(omx_fname, unique_tazs)

# transit skims
crop_omx('transit_skims_processed', taps.TAP.values)
