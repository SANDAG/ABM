...
def determine_closest_external_station(choosers, skim_dict, origin_col='home_zone_id'):
    ...
    unique_home_zones = choosers[origin_col].unique()
    ...
    for origin_zone in unique_origin_zones:
        ...
        #     print('home: ', origin_zone, '\t external: ', ext_zone)
        #     print('home type: ', type(origin_zone), '\t external type: ', type(ext_zone))
        #     print(skim_dict.lookup([origin_zone], [ext_zone], 'DIST'))
        print(unique_origin_zones)
        zone_distances = skim_dict.lookup([origin_zone], ext_zones, 'DIST')
        # zone_distances = [skim_dict.lookup([origin_zone], [ext_zone], 'DIST') for ext_zone in ext_zones]
        # closest_zone_idx = np.argmin(skim_dict.lookup([origin_zone], ext_zones, 'DIST'))
        ...
        choosers.loc[choosers[origin_col] == origin_zone,
...
def external_identification(model_settings, estimator, choosers, network_los, chunk_size, trace_label):
...
def external_worker_identification(
    ...
    choices = external_identification(model_settings, estimator, choosers, network_los, chunk_size, trace_label)
...
@inject.step()
def external_nonmandatory_identification(
        persons_merged, persons, network_los,
        chunk_size, trace_hh_id):
    """
    ...
    tours[external_col_name] = choices.reindex(tours.index).fillna(0).astype(bool)
    tours[internal_col_name] = (tours[filter_col] & ~tours[external_col_name])
    ...
    tracing.print_summary(external_col_name, tours[external_col_name], value_counts=True)
