...
def determine_closest_external_station(choosers, skim_dict, origin_col='home_zone_id'):
    ...
    for origin_zone in unique_origin_zones:
        # FIXME not sure why I get an unhashable type: numpy.ndarray when trying to do them all at once...
        # zone_distances = skim_dict.lookup(origin_zone, ext_zones, 'DIST')
        # doing it one at a time instead
        ...
        dist_to_closest_zone = zone_distances[closest_zone_idx][0]
...
def external_worker_identification(
    ...
    persons[external_col_name] = (choices == 0).reindex(persons.index).fillna(False).astype(bool)
...
def set_external_tour_variables(tours, choices, model_settings, trace_label):
    ...
    tours.loc[choices.index, external_col_name] = (choices == 0).reindex(
        tours.index).fillna(False).astype(bool)
...
def external_non_mandatory_identification(
        tours_merged, tours, network_los,
        chunk_size, trace_hh_id):
    """
