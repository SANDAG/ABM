...
def create_metadata_df(input_dir, unique_id, ts, time_to_write, EMME_metadata):
    """
    ...
    metadata = {
        ...
        "iteration" : [EMME_metadata["iteration"]],
...
def write_metadata_to_datalake(data_dir, t0, EMME_metadata, container, guid, now):
    """
    ...
    scenario_df = create_metadata_df(data_dir[0], guid, now, write_time, EMME_metadata)
...
def get_scenario_metadata():
    ...
    if os.path.isfile(datalake_metadata_path):
        ...
        if not model_metadata_dict["iteration"]:
...
def write_to_datalake(
    ...
    if cloud_bool:
        ...
        prefix = output_tables_settings.get("prefix", "final_")
        EMME_metadata = get_scenario_metadata()
    ...
    if cloud_bool:
        ...
        write_metadata_to_datalake(data_dir, t0, EMME_metadata, container, guid, now)
